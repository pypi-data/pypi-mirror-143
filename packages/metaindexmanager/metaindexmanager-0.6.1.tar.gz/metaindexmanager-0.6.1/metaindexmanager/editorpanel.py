...
class EditorLine(InputLine):
    def __init__(self, panel, text=None):
        y, x, _, _ = panel.content_area()
        y += panel.pos[0] + panel.cursor - panel.offset
        x += panel.pos[1] + panel.columns[0]
        if text is None:
            text = panel.app.as_printable(panel.selected_line.value, None)
        super().__init__(panel.app, panel.columns[1], (y, x), text=text, background='░')
