import os
import string
from typing import ClassVar, List, Dict, Any, Callable, Optional, Union
...
class GraphDb(
    ...
    def __init__(self, connection: GraphDbConnection, path_data: str = None):
    ...
    def _get_node_value_from_node_param(self, node: Node) -> GraphTraversalSource.V:
        ...
        """
        search_node = self.connection.driver.V().hasLabel(node.label)
        if not node.id:
            search_node.hasLabel(node.label)
            if node.properties:
                for k, v in node.properties.items():
                    search_node.has(k, v)
        else:
            # otherwise search based on id
            search_node.hasId(node.id)
    ...
    @log_execution_time
    def create_multi_node(self, nodes: List[Node]) -> List[Node]:
        ...
        def create_node(n: Node):
            r = self.create_node(n)
            q.put(r)

        tmp = []
        for o in nodes:
            t = Thread(target=create_node, args=(o,))
            t.start()
            tmp.append(t)
    ...
    def remove_node_property(self, node: Node, properties: List[str]) -> Node:
        ...
        self.connection.driver.V(node_id).properties(*properties).drop().iterate()
    ...
    def delete_node(self, node: Node) -> Node:
        ...
        node_id = s.next().id
        # delete current node
        self.connection.driver.V(node_id).drop().iterate()
        # assign node id to current object node
        node.id = node_id
        # set current property to null
        node.properties = None
    ...
    def create_relationship_without_upsert(
        ...
        # get result
        res = self.connection.driver.V(n_f).addE(rel.relationship_name).to(__.V(n_t))
        rel.id = res.next().id
        return rel
    ...
    def create_relationship_with_upsert(
        ...
        e = self.connection.driver.V(n_f).outE().hasLabel(rel.relationship_name)
        update_val = __.unfold()
    ...
    def update_relationship_property(
        ...
        if rel.id is not None:
            ...
            n_e = self.connection.driver.E(rel.id)
            for k, v in update_query.items():
                n_e.property(k, v)
            n_e.iterate()
        ...
        # node relationship
        n_rel = self.connection.driver.V(n_f).outE().hasLabel(rel.relationship_name)
        ...
        n_rel.where(__.V().hasId(n_t))
        # get rel id
        rel_id = n_rel.next().id
        # update old property with current property
        n_e = self.connection.driver.E(rel_id)
        for k, v in update_query.items():
            n_e.property(k, v)
        # update query
        n_e.iterate()
    ...
    def replace_relationship_property(
        ...
        # if we want to delete relationship by id then search only relationship id
        if rel.id is not None:
            self.connection.driver.E(rel.id).properties().drop().iterate()
            # set new property
            n_e = self.connection.driver.E(rel.id)
            for k, v in update_query.items():
                n_e.property(k, v)
        ...
        self.connection.driver.E(rel_id).properties().drop().iterate()
    ...
    def remove_all_relationship_property(
        ...
        # if we want to delete relationship by id then search only relationship id
        if rel.id is not None:
            self.connection.driver.E().hasId(rel.id).properties().drop().iterate()
            # set current property to null
            rel.properties = None
            return rel
    ...
    def remove_relationship_property(
        ...
        # if we want to delete relationship by id then search only relationship id
        if rel.id is not None:
            self.connection.driver.E(rel.id).hasId(rel.id).properties(
                *properties
            ).drop().iterate()
            # remove property from payload
            if rel.properties:
        ...
        n_id = n_rel.next().id
        # delete current edge based on specified properties
        self.connection.driver.E(n_id).properties(*properties).drop().iterate()
        # remove property from payload
        if rel.properties:
            _ = [rel.properties.pop(val, None) for val in properties]
        # assign rel id to current object rel
    ...
    def delete_relationship(
        ...
        # if we want to delete relationship by id then search only relationship id
        if rel.id is not None:
            self.connection.driver.E(rel.id).drop().iterate()
        ...
        n_id = n_rel.next().id
        # delete current node
        self.connection.driver.E(n_id).drop().iterate()
        # assign rel id to current object rel
        rel.id = n_id
        # set current property to null
        rel.properties = None
