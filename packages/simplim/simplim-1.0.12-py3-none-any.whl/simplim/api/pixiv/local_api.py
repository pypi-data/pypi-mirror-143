...
def getIllustByUrl(url,path='',use_curl=False):
	...
	open(os.path.join(path,simplim.gettemp()+'.jpg'),'wb').write(requests.get(url.replace('i.pximg.net',pximg_host), headers=headers, verify=False).content)
