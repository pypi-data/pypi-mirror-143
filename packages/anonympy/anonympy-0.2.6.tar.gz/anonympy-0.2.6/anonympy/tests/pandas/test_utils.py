...
def test_load_dataset(load_df):
    if load_df is None:
        assert False, "`load_dataset('big')` should have returned a pandas DataFrame, which is fetched from https://github.com/ArtLabss/open-data-anonymizer/blob/main/examples/files/new.csv"
    else:
        assert True
    ...
    for i in range(ord('a'), ord('z')+1):
        ...
        for line in faker_methods.split('\n'):
                if line[0] == letter.upper():
                    expected = line[3:]
                    assert (captured.out).strip() == expected.strip(), f"`fake_methods()` returned unexpected methods for letter - `{letter.upper()}`"
...
def test_available_methods(capsys):
    ...
    for dtype in splitted.keys():
        ...
        captured = capsys.readouterr()
        assert (captured.out).strip() == '\t'.join(splitted[dtype]), f"`available_methods()` returned unexpected methods for dtype - `{dtype}`"


def test_get_numeric_columns_small(anonym_small):
    assert anonym_small.numeric_columns == ['age', 'salary'], "Fucntion `get_numeric_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('small')`"


def test_get_numeric_columns_big(anonym_big):
        if anonym_big is None:
            pytest.skip("Failed to fetch the DataFrame")
        assert anonym_big.numeric_columns == ['salary', 'age'], "Fucntion `get_numeric_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('big')`"


def test_get_categorical_columns_small(anonym_small):
    assert anonym_small.categorical_columns == ['name', 'web', 'email', 'ssn'], "Fucntion `get_categorical_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('small')`"

def test_get_categorical_columns_big(anonym_big):
    if anonym_big is None:
            pytest.skip("Failed to fetch the DataFrame")
    assert anonym_big.categorical_columns == ['first_name', 'address', 'city', 'phone', 'email', 'web', 'birthdate'], "Fucntion `get_categorical_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('big')`"


def test_get_datetime_columns_small(anonym_small):
    assert anonym_small.datetime_columns == ['birthdate'], "Fucntion `get_datetime_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('small')`"

def test_get_datetime_columns_big(anonym_big):
    if anonym_big is None:
            pytest.skip("Failed to fetch the DataFrame")
    assert anonym_big.datetime_columns == [], "Fucntion `get_datetime_columns` from anonympy.pandas.utils returned unexpected columns for `load_dataset('big')`"
