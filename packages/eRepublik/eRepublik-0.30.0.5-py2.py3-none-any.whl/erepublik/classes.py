...
from httpx import HTTPError, Response, Client
...
class Reporter:
    ...
    def __init__(self, citizen):
        self._citizen = weakref.ref(citizen)
        self._req = Client(follow_redirects=True, http2=True)
    ...
    def __bot_update(self, data: dict) -> Response:
        if self.__to_update:
            for unreported_data in self.__to_update:
                ...
                r = self.post("/bot/update", json=unreported_data)
                r.raise_for_status()
            self.__to_update.clear()
        data = utils.json.loads(utils.json_dumps(data))
        r = self.post("/bot/update", json=data)
        r.raise_for_status()
        return r
    ...
    def post(self, endpoint, **kwargs):
        return self._req.post(f"{self.url}/{endpoint}", **kwargs)

    def get(self, endpoint, **kwargs):
        return self._req.get(f"{self.url}/{endpoint}", **kwargs)
        ...
        if not self.__registered:
            ...
            if r:
                if not r.json().get("status"):
                    self.post("/bot/register", json=dict(name=self.name, email=self.email, player_id=self.citizen_id))
    ...
    def report_promo(self, kind: str, time_until: datetime.datetime):
        self.post("/promos/add/", data=dict(kind=kind, time_untill=time_until))

    def fetch_tasks(self) -> List[Dict[str, Any]]:
        try:
            task_response = self.post("/api/command", data=dict(citizen=self.citizen_id, key=self.key)).json()
...
class TelegramReporter:
    ...
    def post(self, endpoint, **kwargs):
        return self._req.post(f"{self.api_url}/{endpoint}", **kwargs)

    def get(self, endpoint, **kwargs):
        return self._req.get(f"{self.api_url}/{endpoint}", **kwargs)
    ...
    def __send_messages(self):
        ...
        response = self.post("/sendMessage", json=dict(chat_id=self.chat_id, text=message, parse_mode="Markdown"))
