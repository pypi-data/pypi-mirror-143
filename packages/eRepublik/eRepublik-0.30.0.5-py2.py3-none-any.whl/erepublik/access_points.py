...
from httpx import Response, Client, RequestError
...
class SlowRequests(Client):
    ...
    def __init__(self, *, proxies: str = None, user_agent: str = None):
        if proxies is not None:
            proxies = {"all://": proxies}
        self.proxies = proxies
        super().__init__(http2=True, follow_redirects=True, proxies=proxies)
    ...
    @property
    def _cookies_as_dict(self) -> List[Dict[str, Any]]:
        cookie_attrs = ["name", "value", "port", "domain", "path", "secure", "expires", "discard"]
        return [
            {attr: getattr(c, attr, None) for attr in cookie_attrs if hasattr(c, attr)} for c in self.cookies.jar
        ]
...
class CitizenBaseAPI:
    ...
    def __init__(self, *, proxies=None, user_agent=None):
        ...
        self._req = SlowRequests(proxies=proxies, user_agent=user_agent)
        self.token = ""
    ...
    def _post_main_session_get_challenge(self, captcha_id: int, image_id: str = "") -> Response:
        c = [cookie.name for cookie in self._req.cookies.jar if not cookie.has_nonstandard_attr("HttpOnly")]
