...
class ErepublikErrorHTTTPHandler(handlers.HTTPHandler):
    ...
    def _get_last_response(self) -> Dict[str, Union[bytes, str]]:
    ...
    def mapLogRecord(self, record: logging.LogRecord) -> Dict[str, Any]:
        ...
        if resp and resp.get("content"):
            files.append(("file", BytesIO(resp.get("content"))))

        files += [("file", open(f"log/{f}", "rb")) for f in os.listdir("log") if f.endswith(".log")]
        local_vars_json = self._get_local_vars()
        if local_vars_json:
            files.append(("file", BytesIO(local_vars_json.encode("utf8"))))
        instance_json = self._get_instance_json()
        if instance_json:
            files.append(("file", BytesIO(instance_json.encode("utf8"))))
    ...
    def emit(self, record):
        ...
        try:
            ...
            self.reporter.post(self.url, headers=headers, data=data, files=files)
