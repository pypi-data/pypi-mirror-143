...
class BaseCitizen(access_points.CitizenAPI):
    ...
    def __init__(self, email: str = "", password: str = "", *, proxies=None):
    ...
    def get_csrf_token(self):
        ...
        if re_token:
            ...
            raise classes.ErepublikException("Something went wrong! Can't find token in page!")
        try:
    ...
    def dump_instance(self):
        ...
        cookie_attrs = ["name", "value", "port", "domain", "path", "secure", "expires", "discard"]
        cookies = [
            {attr: getattr(c, attr, None) for attr in cookie_attrs if hasattr(c, attr)} for c in self._req.cookies.jar
        ]
    ...
    def load_from_dump(cls, dump_name: str):
        ...
        player = cls(data["config"]["email"], "", proxies=data.get("proxy"))
        if data.get("cookies"):
    ...
    def _login(self):
        ...
        if str(r.url) == f"{self.url}/login":
            raise classes.ErepublikException("Citizen email and/or password is incorrect!")
        else:
        ...
        for kind, notifications in self.get_all_notifications():
            if notifications:
                if kind == "community":
                    ...
                    raise classes.ErepublikException(f'Unsupported notification kind: "{kind}"!')
...
class _Citizen(
    ...
    def __init__(self, email: str = "", password: str = "", auto_login: bool = False, *, proxies=None):
        if isinstance(proxies, dict):
            host = proxies.get("host")
            port = proxies.get("port")
            proxy_url = f"{host}:{port}"
            username = proxies.get("username")
            password = proxies.get("password")
            if password and username:
                proxy_url = f"{username}:{password}@{proxy_url}"
            proxies = f"{proxies.get('kind', 'socks5')}://{proxy_url}"
        super().__init__(email, password, proxies=proxies)
