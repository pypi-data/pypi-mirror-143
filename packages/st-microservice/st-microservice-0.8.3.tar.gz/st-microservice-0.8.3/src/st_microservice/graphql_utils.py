...
async def external_module_executor(module_name):
    proc = await asyncio.create_subprocess_exec(sys.executable, '-u', '-m', f'scripts.{module_name}',
...
def subscription_permission_check(generator):
    async def new_generator(obj, info):
        try:
        ...
        async for res in generator(obj, info):
            yield res
