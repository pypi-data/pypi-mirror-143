...
def _setup_root_parser():
    ...
    p.add_argument('--addsubcomponent', help="add sub component")
    p.add_argument('--getsubcomponent', help="get sub component")
    p.add_argument('--deletesubcomponent', help="delete sub component")
...
def _setup_action_addsubcomponent_parser(subparsers):
    description = ("Add a subcomponent. "
                   "--product, --component, --subcomponent, --description, --default_assignee ."
                   "--default_docs_contact, --default_qa_contact, --default_cc, --api_key")
    p = subparsers.add_parser("addsubcomponent", description=description)

    # _parser_add_bz_fields(p, "addsub")
    g = p.add_argument_group("'addsubcomponent' specific options")
    g.add_argument('--product', help='product name of subcomponent')
    g.add_argument('--component', help='component name of subcomponent')
    g.add_argument('--subcomponent', help='subcomponent name')
    g.add_argument('--description', help='description of subcomponent')
    g.add_argument('--default_assignee', help='default_assignee of subcomponent')
    g.add_argument('--default_docs_contact', help='default_docs_contact of subcomponent')
    g.add_argument('--default_qa_contact', help='default_qa_contact of subcomponent')
    g.add_argument('--default_cc', action="append", help='default_cc of subcomponent')
    g.add_argument('--api_key', help='api_key of subcomponent')


def _setup_action_getsubcomponent_parser(subparsers):
    description = ("get subcomponents. "
                   "--ids, --api_key")
    p = subparsers.add_parser("getsubcomponent", description=description)

    # _parser_add_bz_fields(p, "addsub")
    g = p.add_argument_group("'getsubcomponent' specific options")
    g.add_argument('--ids', action="append", help='list of subcomponent id')
    g.add_argument('--api_key', help='api_key of subcomponent')


def _setup_action_deletesubcomponent_parser(subparsers):
    description = ("delete a subcomponent. "
                   "--product, --component, --ids, ----api_key")
    p = subparsers.add_parser("deletesubcomponent", description=description)

    # _parser_add_bz_fields(p, "addsub")
    g = p.add_argument_group("'deletesubcomponent' specific options")
    g.add_argument('--product', help='product name of subcomponent')
    g.add_argument('--component', help='component name of subcomponent')
    g.add_argument('--ids', action="append", help='subcomponent id')
    g.add_argument('--api_key', help='api_key of subcomponent')
...
def setup_parser():
    ...
    _setup_action_addsubcomponent_parser(subparsers)
    _setup_action_getsubcomponent_parser(subparsers)
    _setup_action_deletesubcomponent_parser(subparsers)
...
def _addsubcomponent(bz, opt, parser):
    # Parse options that accept comma separated list
    def parse_multi(val):
        return _parse_triset(val, checkplus=False, checkminus=False,
                             checkequal=False, splitcomma=True)[0]
    ...
    if opt.component:
        kwopts["component"] = opt.component
    if opt.subcomponent:
        kwopts["subcomponent"] = opt.subcomponent
    if opt.description:
        kwopts["description"] = opt.description
    if opt.default_assignee:
        kwopts["default_assignee"] = opt.default_assignee
    if opt.default_docs_contact:
        kwopts["default_docs_contact"] = opt.default_docs_contact
    if opt.default_qa_contact:
        kwopts["default_qa_contact"] = opt.default_qa_contact
    if opt.default_cc:
        kwopts["default_cc"] = parse_multi(opt.default_cc)
    if opt.api_key:
        kwopts["api_key"] = opt.api_key
    else:
        default_api_key = bugzilla.Bugzilla.get_rcfile_default_api_key()
        if default_api_key:
            kwopts["api_key"] = default_api_key
        else:
            raise Exception("please enter api_key from --api_key command or bugzillarc file")

    newid = None
    try:
        res = bz.addsubcomponent(kwopts)
        newid = res['id']
        print("Created id=%s product=%s component=%s subcomponent=%s" % (newid, kwopts["product"], kwopts["component"],
                                                                   kwopts["subcomponent"]))
...
def _getsubcomponent(bz, opt, parser):
    # Parse options that accept comma separated list
    ...
    try:
        res = bz.getsubcomponent(kwopts)
        print("got res=%s" % res)
    except Exception as e:
        return


def _deletesubcomponent(bz, opt, parser):
    # Parse options that accept comma separated list
    ...
    try:
        res = bz.deletesubcomponent(kwopts)
        print("deleted res=%s" % res)
    except Exception as e:
        return
...
def _make_bz_instance(opt, force_rest=False):
    """
...
def _main(unittest_bz_instance):
    ...
    if unittest_bz_instance:
        ...
        if action in ['addsubcomponent', 'getsubcomponent', 'deletesubcomponent']:
            bz = _make_bz_instance(opt, force_rest=True)
        else:
    ...
    elif action == 'addsubcomponent':
        buglist = _addsubcomponent(bz, opt, parser)

    elif action == 'getsubcomponent':
        buglist = _getsubcomponent(bz, opt, parser)

    elif action == 'deletesubcomponent':
        buglist = _deletesubcomponent(bz, opt, parser)
