...
class Bugzilla(object):
    ...
    @staticmethod
    def get_rcfile_default_api_key():
        """
        Helper to check all the default bugzillarc file paths for
        a [DEFAULT] url=X section, and if found, return it.
        """
        ...
        rcfile = _BugzillaRCFile()
        rcfile.set_configpaths(configpaths)
        return rcfile.get_default_api_key()
    ...
    ###############################
    # subcomponent adding/editing #
    ###############################

    def addsubcomponent(self, data):
        """
        A method to create a subcomponent in Bugzilla. Takes a dict, with the
        following elements:

        product: The product to create the subcomponent in
        component: The component to create the subcomponent in
        subcomponent: The name of the subcomponent to create
        description: A one sentence summary of the subcomponent
        default_assignee: The bugzilla login (email address) of the initial
                          owner of the component
        default_qa_contact (optional): The bugzilla login of the
                                       initial QA contact
        default_cc: (optional) The initial list of users to be CC'ed on
                               new bugs for the component.
        default_docs_contact: (optional) The bugzilla login of the
                                       initial DOCS contact
        """
        data = data.copy()
        return self._backend.subcomponent_create(data)

    def getsubcomponent(self, data):
        """
        A method to get a subcomponent in Bugzilla. Takes a dict, with the
        following elements:

        ids: list of subcomponent id
        """
        data = data.copy()
        return self._backend.subcomponent_get(data)

    def deletesubcomponent(self, data):
        """
        A method to delete a subcomponent in Bugzilla. Takes a dict, with the
        following elements:
        ...
        """
        data = data.copy()
        return self._backend.subcomponent_delete(data)
