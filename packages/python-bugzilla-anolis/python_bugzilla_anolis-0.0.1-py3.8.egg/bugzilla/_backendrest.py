...
class _BackendREST(_BackendBase):
    ...
    def _op(self, method, apiurl, paramdict=None):
        ...
        response_body = dict()
        try:
            response_body = self._handle_response(response.text)
        except Exception as e:
            try:
                msg_all = response.text.split("<div id=\"error_msg\" class=\"throw_error\">")
                eror_msg = msg_all[1].split("</div>")[0]
                response_body['error_msg'] = eror_msg
                print("you have a error, msg: {}".format(eror_msg))
            except Exception as e:
                print("get error msg e: {}".format(e))
    ...
    def _delete(self, *args, **kwargs):
        return self._op("DELETE", *args, **kwargs)
    ...
    def subcomponent_create(self, paramdict):
        return self._post("/subcomponent", paramdict)

    def subcomponent_get(self, paramdict):
        return self._get("/subcomponent", paramdict)

    def subcomponent_delete(self, paramdict):
        return self._delete("/subcomponent", paramdict)
