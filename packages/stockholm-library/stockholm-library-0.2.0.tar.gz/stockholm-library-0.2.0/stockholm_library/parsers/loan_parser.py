...
class LoanParser:
    ...
    def parse_loan(el: Tag, can_re_borrow: bool) -> Loan:
        ...
        loan.borrowed_date = LoanParser.parse_borrowed_date(tds[2])
    ...
    @staticmethod
    def parse_borrowed_date(td: Tag) -> datetime.date:
        borrowed_date = td.findChild("a", {"class": "hidden-phone"}).findChild("span").text.strip()
        return datetime.date.fromisoformat(borrowed_date)

    @staticmethod
    def parse_borrowed_from(td: Tag) -> str:
        borrowed_date = LoanParser.parse_borrowed_date(td)
        full_text = td.findChild("a", {"class": "hidden-phone"}).text
        trimmed_text = full_text.replace(str(borrowed_date), "").strip()
        return trimmed_text
