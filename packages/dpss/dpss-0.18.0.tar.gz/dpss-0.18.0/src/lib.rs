...
pub fn wasm_find_subset(keys: String, targets: String, max_key_length: usize, max_target_length: usize, n_candidates: usize) -> String {
    ...
    if targets.contains(", ") {
        ...
        let result: Vec<Vec<(Vec<i32>, Vec<i32>)>> = dp::sequence_matcher(&mut keys, &mut targets, max_key_length, max_target_length, n_candidates);
        ...
        let res: Vec<Vec<i32>> = dp::find_subset(keys, targets.parse::<i32>().unwrap(), max_target_length);
