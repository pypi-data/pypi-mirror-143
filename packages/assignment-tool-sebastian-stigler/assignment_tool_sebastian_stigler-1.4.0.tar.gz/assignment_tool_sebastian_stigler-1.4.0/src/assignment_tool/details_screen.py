...
class GenerateMixin:
    ...
    def generate_assignment_file(self, create_empty_file=True):
    ...
    def _get_testat_candidates(self):
        pattern = r"^(\d\d\d\d\d-)?" + self.instance.assignment.base_filename + "$"
        files = [
            entry.path
    ...
    def _unique_assignment_file(self, assignment_filename, create_empty_file):
        """
    ...
    @staticmethod
    def _rename_additional_files(files):
        cnt = 0
        while len(files) > 0:
            filename = files[0]
    ...
    @staticmethod
    def _update_author(filename, student_profile):
...
class DetailsManager(Manager, CheckCodeAssignment):
    screen_name = "details"
