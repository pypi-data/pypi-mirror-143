...
class CyberUtils(dict):
    ...
    def __init__(self, config: dict, log_level: int = logging.INFO):
        ...
        logging.basicConfig(level=log_level,
                            format='%(app_name)s | %(asctime)s | %(levelname)s | %(message)s')
        self.__logger__ = logging.getLogger(__name__)
        self.__logger__.addFilter(ContextFilter(config.get('app_name')))
        self.logger = CyberLogger(dict(url=config.get('logger', 'http://127.0.0.1')), self)
    ...
    def log(self, message: object, status_code: int = None, status_name: str = 'info', method: str = None, log_server=True):
        ...
        method_caller = f'{the_class}.{the_method}()' if the_class and the_method else ''
        ...
        if hasattr(self, "caller") and log_server:
            self.caller.do('post', self.logger, '/log', json_content=dict(message=message, status_code=status_code))
        return dict(message=message, status_code=status_code)


class CyberModel(dict):

    def __init__(self, config, utils: CyberUtils):
        self.log = utils.log
        self.update(config)
        super().__init__()
