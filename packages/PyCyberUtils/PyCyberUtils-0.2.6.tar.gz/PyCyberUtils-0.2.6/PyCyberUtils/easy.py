...
class CyberUtils(dict):
    ...
    def __init__(self, config: ModelConfig, log_level: int = logging.INFO):
        ...
        self.logger = CyberLogger(ModelLogger(**config.get('logger', {})), self)
        self.db = CyberDB(ModelDatabase(**config.get('database', {})), self)
        self.caller = CyberCaller(ModelServices(**config.get('services', {})), self)
        super().__init__()
