...
Saved searches are defined in a **savedsearches.conf** file located in the **/default** directory of the app. For more, see [Save and share your reports](http://docs.splunk.com/Documentation/Splunk/latest/SearchTutorial/Aboutsavingandsharingreports) and [savedsearches.conf](http://docs.splunk.com/Documentation/Splunk/latest/Admin/Savedsearchesconf).
"""
import logging
import os
    ...
    if saved_searches.configuration_file_exists():
        ...
        for search in saved_searches.searches():
            if search.disabled == "1":
                ...
                message = (
                    f"The search [{search.name}] in savedsearches.conf is"
                    f" disabled. File: {file_path}, Line: {lineno}."
                )
...
def check_saved_search_specifies_a_search(app, reporter):
    ...
    if saved_searches.configuration_file_exists():
        ...
        for search in saved_searches.searches():
            if search.searchcmd == "":
                mess = (
                    f"The search '{search.name}' in savedsearches.conf"
                    f" does not specify a search. File: {file_path},"
                    f"Line: {search.lineno}."
                )
...
def check_for_emails_in_saved_search(app, reporter):
    ...
    if saved_searches.configuration_file_exists():
        file_path = os.path.join("default", "savedsearches.conf")
        for search in saved_searches.searches():
            for key, value in iter(search.args.items()):
                if key == "action.email.to":
                    reporter_output = (
                        f"The saved search {search.name} has specified the"
                        " `action.email.to` property with a provided value."
                        "  This should be left empty or removed. "
                        f"  File: {file_path}, Line: {value[1]}."
                    )
                    if value:
    ...
    if app.file_exists("default", "savedsearches.conf"):
        ...
        for saved_search in saved_searches.searches():
            if saved_search.is_real_time_search():
                ...
                reporter_output = (
                    f"The stanza [{saved_search.name}] contains a real-time"
                    " search, which might have impact on performance in"
                    f" high volume environments and network load. File: {file_path},"
                    f" Line: {saved_search.lineno}."
                )
...
@splunk_appinspect.tags("cloud", "savedsearches", "private_app")
...
def check_for_real_time_saved_searches_for_cloud(app, reporter):
    ...
    if app.file_exists("default", "savedsearches.conf"):
        ...
        for saved_search in saved_searches.searches():
            if (
                saved_search.is_real_time_search()
                and saved_search.enable_sched
                and not saved_search.is_disabled
            ):
                reporter_output = (
                    f"The stanza [{saved_search.name}] contains a scheduled real-time"
                    " search, which have impact on performance in high volume"
                    " environments and network load. Please disable this search."
                    f" File: {file_path}, Line: {saved_search.lineno}."
                )
                reporter.fail(reporter_output, file_path, saved_search.lineno)
            elif saved_search.is_real_time_search():
                # https://docs.splunk.com/Documentation/Splunk/latest/Search/Realtimeperformanceandlimitations
                reporter_output = (
                    f"The stanza [{saved_search.name}] contains a real-time"
                    " search, which might have impact on performance in"
...
@splunk_appinspect.tags("splunk_appinspect", "cloud", "savedsearches", "private_app")
@splunk_appinspect.cert_version(min="1.1.20")
        ...
        for saved_search in cron_schedule_saved_search:
            try:
                ...
                if not exp.is_valid():
                    ...
                    occurrences = CronExpression.get_occurrences_in_an_hour(
                        minutes_field
                    )
                    gratuitous_cron_schedule_saved_searches.append(
                        (saved_search, occurrences)
                    )
            except ValueError:
                invalid_cron_schedule_saved_searches.append(saved_search)
        ...
        if cron_schedule_saved_search:
            if gratuitous_cron_schedule_saved_searches:
                for (
                    ...
                    reporter_output = (
                        ...
                        f" is appropriate. File: {file_path}, Line: {lineno}."
                    )
            ...
            if invalid_cron_schedule_saved_searches:
                for saved_search in invalid_cron_schedule_saved_searches:
                    lineno = saved_search.args["cron_schedule"][1]
                    reporter_output = (
                        f"The saved search [{saved_search.name}] was detected "
                        " with an invalid cron_schedule. Please evaluate"
                        f" whether `cron_schedule = {saved_search.cron_schedule}`"
                        f" is valid. File: {file_path}, Line: {lineno}."
                    )
                    reporter.fail(reporter_output, file_path, lineno)
        else:
            reporter_output = "No saved searches with a cron schedule were" " detected."
            reporter.not_applicable(reporter_output)
...
def check_for_empty_saved_search_description(app, reporter):
    ...
    if saved_searches.configuration_file_exists():
        ...
        for section in saved_searches_conf_file.sections():
            if (
                section.has_option("description")
                and section.get_option("description").value.strip() == ""
            ):
...
def check_for_sched_saved_searches_earliest_time(app, reporter):
    ...
    if app.file_exists("default", "savedsearches.conf"):
        ...
        for section in savedsearches_config.sections():
            is_generating_command_search = section.has_option(
                "search"
            ) and section.get_option("search").value.strip().startswith("|")
...
def _is_scheduled_search(section):
    return section.has_option("enableSched") and normalizeBoolean(
        section.get_option("enableSched").value.strip()
    )
    ...
    if app.file_exists("default", "savedsearches.conf"):
        ...
        for section in savedsearches_config.sections():
            ...
            if is_generating_command_search:
                ...
                # create events in real-time so earliest_time isn't needed
                continue
            if section.has_option("enableSched") and normalizeBoolean(
                section.get_option("enableSched").value.strip()
            ):
