...
class ObjectDetectionRNPrimitive(PrimitiveBase[Inputs, Outputs, Params, Hyperparams]):
    """
    This primitive utilizes RetinaNet, a convolutional neural network (CNN), for object detection.
    The methodology comes from "Focal Loss for Dense Object Detection" by Lin et al. 2017
    (https://arxiv.org/abs/1708.02002). The code implementation is based
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            hyperparams=hyperparams, volumes=volumes, random_seed=random_seed
        )
    ...
    def set_training_data(self, *, inputs: Inputs, outputs: Outputs) -> None:
        ...
        Parameters
        ----------
            inputs: d3m dataframe of size (n_images, dimension) containing the d3m Index, image name,
        ...
        self.image_paths = np.array(
            [
                ...
                for self.base_dir, col in zip(self.base_dir, image_cols)
            ]
        ).flatten()
    ...
    def _fill_empty_predictions(
        self, empty_predictions_image_names, d3mIdx_image_mapping
    ):
        """
        ...
        empty_predictions_d3mIdx = [
            d3mIdx_image_mapping.get(key) for key in empty_predictions_image_names
        ]
        empty_predictions_d3mIdx = [
        ...
        empty_predictions_df = pd.DataFrame(
            {"d3mIndex": d3mIdx, "bounding_box": bounding_box, "confidence": confidence}
        )
    ...
    def fit(self, *, timeout: float = None, iterations: int = None) -> CallResult[None]:
        ...
        training_model.save_weights(
            self.hyperparams["weights_path"] + "model_weights.h5"
        )
    ...
    def produce(
        ...
        image_list = [
            x
            for i, x in enumerate(self.image_paths.tolist())
        ...
        boxes = list(
            map(lambda x: [x[0], x[1], x[0], x[3], x[2], x[3], x[2], x[1]], boxes)
        )  # Convert to 8 coordinate format for D3M
        ...
        d3mIdx = [
            item for sublist in d3mIdx for item in sublist
        ]  # Flatten list of lists

        ## Assemble in a Pandas DataFrame
        results = pd.DataFrame(
            {"d3mIndex": d3mIdx, "bounding_box": boxes, "confidence": score_list}
        )
        ...
        if len(empty_predictions_image_names) != 0:
            ...
            results_df = pd.concat([results, empty_predictions_df]).sort_values(
                "d3mIndex"
            )
        else:
            results_df = results
