...
class ImageSegmentationPrimitive(
    SupervisedLearnerPrimitiveBase[Inputs, Outputs, Params, Hyperparams]
):
    """This primitive trains a binary image segmentation model using image-level weak supervision
    ...
    def _load_data(self, inputs, outputs):
        ...
        if label_encoder.classes_.shape[0] > 2:
            raise ValueError(
                "This primitive only supports binary segmentation (2 classes)"
            )
    ...
    def _get_imgs(self, inputs):
        ...
        if len(image_cols) > 1:
            raise ValueError(
                "Primitive only supports fitting model on one image column"
            )
    ...
    def _prepare_loader(self, inputs, outputs=None, shuffle=True):
        ...
        data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=self.hyperparams["batch_size"], shuffle=shuffle
        )
        return data_loader
    ...
    def _train(self, epochs, initial_epoch=0):
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            # self.logger
            print(
                f"Epoch {epoch}, " + f"Train Loss: {round(train_loss, 3)}",
            ...
            if stopping_loss is None:
                ...
                torch.save(
                    self.model.state_dict(), self.hyperparams["weights_filepath"]
                )
