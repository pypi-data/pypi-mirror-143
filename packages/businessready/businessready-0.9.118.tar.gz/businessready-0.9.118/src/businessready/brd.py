...
def IOS_show_cdp_neighbors(hostname, username, password, ip):
    ...
    # -------------------------
                with open(f"{ filename }_Show CDP Neighbors_{ mermaid_type }.md", "w") as fh:
...
def NXOS_learn_arp(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        # -------------------------
            ...
            loop_counter = 0
            for mermaid_type in mermaid_loop:
                parsed_mermaid_output = NXOS_learn_arp_mermaid_template.render(to_parse_arp=learn_arp['statistics'],hostname=hostname,filetype_loop=loop_counter)
                loop_counter = loop_counter + 1

    # -------------------------
    # Save the Mermaid files
    # -------------------------
                with open(f"{ filename } Learn ARP Statistics { mermaid_type }.md", "w") as fh:
                    fh.write(parsed_mermaid_output)               
                    fh.close()

            parsed_pie_output = NXOS_learn_arp_statistics_piechart_template.render(to_parse_arp=learn_arp['statistics'],hostname=hostname,filetype_loop=loop_counter)

    # -------------------------
    # Save the Mermaid files
    # -------------------------
            with open(f"{ filename }_Learn ARP Statistics Pie Chart.md", "w") as fh:
...
def NXOS_learn_interface(hostname, username, password, ip):
    ...
    # Save the Mermaid files
    # -------------------------
            with open(f"{ filename }_Learn Interface Pie Chart.md", "w") as fh:
                fh.write(parsed_pie_output)               
                fh.close()
