...
def NXOS_learn_interface(hostname, username, password, ip):
    ...
    # Create Testbed
        ...
        if learn_interface != f"{ hostname } has no Interface to Learn":
            ...
            NXOS_learn_interface_piechat_template = env.get_template('NXOS_learn_interface_pie.j2')
            loop_counter = 0
        # Render Templates
            for filetype in filetype_loop:
    ...
    # Save the Mermaid files
    # -------------------------
                with open(f"{ filename }_Learn Interface_{ mermaid_type }.md", "w") as fh:
                    fh.write(parsed_mermaid_output)               
                    fh.close()

            parsed_pie_output = NXOS_learn_interface_piechat_template.render(to_parse_interface=learn_interface,hostname=hostname,filetype_loop=loop_counter)
            loop_counter = loop_counter + 1

    # -------------------------
    # Save the Mermaid files
    # -------------------------
            with open(f"{ filename }_Learn Interface_Pie_Chart.md", "w") as fh:
