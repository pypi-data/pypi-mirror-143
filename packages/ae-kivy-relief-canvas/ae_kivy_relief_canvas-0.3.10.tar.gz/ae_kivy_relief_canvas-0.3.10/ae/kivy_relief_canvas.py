...
def relief_colors(color_or_ink: ColorOrInk = (0, 0, 0), darken_factors: ReliefBrightness = (0.6, 0.3)) -> ReliefColors:
    ...
    :param color_or_ink:        color used to calculate the relief colors from, which will first be lightened
                                until one of the color parts (R, G or B) reach the value 1.0; then
                                :paramref:`.darken factors` will be applied to the color parts. If not passed then grey
    ...
    :return:                    tuple with darkened colors calculated from :paramref:`.color_or_ink` and
                                :paramref:`.darken_factors` or an empty tuple if the alpha value of
                                :paramref:`.color_or_ink` has a zero value.
    """
