...
@pytest.mark.parametrize(
    ...
    [
        ResponseTest(
            "/model/parse",
            {'text': '/hello', 'intent': {'name': 'hello', 'confidence': 1.0}, 'intent_ranking': [{'name': 'hello', 'confidence': 1.0}], 'entities': []},
            payload={"text": "/hello"},
        ),
        ResponseTest(
            "/model/parse",
        ...
        ResponseTest(
            "/model/parse",
            {'text': '/hello ńöñàśçií', 'intent': {'name': 'hello ńöñàśçií', 'confidence': 1.0}, 'intent_ranking': [{'name': 'hello ńöñàśçií', 'confidence': 1.0}], 'entities': []},
...
def test_train_stack_success(
    ...
    with ExitStack() as stack:
        domain_file = stack.enter_context(open(default_domain_path, encoding='utf8'))
...
def test_check_tracker_existence(rasa_app: SanicTestClient):
    _, response = rasa_app.get("/conversations/madeupid/tracker/exists")
    content = response.json
    assert response.status == 200
    assert content["sender_id"] == "madeupid"
    assert content["exists"] == False

    rasa_app.get("/conversations/madeupid/tracker")
    _, response = rasa_app.get("/conversations/madeupid/tracker/exists")
    content = response.json
    assert response.status == 200
    assert content["sender_id"] == "madeupid"
