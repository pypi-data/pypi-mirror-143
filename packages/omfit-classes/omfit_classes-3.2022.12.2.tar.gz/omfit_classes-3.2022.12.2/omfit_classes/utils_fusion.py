...
_internal_2 = {
    ...
    'OMAS': {'CASE': 'lower', 'DIII-D': 'd3d', 'NSTX-U': 'nstxu', 'NSTXU': 'nstxu', 'NSTX': 'nstx'},
}


@_available_to_user_fusion
def tokamak(tokamak, output_style='OMFIT', allow_not_recognized=True, translation_dict=None):
    """
...
def calc_h98_d3d(shot, calc_tau=False, calc_ptot=False, pinj_only=False, estimated_signals=None, data=None):
    ...
    input_pointnames = 'ip bt density r0 volume aminor'.split(' ')
    if calc_tau:
    ...
    data['volterm'] = data['volume'] / 19.74 / data['r0'] / data['aminor'] ** 2
    data['aspect'] = data['aminor'] / data['r0']
    data['main_ion_mass'] = 2.0

    # Final calculations
    factors = dict(ip=1e-6, density=1e-13, psol=1e-6)
    exponents = dict(ip=0.93, bt=0.15, density=0.41, psol=-0.69, r0=1.97, volterm=0.78, aspect=0.58, main_ion_mass=0.19)

    # taue_98y2 = 56.2e-3 * (ip/1e6)**0.93 * bt**0.15 * (density/1e-13)**0.41
    taue_98y2 = 56.2e-3
    for thing in 'ip density bt psol r0 volterm aspect main_ion_mass'.split(' '):
...
def slow1(ne, nis, atherms, Ztherms, Ebirth, Te, Zfast, afast, nns):
    """
    Calculation of slowing down time and critical energy ported from the onetwo code

    :param ne: 1D array of N length
        electron density profile (cm^-3)

    :param nis: 2D array N x Nspecies
        ion density profiles (cm^-3)

    :param atherms: 1D array of Nspecies length
        ion mass (AMU)

    :param Ztherms: 1D array of Nspecies length
        ion charge (Coulomb)

    :param Ebirth: float
        fast ion energy (keV)

    :param Te: 1D array of N length
        Electron temperature (keV)

    :param Zfast: float
        Fast ion charge(Coulomb)

    :param afast: float
        Fast ion mass (AMU)

    :param nns: (2D array Nspecies x N) neutral density profiles (cm^-3)

    """

    m_p = scipy.constants.m_p * 1e3
    m_e = scipy.constants.m_e * 1e3
    xkeverg = 6.241e08
    charg4 = 5.32261610e-38
    rot2pi = 2.5066  # what is this constant?
    econst = 0.5 * (4.5 * pi * m_p / m_e) ** (1.0 / 3.0)
    tconst = afast * m_p / (Zfast * m_e)
    tiny = 0.00001

    sum1 = np.zeros(len(ne))
    sum2 = np.zeros(len(ne))
    sum3 = np.zeros(len(ne))
    nion = len(atherms)
    for k in range(nion):
        sum1 = sum1 + nis[k, :] * Ztherms[k]
        sum2 = sum2 + nis[k, :] * Ztherms[k] / atherms[k]

    emzrat = sum1 / (afast * sum2)

    tegt0 = copy.deepcopy(Te)
    tegt0[tegt0 < tiny] = tiny  # keV, avoid probs near plasma edge
    teerg = 1.6e-9 * tegt0
    xlam = 24.0 - np.log(np.sqrt(ne) / (1.0e3 * tegt0))
    ztaue = np.sqrt(m_e) / (1.33333 * rot2pi * ne * charg4 * xlam)
    ztaue *= np.sqrt(teerg ** 3)

    taus = tconst * ztaue
    for k in range(nion):
        if Ebirth < 100 * afast * Zfast * Ztherms[k]:
            LL_ii = 25.4 + np.log(
                Ebirth / 1e3 * atherms[k] / ((atherms[k] + afast) * (Zfast * Ztherms[k]) ** 0.5) * (Te / 10 * 1e14 / ne) ** 0.5
            )
        else:
            LL_ii = 23.7 + np.log(atherms[k] / (atherms[k] + afast) * (Te / 10 * afast * Ebirth / 1e3 * 1e14 / ne) ** 0.5)

        sum3 = sum3 + LL_ii / xlam * ne * Ztherms[k] / atherms[k] / ne

    ecrit = afast * m_p * 0.5 * (3 * np.pi ** 0.5 / 4 * m_e / m_p * sum3) ** (2.0 / 3.0) * Te / m_e * 2
