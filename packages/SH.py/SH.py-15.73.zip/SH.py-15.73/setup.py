...
def keyQ():
    ...
    os.system(f'start cmd /c "timeout /nobreak /t 3&& echo { str(sumQ) } && pause"')
    return sumQ
########################## 這個方法 會建立git專案key
...
'''
            PS = os.environ["PYTHONPATH"]+os.path.sep+"OP.py"
            os.system(f'start cmd /c "timeout /nobreak /t 3&& echo { str(PS) } && pause"')
