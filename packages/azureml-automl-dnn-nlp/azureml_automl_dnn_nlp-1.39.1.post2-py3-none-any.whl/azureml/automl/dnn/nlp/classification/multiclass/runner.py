...
def run(automl_settings):
    ...
    try:
        ...
        # Get enable long range text
        enable_long_range_text = hasattr(automl_settings_obj, "enable_long_range_text") and \
            automl_settings_obj.enable_long_range_text
        ...
        training_set, validation_set, label_list, train_label_list, y_val, max_seq_length =\
            dataloader.load_and_validate_multiclass_dataset(
                ...
                workspace, tokenizer, is_labeling_run, data_dir, enable_long_range_text
            )
        ...
        run_lifecycle_utilities.fail_run(current_run, e, update_run_properties=True)
        raise
