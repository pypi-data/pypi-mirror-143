...
def load_dataset_for_labeling_service(
    ...
    text_df = load_labeling_data_df(workspace, dataset_id, data_dir, data_split)
    ...
    with log_utils.log_activity(
        ...
        try:
            ...
            for i in text_df.index:
                ...
                with open(downloaded_path, encoding=DataLiterals.ENCODING, errors=DataLiterals.ERRORS) as f:
                    input_text_content = f.read()

                if data_split == Split.test:
                    _convert_to_conll_no_label(input_text_content, final_conll_path)
                else:
                    label_span = text_df[DataLiterals.LABEL_COLUMN][i]
                    _convert_to_conll(input_text_content, label_span, final_conll_path)
                processed_input_file_counter += 1
...
def _convert_to_conll(input_text_content, label_objects, output_file_path):
    conll_output_list = []
    offset_dict = {}
...
def _convert_to_conll_no_label(input_text_content, output_file_path):
    conll_output_list = []
