...
class Cyrating(object):
    ...
    def technologies(self, company, assets=None):
        filter = {"where": json.dumps({"company": company["_id"]})}
        technologies = self.findall(TECHNOLOGIES_ENDPOINT, 1, filter)
        ...
        for tech in technologies:
            item = {
                ...
                "entities": assets[tech["asset"]]["entities"]
                if assets and "entities" in assets[tech["asset"]]
                else None,
                "tags": assets[tech["asset"]]["tags"] if assets else None,
            }
            res.append(item)
        return res
            ...
            for event in events["assessment"][key]:
                for source in event["sources"]:
                    item = {
                        ...
                        "entities": assets[event["name"]]["entities"]
                        if assets and "entities" in assets[event["name"]]
                        else None,
                    }
                    res.append(item)
    ...
    def facts(self, company, assets=None, extra_filter=None):
        ...
        for assessment in assessements:
            ...
            entities = (
                assets[assessment["domainname"]]["entities"]
                if assets and ("entities" in assets[assessment["domainname"]])
                else None
            )
