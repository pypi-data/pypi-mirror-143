...
class SQSHandler(object):
    ...
    def __init__(self, processor: Optional[Callable[[dict, dict], Any]], single: Optional[bool] = False,
        ...
        self.starting_precess_logger = None
        self.jsonfy = jsonfy
    ...
    def run(self, event):
        ...
        if 'Records' not in event:
            if self.strict:
                msg = "Fatal :: Could not continue processing, invalid event!"
        ...
        for index, record in enumerate(event["Records"]):
            ...
            try:
                if 'body' not in record:
                    msg = "Fatal :: Invalid item, could not continue processing..."
                ...
                if self.starting_precess_logger:
                    self.starting_precess_logger(body, record)
                ...
                failed = True
                self.__notify_error(x_record, "Fatal :: An error occurred while processing notification data")
                logger.exception(f"An error occurred while processing notification data: {body}\n", exc_info=e)
                if self.re_enqueue:
