...
class Client(object):
    ...
    def make_request(
        self, endpoint, params, method=GET, headers={}, timeout=None
    ):
        ...
        logger.debug(
            "url: %s; endpoint: %s; params: %s", self.url, endpoint, params
        )
    ...
    def add_optional_kwargs(
        ...
        **kwargs
        ...
        if best_value_offer:
            params.update(
                req_cost_range_best_value_offer=True, req_cost_range=True
            )

        if max_saving_offer:
            params.update(
                req_cost_range_max_saving_offer=True, req_cost_range=True
            )

        if min_cost_offer:
            params.update(
                req_cost_range_min_cost_offer=True, req_cost_range=True
            )

        if top_price_offer:
            params.update(
                req_cost_range_top_price_offer=True, req_cost_range=True
            )

        if no_singles_data:
            params.update(
                req_cost_range_no_singles_data=True, req_cost_range=True
            )
        ...
        if availability_with_performances:
            params.update(
                req_avail_details=True, req_avail_details_with_perfs=True
            )

        if tracking_id:
            params.update(
                self.get_tracking_params(custom_tracking_id=tracking_id)
            )
    ...
    def get_events(
        self, event_ids, with_addons=False, with_upsells=False, **kwargs
    ):
    ...
    def list_performances(
        ...
        **kwargs
        ...
        performances = [
            Performance.from_api_data(data) for data in raw_performances
        ]
    ...
    def _trolley_params(
        ...
        **kwargs
        ...
        if seats:
            params.update(
                {"seat{}".format(i): seat for i, seat in enumerate(seats)}
            )
    ...
    def get_upsells(
        ...
        **kwargs
        ...
        upsell_events = [
            Event.from_api_data(data) for data in raw_upsell_events
        ]
    ...
    def get_addons(
        ...
        **kwargs
        ...
        add_on_events = [
            Event.from_api_data(data) for data in raw_add_on_events
        ]
    ...
    def get_reservation(
        ...
        response = self.make_request(
            "reserve_page_archive.v1", params, method=GET
        )
    ...
    def get_purchase(self, transaction_uuid, **kwargs):
        ...
        params = {"transaction_uuid": transaction_uuid}
        response = self.make_request(
            "purchase_page_archive.v1", params, method=GET
        )
    ...
    def process_reservation_response(
        self, response, raise_on_unavailable_order
    ):
    ...
    def cancel_purchase(
        self, transaction_uuid, cancel_items_list=None, **kwargs
    ):
        ...
        if cancel_items_list:
            params.update(
                cancel_items_list=",".join(map(str, cancel_items_list))
            )
