    ...
    def __jsondict__(self, hide_none=True, hide_empty=True):
        def sanitise(obj):
            ...
            if hasattr(obj, "__jsondict__"):
                return obj.__jsondict__(
                    hide_none=hide_none, hide_empty=hide_empty
                )
...
class SeatPricingMixin(object):
    ...
    def non_offer_combined_price(self):
        ...
        """
        assert (
            self.non_offer_seatprice is not None
        ), "non_offer_seatprice data missing"
        assert (
            self.non_offer_surcharge is not None
        ), "non_offer_surcharge data missing"
        return utils.add_prices(
            self.non_offer_seatprice, self.non_offer_surcharge
        )
