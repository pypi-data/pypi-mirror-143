...
class AvailabilityMeta(CurrencyMeta):
    ...
    def from_api_data(cls, data):
        ...
        inst.must_select_whole_seat_block = data.get(
        ...
        for ticket_type in data.get("ticket_type", []):
            ...
            for price_band in ticket_type.get("price_band", []):
                ...
                for raw_details in raw_details_list:
                    ...
                    kwargs["full_seatprice"] = raw_details.get(
                        "full_seatprice", 0.0
                    )
                    kwargs["surcharge"] = raw_details.get("surcharge", 0.0)
                    kwargs["full_surcharge"] = raw_details.get(
                        "full_surcharge", 0.0
                    )
                    kwargs["percentage_saving"] = raw_details.get(
                        "percentage_saving", 0.0
                    )
                    kwargs["absolute_saving"] = raw_details.get(
                        "absolute_saving", 0.0
                    )
                    ...
                    if "valid_quantities" in raw_details:
                        kwargs["valid_quantities"] = raw_details[
                            "valid_quantities"
                        ]
