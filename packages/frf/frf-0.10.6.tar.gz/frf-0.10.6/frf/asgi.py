...
class ServiceNotAvailable(CanonicalException):
...
async def _handle_canonical_exception(request, exc, nolog=False):
    if isinstance(exc, ConnectionRefusedError)\
    or type(exc).__name__ == 'ConnectionRefusedError':
        # Assume here that an external service refused connection.
        return await _handle_canonical_exception(
            request,
            ServiceNotAvailable(),
            nolog=True
        )
    ...
    if isinstance(exc, CanonicalException):
        if exc.http_status_code >= 500 and not nolog:
...
def get_asgi_application(allowed_hosts=None, **kwargs):
    ...
    app = FastAPI(
        ...
        exception_handlers={
            ConnectionRefusedError: _handle_canonical_exception,
