...
#            tstr = str(0.0) + " " + str(dt) + " " + str(t1 - t0)
        ...
        if self.wave:
            ...
            exe_path = os.path.join('"' + cosmos.config.delft3dfm_exe_path,
                                    "x64\\dimr\\scripts\\run_dimr.bat" + '"' + " dimr_config.xml\n")
            fid.write(exe_path)
#            fid.write("set PATH=d:\\programs\\dflowfm\\2.05.04_60632\\x64\\share\\bin\\;%PATH%\n")
#            fid.write("set PATH=d:\\programs\\dflowfm\\2.05.04_60632\\x64\\dflowfm\\bin\\;%PATH%\n")
#            fid.write("set PATH=d:\\programs\\dflowfm\\2.05.04_60632\\x64\\dwaves\\bin\\;%PATH%\n")
#            fid.write("call d:\\programs\\dflowfm\\2.05.04_60632\\x64\dimr\\bin\\dimr dimr_config.xml\n")
            fid.write("move running.txt finished.txt\n")
            fid.write("exit\n")
            fid.close()
            
            # And the DIMR xml file
            ...
            fid = open(batch_file, "w")
            fid.write("@ echo off\n")
            fid.write("set dflowfmdir=d:\\programs\\dflowfm\\dflowfm-x64-1.1.204.49395\n")
            fid.write("copy %dflowfmdir%\\unstruc.ini .\\unstruc.ini\n")
            fid.write("DATE /T > running.txt\n")
            fid.write("%dflowfmdir%\\dflowfm.exe " + self.runid + ".mdu\n")
            fid.write("move running.txt finished.txt\n")
            fid.write("exit\n")
            fid.close()
