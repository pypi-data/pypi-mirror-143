...
def open_sftp_connection():
    ...
    hostname   = "c-oet0074.directory.intra"
    f = SSHSession(hostname, username=cosmos.config.username,
                       password=cosmos.config.password)
...
def upload_latest_json(f):
    ...
    target_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/latest.json"
    f.put(source_path, target_path)

def upload_tide_gauges(f):

    target_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/timeseries"
    try:
        ...
        if model.station and model.flow:
            for station in model.station:                
                if station.type == "tide_gauge" and station.upload:
                    ...
                    remote_file_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/timeseries/" + csv_file

                    f.put(local_file_path, remote_file_path)
                    if obs_file:
                        remote_file_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/timeseries/" + obs_file
    ...
    remote_file_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/stations.geojson"
    f.put(local_file_path, remote_file_path)
...
def upload_wave_buoys(f):
    ...
    remote_file_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/wavebuoys.geojson"
    f.put(local_file_path, remote_file_path)
...
def upload_wind_fields(f):
    ...
    target_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/gfs_conus.json"
    f.put(source_path, target_path)
...
def upload_png_tiles(f):
    ...
    if cosmos.config.make_flood_maps and os.path.exists(source_path):
        ...
        target_path = "../../opt/websites/static/deltares/cosmos/" + cosmos.scenario.name + "/tiles"
