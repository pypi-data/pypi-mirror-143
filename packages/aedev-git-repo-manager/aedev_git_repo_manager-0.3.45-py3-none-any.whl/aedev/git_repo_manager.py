...
from typing import (
    Any, AnyStr, Callable, Dict, Iterable, Iterator, List, Optional, OrderedDict as OrderedDictType, Sequence, Set,
    Tuple, Union, cast)
...
def pypi_versions(package_name: str) -> List[str]:
    ...
    output: List[str] = []
    # alternativ command `pip index versions {package_name}`, added in PIP v21 in June 2021 but is in March 2022 still
    ...
    if len(output) != 4 or start_marker not in output[1] or start_marker + 'none' in output[1]:
        return [""]
    return output[1][output[1].index(start_marker) + len(start_marker): -1].split(", ")


def refresh_templates(pdv: PdvType, *exclude_folders: str, logger: Callable = print, **replacer: Replacer) -> Set[str]:
    ...
    for tpl_prj in cast(List[RegisteredTemplateProject], pdv_val(pdv, 'tpl_projects')):
        ...
        for tpl_file_path in [file for file in glb(join(tpl_path, "**/.*")) + glb(join(tpl_path, "**/*")) if isf(file)]:
            dst_path = os.path.relpath(cast(str, os.path.dirname(tpl_file_path)), tpl_path)
            if not any(dst_path.startswith(folder_prefix) for folder_prefix in exclude_folders):
                if dst_path == tpl_folder:
                    dst_path = os.path.join(pdv_str(pdv, 'package_path'), dst_path)
                deploy_template(cast(str, tpl_file_path), dst_path, patcher, pdv,
...
def _children_desc(pdv: PdvType, children_pdv: Iterable[PdvType] = ()) -> str:
    namespace_name = pdv_str(pdv, 'namespace_name')
...
def _init_act_exec_args() -> Tuple[PdvType, str, tuple]:
    ...
    * run optional pre_action.
    ...
    if 'children_pdv' in act_spec['annotations']:   # and '_children' in act_name
        ...
        extra_msg += f" :: {len(act_args) - arg_count} {_children_desc(ini_pdv, children_pdv=act_args[arg_count:])}"
...
class GitlabCom(_RemoteHost):
    ...
    def release_children(self, ini_pdv: PdvType, *children_pdv: PdvType):
        """ release specified children projects to remotes/origin. """
        for chi_pdv in children_pdv:
    ...
    def request_children_merge(self, ini_pdv: PdvType, *children_pdv: PdvType):
        ...
        for chi_pdv in children_pdv:
            ...
            if chi_pdv != children_pdv[-1]:
                _wait()
        cae.po(f"  === requested the merge of {len(children_pdv)} {_children_desc(ini_pdv)}")
    ...
    def request_merge(self, ini_pdv: PdvType):
        ...
        .. note:: the name of the main branch is specified by the constant :data:`MAIN_BRANCH`.
        """
    ...
    def show_children_repos(self, ini_pdv: PdvType, *children_pdv: PdvType):
        ...
        cae.po(f"  === dumped info of {len(children_pdv)} {_children_desc(ini_pdv)}")
...
def clone_project(ini_pdv: PdvType, package_or_portion: str = "") -> str:
    ...
    cae.po(f"   == project {pkg_and_ver} cloned from {repo_root} into project path {project_path}")
...
def commit_children(ini_pdv: PdvType, *children_pdv: PdvType):
    ...
    cae.po(f"  === committed {len(children_pdv)} {_children_desc(ini_pdv, children_pdv=children_pdv)}")
...
def delete_children_file(ini_pdv: PdvType, file_name: str, *children_pdv: PdvType) -> bool:
    ...
    cae.po(f"  === wiped {file_name} in {len(c_del)}/{len(children_pdv)} {_children_desc(ini_pdv, children_pdv=c_del)}")
    return len(c_del) == 1 + len(children_pdv)
...
def install_children_editable(ini_pdv: PdvType, *children_pdv: PdvType):
    ...
    cae.po(f"  === installed {len(children_pdv)} {_children_desc(ini_pdv, children_pdv=children_pdv)}")
...
def new_children(ini_pdv: PdvType, *children_pdv: PdvType) -> List[PdvType]:
    ...
    cae.po(f"  === renewed {len(new_vars)} {_children_desc(ini_pdv, children_pdv=new_vars)}")
    return new_vars
...
def prepare_children_commit(ini_pdv: PdvType, title: str, *children_pdv: PdvType):
    ...
    cae.po(f"  === prepared commit of {len(children_pdv)} {_children_desc(ini_pdv, children_pdv=children_pdv)}")
...
def refresh_children_outsourced(ini_pdv: PdvType, *children_pdv: PdvType):
    ...
    cae.po(f"  === {len(children_pdv)} {_children_desc(ini_pdv, children_pdv=children_pdv)} refreshed")
...
def show_children_status(_parent_pdv: PdvType, *children_pdv: PdvType):
...
def show_children_versions(_parent_pdv: PdvType, *children_pdv: PdvType):
...
def update_children(_parent_pdv: PdvType, *children_pdv: PdvType):
