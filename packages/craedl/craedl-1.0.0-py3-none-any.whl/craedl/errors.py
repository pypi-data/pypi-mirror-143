...
class Invalid_Token_Error(Exception):
  def __init__(self):
    self.message = '''\n
Your configured authentication token is invalid. Obtain an API token from
https://craedl.org/#tokens, then
...
class Missing_Token_Error(Exception):
  def __init__(self):
    self.message = '''\n
You have not configured an authentication token. Obtain an API token from
https://craedl.org/#tokens, then
  (A) Configure your account through a system shell:
    `python -m craedl`
  (B) or configure your account through a Python interpreter:
...
class Not_A_Directory_Error(Exception):
  def __init__(self):
    self.message = '''\n
