...
class Client(ABC):
    ...
    def store_in_cache(
        self, object_id: int, object_type_id: int = None, checkout: bool = False
    ) -> List[ResultFile]:
        ...
        if result.return_code != 0:
            raise (
                RuntimeError(
                    f"Received return code {result.return_code}: {result.error_message}"
                )
            )
    ...
    def xml_import(
        ...
        for field_internal_name, field_value in search_fields.items():
            field = XmlElement(
                "Field", a={"internal_name": field_internal_name}, t=field_value
            )
        ...
        if table_fields != None and len(table_fields):
            ...
            for table_field_internal_name, table_field_rows in table_fields.items():
                table_field = XmlElement(
                    "TableField", {"internal_name": table_field_internal_name}
                )
    ...
    def get_object_details(
        ...
        for system_field in system_fields or []:
            query_field = XmlElement(
                "Field", {"internal_name": system_field.name, "system": "1"}
            )
                ...
                XmlElement(
                    ...
                    [
                        XmlElement(
                            ...
                            [
                                XmlElement(
                                    "Fields", {"field_schema": "ALL"}, query_fields
                                ),
                                XmlElement(
                                    "Conditions",
                                    {},
        ...
        obj_xml = result_xml["Archive"][0]["ObjectType"][0]["ObjectList"][0]["Object"][
            0
        ]
        ...
        if table_fields:
            for table_field in table_fields["TableField"]:
                table_field_result = []
                cols = [
                    col.attributes["internal_name"]
                    for col in table_field["Columns"][0]["Column"]
                ]
    ...
    def execute_sql(self, sql_command: str) -> Union[List[Dict[str, str]], None]:
        ...
        if result.files:
            ...
            data = [
                row.attributes
                for row in xml_result["{urn:schemas-microsoft-com:rowset}data"][0][
                    "{#RowsetSchema}row"
                ]
            ]
            return data
    ...
    def lol_query(
        ...
        include_file_info -- (Optional) bool indicating, if file size, file extension and mime type should be added to document results, default = False.
        follow_doc_links -- (Optional) bool indicating, the include_file_info returns the data of referenced files ("green arrows"), default = False.
        include_status -- (Optional) bool indicating, if status field should be added to the result, default = False.
        garbage_mode -- (Optional) bool indicating, if the query searches the recycle bin instead of non-deleted objects, default = False.
        context_user -- (Optional) string to change the job context to another user name.
        """
        ...
        if conditions:
            ...
            for internal_name in grouped_conditions:
                conditions_my_name = grouped_conditions[internal_name]
        ...
        if result_fields and len(result_fields) > 0:
            ...
            for result_field in (
                result_fields if isinstance(result_fields, list) else [result_fields]
            ):
                if isinstance(result_field, QueryResultField):
                    fields_element.append(result_field.to_xml_element())
                else:
                    fields_element.append(
                        QueryResultField(result_field).to_xml_element()
                    )
        ...
        while (not max_hits) or (combined_count <= max_hits):
            ...
            if result.return_code != 0:
                raise RuntimeError(
                    f"enaio error {result.return_code}: {result.error_message}"
                )
            ...
            if not columns:
                columns = {
                    column.attributes["internal_name"]: column.attributes["datatype"]
                    for column in rowset["Columns"][0]["Column"]
                }
            ...
            for row in rowset["Rows"][0]["Row"]:
                ...
                yield_result["OBJECT_INTERNAL_NAME"] = objecttype.attributes[
                    "internal_name"
                ]
                ...
                for name, value in zip(columns, row["Value"]):
                    ...
                    if datatype == "TEXT":
                        ...
                        yield_result[name] = (
                            datetime.strptime(value.text, "%Y-%m-%d").date()
                            if value.text
                            else None
                        )
                    elif datatype == "DATETIME":
                        yield_result[name] = (
                            datetime.strptime(value.text, "%Y-%m-%d %H:%M:%S")
                            if value.text
                            else None
                        )
                    else:
            ...
            if max_hits and (max_hits - combined_count == 0):
                ...
                job.update(
                    Param("PageSize", ParamTypes.INTEGER, max_hits - combined_count)
                )

            current_offset += page_size
            job.update(Param("Offset", ParamTypes.INTEGER, current_offset))
            logging.debug(
                f"Paging to next result frame with offset {current_offset} after {combined_count} combined hits."
            )
