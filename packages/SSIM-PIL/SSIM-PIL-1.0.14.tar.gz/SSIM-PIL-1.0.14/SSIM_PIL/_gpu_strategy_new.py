...
def _get_image_buffer(image0, image1):
    """
    Create the buffer object for a image
    :param image: PIL image object
    :return: CL buffer object
    """
    image0 = asarray(image0)
    image1 = asarray(image1)
    x, y, z = image0.shape
    layer = np.zeros(shape=(x, y, 4 - z), dtype=np.uint8)
    image0 = np.concatenate((image0, layer), axis=2)
    image1 = np.concatenate((image1, layer), axis=2)

    return cl.image_from_array(_context, image0, num_channels=4, mode="r", norm_int=False), \
           cl.image_from_array(_context, image1, num_channels=4, mode="r", norm_int=False)
...
def get_ssim_sum(image_0, image_1, tile_size: int, pixel_len: int, width: int, height: int, c_1: float, c_2: float) -> (float, bool):
    ...
    with Timer('CONV IMAGES', log_function=info):
        ...
        image_0, image_1 = _get_image_buffer(image_0, image_1)
