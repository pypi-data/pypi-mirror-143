...
class TradeGate:
    ...
    @staticmethod
    def _getQuantity(enterPrice, quantity, quoteQuantity, stepPercision):
        ...
        if quantity is None:
            if float(stepPercision) > 0.5:
                quantity = round(quoteQuantity / enterPrice, len(str(float(stepPercision))) - 3)
            else:
                quantity = round(quoteQuantity / enterPrice, len(str(float(stepPercision))) - 2)
        return quantity
