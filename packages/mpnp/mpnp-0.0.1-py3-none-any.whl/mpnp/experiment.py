...
def nkcv( X, y, k_outer, k_inner, model = 'DT', param_grid = [ { } ], scoring = 'accuracy', check_balance = True, n_jobs = None, verbose = 1, experiment_name = '', results_folder = './', random_state = None ):
	...
	if verbose > 0: print( 'experiment_name', experiment_name )
	...
	param_grid = set_param_grid( X, y, model, k_inner, check_balance, param_grid, random_state )
...
def set_param_grid( X, y, model, k_inner, check_balance, param_grid, random_state ):
	...
	if 'max_depth' not in param_grid[ 0 ]: 
		...
		# Paper based:
		# The usage on a tree over X and Y is based on -- https://scikit-learn.org/stable/auto_examples/tree/plot_cost_complexity_pruning.html#sphx-glr-auto-examples-tree-plot-cost-complexity-pruning-py -- However, all X and y is used and not just the training set. ?? ??
		...
		max_depth.extend( list( range( 1, max_depth_value + 1 ) ) ) # max_depth.extend( list( range( MIN, MAX + 1 ) ) ) # list( range( 5, 22 ) ) = [ 5, 6, ..., 20, 21 ]
	...
	if 'min_weight_fraction_leaf' not in param_grid[ 0 ]: 
		...
		param_grid[ 0 ][ 'min_weight_fraction_leaf' ] = min_weight_fraction_leaf # em um próximo experimento com class_weight = 'balanced' testar se este parâmetro tem influência ??
	...
	if 'max_features' not in param_grid[ 0 ]:  # int, float or {“auto”, “sqrt”, “log2”}, default= 'None' for DT and 'auto' for RF
		...
		param_grid[ 0 ][ 'max_features' ] = max_features # The numbers 0.2 and 0.5 are taken of my head. ??
	...
	if 'class_weight' not in param_grid[ 0 ]:
		...
		if check_balance == True:
			if ( is_balanced( y ) == False ): class_weight = [ 'balanced' ]
	...
	if model == 'DT':
		...
		if 'n_estimators' not in param_grid[ 0 ]:
			...
			n_estimators = [ 32, 64, 128, 256, 512, 1024, 2048, 4096 ] # It does not make sense to create RFs with fewer DTs
		...
		if 'max_samples' not in param_grid[ 0 ]:
			...
			print( len( param_grid[ 0 ][ 'bootstrap' ] ) )
			print( param_grid[ 0 ][ 'bootstrap' ][ 0 ] )
...
def summarize_results( results_folder = './', fold_results = False, calc_ss_matrix = True ):
