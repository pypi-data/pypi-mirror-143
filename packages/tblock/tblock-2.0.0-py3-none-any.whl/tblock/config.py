...
class Var:
    ...
    try:
        DEFAULT_PERMISSIONS = FilterPermissions(__config.get("default", "default_permissions"))
    except (configparser.NoSectionError, configparser.NoOptionError, ValueError):
        DEFAULT_PERMISSIONS = FilterPermissions('b')
    try:
        DEFAULT_IP = __config.get("default", "default_ip")
    except (configparser.NoSectionError, configparser.NoOptionError, ValueError):
        DEFAULT_IP = "127.0.0.1"
    try:
        DEFAULT_IPV6 = __config.get("default", "default_ipv6")
    except (configparser.NoSectionError, configparser.NoOptionError, ValueError):
        DEFAULT_IPV6 = "::1"
    try:
        ALLOW_IPV6 = __config.getboolean("default", "allow_ipv6")
    except (configparser.NoSectionError, configparser.NoOptionError, ValueError):
        ALLOW_IPV6 = False
    try:
        PREFER_ONION = __config.getboolean("default", "prefer_onion")
    except (configparser.NoSectionError, configparser.NoOptionError, ValueError):
        PREFER_ONION = False
    ...
    if PREFER_ONION:
        REPO_MIRRORS = [
            ...
            "https://codeberg.org/tblock/repo/raw/branch/2.0.0/index.xml",
            "https://framagit.org/twann/repo/-/raw/2.0.0/index.xml",
            "https://git.disroot.org/tblock/repo/raw/branch/2.0.0/index.xml",
            "https://0xacab.org/twann/repo/-/raw/2.0.0/index.xml"
        ]
    else:
        REPO_MIRRORS = [
            "https://codeberg.org/tblock/repo/raw/branch/2.0.0/index.xml",
            ...
            "https://git.disroot.org/tblock/repo/raw/branch/2.0.0/index.xml",
            "https://0xacab.org/twann/repo/-/raw/2.0.0/index.xml",
            "http://wmj5kiic7b6kjplpbvwadnht2nh2qnkbnqtcv3dyvpqtz7ssbssftxid.onion/twann/repo/-/raw/2.0.0/index.xml"
        ]
...
def hosts_are_safe() -> bool:
    ...
    with sqlite3.connect(Path.DATABASE) as conn:
        hosts_backup_shasum = conn.cursor().execute(
            "SELECT value FROM system WHERE variable='hosts_shasum';").fetchone()[0]
