...
class Arguments:
    ...
    def add_operation(self, operation: str, value_type, description: str = None,
        ...
        :param default_value. Specify the default value to use if the argument is not passed. Defaults to None or False for bool.
        :return:
        """
        if operation in self.operations.keys():
            ...
            raise InvalidOperationError(f'short operation can only be one-character long when no dash, not "{short_operation}"')
        elif short_operation is not None and len(short_operation) > 2:
            raise InvalidOperationError(f'short operation can contain a dash and a character maximum, not "{short_operation}"')
            ...
            if default_value is None and value_type == bool:
                ...
                raise ArgumentValueError(f'default value for operation {operation} is from type "{type(default_value)}" instead of "{value_type}"')
    ...
    def add_operation_group(self, group_name: str, name: str = None) -> None:
        ...
        :param name: Optional. Specify a different name to show on the help page. This can be useful for multi-language support
        :return:
        """
    ...
    def add_option(self, option: str, value_type, description: str = None,
        ...
        if option in self.options.keys():
            ...
            raise InvalidOptionError(f'short option must start with a dash and finish with a character, not "{short_option}"')
        else:
            if default_value is None and value_type == bool:
                default_value = False
            elif default_value is not None and type(default_value) != value_type:
                raise ArgumentValueError(f'default value for operation {option} is from type "{type(default_value)}" instead of "{value_type}"')
    ...
    def __match_option(self, option_title: str, real_option: str, real_argument: str) -> None:
        ...
        if self.active_operation is not None and self.options[option_title]['available_with_operations'] is not None \
            ...
            raise SystemExit(1)
                ...
                and self.operations[self.active_operation]['group'] not in self.options[option_title]['available_with_groups']:
            print(f'Error: option "{real_option}" is not available with operation "{self.active_operation}"')
            raise SystemExit(1)
    ...
    def parse(self) -> [dict, dict]:
        ...
        for argument in self.arguments:
            ...
            if argument in self.operations.keys():
                ...
                self.output_operations[self.active_operation].append(argument)
                    ...
                    and self.operations[self.active_operation]['value_type'] == str and self.output_operations[self.active_operation] is None:

                self.output_operations[self.active_operation] = argument

            elif self.isolate_unmatched and self.active_operation is not None and not re.match(r'-[A-z0-9_.\-]*', argument) \
                    and self.operations[self.active_operation]['value_type'] == int and self.output_operations[self.active_operation] is None:
        ...
        # Check if all active operations that require a list, a str or a int have at least one value
        for active_operation in self.output_operations.keys():
            if self.operations[active_operation]['value_type'] == list and self.output_operations[active_operation] == []:
                ...
                raise SystemExit(1)
            elif self.operations[active_operation]['value_type'] in (str, int) and self.output_operations[active_operation] is None:
                print(f'Error: operation "{active_operation}" requires at least one argument')
                raise SystemExit(1)
        ...
        for option in self.options:
            ...
            if self.options[option]['required_by_groups'] is not None and option not in self.output_options:
                for required_group in self.options[option]['required_by_groups']:
                    for operation in self.operations:
                        if self.operations[operation]['group'] == required_group:
