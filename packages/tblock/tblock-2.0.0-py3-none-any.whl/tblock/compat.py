...
def setup_new_database(quiet: bool = False) -> None:
    """
    Create a new database, compatible with TBlock version 2.0.0

    :param quiet: Optional. Do not display an output (false by default)
    """
    if not quiet:
        print(f'{Font.BOLD}==> Creating new database{Font.DEFAULT}')
        log_message(f"[core]    INFO:  creating new sqlite database under: {Path.DATABASE}")
    with sqlite3.connect(Path.DATABASE) as conn:
        conn.cursor().execute('''CREATE TABLE IF NOT EXISTS "filters" (
        ...
        conn.cursor().execute('''CREATE TABLE IF NOT EXISTS "rules" (
        ...
        conn.cursor().execute('''CREATE TABLE IF NOT EXISTS "system" (
...
def update_database_1_3_2_to_2_0_0(quiet: bool = False) -> None:
    """
    :param quiet: Optional. Do not display an output (false by default)
    """
    if not quiet:
        print(f'{Font.BOLD}==> Updating database from v1.3.2 to v2.0.0{Font.DEFAULT}')
        log_message(f"[core]    INFO:  updating sqlite database from v1.3.2 to v2.0.0 under: {Path.DATABASE}")
    ...
    with sqlite3.connect(Path.DATABASE) as conn:
        ...
        cursor.execute('INSERT INTO rules_backup SELECT domain,policy,filter_id,ip FROM rules;')
        ...
        cursor.execute('UPDATE rules SET policy=? WHERE policy=?', (RulePolicy.ALLOW, "allow"))
        cursor.execute('UPDATE rules SET policy=? WHERE policy=?', (RulePolicy.BLOCK, "block"))
        cursor.execute('UPDATE rules SET policy=? WHERE policy=?', (RulePolicy.REDIRECT, "redirect"))
    conn.commit()
    ...
    if os.path.isfile(os.path.join(Path.PREFIX, 'repo')):
        with open(os.path.join(Path.PREFIX, 'repo'), "rt") as r:
            conn.cursor().execute('INSERT INTO system (variable, value) VALUES ("repo_version", ?);', (int(r.read()),))
    ...
    if os.path.isfile(os.path.join(Path.PREFIX, 'hosts_protection')):
        ...
        if hosts_shasum == hosts_backup_shasum:
            conn.cursor().execute('UPDATE system SET value=? WHERE variable="hosts_shasum";', (hosts_shasum,))
        os.remove(os.path.join(Path.PREFIX, 'hosts_protection'))
    conn.commit()


def update_database_1_2_0_to_2_0_0(quiet: bool = False) -> None:
    """
    :param quiet: Optional. Do not display an output (false by default)
    """
    if not quiet:
        print(f'{Font.BOLD}==> Updating database from v1.2.0 to v2.0.0{Font.DEFAULT}')
        log_message(f"[core]    INFO:  updating sqlite database from v1.2.0 to v2.0.0 under: {Path.DATABASE}")
    with sqlite3.connect(Path.DATABASE) as conn:
        conn.cursor().execute('ALTER TABLE "filters" ADD COLUMN "mirrors";')
        conn.commit()
    update_database_1_3_2_to_2_0_0(quiet=True)
...
def init_db(quiet: bool = False) -> None:
    ...
    if not os.path.isfile(Path.DATABASE):
        ...
        try:
            ...
            update_database_1_2_0_to_2_0_0(quiet=quiet)
        else:
            try:
                with sqlite3.connect(Path.DATABASE) as conn:
                ...
                update_database_1_3_2_to_2_0_0(quiet=quiet)
