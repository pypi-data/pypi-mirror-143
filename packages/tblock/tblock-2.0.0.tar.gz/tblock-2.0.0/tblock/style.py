...
def status_tblock_enabled() -> str:
    if not hosts_are_default():
        return Fore.GREEN + "active"
    else:
        return Fore.RED + "inactive"
...
def status_hosts_hijack() -> str:
    if hosts_are_safe():
        return Fore.GREEN + "undetected"
    elif hosts_are_default():
        return Fore.RED + "tblock is inactive"
    else:
        return Fore.RED + "detected"


def status_daemon_running() -> str:
    if os.path.isfile(Path.DAEMON_PID):
        return Fore.GREEN + "running"
    else:
        return Fore.RED + "not running"
...
def show_status() -> None:
    ...
    with sqlite3.connect(Path.DATABASE) as db:
        ...
        total_filter_lists = db.cursor().execute('SELECT COUNT() FROM filters WHERE subscribing=1;').fetchone()[0]
    print(
