...
class SmartIq_Helper_v2():
    ...
    def get_quotation(self, car):
        ...
        if retail:
            return self.return_valid_quotation(retail, io, matching_labels_ids[5][2], 'OK (cache)')
        ...
        try:
            ...
            if 'prediction' not in data:
                return self.return_error_quotation(f"trim={matching_labels_ids[5][2]}, error={data['status_code']}")
            ...
            return self.return_valid_quotation(retail, io, matching_labels_ids[5][2], 'OK (request)')
    ...
    def get_cached_price(self, vehicle_str, km):
        ...
        # logging.info(f"a={(x1, y1)}, b={x2, y2}, c0={c0}, c1={c1} ")
        if not all([x1, x2, c0, c1]) or not int(x1) <= km <= int(x2): # only interpolate, do not extrapolate
            return None, None
    ...
    def cache_price(self, vehicle_str, labels, km, retail, io):
        ...
        if not x1: # First point
            self.redis_conn.hmset('siq-price:' + vehicle_str, {'x1': km, 'y1': retail})
            return

        x1, y1 = int(x1), int(y1)
        if abs(x1 - km) < 100: # to close, ignore
            return
