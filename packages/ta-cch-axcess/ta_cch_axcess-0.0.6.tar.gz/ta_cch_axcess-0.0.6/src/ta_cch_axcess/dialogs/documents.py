...
class Documents(CchDialog):
    ...
    def get_files_by_client_id(self, client_id, folder_name):
        ...
        self.window.child_window(auto_id="txtClientFilter", class_name="TextBox").click_input()
        self.window.child_window(auto_id="txtClientFilter", class_name="TextBox").type_keys(client_id + "{ENTER}")
        self.window.child_window(auto_id="txtClientFilter", class_name="TextBox").type_keys("{DOWN}{ENTER}")

        try:
            self.window.child_window(auto_id="cmbRecordsPerPage").wait("ready", timeout=15)
            self.window.child_window(auto_id="cmbRecordsPerPage").select("2000")
                    ...
                    if "FileSearchResultList" in result.legacy_properties()["Name"]:
                        ...
                        for datagrid in datagrids:
                            if "Records" in datagrid.legacy_properties()["Name"]:
                                ...
                                cell_to_click = list_data[int(files_number) - 1].children(class_name="Cell")
