...
class Size:
    ...
    def size_property(fre):
        latest_asofdate = pd.read_sql("select max(asofdate) as asofdate from nav_style_property_size where fre='{0}'".format(fre), con=localdb)['asofdate'][0]
        sql = "select jjdm, shift_ratio, centralization, shift_ratio_rank, centralization_rank, 大盘_mean, 中盘_mean, 小盘_mean, manager_change, asofdate from nav_style_property_size where fre='{0}' and asofdate='{1}'".format(fre, latest_asofdate)
    ...
    def get_size(self, fre, show_num=200, shift_ratio_threshold=0.5, centralization_threshold=0.5):
        ...
        size_property.columns = ['jjdm', '换手率', '集中度', '换手率排名', '集中度排名', '大盘', '中盘', '小盘', '基金经理变更', 'asofdate']
        ...
        big_size = big_size[['jjdm', 'jjjc', 'asofdate', '大盘', '基金经理变更']]
        medium_size = medium_size[['jjdm', 'jjjc', 'asofdate', '中盘', '基金经理变更']]
        small_size = small_size[['jjdm', 'jjjc', 'asofdate', '小盘', '基金经理变更']]
...
class Value:
    ...
    def value_property(fre):
        latest_asofdate = pd.read_sql("select max(asofdate) as asofdate from nav_style_property_value where fre='{0}'".format(fre), con=localdb)['asofdate'][0]
        sql = "select jjdm, shift_ratio, centralization, shift_ratio_rank, centralization_rank, 成长_mean, 价值_mean, manager_change, asofdate from nav_style_property_value where fre='{0}' and asofdate='{1}'".format(fre, latest_asofdate)
    ...
    def get_value(self, fre, show_num=200, shift_ratio_threshold=0.5, centralization_threshold=0.5):
        ...
        value_property.columns = ['jjdm', '换手率', '集中度', '换手率排名', '集中度排名', '成长', '价值', '基金经理变更', 'asofdate']
        ...
        growth = growth[['jjdm', 'jjjc', 'asofdate', '成长', '基金经理变更']]
        value = value[['jjdm', 'jjjc', 'asofdate', '价值', '基金经理变更']]
        ...
        value = value.merge(holding_property.drop('asofdate', axis=1), on=['jjdm'], how='left')
        absolute_pe_value = value.sort_values('PE排名')
        absolute_pb_value = value.sort_values('PB排名')
        relative_pe_value = value.sort_values('PE相对行业均值排名')
        relative_pb_value = value.sort_values('PB相对行业均值排名')
        dividend_value = value.sort_values('股息率排名', ascending=False)
        reverse_value = value.sort_values('ROE排名')
        high_quality_value = value.sort_values('ROE排名', ascending=False)

        absolute_pe_value = absolute_pe_value[['jjdm', 'jjjc', 'asofdate', '价值', 'PE排名', '基金经理变更']]
        absolute_pb_value = absolute_pb_value[['jjdm', 'jjjc', 'asofdate', '价值', 'PB排名', '基金经理变更']]
        relative_pe_value = relative_pe_value[['jjdm', 'jjjc', 'asofdate', '价值', 'PE相对行业均值排名', '基金经理变更']]
        relative_pb_value = relative_pb_value[['jjdm', 'jjjc', 'asofdate', '价值', 'PB相对行业均值排名', '基金经理变更']]
        dividend_value = dividend_value[['jjdm', 'jjjc', 'asofdate', '价值', '股息率排名', '基金经理变更']]
        reverse_value = reverse_value[['jjdm', 'jjjc', 'asofdate', '价值', 'ROE排名', '基金经理变更']]
        high_quality_value = high_quality_value[['jjdm', 'jjjc', 'asofdate', '价值', 'ROE排名', '基金经理变更']]
