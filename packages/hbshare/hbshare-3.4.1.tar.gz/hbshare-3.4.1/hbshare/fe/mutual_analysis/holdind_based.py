...
class General_holding:
    ...
    def get_hld_property(self,jjdm_list,start_date,end_date,add=False):
        ...
        hld = hld.reset_index()
        hld = hld.merge(stock_industry[['TICKER_SYMBOL', 'INDUSTRY_NAME', 'BEGIN_DATE', 'END_DATE']].rename(columns={'TICKER_SYMBOL': 'zqdm'}), on=['zqdm'], how='left')
        hld = hld[(hld['jsrq'] >= hld['BEGIN_DATE']) & (hld['jsrq'] < hld['END_DATE'])]
        cal, trade_cal = self.get_cal_and_trade_cal('20180101', '20211231')
        hld = hld.merge(cal[['TRADE_DATE', 'RECENT_TRADE_DATE']].rename(columns={'TRADE_DATE': 'jsrq', 'RECENT_TRADE_DATE': 'TRADE_DATE'}), on=['jsrq'], how='left')
        ...
        stock_valuation = stock_valuation.rename(columns={'SECUCODE': 'TICKER_SYMBOL', 'TRADINGDAY': 'TRADE_DATE', 'TOTALMV': 'MARKET_VALUE', 'PE': 'PE', 'PB': 'PB'})
        star_stock_valuation = star_stock_valuation.rename(columns={'SECUCODE': 'TICKER_SYMBOL', 'TRADINGDAY': 'TRADE_DATE', 'TOTALMV': 'MARKET_VALUE', 'PE': 'PE', 'PB': 'PB'})
        stock_valuation = stock_valuation[['TRADE_DATE', 'TICKER_SYMBOL', 'MARKET_VALUE', 'PE', 'PB']]
        star_stock_valuation = star_stock_valuation[['TRADE_DATE', 'TICKER_SYMBOL', 'MARKET_VALUE', 'PE', 'PB']]
        stock_valuation = pd.concat([stock_valuation, star_stock_valuation]).sort_values(['TRADE_DATE', 'TICKER_SYMBOL'])
        stock_valuation = stock_valuation.merge(stock_industry[['TICKER_SYMBOL', 'INDUSTRY_NAME', 'BEGIN_DATE', 'END_DATE']], on=['TICKER_SYMBOL'], how='left')
        stock_valuation = stock_valuation[(stock_valuation['TRADE_DATE'] >= stock_valuation['BEGIN_DATE']) & (stock_valuation['TRADE_DATE'] < stock_valuation['END_DATE'])]
        stock_valuation = stock_valuation[stock_valuation['TRADE_DATE'].isin(hld['TRADE_DATE'].unique().tolist())]
        industry_pe_valuation = stock_valuation[['TRADE_DATE', 'INDUSTRY_NAME', 'PE']].groupby(['TRADE_DATE', 'INDUSTRY_NAME']).mean().reset_index().rename(columns={'PE': 'PE_indu'})
        industry_pb_valuation = stock_valuation[['TRADE_DATE', 'INDUSTRY_NAME', 'PB']].groupby(['TRADE_DATE', 'INDUSTRY_NAME']).mean().reset_index().rename(columns={'PB': 'PB_indu'})
        stock_valuation = stock_valuation.merge(industry_pe_valuation, on=['TRADE_DATE', 'INDUSTRY_NAME'], how='left')
        stock_valuation = stock_valuation.merge(industry_pb_valuation, on=['TRADE_DATE', 'INDUSTRY_NAME'], how='left')
        stock_valuation['pe_rel'] = stock_valuation['PE'] - stock_valuation['PE_indu']
        stock_valuation['pb_rel'] = stock_valuation['PB'] - stock_valuation['PB_indu']

        hld = hld.merge(stock_valuation[['TICKER_SYMBOL', 'TRADE_DATE', 'pe_rel', 'pb_rel']].rename(columns={'TICKER_SYMBOL': 'zqdm'}), on=['TRADE_DATE', 'zqdm'], how='left')
        hld = hld.set_index('jsrq').drop(['INDUSTRY_NAME', 'BEGIN_DATE', 'END_DATE', 'TRADE_DATE'], axis=1)
        ...
        for jjdm in new_jjdm_list:
            ...
            financial_property = full_hld.groupby('jsrq').mean()[['pe_rel', 'pb_rel']].mean()
            pe_rel_list.append(financial_property['pe_rel'])
            pb_rel_list.append(financial_property['pb_rel'])
        ...
        outputdf['PE_REL'] = pe_rel_list
        outputdf['PB_REL'] = pb_rel_list
        outputdf['asofdate']=asofdate
        ...
        outputdf[[x + "_rank" for x in ['PE_REL', 'PB_REL']]] = \
            outputdf[['PE_REL', 'PB_REL']] \
                .rank(method='min') / len(outputdf)
