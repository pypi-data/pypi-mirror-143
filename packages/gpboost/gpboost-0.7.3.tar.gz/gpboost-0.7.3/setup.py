...
def find_lib() -> List[str]:
    ...
    logger.info(f"Installing lib_gpboost from: {LIB_PATH}")
    return LIB_PATH
...
def copy_files(integrated_opencl: bool = False, use_gpu: bool = False) -> None:
    ...
    if not IS_SOURCE_FLAG_PATH.is_file():
        ...
        # copyfile(CURRENT_DIR.parent / "windows" / "GPBoost.sln",
        #          CURRENT_DIR / "compile" / "windows" / "GPBoost.sln")
        # copyfile(CURRENT_DIR.parent / "windows" / "GPBoost.vcxproj",
        #          CURRENT_DIR / "compile" / "windows" / "GPBoost.vcxproj")
...
def compile_cpp(
    ...
    if system() in {'Windows', 'Microsoft'}:
        if use_mingw:
            ...
            status = 1
            lib_path = CURRENT_DIR / "compile" / "windows" / "x64" / "DLL" / "lib_gpboost.dll"
            ## Installion using VS solution currently not supported
            ...
            #                               str(CURRENT_DIR / "compile" / "windows" / "GPBoost.sln"),
            #                               "/p:Configuration=DLL",
            #                               "/p:Platform=x64",
...
class CustomInstallLib(install_lib):
    ...
    def install(self) -> List[str]:
        ...
        src = find_lib()[0]
        dst = Path(self.install_dir) / 'gpboost'
...
class CustomSdist(sdist):
    ...
    def run(self) -> None:
        ...
        rmtree(CURRENT_DIR / 'gpboost' / 'Release', ignore_errors=True)
        rmtree(CURRENT_DIR / 'gpboost' / 'windows' / 'x64', ignore_errors=True)
        lib_file = CURRENT_DIR / 'gpboost' / 'lib_gpboost.so'
        if lib_file.is_file():
            lib_file.unlink()
        sdist.run(self)
...
if __name__ == "__main__":
    ...
    LOG_PATH = Path.home() / 'GPBoost_compilation.log'
    ...
    _version_dst = CURRENT_DIR / 'gpboost' / 'VERSION.txt'
