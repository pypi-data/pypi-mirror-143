...
from AOMUSE.db.Database import Exposure, Processed_Exposure, db, create_conn
import pandas as pd
import json
...
class muse_db():
    ...
    def __init__(self, db, provider, host, user, passwd, database):
        create_conn(db, provider=provider, host=host, user=user, passwd=passwd, db=database)
    ...
    def get_proocessed_exposures(self):
        pes = select(pe for pe in Processed_Exposure)
        data = []
        columns = [
            "observation_time", "target", "obs_id", "insMode", 
