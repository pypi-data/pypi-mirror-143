...
class TestIAEJobRun(unittest.TestCase):
    ...
    def test_negative_scenarios(self):
        ...
        try:
            client = IAEInstanceClient(
                server_url, service_instance_name, volume, token)
    ...
    def test_get_job_payload(self):
        iae_client = IAEEngineClient(
            "server_url", "instance_name", "volume", "token")
        job_params = {
            "spark_settings": {
                "max_num_executors": 4,
                "executor_cores": 1,
        ...
        job_payload = iae_client.get_job_payload(
            'Sample', 'SampleJob', job_params, 'temp_file')[0]
        assert job_payload is not None
        assert "conf" in job_payload["engine"]
        assert job_payload["engine"]["conf"] == job_params["conf"]
        assert "env" in job_payload["engine"]
        for k, v in job_params["env"].items():
            assert k in job_payload["engine"]["env"]
            assert v == job_payload["engine"]["env"][k]
        ...
        assert job_payload is not None
        assert "conf" in job_payload["engine"]
        assert job_payload["engine"]["conf"] == {"spark.app.name": "Sample"}
