...
from ibm_wos_utils.joblib.utils.constants import UPLOAD_FILE_RETRY_COUNT, UPLOAD_FILE_RETRY_STATUS_CODES, IAE_VOLUME_MOUNT_PATH
...
class IAEInstanceClient():
    ...
    def _get_instance(self, name=None):
        ...
        if instance is None:
            raise ObjectNotFoundError(
                "Spark instance with name '{}' could not be found.".format(name))
        return instance
    ...
    def run_job(self, job_json, background=True, timeout=constants.SYNC_JOB_MAX_WAIT_TIME):
        ...
        # Check resource quotas for the spark instance
        resource_quota = self.get_resource_quota(spark_instance.instance_url)
        self.check_resource_quota(
            resource_quota, job_json)
        logger.info("Submitting spark job to IAE instance {}. Job URL: {}".format(
            spark_instance.instance_id, job_url))
        ...
        print("\nSuccessfully submitted spark job to IAE instance {}. Job ID: {}, Status {}".format(
            spark_instance.instance_id, job_id, state))
    ...
    def get_job_state(self, job_id, instance_token=None, job_url=None):
        ...
        job_response = response.json()
        logger.info("Response of GET job status API: {}".format(
            str(job_response)))
        job_state = None
        driver_state = None
        ...
        if str(job_state).lower() in constants.JOB_FINISHED_STATES and str(driver_state).lower() not in constants.JOB_FINISHED_STATES:
            job_state = driver_state
        return job_state
...
class IAESparkInstance():
    def __init__(self, server_url, instance):
        ...
        connection_info = instance["CreateArguments"]["connection-info"]
        spark_jobs_endpoint = connection_info.get("Spark jobs V2 endpoint")
        if spark_jobs_endpoint is None or spark_jobs_endpoint.strip() == "":
            spark_jobs_endpoint = connection_info.get("Spark jobs endpoint")
        self.jobs_url = spark_jobs_endpoint.replace(
            "$HOST", server_url)
        # Add instance_url which is used to check available resource quota
        # If it is V3 instance then use V3 endpoint for resource quota APIs
        instance_endpoint = connection_info.get("Spark jobs V3 endpoint")
        if instance_endpoint:
            instance_endpoint = instance_endpoint.split(
                "/spark/applications")[0]
        else:
