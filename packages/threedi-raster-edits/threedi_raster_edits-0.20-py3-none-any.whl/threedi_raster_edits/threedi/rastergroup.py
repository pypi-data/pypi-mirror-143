...
class ThreediRasterGroup(RasterGroup):
    ...
    def null_raster(self):
        ...
        copy = self.base_raster.copy(shell=True)
    ...
    def generate_infiltration(self):
        ...
        output = self.base_raster.copy(shell=True)
    ...
    def write(self, folder_path, output_names: dict = OUTPUT_NAMES):
        ...
        for table_name, table_path in output_names.items():
            ...
            if hasattr(self, table_name):
                ...
                final_paths[table_name] = str(p / table_path)
        return final_paths  # primarily used for qgis
...
def isnumber(number):
    ...
    try:
        float(number)
    except (ValueError, TypeError):  # also expect a none type
        return False
    else:
        return True
