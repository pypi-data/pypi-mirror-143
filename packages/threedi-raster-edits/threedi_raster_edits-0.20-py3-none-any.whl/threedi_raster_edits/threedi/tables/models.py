...
# local imports
from threedi_raster_edits.threedi.vector import ThreediVector
from threedi_raster_edits.threedi.vector import ThreediDemVector
...
from threedi_raster_edits import LineString
...
class GlobalSettings(ThreediDemVector):
    required = {"scenario": 1, "dem": None}

    def __init__(self, ds, layer_name, dem, scenario=1):
        ThreediDemVector.__init__(self, ds, layer_name, dem)
...
class Channel(ThreediVector):
    constraints = {}
    required = {"cross_section_locations": None}

    def __init__(self, ds, layer_name, cross_section_locations):
        super().__init__(ds, layer_name)
...
class Manhole(ThreediDemVector):
    constraints = {"drain_level": {"comparison": (operator.ge, "bottom_level")}}

    def __init__(self, ds, layer_name, dem):
        super().__init__(ds, layer_name, dem)
...
class CrossSectionDefinition(ThreediVector):
    ...
    def __getitem__(self, fid):
        ...
        # add specific csd functions for feature/row
        # if feature["shape"] == 6:
        #     feature.plot = self.custom_plot(feature)
...
class Obstacle(ThreediDemVector):
...
class GridRefinementArea(ThreediVector):
    required = {"global_setting": None}

    constraints = {}

    def __init__(self, ds, layer_name, global_setting):
        super().__init__(ds, layer_name)
