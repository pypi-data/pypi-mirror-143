...
import numpy as np
import sys
from thepack import credentials
...
def get_pack_sso_token(system_name = 'sinppack_admin'):
    if sys.platform == 'win32':
        session = requests.Session()
        session.trust_env = False
        r = session.get("https://packsso:10002", auth=HttpNegotiateAuth(), verify=False)
    else:
        session = requests.Session()
        username, password = credentials.get_credentials(system_name)
        session.auth = HttpNtlmAuth(username, password)
        r = session.get("https://packsso:10002", verify=False)
    return r.json()['jwt']
...
def _get_all(url, env='local'):
    ...
    resp = requests.get("{}/{}/".format(base_url, url), headers=headers, verify=False, proxies=proxies)
    try:
