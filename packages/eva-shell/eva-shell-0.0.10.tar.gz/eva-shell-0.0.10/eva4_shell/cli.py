...
class CLI:
    ...
    def update(self, repository_url, yes, info_only, unsafe, test):
        import json
        from .tools import safe_download, set_file_lock, remove_file_lock, get_arch, get_public_key
        arch = get_arch()
        ...
        try:
            ...
            if build == new_build:
                return
            if build > new_build:
                raise RuntimeError('Your build is newer than update server has')
            manifest = json.loads(
            ...
            try:
                ...
                for f in update_files:
                    with open(f, 'wb') as fh:
                        fh.write(
                            safe_download(
                                f'{repository_url}/{new_version}/nightly/{f}',
                                manifest=manifest,
