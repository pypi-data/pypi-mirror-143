    ...
    if manifest:
        ...
        if public_key:
            try:
                ...
                pyaltt2.crypto.verify_signature(r.content, signature,
                                                public_key)
            except ModuleNotFoundError:
                raise RuntimeError(
                    'Please either install pyaltt2, pycryptodomex and cryptography Python modules or '
                    'download files without signature verifications')
            except:
...
def get_public_key():
    if common.public_key is None:
        with open(f'{common.dir_eva}/share/eva.key') as fh:
            common.public_key = fh.read()
