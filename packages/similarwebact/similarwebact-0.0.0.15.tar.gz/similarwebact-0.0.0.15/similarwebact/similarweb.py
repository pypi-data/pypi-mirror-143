...
# def refineSWFrame_SA(segment_id, start_date, end_date, site_code, metric, description):
#     dataFrame = getSimilarwebData(inputConverter('website', company), 'null', start_date, end_date, inputConverter('site_code', site_code), channel, 'conversion')
#     defaultColumn(dataFrame, site_code)
#     dataFrame.insert(2, "device_type", 'desktop', True)
#     dataFrame.insert(3, "company_name", inputConverter('company', company), True)
        ...
        for j in range(len(site_code)):
            for p in range(len(device_type)):
                while True:
                    ...
                    break
## New
# def SW_segmentAnalysis(start_date, end_date, site_code, dbTableName, metric, segment_id, description):
#     for i in range(len(segment_id)):
#         for j in range(len(site_code)):
#             while True:
#                 try:
#                     stackTodb(refineSWFrame_SA(segment_id[i], start_date, end_date, site_code[j], metric, description[i]), dbTableName)
            
#                 except urllib.error.HTTPError:
#                     print('No Matching Data : {segment_id} X {site_code}'.format(segment_id = segment_id[i], site_code = site_code[j]))
...
# visit
    ...
    # SW_segmentAnalysis(start_date, end_date, site_code, dbTableName, metric, segment_id, description)