...
class Type(t.Generic[S, I], Describable, abstract=True):
    ...
    def gettypeattr(self, typeattr: t.Union[str, int, None]) -> Type:
        ...
        To traverse a MappingType, either use 'keys', 'values', or 'items'::
           ...
           TupleType(bases=(StringType(), IntegerType()))
    ...
    def traverse(self, path: t.Sequence[t.Union[str, int]]) -> Type:
...
class Selectable(t.Generic[T]):
    ...
    @property
    def selection_full_repr(self):
        if self.selection is not None:
            return {
                "selection": [
                    {"value": s.value, "name": s.name} for s in self.selection
                ]
            }
        else:
            return {}
    ...
    @classmethod
    def _from_full_repr_arg_selection(cls, val) -> Selection:
        return Selection.from_pairs(
            (item["value"], item["name"]) for item in val
        )

    def _ensure_value_in_selection(self, value: T):
        "Raise a ValueError is value is not valid in the selection"
        if self.selection and not any(
            sel.value == value for sel in self.selection
        ):
            raise ValueError(
...
class MinMax(t.Generic[TOrd]):
    ...
    @property
    def minmax_full_repr(self):
        res = {}
        if self.min_value is not None:
            res["min_value"] = self.dump(self.min_value, validate=False)
            res["min_included"] = self.min_included
        if self.max_value is not None:
            res["max_value"] = self.dump(self.max_value, validate=False)
            res["max_included"] = self.max_included
        return res
    ...
    def __repr__(self):
        args = self.minmax_full_repr
        args_repr = ", ".join(f"{name}={val!r}" for name, val in args.items())
        Type = type(self).__name__
        return f"{Type}({args_repr})"
...
class BaseNumberType(Type, MinMax[TOrd], Selectable[T], abstract=True):
    ...
    def full_repr(self):
        return {
            ":ns:": self.namespace,
            ":base:": self.constructor_name,
            **self.minmax_full_repr,
            **self.selection_full_repr,
        }
    ...
    def __repr__(self):
        ...
        if self.selection:
            if args_repr:
                args_repr += f", selection={self.selection}"
            else:
                args_repr = f"selection={self.selection}"
        Type = type(self).__name__
        return f"{Type}({args_repr})"
    ...
    *serialized form*.
    ...
    def __repr__(self):
        if not self.selection:
            ...
            return f"StringType(selection={self.selection})"
    ...
    @property
    def full_repr(self):
        res = {":ns:": self.namespace, ":base:": self.constructor_name}
        res.update(self.selection_full_repr)
        if not self.selection and (max_length := self.max_length) is not None:
    ...
    def _parse_dump(self, value: str, *, validate: bool = True) -> str:
        if not isinstance(value, str):
            raise TypeError(
                f"Unexpected type of value {value!r}, expected a str"
            )
        if validate:
            self._ensure_value_in_selection(value)
            if not self.selection and self.max_length:
                if len(value) > self.max_length:
                    raise TypeError(
                        f"Value {value!r} exceeds the maximum length allowed."
                    )
        return value
...
class DateType(MinMaxType[str, date]):
    ...
    @classmethod
    def _from_full_repr_arg_min_value(cls, val: str) -> date:
        return datetime.strptime(val, "%Y-%m-%d").date()

    _from_full_repr_arg_max_value = _from_full_repr_arg_min_value
...
class DateTimeType(MinMaxType[str, datetime]):
    ...
    _from_full_repr_arg_max_value = _from_full_repr_arg_min_value = _parse
...
class DurationType(MinMaxType[t.Tuple[int, int, int], timedelta]):
    ...
    @classmethod
    def _parse_from_repr(cls, val: t.Tuple[int, int, int]) -> timedelta:
        days, secs, ms = val
        return timedelta(days=days, seconds=secs, microseconds=ms)

    _from_full_repr_arg_min_value = _parse_from_repr
    _from_full_repr_arg_max_value = _parse_from_repr
...
class TupleType(Type[t.Tuple[t.Any, ...], t.Tuple[t.Any, ...]]):
    ...
    def full_repr(self):
        return {
            ":ns:": self.namespace,
            ":base:": self.constructor_name,
            "bases": [base.full_repr for base in self.bases],
        }
    ...
    @classmethod
    def _from_full_repr_arg_bases(cls, val) -> t.Sequence[Type]:
        return tuple(Type.from_full_repr(base) for base in val)
    ...
    def parse(self, raw_value: t.Tuple[t.Any, ...]) -> t.Tuple[t.Any, ...]:
        ...
        return tuple(
            base.parse(value) for base, value in zip(self.bases, raw_value)
        )
    ...
    def dump(
        ...
        *,
        ...
        if (many := len(self.bases)) != (this_many := len(value)):
            raise ValueError(f"Expected {many} values, got {this_many}")
        return tuple(
            base.dump(v, validate=validate)
            for base, v in zip(self.bases, value)
        )
...
class ObjectType(Type):
    ...
    def __repr__(self):
        ...
        return f"ObjectType(shape={dict(shape)})"
    ...
    @classmethod
    def _from_full_repr_arg_shape(cls, val) -> Shape:
        return {attr: Type.from_full_repr(value) for attr, value in val.items()}
...
class ListType(Type[t.List[S], t.Sequence[I]]):
    ...
    def full_repr(self):
        return {
            ":ns:": self.namespace,
            ":base:": self.constructor_name,
            "of": self.of.full_repr,
        }

    @classmethod
    def _from_full_repr_arg_of(cls, val) -> Type:
        return Type.from_full_repr(val)
...
class OptionalType(Type[t.Optional[S], t.Optional[I]]):
    ...
    def full_repr(self):
        return {
            ":ns:": self.namespace,
            ":base:": self.constructor_name,
            "type": self.type.full_repr,
        }

    @classmethod
    def _from_full_repr_arg_type(cls, val) -> Type:
        return Type.from_full_repr(val)
...
class MappingType(Type[t.Mapping[KS, VS], t.Mapping[KI, VI]]):
    ...
    def full_repr(self):
        return {
            ":ns:": self.namespace,
            ":base:": self.constructor_name,
            "key_type": self.key_type.full_repr,
            "value_type": self.value_type.full_repr,
        }

    @classmethod
    def _from_full_repr_arg_key_type(cls, val) -> Type:
        return Type.from_full_repr(val)

    @classmethod
    def _from_full_repr_arg_value_type(cls, val) -> Type:
        return Type.from_full_repr(val)
