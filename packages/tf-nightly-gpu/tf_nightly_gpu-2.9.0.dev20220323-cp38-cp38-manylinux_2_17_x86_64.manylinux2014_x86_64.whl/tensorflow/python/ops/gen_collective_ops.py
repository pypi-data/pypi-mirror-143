...
def collective_assign_group_v2(group_assignment, device_index, name=None):
  ...
  if tld.is_eager:
    ...
    try:
      return collective_assign_group_v2_eager_fallback(
          group_assignment, device_index, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
  _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "CollectiveAssignGroupV2", group_assignment=group_assignment,
                                   device_index=device_index, name=name)
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ()
    _inputs_flat = _op.inputs
...
def collective_assign_group_v2_eager_fallback(group_assignment, device_index, name, ctx):
  group_assignment = _ops.convert_to_tensor(group_assignment, _dtypes.int32)
  device_index = _ops.convert_to_tensor(device_index, _dtypes.int32)
  _inputs_flat = [group_assignment, device_index]
  _attrs = None
  _result = _execute.execute(b"CollectiveAssignGroupV2", 1,
