...
from bitfount.types import _DataFrameType, _Dtypes, _JSONDict
...
class TableSchema(_TableSchemaMarshmallowMixIn):
    ...
    def _add_dataframe_features(
        ...
        """
        table_dtypes = datasource.get_dtypes(self.name)
        ...
        semantic_types = self._combine_existing_stypes_with_forced_stypes(
            inferred_semantic_types, force_stype, table_dtypes
        )
        if SemanticType.CATEGORICAL in semantic_types.keys():
            values = datasource.get_values(
                semantic_types[SemanticType.CATEGORICAL], self.name
            )
        ...
        for stype, features in semantic_types.items():
            ...
            for feature_name in features:
                ...
                if stype == SemanticType.TEXT:
                    ...
                    for img in datasource.get_column(feature_name, self.name):
                        im = Image.open(img)
...
class BitfountSchema(_BitfountSchemaMarshmallowMixIn):
    """A schema that defines the tables of a `DataSource`.

    It lists all the tables found in the `DataSource` and the features in those tables.
    """

    def __init__(self, datasource: Optional[DataSource] = None, **kwargs: Any):
    ...
    def add_datasource_tables(
        ...
        """Adds the tables from a `DataSource` to the schema.

        Args:
            datasource: The `DataSource` to add the tables from.
        ...
        if not self._frozen:
            ...
            if datasource.multi_table:
                assert isinstance(datasource.data_ref, DatabaseConnection)  # nosec
                assert datasource.data_ref.table_names is not None  # nosec
