...
@fixture
def tabular_dataset(dataframe: pd.DataFrame) -> _Dataset:
...
@fixture
def image_dataset(dataframe: pd.DataFrame) -> _Dataset:
...
@fixture
def image_tab_dataset(dataframe: pd.DataFrame) -> _Dataset:
...
@fixture
def multiimage_dataset(multiimage_dataframe: pd.DataFrame) -> _Dataset:
...
def db_session_with_index(
    ...
    # The tables should never already exist in the database so we set it to fail
    # if it does to catch any potential setup errors.
    df.to_sql("dummy_data", engine, if_exists="fail", index=True)
    df2.to_sql("dummy_data_2", engine, if_exists="fail", index=True)
