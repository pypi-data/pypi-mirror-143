...
def basemodel_to_str(
    model: BaseModelType, default_replace: bool = False
) -> T.Tuple[str, T.Dict[str, T.Any]]:
    # should exclude_none = True?
    from .resolver import Resolver, UpdateOperation

    model_d = model.dict(exclude_none=True)
...
class Node(BaseModel, T.Generic[InsertType]):
    ...
    async def add(
        ...
        if batch and upsert_given_conflict_on:
            raise NodeException("You cannot batch add with upsert fields.")
        ...
        if batch:
            ...
            # should I screw up the types for this?
            return None
        conflict_str = ""
        if custom_on_conflict_str:
            conflict_str = custom_on_conflict_str
        elif upsert_given_conflict_on:
            conflict_str = f"UNLESS CONFLICT ON .{upsert_given_conflict_on} ELSE (UPDATE {cls._model_name} SET {s})"
