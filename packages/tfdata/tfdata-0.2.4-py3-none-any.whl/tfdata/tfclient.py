...
from tfdata.utils.comm_utils import timenow,postfix,pro_ts,get_codetype
...
class TFclient():
    ...
    def _get_hf(self,code,hf_type,date,timeout=200):
        ...
        """        
        if hf_type not in ['hf_tick','hf5']:
            return '非法参数'
        if '.' not in code:
            return '请输入正确的带后缀代码'
        code_type = get_codetype(code)
        if code_type not in ['ETF_CN','stock_A']:
    ...
    def get_daily(self,code,start_date=None,end_date=None,sample_size=None,timeout=200,adj_price=None):
        ...
        """   
        df =  self._get_tstable(code=code,att=['open','high','low','close','pct_chg','vol','amount','pindex'],\
    ...
    def _get_tstable(self,code,att,start_date=None,end_date=None,sample_size=None,timeout=200):
        ...
        pcode = postfix(code)
        if '.' not in pcode:
            return '证券代码不符合规范,请参阅self。menu["code"]'
        asset_type = get_codetype(code)
        if asset_type == 'other' or asset_type == '.':
            return f'暂不支持代码{code}数据请求'
        ...
        if self.__ens_pw is not None:
            arg = {'code':pcode,'yyhh':self.__acct,'dpw':self.__ens_pw,'rt':'tstable','start_date':start_date,'end_date':end_date,'sample_size':sample_size}
            if asset_type == 'stock_A':
                dff = list(set(att) - set(self.att_menu['get_daily_pro_att']['stock_A'].keys()))
                if len(dff)>0:
                    return f'无效的数据标签{dff},详见self.get_menu("get_daily_pro_att")'               
            elif asset_type == 'ETF_CN':
                dff = list(set(att) - set(self.att_menu['get_daily_pro_att']['ETF_CN'].keys()))
                if len(dff)>0:
                    return f'无效的数据标签{dff},详见self.get_menu("get_daily_pro_att")'                
