...
class StateTimestampMixin(BaseTimestampMixin):
    ...
    An object is considered active if its ``state`` attribute, which needs to be present
    in an inheriting model, is ``"active"``. Besides the object itself, changes in
    relationship-based collections consisting of inactive objects only are also ignored.
    """
    ...
    def update_timestamp(self):
        ...
        Only actually triggers the update if the ``state`` of the object is "active".
        """
