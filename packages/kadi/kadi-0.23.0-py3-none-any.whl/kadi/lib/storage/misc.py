...
def create_misc_storage(max_size=None):
    ...
    configuration as root directory for the storage and two directories of length 2 for
    all generated file paths via :meth:`.LocalStorage.create_filepath`.
...
def save_as_thumbnail(image_identifier, file_object, max_image_size=(512, 512)):
    """Save an image file as JPEG thumbnail.

    Uses the local storage as returned by :func:`create_misc_storage` to store the
    thumbnails with a maximum size given by ``MAX_IMAGE_SIZE`` as defined in the
    ...
    :param file_object: The binary image file object. The image must be one of the image
        types specified in ``IMAGE_MIMETYPES`` in the application's configuration.
    ...
    try:
        ...
        if mimetype not in current_app.config["IMAGE_MIMETYPES"]:
            return False
