...
from kadi.lib.validation import validator
...
def _validate_record_id(record_id):
    # Validating the existence of a record is especially useful when using the schema in
    # a nested context, e.g. when linking records.
...
class UploadSchema(KadiSchema):
    ...
    storage = fields.Nested(StorageSchema, load_default={"storage_type": "local"})
