...
class BaseProvider(ABC):
    ...
    @classmethod
    def create_identity(cls, *, system_role, identity_model, **kwargs):
        ...
        # Apply all existing role rules to the newly registered user.
...
def init_auth_providers(app):
    ...
    application's configuration. During initialization, the ``AUTH_PROVIDERS``
    configuration will be modified to use an ordered dictionary instead and will also
    include the following additional keys:
