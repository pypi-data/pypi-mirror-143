...
<template>
  <div class="form-group" :class="{'required': field.validation.required}">
    ...
    <small class="form-text text-muted" v-if="errors.length === 0">{{ description_ }}</small>
  </div>
</template>
...
export default {
  ...
  props: {
    field: Object,
    // An additional static description that should always appear next to the field's description.
    description: {
      type: String,
      default: '',
    },
