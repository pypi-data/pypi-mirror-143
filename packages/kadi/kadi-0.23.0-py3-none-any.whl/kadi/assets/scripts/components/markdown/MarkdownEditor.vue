    ...
    <div class="card toolbar">
      <div class="card-body px-0 py-0">
        <span v-for="tool in toolbar" :key="tool.title">
          <span class="separator" v-if="tool === '|'"></span>
          <button type="button"
                  :class="toolbarBtnClasses"
                  :title="tool.title"
                  :disabled="previewActive"
                  @click="handleToolbarButton(tool.handler)"
                  v-else>
            <i :class="tool.icon"></i>
          </button>
        </span>
  ...
  watch: {
    input() {
      ...
      if (this.checkpointTimeoutHandle !== null) {
        window.clearTimeout(this.checkpointTimeoutHandle);
      }

      this.checkpointTimeoutHandle = window.setTimeout(() => {
        this.saveCheckpoint();
      }, 500);
    },
  },
  methods: {
    ...
    insertLink(toggleSelection) {
      if (toggleSelection && this.linkEndpoint) {
        ...
        const link = `[${$t('Link text')}](https:// "${$t('Title')}")`;
        this.insertText(link);
      }
    },

    selectLink(file) {
      const link = `[${file.text}](${file.view_endpoint} "${file.text}")`;
      this.insertText(link);
    },
    ...
    insertImage() {
      if (this.imageEndpoint) {
        ...
        const image = `![${$t('Alternative text')}](https:// "${$t('Title')}")`;
        this.insertText(image);
      }
    },

    selectImage(file) {
      const imageText = `![${file.text}](${file.preview_endpoint} "${file.text}")`;
