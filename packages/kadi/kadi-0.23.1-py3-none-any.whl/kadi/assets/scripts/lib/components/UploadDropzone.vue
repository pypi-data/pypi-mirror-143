...
<template>
  <div>
    <div class="dropzone"
      ...
      <div class="text-muted">{{ $t('Drop files here or click to upload.') }}</div>
    </div>
    <input multiple type="file" class="input" @change="inputChange" ref="input">
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    inputChange(e) {
      ...
      // Clear the input afterwards, as otherwise some browsers only seem to trigger the change event again if a
      // different file is selected the next time the input is used directly.
