      ...
      if path is not None:
        ...
        try:
          ...
          for file in file_list:
            ...
            if not os.path.isfile(filepath) or any([file.endswith(ext) for ext in ['.mold.json','.bundle.json','.vydir']]):
              continue
            item['philes'].append({
      ...
      if type_name.startswith('bundle:'):
        ...
        for e in item['bundles']:
          if e['name'] in items:
            ...
            successi = False
            vytools.printer.print_fail('bundle "{n}" has an reference to a bundle {t}'.format(n=name, t=e['name']))
        success &= successi
...
def upload(lst, url, uname, token, check_first, update_list, items=None):
  ...
  su.upload('bundle', lst, url, uname, token, check_first, update_list, onsuccess, items=items)