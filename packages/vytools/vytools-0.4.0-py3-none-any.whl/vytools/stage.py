import os, re, subprocess, time, shlex, json
...
def dependencies(stage_text):  
    ...
    if m:
      ...
      if len(lsplit) > 2:
        if lsplit[1].lower() == 'context':
          ...
          if build_context.startswith('vydir:'):
            dependency.append(build_context)
          elif build_context.startswith('repo:'):
            dependency.append(build_context)
  ...
  return (success, dependency, args, build_context)
...
def parse(namein, pth, items):
  ...
  (success, dependency, args, build_context) = dependencies(stage_text)
  if not success:
    vytools.printer.print_fail('Stage "{}" references a build context "{}" which does not exist'.format(name,build_context))
    return False
    ...
    try:
      if build_context.startswith('vydir:'):
        if build_context in items:
          build_context = Path(items[build_context]['path']).resolve()
        else:
          vytools.printer.print_fail('Stage build context "{p}" for stage {n} is not a known vydir'.format(p=build_context,n=name))
          return False
      elif build_context.startswith('repo:'):
        if build_context in items:
          build_context = Path(items[build_context]['path']).resolve()
        else:
          vytools.printer.print_fail('Stage build context "{p}" for stage {n} is not a known repo'.format(p=build_context,n=name))
          return False
      else:
...
def __build__(stage, already_tagged, build_arg_dict, items, checked_versions, jobpath=None):
  ...
  contents = Path(stage['path'].replace(':',';')).read_text() # use semicolon for windows friendly
  ...
  with open(df+'.log','w') as logfile:
    vytools.printer.print_def(' '.join(cmd))
    proc = subprocess.Popen(cmd, env={'DOCKER_BUILDKIT':'1'}, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
