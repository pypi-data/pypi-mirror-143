      ...
      if type_name.startswith('mold:'):
        ...
        for key in ['pubu','prbu']:
          ...
          if val == 'orph':
            ...
            successi = False
            vytools.printer.print_fail('mold "{n}" references "{r}" as the {p}, it should reference a valid bundle name (e.g. "bundle:mybundle").'.format(n=name, p=key, r=val))
          success &= successi
...
def upload(lst, url, uname, token, check_first, update_list, items=None):
  ...
  uploads.upload('mold', lst, url, uname, token, check_first, update_list, onsuccess, items=items)