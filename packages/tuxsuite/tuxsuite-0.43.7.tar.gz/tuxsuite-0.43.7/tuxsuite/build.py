    ...
    def submit(self):
        ...
        if builds_to_submit:
            ...
            for b in builds_to_submit:
                build_entry, patch = b.generate_build_request(plan=self.plan)
                req_data["builds"].append(build_entry)
                req_data["patches"].update(patch)
            ret = post_request(f"{self.url}/builds", self.headers, req_data)
...
class Bitbake(HeadersMixin):
    ...
    def generate_build_request(self, plan=None):
        """return a build data in a python dict"""
        return self.build_definition.__dict__

    def build(self, plan=None):
        """Submit the build request"""
        data = self.generate_build_request(plan=plan)
