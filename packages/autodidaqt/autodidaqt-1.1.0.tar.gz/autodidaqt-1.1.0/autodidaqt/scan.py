...
import numpy as np
from autodidaqt_common.enum import _try_unwrap_value
from autodidaqt_common.path import AxisPath

from autodidaqt.instrument.spec import ChoicePropertySpecification, DataclassPropertySpecification
from autodidaqt.utils import AccessRecorder, InstrumentScanAccessRecorder, PathType
...
class ScanProperty(ScanDegreeOfFreedom):
    def __init__(self, device_name: Union[str, AccessRecorder], spec):
        ...
        if isinstance(device_name, InstrumentScanAccessRecorder):
            ...
            self.devices = AxisPath.to_tuple(device_name)
...
def field_to_ui_fields(field_name, field):
    raise NotImplementedError
...
def scan(
    ...
    **axes: Union[str, ScanDegreeOfFreedom, AccessRecorder],
    ...
    def sequence_scan(self, experiment, **kwargs):
        dependent = {
            read_name: AxisPath.to_tuple(read_device) for read_name, read_device in read.items()
        }
