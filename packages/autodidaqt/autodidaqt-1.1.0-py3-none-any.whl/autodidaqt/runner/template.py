...
class Templater:
    ...
    def dir_for_template(self) -> Path:
        variant_dir = autodidaqt_LIB_ROOT / "resources" / "templates" / self.variant.value
    ...
    def template(self):
        ...
        # this would get created by autodidaqt when the project is run but the
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create autodidaqt projects from a template.")
    ...
    if not Path(args.project_path).exists():
        print(f"Path {args.project_path} does not exist")
        exit(1)
