...
class ExperimentPanel(Panel):
    ...
    def widget_for_scan_config(self, item, index):
        ...
        try:
            ...
            if len(fields) < 3:
                rest = vertical(
                    *[label(f"{f}: {str(getattr(item, f))}") for f, v in fields.items()]
                )
            else:
                split = len(fields) // 2
                rest = horizontal(
    ...
    def update_queue_ui(self):
        ...
        for i, config in enumerate(self.experiment.scan_deque):
            queue_main_widget.layout().addWidget(self.widget_for_scan_config(config, i))
    ...
    def soft_update(self, force=False, render_all=False):
        ...
        if not self.dynamic_state_mounted:
            ...
            # ('a', 0, 'b') -> 'a[0].b'
            display_names = {
                ks: ".".join([k if isinstance(k, str) else f" [{k}] " for k in ks])
                .replace(". ", "")
                .replace(" .", "")
                .strip()
        ...
        if (
            ...
            and (now - self.last_plot_update).total_seconds() < self.update_every
            and not force
        ):
            return
    ...
    def update_user_stream_plot(self, user_plot_key):
        ...
        if len(ind) > 1:
            ...
            xs, ys = (
                self.experiment.current_run.streaming_daq_ys[ind[0]],
