...
from autodidaqt.instrument import AxisListSpecification, AxisSpecification, ManagedInstrument
from autodidaqt.instrument.axis import PolledRead, PolledWrite
...
class RandomlyMove(MessagingActor):
    async def run_step(self):
        ...
        await asyncio.gather(
            ...
            *[
                self.app.managed_instruments["motion_controller"].stages[i].write(100 * random())
                for i in range(3)
            ],
        )
        await asyncio.gather(
            *[self.app.managed_instruments["motion_controller"].stages[i].read() for i in range(3)]
        )
