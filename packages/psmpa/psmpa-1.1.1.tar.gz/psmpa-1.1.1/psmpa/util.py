...
def convert_humann2_to_picrust2(infiles, outfile, stratified):
    ...
    # unstratified format and split index into two columns.
        ...
        split_df = pd.DataFrame.from_records(list(humann2_combined.index.str.split('\\|', 1)),
                                             columns=[first_col, 'sequence'])
    ...
    # Otherwise read in stratified and unstratified tables.
        ...
        def set_picrust2_tab_index(in_table):
            ...
            if 'sequence' in in_table.columns:
                in_table.index = in_table.loc[:, in_table.columns[0]].str.cat(
                    others=in_table.loc[:, in_table.columns[1]],
                    sep='|')
...
def create_output_folder(output_folder):
    ...
    try:
        ...
        raise Exception('Stopping since output directory ' + output_folder + ' already exists.')
