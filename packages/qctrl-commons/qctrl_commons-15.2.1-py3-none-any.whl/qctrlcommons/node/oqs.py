...
class DensityMatrixEvolutionPwc(Node):
    ...
    error_tolerance : float, optional
        ...
        Note that using the exact solution can be very computationally demanding in calculations
        involving a large Hilbert space or a large number of segments. You can set a value for this
        tolerance to use a faster and more memory-efficient approximate method to solve the master
        equation. The 2-norm of the difference between the propagated state and the exact solution
    ...
    Warnings
    --------
    Not setting an `error_tolerance` parameter will default to calculating an exact
    piecewise-constant solution, which can be very computationally demanding in calculations
    involving a large Hilbert space or a large number of segments.
    ...
    :math:`\dot{\rho} = -i[\sigma_z,\, \rho] + \mathcal{D}[\sigma_-]\rho`.

    >>> duration = 20
    >>> sigma_z = np.array([[1, 0], [0, -1]])
    >>> sigma_m = np.array([[0, 1], [0, 0]])
    >>> initial_density_matrix = np.array([[0, 0], [0, 1]])
    >>> hamiltonian = graph.constant_pwc_operator(duration=duration, operator=sigma_z / 2)
    >>> lindblad_terms = [(1, sigma_m)]
