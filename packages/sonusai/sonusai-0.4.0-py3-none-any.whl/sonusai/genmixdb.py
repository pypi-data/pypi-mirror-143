...
from sonusai.mixture import check_truth_setting
...
def genmixdb(file: str, logging: bool = True, show_progress: bool = False) -> dict:
    ...
    noises = get_input_files(records=config['noises'])
    if len(noises) == 0:
        if logging:
            logger.warning(f'Did not find any noises; using default noise: {sonusai.mixture.default_noise}')
        noises = get_input_files(records=[sonusai.mixture.default_noise])
    if logging:
    ...
    if config['truth_mode'] not in ['normal', 'mutex']:
        logger.error(f'invalid truth_mode: {config["truth_mode"]}')
        raise SonusAIError
            ...
            for target_index, target in enumerate(targets):
                for target_augmentation_index, target_augmentation in enumerate(target_augmentations):
                    ...
                    for truth_setting in target['truth_settings']:
                        check_truth_setting(truth_setting, config['truth_settings'])
                        ...
                        if any(idx > max_class for idx in truth_setting['index']):
                            logger.error('invalid truth index')
                            raise SonusAIError
...
def append_input_files(files: list, in_record, truth_settings: list, tokens=None):
    ...
    if isinstance(in_record, dict):
        if 'target_name' in in_record:
            ...
            logger.error('Target list contained record without target_name')
            raise SonusAIError
    ...
    names = glob(in_name)
    if not names:
        logger.error(f'Could not find {in_name}. Make sure path exists')
        raise SonusAIError
    for name in names:
        ...
        if isdir(name):
            ...
            try:
                if ext == '.txt':
                    ...
                    try:
                        with open(file=name, mode='r') as yml_file:
...
def expand_augmentations(augmentations: list, rule: dict, target: bool):
    ...
    for key in rule:
        if key not in sonusai.mixture.valid_augmentations:
            logger.error(f'Invalid augmentation: {key}')
            raise SonusAIError
        ...
        if key in ['eq1', 'eq2', 'eq3']:
            ...
            if not valid:
                logger.error(f'Invalid augmentation value for {key}: {rule[key]}')
                raise SonusAIError
...
def get_output_from_config(config: dict, config_name: str) -> str:
    try:
        ...
        logger.error(f'Error getting genmixdb base name: {e}')
        raise SonusAIError


def _process_mixture(mixture: dict) -> dict:
...
def main():
    try:
        ...
        for config_file in args['CONFIG']:
            ...
            mixdb = genmixdb(file=config_file, show_progress=True)
