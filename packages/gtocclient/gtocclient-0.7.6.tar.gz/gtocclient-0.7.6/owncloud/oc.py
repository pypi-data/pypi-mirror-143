...
from . import exit_codes, progress, utils, exceptions
...
class ResponseError(Exception):
    ...
    def get_error_code(self):
        if self.status_code in [301, 307]:
            return exit_codes.EX_SERVERMOVED
        elif self.status_code in [400, 405, 411, 416, 417, 501, 504]:
            return exit_codes.EX_SERVERERROR
        elif self.status_code == 403:
            return exit_codes.EX_ACCESSDENIED
        elif self.status_code == 404:
            return exit_codes.EX_NOTFOUND
        elif self.status_code == 409:
            return exit_codes.EX_CONFLICT
        elif self.status_code == 412:
            return exit_codes.EX_PRECONDITION
        elif self.status_code == 500:
            return exit_codes.EX_SOFTWARE
        elif self.status_code in [429, 503]:
            return exit_codes.EX_SERVICE
        else:
            return exit_codes.EX_SOFTWARE
...
class FileInfo(object):
    ...
    @staticmethod
    def get_all_property_arguments():
        return [
            'd:getlastmodified',
            'd:getetag',
            'd:getcontenttype',
...
class Client(object):
    ...
    def __init__(self, url, **kwargs):
        ...
        self.progress_class = kwargs.get('progress_class', progress.ProgressCR)
        self.oc_path_format = kwargs.get('oc_path_format', False)
    ...
    def login(self, user_id, password):
        ...
        try:
            ...
            if self._dav_endpoint_version == 1:
                self._davpath = url_components.path + \
                                'remote.php/dav/files/' + parse.quote(user_id)
                self._webdav_url = self.url + \
                                   'remote.php/dav/files/' + parse.quote(user_id)
            else:
    ...
    def exists(self, path):
        ...
        """
        headers = {}
        headers['User-Agent'] = 'gtocclient/{} ({}; {})'.format(pkg_info.version, getpass.getuser(),
                                                               socket.gethostname())
    ...
    def file_info(self, path, properties=None):
        ...
        if properties:
            ...
            data = None
        res = self._make_dav_request('PROPFIND', path, headers={
                                     'Depth': '0'}, data=data)
        if res:
            return res[0]
        return None
    ...
    def list(self, path, depth=1, properties=None):
        ...
        res = self._make_dav_request(
            'PROPFIND', path, headers=headers, data=data)
        # first one is always the root, remove it from listing
        if res:
            return res[1:]
        return None

    def get_file_contents(self, path, **kwargs):
        ...
        if res.status_code == 200:
            if not kwargs.get('dest_stream'):
                dest_stream = io.BytesIO()
                for chunk in res.iter_content(8192):
                    dest_stream.write(chunk)
                return dest_stream.getvalue()
            else:
                dest_stream = kwargs.get('dest_stream')
                for chunk in res.iter_content(8192):
                    dest_stream.write(chunk)
                    dest_stream.flush()
            return True
    ...
    def get_file(self, remote_path, local_file=None,**kwargs):
        ...
        debug(u'Download %s to %s' % (self._format_oc_path_for_output(remote_path), local_file))
        res = self._session.get(
        ...
        if res.status_code == 200:
            ...
            if self.progress:
                props = self.file_info(remote_path)
                labels = {'source': self._format_oc_path_for_output(remote_path), 'destination': local_file,'action':'Download',
                          'extra':kwargs.get('extra_label') if kwargs.get('extra_label') else ""}
                size = 0
                try:
                    size = int(props.attributes.get('{DAV:}getcontentlength'))
                except Exception:
                    pass
                progress_bar = self.progress_class(labels, size)
                file_handle = open(local_file, 'wb', 8192)
                for chunk in res.iter_content(8192):
                    file_handle.write(chunk)
                    progress_bar.update(delta_position=len(chunk))
                progress_bar.update(current_position=size)
                progress_bar.done("done")
                file_handle.close()
    ...
    def get_directory_as_zip(self, remote_path, local_file):
        ...
        if res.status_code == 200:
            if local_file is None:
                ...
                local_file = os.path.basename(remote_path)
            labels = {'source': self._format_oc_path_for_output(remote_path), 'destination': local_file, 'action': 'download', 'extra': ""}
            progress_bar = self.progress_class(labels, -1)
            file_handle = open(local_file, 'wb', 8192)
            for chunk in res.iter_content(8192):
                file_handle.write(chunk)
                progress_bar.update(delta_position=len(chunk))
    ...
    def put_file_contents(self, remote_path, data, **kwargs):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        # if self.file_info(remote_path).file_type == "dir":
        #     raise Exception("远程路径为dir类型，无法上传")
        mime_type = magic.from_buffer(data,mime=True)
        return self._make_dav_request('PUT', remote_path, data=data, headers={'Content-Type':mime_type}, **kwargs)
    ...
    def put_file(self, remote_path, local_source_file, **kwargs):
        ...
        :param force: (optional) overwrite the remote file
        :param skip_existing: (optional) skip when the remote file exists
        :returns: True if the operation succeeded, False otherwise
        :raises: HTTPResponseError in case an HTTP error status was returned
        """

        try:
            file_info = self.file_info(remote_path)
            if file_info.file_type == "dir":
                ...
                file_info = self.file_info(remote_path)
                if self.file_info(remote_path).file_type == 'dir':
                    raise exceptions.OCUnexpectedDirError(remote_path)
            if file_info.file_type == 'file':
                if kwargs.get('skip_existing', False):
                    sys.stdout.write('Skip over existing File: %s.\n' % self._format_oc_path_for_output(utils.convert(remote_path)))
                    sys.stdout.flush()
                    return
                if not kwargs.get('force', False) and not kwargs.get('put_continue', False):
                    raise exceptions.OCFileExistsError(remote_path)
        except HTTPResponseError as e:
            pass

        debug(u'Upload %s to %s' % (local_source_file, self._format_oc_path_for_output(remote_path)))
        if kwargs.get('chunked', False) or kwargs.get('put_continue',False):
        ...
        if self.progress:
            labels = {'source': local_source_file, 'destination': self._format_oc_path_for_output(remote_path), 'action': 'Upload',
                      'extra': kwargs.get('extra_label') if kwargs.get('extra_label') else ""}
            prog = self.progress_class(labels, os.path.getsize(local_source_file))
            upload_iterator = utils.UploadIterator(file_handle, prog)
            try:
                res = self._make_dav_request(
                    'PUT',
                    remote_path,
                    data=utils.IterableToFileAdapter(upload_iterator),
                    headers=headers
                )
                file_handle.close()
                prog.done("done")
    ...
    def put_directory(self, target_path, local_directory, **kwargs):
        ...
        if not local_directory.endswith(os.path.sep):
            local_directory += os.path.sep

        basedir = os.path.basename(local_directory[:-1]) + '/'
        # gather files to upload
        for path, _, files in os.walk(local_directory):
            gathered_files.append(
                (path, basedir + path[len(local_directory):].replace(os.path.sep,'/'), files)
            )
        ...
        if kwargs.get('chunked', False) or kwargs.get('put_continue',False):
            try:
                executor = ThreadPoolExecutor(max_workers=10, thread_name_prefix="owncloud_")
            except Exception:
                pass
        try:
            for path, basename, files in gathered_files:
                if basename.startswith('.' + os.path.sep):
                    basename = basename[2:]
                if basename == '': continue
                _path = target_path + basename
                if not self.exists(_path):
                    info(u'Making dir: %s' % self._format_oc_path_for_output(_path))
                    self.mkdir(_path)
                elif not kwargs.get('skip_existing', False) and not kwargs.get('force', False) \
                        and not kwargs.get('put_continue', False):
                    raise exceptions.OCDirExistsError(_path)
                for name in files:
                    res = self.put_file(_path, os.path.join(path, name), executor=executor, **kwargs)
                    try:
    ...
    def _put_file_chunked(self, remote_path, local_source_file, **kwargs):
        ...
        :raises: HTTPResponseError in case an HTTP error status was returned
        """
        chunk_size = kwargs.get('chunk_size', 15 * 1024 * 1024)
        result = True
        now = int(time.time())
        ...
        st_mtime = int(stat_result.st_mtime)
        if kwargs.get('keep_mtime', True):
            headers['X-OC-MTIME'] = str(st_mtime)
        # global downloaded
        # uploaded = 0
        # console_size = 50
        ...
        #     nonlocal uploaded
        #     uploaded += chunk
        #     done = int(console_size * uploaded / float(size))
        #     percent = 100 * uploaded / float(size)
        #     remain_time = (int(time.time()) - now) * \
        #                   ((100 - percent) / percent)
        #     if percent >= 100:
        #         sys.stdout.write("\rUploading " + local_source_file + " [" + '#' * done + '.' * (
        #                 console_size - done) + "] " + '{:.2f}'.format(percent) + "% Time: " + utils.format_time(
        #             int(time.time()) - now))
        #     else:
        #         sys.stdout.write("\rUploading " + local_source_file + " [" + '#' * done + '.' * (
        #                 console_size - done) + "] " + '{:.2f}'.format(percent) + "% ETA: " + utils.format_time(
        #             remain_time))
        #     sys.stdout.flush()

        labels = {'source': local_source_file, 'destination': self._format_oc_path_for_output(remote_path),
                  'action': 'Continue Upload' if kwargs.get('put_continue', False) else 'Upload',
                  'extra': kwargs.get('extra_label') if kwargs.get('extra_label') else ""}
        prog = self.progress_class(labels, os.path.getsize(local_source_file))
        if size == 0:
            try:
        ...
        index = 0
        cache_file_name = hashlib.md5(
            u'{}-{}-{}-{}-{}'.format(
                self._session.auth[0] if self._session else None,
                local_source_file,
                remote_path,
                st_mtime,
                chunk_count).encode('UTF-8')).hexdigest()

        if kwargs.get('put_continue', False):
            if self.exists(remote_path):
                prop = self.file_info(remote_path, ['oc:checksums'])
                try:
                    checksums = prop.attributes['{http://owncloud.org/ns}checksums']['{http://owncloud.org/ns}checksum'][0]
                    remote_md5 = checksums.split(' ')[1].split(':')[1]
                    md5 = hashlib.md5()
                    with open(local_source_file, 'rb') as fr:
                        while True:
                    ...
                    if remote_md5 == local_md5:
                        if self.progress:
                            prog.update(delta_position=size)
                            prog.done("done")
                        return True
                except BaseException:
                    pass
            cached = utils.read_cache_file(cache_file_name, datetime.timedelta(hours=kwargs.get('cache_valid_hour', 8)))
            try:
                index = int(cached)
            except BaseException:
                pass
            file_handle.seek(index * chunk_size, os.SEEK_SET)
            # uploaded = index * chunk_size
            if self.progress:
                prog.update(current_position=index*chunk_size)
        for chunk_index in range(index, int(chunk_count)):
            data = file_handle.read(chunk_size)
            if self.progress:
                data = utils.IterableToFileAdapter(utils.UploadIterator(io.BytesIO(data),prog,len(data)))
            try:
                if chunk_count > 1:
                    chunk_name = '%s-chunking-%s-%i-%i' % \
                                 (remote_path, st_mtime, chunk_count,
                                  chunk_index)
                    if chunk_index == chunk_count - 1 and kwargs.get('executor', None):
                        def task():
                            res = self._make_dav_request('PUT', chunk_name, data=data, headers=headers)
                            utils.delete_cache_file(cache_file_name)
                            file_handle.close()
                            if self.progress:
                                prog.done("done")
                            return res
                        return kwargs.get('executor').submit(task)
                    else:
                        result = self._make_dav_request('PUT', chunk_name, data=data, headers=headers)
                        utils.write_cache_file(cache_file_name, str(chunk_index + 1))
                else:
                    result = self._make_dav_request('PUT', remote_path, data=data, headers=headers)
                    file_handle.close()
            except HTTPResponseError as e:
                if self.progress:
                    prog.done("failed")
    ...
    def _check_ocs_status(tree, accepted_codes=[100]):
        ...
        if code_el is not None and int(code_el.text) not in accepted_codes:
            ...
            if msg_el is None:
                # fallback to the entire ocs response, if we find no message.
                msg_el = tree
    ...
    def _make_ocs_request(self, method, service, action, **kwargs):
        ...
        attributes['headers'] = attributes.get('headers', {})
        attributes['headers']['User-Agent'] = 'gtocclient/{} ({}; {})'.format(pkg_info.version, getpass.getuser(),
                                                               socket.gethostname())
        ...
        res = self._session.request(method, self.url + path, proxies=self.proxies, **attributes)
        return res
    ...
    def _make_dav_request(self, method, path, **kwargs):
        ...
        headers['User-Agent'] = 'gtocclient/{} ({}; {})'.format(pkg_info.version,getpass.getuser(),socket.gethostname())
    ...
    def _parse_dav_element(self, dav_response):
        ...
        def recurse(element):
            ...
            result = {}
            for child in element:
                if not result.get(child.tag):
                    result[child.tag] = []
                result[child.tag].append(recurse(child))
            return result

        for attr in attrs:
            file_attrs[attr.tag] = recurse(attr)
    ...
    def _format_oc_path_for_output(self, path):
        if self.oc_path_format:
            return utils.format_oc_path_output(path)
        else:
            return path
