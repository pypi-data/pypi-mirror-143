    ...
    def get_asset_config_for_layers(self):
        ...
        for xmlasset in self.xmlassets:
            if xmlasset.hasAttribute("name"):
                ...
                assetcat = None
                if len(xmlasset.getElementsByTagName("category"))> 0 and \
                        len(xmlasset.getElementsByTagName("category")[0].childNodes) > 0:
                ...
                if lookup_chk and len(lookup_chk) > 0:
                    try:
                        ...
                        pass
                if isinstance(lookup_cfg, bool) or lookup_cfg.upper() == "TRUE":
                    lookup = bool(lookup_cfg)
                else:
                    lookup = False
                    self.commontools.new_message(
                ...
                update_skip_defaults_cfg = False
                update_skip_defaults_chk = xmlasset.getElementsByTagName(
                    "skip_defaults_on_update")
                if update_skip_defaults_chk and \
                        len(update_skip_defaults_chk) > 0:
                    try:
                        update_skip_defaults_cfg = xmlasset.getElementsByTagName(
                            "skip_defaults_on_update")[0].childNodes[0].data
                    except:
                        pass
                if isinstance(update_skip_defaults_cfg, bool) \
                        or update_skip_defaults_cfg.upper() == "TRUE":
                    update_skip_defaults = bool(update_skip_defaults_cfg)
                else:
                    update_skip_defaults = False
                    self.commontools.new_message(
                        "skip_defaults_on_update to indicate updates should "
                        "not include defaults when updating is not set "
                        "correctly in tag <skip_defaults_on_update>."
                        "\nCurrent setting is '{0}', use"
                        "one of True or False."
                        "\nDefaulting to status 'False'".format(
                            update_skip_defaults_cfg))
                ...
                lyr_config["skip_defaults_on_update"] = update_skip_defaults
