...
def draw_clustergraph(via_coarse, type_data='gene', gene_exp='', gene_list='', arrow_head=0.1,
    ...
    for i in range(n):
        ...
        ax_i = plot_edge_bundle(ax_i, via_coarse.hammer_bundle, layout=via_coarse.graph_node_pos, CSM=via_coarse.CSM,
                                velocity_weight=via_coarse.velo_weight, pt=pt, headwidth_bundle=arrow_head, alpha_bundle=0.4)
...
def animated_streamplot(via_coarse, embedding , density_grid=1,
 linewidth=0.5,min_mass = 1, cutoff_perc = None,scatter_size=500, scatter_alpha=0.2,marker_edgewidth=0.1, smooth_transition=1, smooth_grid=0.5, color_scheme = 'annotation', add_outline_clusters=False, cluster_outline_edgewidth = 0.001,gp_color = 'white', bg_color='black' ,  title='Streamplot', b_bias=20, n_neighbors_velocity_grid=None, fontsize=8, alpha_animate=0.7,
                        cmap_scatter = 'rainbow', cmap_stream='Blues_r', color_stream = None, segment_length=1, saveto='/home/shobi/Trajectory/Datasets/animation.gif'):
    '''
    ...
    V_emb = via_coarse.velocity_embedding(embedding, smooth_transition, b=b_bias)

    V_emb *= 100 #the velocity of the samples has shape (n_samples x 2).
    print('V_emb before grid in animation')
    print(V_emb)
    ...
    n_true = len(set(via_coarse.true_label))
    ...
    if color_scheme == 'annotation':
        ...
        for color, group in zip(lin_col, sorted(set(via_coarse.true_label))):
...
class VIA:
    def __init__(self, data, true_label=None, anndata=None, dist_std_local=2, jac_std_global='median',
                 ...
                 piegraph_edgeweight_scalingfactor=1.5, max_visual_outgoing_edges=2, via_coarse=None, velocity_matrix=None, gene_matrix=None, velo_weight=0.5, edgebundle_pruning=None, A_velo = None, CSM = None, edgebundle_pruning_twice=False, pca_loadings = None):
    ...
    def sc_transition_matrix(self,smooth_transition,b=10):
        '''
        ...
        labels = self.labels
        T = self.csr_array_locally_pruned #single cell edges are weighted as inverse of distance
        #T = self.csr_full_graph #single cell


        thr_global = np.mean(T.data) - 2*np.std(T.data)
        T.data[T.data < thr_global] = 0
        T.eliminate_zeros()

        size_T = T.size
        print('sizeT number of edges in sc graph', size_T)
        ...
        if self.CSM is not None:
            for i in range(size_T):
                ...
                #if (csm_ >0) & (t_dif > 0): csm_ *= 10
                if i%10000==0:
                    print(i,'out of', size_T, 'start', labels[start], 'end', labels[end], 'csm_', round(csm_,3), 't_diff', round(t_dif,3))
            ...
            bias_weight_velo = 2.0*np.asarray(bias_weight_velo)/np.mean(np.array(bias_weight_velo))
            print('velo weight' , bias_weight_velo)
    ...
    def velocity_embedding(self, X_emb, smooth_transition, b):
        '''
        ...
        T = self.sc_transition_matrix(smooth_transition, b)
    ...
    def make_csrmatrix_noselfloop(self, neighbors: np.ndarray, distances: np.ndarray,
                                  auto_: bool = True, distance_factor=.01, weights_as_inv_dist = True, min_max_scale=False) -> csr_matrix:
        """
        ...
        min_max_scale: bool, default = False
            ...
            distances which then play a role in the transition matrix for trajectories.
        ...
        # mean distance.
        if (weights_as_inv_dist==True) & (min_max_scale==False):
            ...
            cols = neighbors[msk]
        if min_max_scale==True: #the jaccard similarity does not consider edge weights, we will use these min-max scaled weights to make a composite weight together with Jaccard to retain the distance based info
            ...
            weights = 1/weights
            #print('inved weights for current pc-dist', weights)
    ...
    def run_subPARC(self):
        ...
        sim = np.array(ig.Graph(n=self.nsamples, edges=edges.tolist(), edge_attrs={'weight': adjacency.data}).similarity_jaccard(pairs=edges)) #used to do .similarity_jaccard(pairs=edges) here but then it meant that we dont carry the weight information towards the global pruning
        #sim = adjacency.data
        tot = len(sim)
        ...
        if self.is_coarse:
            ...
            csr_full_graph = self.make_csrmatrix_noselfloop(neighbors, distances, auto_=False, min_max_scale=False) #no local pruning: auto_ set to false #min_max_scale=True
            ...
            if ((self.pca_loadings is not None) & (self.velocity_matrix is not None)) &(self.gene_matrix is not None):
                ...
                self.ig_full_graph = ig.Graph(edges_preClustergraph,
                                              edge_attrs={'weight': composite_weight_projected.tolist()}).simplify( combine_edges='sum')
            ...
            #ig_fullgraph = ig.Graph(edges, edge_attrs={'weight': g_global_jac_preClustergraph}).simplify(combine_edges='sum')
                ...
                composite_weight = np.multiply(csr_full_graph.data, np.array(global_jac_preClustergraph_sim_list))
                ...
                self.ig_full_graph = ig.Graph(edges_preClustergraph,    edge_attrs={'weight': composite_weight.tolist()}).simplify(          combine_edges='sum')

            # for VIA we local and global prune the vertex cluster graph *after* making the clustergraph
            self.csr_array_locally_pruned = adjacency # this graph has only been locally pruned. it then subsequently in later code will be subject to global pruning, followed by Leiden clustering. saving it here for non-coarse runs
        ...
        #if using a different global pruning than the one specified in self.cluster_graph_pruning_std
        else:
        ...
        if (self.velocity_matrix is not None) & (self.gene_matrix is not None):
            ...
            self.A_velo, self.CSM = velocity_transition(A=df_graph.values, V=velocity_mean,
        ...
        for comp_i in large_components:  # range(n_components):
            ...
            if self.dataset in ['toy','faced','mESC','iPSC','group']:#((self.dataset == 'toy') | (self.dataset == 'faced')):
                ...
                if comp_i > len(self.root_user) - 1:
                    ...
                    for ri in self.root_user:
                        print(self.root_user, 'ri in rootuser', ri)
        ...
        else: final_transition_matrix_all_components = (self.velo_weight)*self.A_velo + (1-self.velo_weight)*(temp_csr.toarray())
        ...
        if self.edgebundle_pruning_twice == True:
            print('Visual cluster graph pruning for edge bundling in additional to initial single-cell level global pruning. Visual cluster pruning is at level:', self.visual_cluster_graph_pruning)
    ...
    def draw_piechart_graph(self, type_data='pt', gene_exp='', title='', cmap=None, ax_text=True, dpi=150,headwidth_bundle = 0.1):
        '''
        ...
        ax = plot_edge_bundle(ax, self.hammer_bundle, layout=self.graph_node_pos, CSM=self.CSM,
                                velocity_weight=self.velo_weight, pt=pt, headwidth_bundle=headwidth_bundle,
                                alpha_bundle=0.4)
        ...
        for i, ax_i in enumerate([ax1]):
            ...
            # print(gp_scaling, 'gp_scaling')
            group_pop_scale = group_pop * gp_scaling * 0.5
            ax_i=plot_edge_bundle(ax_i, self.hammer_bundle, layout=self.graph_node_pos,CSM=self.CSM, velocity_weight=self.velo_weight, pt=pt,headwidth_bundle=headwidth_bundle, alpha_bundle=0.4)
            ## ax_i.plot(hammer_bundle.x, hammer_bundle.y, 'y', zorder=2, linewidth=2, color='gray', alpha=0.8)
