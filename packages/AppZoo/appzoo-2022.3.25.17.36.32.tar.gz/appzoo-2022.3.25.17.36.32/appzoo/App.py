...
class App(object):
    ...
    def add_apps(self, app_dir='apps', prefix='/', **kwargs):
        """加载当前app_dir文件夹下的所有app（递归）, 入口函数都是main
        1. 过滤掉 _ 开头的py文件
        2. 支持单文件
        """

        sys.path.append(Path(app_dir).absolute())
        logger.info(Path(app_dir).absolute())
        routes = []
        if Path(app_dir).is_dir():
            for p in Path(app_dir).rglob('*.py'):
                if not p.stem.startswith('_'):  # 过滤_开头的py
                    ...
                    self.add_route(f"{Path(prefix) / Path(route)}", func, method='POST', **kwargs)

        else:
            route = app_dir = Path(app_dir).stem
            routes.append(app_dir)
