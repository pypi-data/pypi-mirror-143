...
def addLetterIn(request):
	...
	if request.method == "POST":
		newid = getlastid_kinos(LetterIn,str(userAddress.employee.village.id))
...
def addLetterOut(request):
	...
	userAddress = get_object_or_404(EmployeeUser,user__id=request.user.id)
	if request.method == "POST":
		newid = getlastid_kinos(LetterOut,str(userAddress.employee.village.id))
		hashid = hash_md5(str(newid))
...
def addExpedition(request,hashid):
	...
	if request.method == "POST":
		newid = getlastid_kinos(LetterOutExpedition,str(userAddress.employee.village.id))
		hashid = hash_md5(str(newid))
...
def uploadExpeditionFile(request,hashid):
	...
	letterOutData = get_object_or_404(LetterOut,hashed=hashid)
	if request.method == "POST":
		newid = getlastid_kinos(AttachedExpedition,str(userAddress.employee.village.id))
		hashid = hash_md5(str(newid))
...
def addComplaint(request):
	...
	userAddress = get_object_or_404(EmployeeUser,user__id=request.user.id)
	if request.method == "POST":
		newid = getlastid_kinos(Complaint,str(userAddress.employee.village.id))
		hashid = hash_md5(str(newid))
