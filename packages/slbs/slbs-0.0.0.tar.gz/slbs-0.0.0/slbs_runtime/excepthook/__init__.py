"""
By default, slbs overwrites sys.excepthook for better error reporting:

 1) Applications based on PyQt5 or PySide2 are missing some stack trace entries
    in their tracebacks (see add_missing_qt_frames(...) below). slbs makes sure
    they are displayed, for easier debugging.
 2) Python < 3.8 does not call sys.excepthook for non-main threads. slbs ensures
    that its own excepthook (and thus eg. the benefits of 1) above) does get
    called, so you see all errors.
 3) slbs lets you define multiple `ExceptionHandler`s - see below. This lets you
...
class _Excepthook:
    """
    slbs's excepthook. Forwards exceptions to the given handlers, until one of
