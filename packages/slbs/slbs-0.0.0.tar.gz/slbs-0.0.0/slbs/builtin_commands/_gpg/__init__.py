...
from slbs.builtin_commands._docker import _run_docker
from slbs.builtin_commands._util import prompt_for_value, update_json, \
...
_DOCKER_IMAGE = 'slbs/gpg-generator'
...
def gengpgkey():
    ...
    if exists(_DEST_DIR):
        raise slbsError('The %s folder already exists. Aborting.' % _DEST_DIR)
    try:
    ...
    result = _run_docker(args, check=True, stdout=PIPE,
