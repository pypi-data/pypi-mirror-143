...
def get_conductor_corners(roxie_data: APIdata):
                ...
                for winding_key, winding in layer.windings.items():
                    for block_key, block in winding.blocks.items():
                        for halfTurn_nr, halfTurn in block.half_turns.items():
                            idx_half_turn = idx_half_turn + 1
                            insulated = halfTurn.corners.insulated
                            bare = halfTurn.corners.bare
                            x_insulated.append([insulated.iL.x, insulated.oL.x, insulated.oR.x, insulated.iR.x])
                            y_insulated.append([insulated.iL.y, insulated.oL.y, insulated.oR.y, insulated.iR.y])
                            x_bare.append([bare.iL.x, bare.oL.x, bare.oR.x, bare.iR.x])
                            y_bare.append([bare.iL.y, bare.oL.y, bare.oR.y, bare.iR.y])
                            i_conductor.append(winding.cable_current * block.current_sign)

                            for strand_gr_nr, strand_gr in halfTurn.strand_groups.items():
                                for strand_nr, strand in strand_gr.strand_positions.items():
                                    x_strand.append(strand.x)
                                    y_strand.append(strand.y)
                                    i_strand.append(winding.strand_current * block.current_sign)
                                    strandToHalfTurn.append(idx_half_turn)

    return x_insulated, y_insulated, x_bare, y_bare, i_conductor, x_strand, y_strand, i_strand, strandToHalfTurn
                                ...
                                if lineType == 'Line':
                                    ...
                                    if alpha != 0:
                                        case = [(abs(alpha) < np.pi / 2) & (alpha > 0),
                                                ...
                                                (abs(alpha) < np.pi / 2) & (alpha < 0),
                                                (abs(alpha) > np.pi / 2) & (alpha < 0)]
                                        line_a = geometricFunctions.findLineThroughTwoPoints(
                                    ...
                                    if beta != 0:
                                        case = [(abs(beta) < np.pi / 2) & (beta > 0),
                                                ...
                                                (abs(beta) < np.pi / 2) & (beta < 0),
                                                (abs(beta) > np.pi / 2) & (beta < 0)]
                                        line_b = geometricFunctions.findLineThroughTwoPoints(
