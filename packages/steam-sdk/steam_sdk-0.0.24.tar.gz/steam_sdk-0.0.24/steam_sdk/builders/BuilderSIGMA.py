...
class BuilderSIGMA:
    ...
    def save_compile_and_open_bat(self):
        ...
        script_lines += [
            f'"{self.COMSOL_compile_path}" -jdkroot "{self.java_jdk_path}" {class_paths}'
            f'"{self.model_java_file_path}"',
...
if __name__ == "__main__":
    ...
    magnet_name = 'MQY_2in1'  #'MQXF_V2'  # 'MBX'  #
    current_path = Path(__file__).parent
    path = Path.joinpath(current_path.parent.parent, 'tests', 'builders', 'model_library','magnets', magnet_name, 'input')
    ...
    path_cadata = Path.joinpath(path.parent, 'roxie.cadata')
