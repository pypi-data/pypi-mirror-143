...
from steam_sdk.data import DataModel, DictionaryProteCCT
...
class BuilderProteCCT:
    ...
    def __init__(self,
                 input_model_data: DataModel = None,
        ...
        if not self.model_data and flag_build:
            raise Exception('Cannot build model instantly without providing DataModel')

        if flag_build:
            # Add method to translate all ProteCCT parameters from model_data to ProteCCT dataclasses
            self.translateModelDataToProteCCT()

            # Load conductor data from DataModel keys, and calculate+set relevant parameters
    ...
    def translateModelDataToProteCCT(self):
        """"
            Translates and sets parameters in self.DataModel to DataProteCCT if parameter exists in ProteCCT
        """
    ...
    def loadConductorData(self):
        '''
            Load selected conductor data from DataModel keys, check inputs, calculate and set missing variables
        '''
