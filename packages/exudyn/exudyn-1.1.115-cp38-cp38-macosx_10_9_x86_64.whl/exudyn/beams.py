...
def GenerateStraightLineANCFCable2D(mbs, positionOfNode0, positionOfNode1, numberOfElements, cableTemplate,
    ...
    if len(positionOfNode0) != 3:
        print('WARNING: GenerateStraightLineANCFCable2D: positionOfNode0 should be a 3D vector')
    if len(positionOfNode1) != 3:
        print('WARNING: GenerateStraightLineANCFCable2D: positionOfNode1 should be a 3D vector')
    ...
    for i in range(numberOfElements): 
        ...
        # print('p'+str(i)+'=',positionOfCurrentNode)
...
def GenerateCircularArcANCFCable2D(mbs, positionOfNode0, radius, startAngle, arcAngle, 
    ...
    cableCoordinateConstraintList=[]
    if len(positionOfNode0) != 3:
        print('WARNING: GenerateCircularArcANCFCable2D: positionOfNode0 should be a 3D vector')
    ...
    if (nodeNumber0!=-1):
        ...
        cableNodeList+=[nCable0]
        if verboseMode:
            print('  node 0: pos=', positionOfNode0[0],',',positionOfNode0[1],', slope=',cableSlopeVec[0],',',cableSlopeVec[1])
    ...
    for i in range(numberOfElements): 
        ...
        # print('pArc'+str(i)+'=',pArc)
        # print('vArc'+str(i)+'=',vArc)
        
        positionOfCurrentNode=[pArc[0],pArc[1],0.]
        cableNodePositionList+=[positionOfCurrentNode]

        if verboseMode:
            print('  node '+str(i+1)+': pos=', positionOfCurrentNode[0],',',positionOfCurrentNode[1],', slope=',vArc[0],',',vArc[1])
        ...
        # print('cableNodeList=',cableNodeList)
...
def CreateReevingCurve(circleList, drawingLinesPerCircle = 64, numberOfANCFnodes=-1, 
    ...
    for i, circle in enumerate(circleList):
        ...
        vv = pos1-pos0
        L = np.linalg.norm(vv)
        if L==0: raise ValueError('CreateReevingCurve: distance of circles is zero')
        vv0 = (1./L)*vv
        ...
        # print('circle',i,', alpha=',alpha,', p0=',p0,', p1=',p1)
        # print('  vv0=',vv0,', nv0=', nv0, ', sign0=',sign0, ', sign1=',sign1)
        # print('  r0=',r0,', r1=', r1,', pos0=', pos0,', pos1=', pos1)
        ...
        if not ((removeFirstLine and i==0) or
                (removeLastLine and i==len(circleList)-1)):
            curve2D += [{'type':'LINE', 'point0':p0, 'point1':p1}]
            totalLength += np.linalg.norm(p1-p0)
    ...
    print('Reeving system total length=', totalLength)
    # print('ANCF nodes=', ancfPointsSlopes)
    ...
    return {'ancfPointsSlopes':ancfPointsSlopes, 'elementLengths':elementLengths, 
            'totalLength':totalLength, 'circleData':curve2D, 
