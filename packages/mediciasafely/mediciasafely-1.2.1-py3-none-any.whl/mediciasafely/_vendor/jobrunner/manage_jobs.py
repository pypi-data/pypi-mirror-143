...
from mediciasafely._vendor.jobrunner import config
from mediciasafely._vendor.jobrunner.lib import docker
from mediciasafely._vendor.jobrunner.lib.database import find_one
from mediciasafely._vendor.jobrunner.lib.git import checkout_commit
from mediciasafely._vendor.jobrunner.lib.path_utils import list_dir_with_ignore_patterns
...
from mediciasafely._vendor.jobrunner.models import SavedJobRequest, State, StatusCode
from mediciasafely._vendor.jobrunner.project import (
...
import mediciasafely._vendor.jobrunner.patients
...
# created
TIMESTAMP_REFERENCE_FILE = ".mediciasafely-timestamp"
...
def start_job(job):
    ...
    env = {"MEDICIASAFELY_BACKEND": config.BACKEND}
...
def finalise_job(job):
    ...
    log_dir = get_log_dir(job) # Only for temporary saving log files.
    ...
    for filename in job.output_files:
        ...
        if job.run_command.lower().strip().find("cohortextractor:", 0, 16) == 0:
            ...
            result, detected_columns_names, unique_columns_names = mediciasafely._vendor.jobrunner.patients.inspect_pat_ID(data_file_path=workspace_dir / filename)
            ...
            if result:
                ...
                data_hashed = mediciasafely._vendor.jobrunner.patients.hash_columns(data_file_path=workspace_dir / filename, 
...
def find_matching_outputs(job):
    ...
    # print("volume_name(job)", volume_name(job))
    # print("all_matches", all_matches)
    # print("outputs", outputs)
    # print("unmatched_patterns", unmatched_patterns)
    # print("####################")
...
SAFE_ENVIRONMENT_VARIABLES = set(
    ...
    MEDICIASAFELY_BACKEND TZ TEMP_DATABASE_NAME PYTHONPATH container LANG LC_ALL
    """.split()
)
