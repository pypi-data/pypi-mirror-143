...
from mediciasafely._vendor.jobrunner import config, executors, manifest_to_database_migration
from mediciasafely._vendor.jobrunner.create_or_update_jobs import (
    RUN_ALL_COMMAND,
    JobRequestError,
    NothingToDoError,
...
from mediciasafely._vendor.jobrunner.lib import database, docker
from mediciasafely._vendor.jobrunner.lib.database import find_where
from mediciasafely._vendor.jobrunner.lib.log_utils import configure_logging
from mediciasafely._vendor.jobrunner.lib.string_utils import tabulate
from mediciasafely._vendor.jobrunner.lib.subprocess_utils import subprocess_run
from mediciasafely._vendor.jobrunner.manage_jobs import METADATA_DIR
from mediciasafely._vendor.jobrunner.models import Job, JobRequest, State, StatusCode, random_id
from mediciasafely._vendor.jobrunner.project import UnknownActionError, get_all_actions
from mediciasafely._vendor.jobrunner.queries import calculate_workspace_state
from mediciasafely._vendor.jobrunner.reusable_actions import (
    ReusableActionError,
    resolve_reusable_action_references,
)
from mediciasafely._vendor.jobrunner.run import main as run_main
...
def main(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="mediciasafely_"))
    if not debug:
...
def create_and_run_jobs(
    ...
    # The project_dir points to the research-study project.
    ...
    # print("project_dir.parent", project_dir.parent)
    # print("config.HIGH_PRIVACY_WORKSPACES_DIR", config.HIGH_PRIVACY_WORKSPACES_DIR)
    # print("config.DATABASE_FILE", config.DATABASE_FILE)
    # print("%%%%%%%%%%%%%%%%%%%%%%")
    ...
    if uses_stata and config.STATA_LICENSE is None:
        ...
        if config.STATA_LICENSE is None:
            print(
                ...
                "\n"
                "If you are a member of MediciaSAFELY we should have been able to fetch\n"
                ...
                "\n"
                "If you are not a member of MediciaSAFELY you will have to provide your\n"
    ...
    for job in final_jobs:
        ...
        if format_output_for_github:
            print(f"::group:: log file: {log_file} {ANSI.Grey}(click to view){ANSI.Reset}")
...
def get_stata_license(repo=config.STATA_LICENSE_REPO):
    ...
    cached = Path(f"{tempfile.gettempdir()}/mediciasafely-stata.lic")
    ...
    if fetch:
        try:
            tmp = tempfile.TemporaryDirectory(suffix="mediciasafely")
