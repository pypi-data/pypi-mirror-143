...
import mediciasafely

# from mediciasafely._vendor import requests
import requests

DESCRIPTION = "Upgrade the mediciasafely cli tool."

CACHE_FILE = Path(tempfile.gettempdir()) / "mediciasafely-latest-version"
...
def main(version):
    ...
    if not need_to_update(version):
        print(f"mediciasafely is already at version {version}")
        return 0
    ...
    if os.name == "nt" and entrypoint_bin.exists():
        tmp_file = entrypoint_bin.with_suffix(".exe._mediciasafely_.tmp")
...
def get_latest_version(force=False):
    ...
    if force or latest is None:
        resp = requests.get("https://pypi.org/pypi/mediciasafely/json").json()
...
def need_to_update(latest):
    current = None
    current = mediciasafely.__version__.lstrip("v")
...
def check_version():
    ...
    if update:
        print(
            f"Warning: there is a newer version of mediciasafely available ({latest}) - please upgrade by running:\n"
            "    mediciasafely upgrade\n"
        )
    return update
