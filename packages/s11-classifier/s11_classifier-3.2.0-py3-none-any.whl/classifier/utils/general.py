...
from classifier.utils.config import Configuration, setup_config

UTILS_GENERAL_LOGGER = logging.getLogger(__name__)


def get_available_model_args(model_args: dict, function: Callable) -> dict:
...
def cli_init(
    ...
    rois: Optional[str] = None) \
        -> Tuple[Path, List[Path], Optional[Path], Configuration]:
    ...
    Args:
        ...
        config_location (str, Path): location of the config file
    ...
    config.tmp_dir = output_directory / 'tmp'
...
def impute_values(dataset: pd.DataFrame, config: Configuration) -> np.ndarray:
    """Impute values
...
def write_tifs(
        temp_dir: Path, window: rasterio.windows.Window, meta: dict,
...
def impute(data_array: np.ndarray,
           config: Configuration) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    Returns:
        imputed_array (np.ndarray): imputed array
...
def get_raster_paths(workspace: Path, rasters: List[str]) -> List[Path]:
    ...
    for raster in rasters:
        ...
        if path.is_dir():
            ...
            paths.append(path)
    return [path for path in paths if path.suffix in RASTER_EXTENSIONS]
