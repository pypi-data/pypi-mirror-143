...
class RequestError(PointError):
    ...
    def __init__(self, url=None, detail=None, status_code=None):
    ...
    def __str__(self):
        return f'{self.__status_code}:{self.__url}:{self.__detail}'
    ...
    def get_full_details(self):
        if not status.is_client_error(self.__status_code):
            raise PointError('Full details only for client errors')
        return {
            ...
            if not status.is_success(response.status_code):
                if status.is_client_error(response.status_code):
                    if status.is_not_found(response.status_code):
                        ...
                        raise ClientError(self._point_id.url, response.json(), response.status_code)
                else:
                    raise ServerError(self._point_id.url, response.text, response.status_code)
