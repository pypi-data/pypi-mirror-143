...
class LixingrenIndexData:
    ...
    def __init__(self, code: str, start_date: str = None, end_date: str = None, force_update_db: bool = False):
        self.__v_init(code=code, start_date=start_date, end_date=end_date)
        # 从db中取回现有日期
        ...
        if (len(item_list) == 0) or force_update_db:  # 需要从API获取至db
            ...
            self.__dict_data_raw = self.__save_index_data_to_db(self.__retrieve_index_data())  # 重新获取
            ...
            if today_period > existed_period:  # 差异超过周期，更新数据！
                ...
                self.__save_index_data_to_db(self.__retrieve_index_data(start_date=start_date.strftime('%Y-%m-%d')))
            else:
                logger.info('code：%s数据存在，db中最新日期是%s，无需更新....' % (self.code, existed_latest_date))

        self.__populate_item_raw_to_object()

    def __v_init(self, code: str, start_date: str = None, end_date: str = None):
        self.code = code
    ...
    def __retrieve_index_data(self, start_date: str = '2000-01-01'):
        logger.info(
            "retrieving index data for %s - from <%s> to <%s>" % (
                self.code, start_date, self.__today_date.strftime("%Y-%m-%d")))
        request_params = {
            "token": const.LIXINGREN_TOKEN,
            "startDate": start_date,
            "endDate": self.__today_date.strftime("%Y-%m-%d"),
            "stockCodes": [self.code],
            "metricsList": self.__field_params
        }
        if self.code == '.INX':  # 处理标普500指数
            api_url = 'https://open.lixinger.com/api/us/index/fundamental'
        else:
            api_url = 'https://open.lixinger.com/api/cn/index/fundamental'
        dict_index = ut.request_post_json(
            api_url=api_url,
    ...
    def __save_index_data_to_db(self, dict_index_data):
        for item in dict_index_data:  # 修改日期格式为YYYYMMDD
            item['date'] = pd.to_datetime(item['date']).strftime('%Y%m%d')
        ut.db_save_dict_to_mongodb(
            mongo_db_name=const.MONGODB_DB_LXR,
            col_name=const.MONGODB_COL_LXR_INDEX,
            target_dict=dict_index_data
        )
        return dict_index_data

    def __populate_item_raw_to_object(self):
        self.name = const.PARAMS['lxr_index'][self.code]
    ...
    def build_index_df(self, type: str, granularity: str, metrics_type: str):
        dt = {}
