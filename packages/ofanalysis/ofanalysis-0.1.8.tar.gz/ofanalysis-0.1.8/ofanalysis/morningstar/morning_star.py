...
from ofanalysis.morningstar.lib.mysnowflake import IdWorker
import numpy as np
import requests
...
class MorningStar:
    ...
    def get_fund_list(self):
        ...
        chrome_driver = webdriver.Chrome(executable_path=self.web_driver_path, options=chrome_options)
        ...
        while page_num <= page_num_total:
            ...
            xpath_str = '/html/body/form/div[8]/div/div[4]/div[3]/div[3]/div[1]/a[%s]' % (
                num)
            logger.info(f'processing page_num {page_num}')
            # 等待，直到当前页（样式判断）等于page_num
            WebDriverWait(chrome_driver, timeout=600).until(self.__text_to_be_present_in_element(
