...
class Client:
        ...
        for c in content:
            if isinstance(c, str):
                ...
                if c.content_type in ('text', 'blob'):
                    ...
                    raise TypeError(f'unsupported type {c!r} {c.content_type}')
            else:
                raise TypeError(f'unsupported type {c!r}')
    ...
    def _get_post_payload(self, content, kwargs):
        return dict(
            ...
            request_size=kwargs.get('batch_size', 128),
        )
    ...
    def profile(self, content: Optional[str] = '') -> None:
        """Profiling the full roundtrip including network and compuation latency. Results is summarized in a table.
