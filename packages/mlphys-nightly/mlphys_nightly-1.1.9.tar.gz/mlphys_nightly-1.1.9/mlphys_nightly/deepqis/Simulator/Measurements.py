...
class Ideal_Measurements:
    ...
    def __init__(self, qs=2):
        self._qs = qs
        if not os.path.exists(f'./projectors/projectors_array_qs_{self._qs}.pickle'):
            print('| To accelerate the simulation, a new projector folder is created.')
            os.mkdir('projectors')
            mqs = MultiQubitSystem(qubit_size=self._qs)
            proj = mqs.NISQ_Projectors()
            with open(f'./projectors/projectors_array_qs_{self._qs}.pickle', 'wb') as f:
                pkl.dump(proj, f, -1)
            self.projectors = proj
        else:
            self.projectors = pd.read_pickle(f'./projectors/projectors_array_qs_{self._qs}.pickle')
...
class Random_Measurements:
    ...
    def __init__(self, qs=2, n_meas=1024):
        ...
        if not os.path.exists(f'./projectors/projectors_array_qs_{self._qs}_general_scheme.pickle'):
            print('| To accelerate the simulation, General Scheme Projector file is created in a new projectors folder.')
            mqs = MultiQubitSystem(qubit_size=self._qs)
...
class NISQ_Shots:
    ...
    def __init__(self, qs=2, shots=1024):
        ...
        if not os.path.exists(f'./projectors/projectors_array_qs_{self._qs}.pickle'):
            print('| To accelerate the simulation, Projectors file is created in a new projectors folder.')
            mqs = MultiQubitSystem(qubit_size=self._qs)
            proj = mqs.NISQ_Projectors()
            os.mkdir('projectors')
