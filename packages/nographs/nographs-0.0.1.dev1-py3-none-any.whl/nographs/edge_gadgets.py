...
def adapt_edge_index(
    ...
    :param index: Mapping or Sequence with vertices as key resp. index. For each vertex, you provide
        an Iterable of adjacent vertices (this defines unlabeled edges) or an Iterable of the
        labeled edges that start at this vertex. A labeled edge to some neighbor need to be a
        Sequence of the form (neighbor, additional data...).

    :param add_inverted: The resulting neighbor function will yield the edges of m.
        Additionally, if add_inverted_edges is True, it will provide the same edges inverted (start
        end end vertex exchanged).

    :param labeled: Give True, if *index* provides labeled edges. In this case, you can use the
        resulting neighbor function as next_edges function. Give False, if m provides neighbor
        vertices for a given vertex. In this case, you can use the resulting neighbor function as
        next_vertices function.

    :return: Neighbor function that can be used as parameter for one of the traversal algorithms.
    """
...
def adapt_edge_iterable(
    ...
    :param add_inverted: The resulting neighbor function will yield your edges.
        Additionally, if add_inverted_edges is True, it will provide the same edges inverted (start
        and end vertices exchanged). Note: when this option is used, a copy of your graph will be
        held by the returned neighbor function.

    :param labeled: If set to True, the resulting neighbor function will yield labeled edges (
        to_vertex, additional data ...) for a given vertex and can be used as next_edges function.
        If set to False, it will yield neighbor vertices, can be used as next_vertices function,
        and additional data that your edges may contain will be ignored.
    ...
    if add_inverted:
        if labeled:
            ...
            # Only start and end vertices of the edge should be used. Optional additional data is
            # ignored.
            for from_vertex, to_vertex, *data in edges:
