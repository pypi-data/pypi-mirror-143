...
def is_version_upgrade(app="neo", neocli_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, neocli_path=neocli_path)
	...
	local_version = get_major_version(get_current_version(app, neocli_path=neocli_path))
...
def switch_branch(branch, apps=None, neocli_path=".", upgrade=False, check_upgrade=True):
	import git
	from neocli.neocli import NeoCLI
	from neocli.utils import log, exec_cmd
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, neocli_path=neocli_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `neocli update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		NeoCLI(neocli_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, neocli_path=".", upgrade=False):
	switch_branch(branch, apps=apps, neocli_path=neocli_path, upgrade=upgrade)


def switch_to_develop(apps=None, neocli_path=".", upgrade=True):
	switch_branch("develop", apps=apps, neocli_path=neocli_path, upgrade=upgrade)
...
def get_develop_version(app, neocli_path="."):
	repo_dir = get_repo_dir(app, neocli_path=neocli_path)
...
def get_upstream_version(app, branch=None, neocli_path="."):
	repo_dir = get_repo_dir(app, neocli_path=neocli_path)
	if not branch:
...
def get_current_neo_version(neocli_path="."):
	try:
		return get_major_version(get_current_version("neo", neocli_path=neocli_path))
	except IOError:
		return 0


def get_current_branch(app, neocli_path="."):
	from neocli.utils import get_cmd_output

	repo_dir = get_repo_dir(app, neocli_path=neocli_path)
	return get_cmd_output("basename $(git symbolic-ref -q HEAD)", cwd=repo_dir)


def get_remote(app, neocli_path="."):
	repo_dir = get_repo_dir(app, neocli_path=neocli_path)
	contents = subprocess.check_output(
...
def get_app_name(neocli_path, repo_name):
	app_name = None
...
def get_current_version(app, neocli_path="."):
