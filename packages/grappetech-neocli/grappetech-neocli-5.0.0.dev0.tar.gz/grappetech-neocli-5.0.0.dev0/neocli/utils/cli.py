...
def print_neocli_version(ctx, param, value):
	"""Prints current neocli version"""
	if not value or ctx.resilient_parsing:
		return

	import neocli

	click.echo(neocli.VERSION)
	ctx.exit()
...
def use_experimental_feature(ctx, param, value):
	...
	if value == "dynamic-feed":
		...
		neocli.cli.dynamic_feed = True
		neocli.cli.verbose = True
	else:
		from neocli.exceptions import FeatureDoesNotExistError

		raise FeatureDoesNotExistError(f"Feature {value} does not exist")

	from neocli.cli import is_envvar_warn_set

	if is_envvar_warn_set:
		return

	click.secho(
		"WARNING: neocli is using it's new CLI rendering engine. This behaviour has"
