...
from neocli import PROJECT_NAME, VERSION

from neocli.exceptions import CommandFailedError, InvalidRemoteException, ValidationError


logger = logging.getLogger(PROJECT_NAME)
neocli_cache_file = ".neocli.cmd"
paths_in_app = ("hooks.py", "modules.txt", "patches.txt")
paths_in_neocli = ("apps", "sites", "config", "logs", "config/pids")
sudoers_file = "/etc/sudoers.d/neo"


def is_neocli_directory(directory=os.path.curdir):
	is_neocli = True

	for folder in paths_in_neocli:
		path = os.path.abspath(os.path.join(directory, folder))
		is_neocli = is_neocli and os.path.exists(path)

	return is_neocli


def is_neo_app(directory: str) -> bool:
	is_neo_app = True
	...
	for folder in paths_in_app:
		...
		is_neo_app = is_neo_app and path

	return bool(is_neo_app)
...
def log(message, level=0, no_log=False):
	...
	if neocli.cli.from_command_line and neocli.cli.dynamic_feed:
		neocli.LOG_BUFFER.append(
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/grappetech-neocli/json")
	except Exception:
	...
	if pypi_request.status_code == 200:
		...
		if pypi_version > local_version:
			log(f"A newer version of neocli is available: {local_version} → {pypi_version}")
...
def setup_logging(neocli_path=".") -> "logger":
	LOG_LEVEL = 15
	...
	if os.path.exists(os.path.join(neocli_path, "logs")):
		log_file = os.path.join(neocli_path, "logs", "neocli.log")
...
def run_neo_cmd(*args, **kwargs):
	from neocli.cli import from_command_line
	from neocli.utils.neocli import get_env_cmd

	neocli_path = kwargs.get("neocli_path", ".")
	f = get_env_cmd("python", neocli_path=neocli_path)
	sites_dir = os.path.join(neocli_path, "sites")
	...
	p = subprocess.Popen(
		(f, "-m", "neo.utils.neocli_helper", "neo") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_neocli_name(neocli_path):
	return os.path.basename(os.path.abspath(neocli_path))


def set_git_remote_url(git_url, neocli_path="."):
	"Set app remote git url"
	from neocli.app import get_repo_dir
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(neocli.__path__[0], "playbooks"))


def find_neoclies(directory: str = None) -> List:
	if not directory:
	...
	if is_neocli_directory(directory):
		if os.path.curdir == directory:
			print("You are in a neocli directory!")
		else:
			print(f"{directory} is a neocli directory!")
		return
	...
	for sub in os.listdir(directory):
		...
		if os.path.isdir(sub) and not os.path.islink(sub):
			if is_neocli_directory(sub):
				print(f"{sub} found!")
				neoclies.append(sub)
			else:
				neoclies.extend(find_neoclies(sub))
...
def find_parent_neocli(path: str) -> str:
	"""Checks if parent directories are neoclies"""
	if is_neocli_directory(directory=path):
		return path
	...
	if path not in {home_path, root_path}:
		...
		return find_parent_neocli(parent_dir)


def generate_command_cache(neocli_path=".") -> List:
	"""Caches all available commands (even custom apps) via Neo
	Default caching behaviour: generated the first time any command (for a specific neocli directory)
	"""
	from neocli.utils.neocli import get_env_cmd
	...
	if os.path.exists(neocli_cache_file):
		os.remove(neocli_cache_file)

	try:
		output = get_cmd_output(
			f"{python} -m neo.utils.neocli_helper get-neo-commands", cwd=sites_path
		)
		with open(neocli_cache_file, "w") as f:
...
def clear_command_cache(neocli_path="."):
	...
	Default invalidation behaviour: destroyed on each run of `neocli update`
	"""

	if os.path.exists(neocli_cache_file):
		os.remove(neocli_cache_file)
	else:
		print("NeoCLI command cache doesn't exist in this folder!")
...
class _dict(dict):
	...
	# neocli port of neo._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix neo#14833 via DFP
