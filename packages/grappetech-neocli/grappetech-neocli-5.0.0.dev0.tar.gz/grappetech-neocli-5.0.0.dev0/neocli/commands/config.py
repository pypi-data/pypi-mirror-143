...
from neocli.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help='Change neocli configuration')
def config():
	pass
...
@click.command('dns_multitenant', help='Enable/Disable neocli multitenancy on running neocli update')
@click.argument('state', type=click.Choice(['on', 'off']))
...
def set_common_config(configs):
	...
	update_config(common_site_config, neocli_path='.')


@click.command('remove-common-config', help='Remove specific keys from current neocli\'s common config')
@click.argument('keys', nargs=-1)
def remove_common_config(keys):
	from neocli.neocli import NeoCLI
	common_site_config = NeoCLI('.').conf
	for key in keys:
