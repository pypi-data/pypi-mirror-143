...
from neocli.utils.neocli import post_upgrade, patch_sites, build_assets


@click.command('update', help="Performs an update operation on current neocli. Without any flags will backup, pull, setup requirements, build, run patches and restart neocli. Using specific flags will only do certain tasks instead of all")
@click.option('--pull', is_flag=True, help="Pull updates for all the apps in neocli")
@click.option('--apps', type=str)
@click.option('--patch', is_flag=True, help="Run migrations for all sites in the neocli")
@click.option('--build', is_flag=True, help="Build JS and CSS assets for the neocli")
@click.option('--requirements', is_flag=True, help="Update requirements. If run alone, equivalent to `neocli setup requirements`")
...
def update(pull, apps, patch, build, requirements, restart_supervisor, restart_systemd, no_backup, no_compile, force, reset):
	from neocli.utils.neocli import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from neocli.utils.app import switch_to_branch
	switch_to_branch(branch=branch, apps=list(apps), upgrade=upgrade)


@click.command('switch-to-develop')
def switch_to_develop(upgrade=False):
	"Switch neo and erpneo to develop branch"
	from neocli.utils.app import switch_to_develop
	switch_to_develop(apps=['neo', 'erpneo'])
