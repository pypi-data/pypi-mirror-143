...
from neocli.utils import exec_cmd, run_playbook


@click.group(help="Setup command group for enabling setting up a Neo environment")
def setup():
	pass
...
def setup_sudoers(user):
	from neocli.utils.system import setup_sudoers
	setup_sudoers(user)
...
def setup_nginx(yes=False):
	...
	make_nginx_conf(neocli_path=".", yes=yes)


@click.command("reload-nginx", help="Checks NGINX config file and reloads service")
def reload_nginx():
	from neocli.config.production_setup import reload_nginx
...
def setup_supervisor(user=None, yes=False, skip_redis=False):
	from neocli.config.supervisor import update_supervisord_config, generate_supervisor_config

	update_supervisord_config(user=user, yes=yes)
	generate_supervisor_config(neocli_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
@click.command("fonts", help="Add Neo fonts to system")
def setup_fonts():
	from neocli.utils.system import setup_fonts
	setup_fonts()


@click.command("production", help="Setup Neo production environment for specific user")
@click.argument("user")
@click.option("--yes", help="Yes to regeneration config", is_flag=True, default=False)
def setup_production(user, yes=False):
	from neocli.config.production_setup import setup_production
	setup_production(user=user, yes=yes)


@click.command("backups", help="Add cronjob for neocli backups")
def setup_backups():
	from neocli.neocli import NeoCLI
	NeoCLI(".").setup.backups()


@click.command("env", help="Setup virtualenv for neocli")
@click.option("--python", type = str, default = "python3", help = "Path to Python Executable.")
def setup_env(python="python3"):
	from neocli.neocli import NeoCLI
	return NeoCLI(".").setup.env(python=python)
...
def setup_firewall(ssh_port=None, force=False):
	...
	run_playbook("roles/neocli/tasks/setup_firewall.yml", {"ssh_port": ssh_port})
...
def set_ssh_port(port, force=False):
	...
	run_playbook("roles/neocli/tasks/change_ssh_port.yml", {"ssh_port": port})
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	from neocli.config.lets_encrypt import setup_letsencrypt
	setup_letsencrypt(site, custom_domain, neocli_path=".", interactive=not non_interactive)


@click.command("wildcard-ssl", help="Setup wildcard SSL certificate for multi-tenant neocli")
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	from neocli.config.lets_encrypt import setup_wildcard_ssl
	setup_wildcard_ssl(domain, email, neocli_path=".", exclude_base_domain=exclude_base_domain)


@click.command("procfile", help="Generate Procfile for neocli start")
def setup_procfile():
...
def setup_requirements(node=False, python=False, dev=False):
	...
	if not (node or python or dev):
		...
		from neocli.utils.neocli import install_python_dev_dependencies
...
@click.command("manager", help="Setup neocli-manager.local site with the neocli_manager app installed on it")
@click.option("--yes", help="Yes to regeneration of nginx config file", default=False, is_flag=True)
@click.option("--port", help="Port on which you want to run neocli manager", default=23624)
@click.option("--domain", help="Domain on which you want to run neocli manager")
def setup_manager(yes=False, port=23624, domain=None):
	from neocli.neocli import NeoCLI
	from neocli.config.nginx import make_neocli_manager_nginx_conf

	create_new_site = True

	if "neocli-manager.local" in os.listdir("sites"):
		create_new_site = click.confirm("Site already exists. Overwrite existing site?")

	if create_new_site:
		exec_cmd("neocli new-site --force neocli-manager.local")

	if "neocli_manager" in os.listdir("apps"):
		print("App already exists. Skipping app download.")
	else:
		exec_cmd("neocli get-app neocli_manager")

	exec_cmd("neocli --site neocli-manager.local install-app neocli_manager")

	neocli_path = "."
	neocli = NeoCLI(neocli_path)

	if neocli.conf.get("restart_supervisor_on_update") or neocli.conf.get("restart_systemd_on_update"):
		# implicates a production setup or so I presume
		if not domain:
			print("Please specify the site name on which you want to host neocli-manager using the 'domain' flag")
			sys.exit(1)

		if domain not in neocli.sites:
			raise Exception("No such site")

		make_neocli_manager_nginx_conf(neocli_path, yes=yes, port=port, domain=domain)
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	from neocli.config.site_config import add_domain
	add_domain(site, domain, ssl_certificate, ssl_certificate_key, neocli_path=".")
...
def remove_domain(domain, site=None):
	...
	from neocli.config.site_config import remove_domain
	remove_domain(site, domain, neocli_path=".")
...
def sync_domains(domain=None, site=None):
	...
	from neocli.config.site_config import sync_domains
	changed = sync_domains(site, domains, neocli_path=".")
...
@click.option("--stop", help="Stop neocli services", is_flag=True, default=False)
...
def setup_systemd(user=None, yes=False, stop=False, create_symlinks=False, delete_symlinks=False):
	from neocli.config.systemd import generate_systemd_config
	generate_systemd_config(neocli_path=".", user=user, yes=yes,
