...
from neocli.utils import get_neocli_name


def make_nginx_conf(neocli_path, yes=False):
	conf_path = os.path.join(neocli_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		if not click.confirm('nginx.conf already exists and this will overwrite it. Do you want to continue?'):
			return

	template = neocli.config.env().get_template('nginx.conf')
	neocli_path = os.path.abspath(neocli_path)
	sites_path = os.path.join(neocli_path, "sites")

	config = NeoCLI(neocli_path).conf
	sites = prepare_sites(config, neocli_path)
	...
	if allow_rate_limiting:
		template_vars.update({
			'neocli_name_hash': hashlib.sha256(neocli_name).hexdigest()[:16],
...
def make_neocli_manager_nginx_conf(neocli_path, yes=False, port=23624, domain=None):
	from neocli.config.site_config import get_site_config

	template = neocli.config.env().get_template('neocli_manager_nginx.conf')
	neocli_path = os.path.abspath(neocli_path)
	sites_path = os.path.join(neocli_path, "sites")

	config = NeoCLI(neocli_path).conf
	site_config = get_site_config(domain, neocli_path=neocli_path)
	neocli_name = get_neocli_name(neocli_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"neocli_manager_site_name": "neocli-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get('webserver_port'),
		"socketio_port": config.get('socketio_port'),
	...
	neocli_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(neocli_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm('nginx.conf already exists and neocli-manager configuration will be appended to it. Do you want to continue?',
			abort=True)

	with open(conf_path, "a") as myfile:
		myfile.write(neocli_manager_nginx_conf)

def prepare_sites(config, neocli_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(neocli_path=neocli_path)
...
def get_sites_with_config(neocli_path):
	from neocli.neocli import NeoCLI
	from neocli.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, neocli_path=neocli_path)
	...
	use_wildcard_certificate(neocli_path, ret)

	return ret

def use_wildcard_certificate(neocli_path, ret):
	'''
		stored in common_site_config.json as:
		"wildcard": {
			"domain": "*.erpneo.com",
			"ssl_certificate": "/path/to/erpneo.com.cert",
			"ssl_certificate_key": "/path/to/erpneo.com.key"
		}
	'''
	from neocli.neocli import NeoCLI
	config = NeoCLI(neocli_path).conf
...
def get_error_pages():
	import neocli
	neocli_app_path = os.path.abspath(neocli.__path__[0])
	templates = os.path.join(neocli_app_path, 'config', 'templates')
