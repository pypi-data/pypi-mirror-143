...
from neocli.config.production_setup import service
from neocli.config.site_config import get_domains, remove_domain, update_site_config
from neocli.neocli import NeoCLI
from neocli.utils import exec_cmd
from neocli.utils.neocli import update_common_site_config
from neocli.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, neocli_path, interactive):

	site_path = os.path.join(neocli_path, "sites", site, "site_config.json")
	...
	if not NeoCLI(neocli_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, neocli_path, interactive)
	setup_crontab()


def create_config(site, custom_domain):
	config = neocli.config.env().get_template('letsencrypt.cfg').render(domain=custom_domain or site)
...
def run_certbot_and_setup_ssl(site, custom_domain, neocli_path, interactive=True):
	service('nginx', 'stop')
	get_certbot()
	...
	if custom_domain:
		remove_domain(site, custom_domain, neocli_path)
		domains = get_domains(site, neocli_path)
		ssl_config['domain'] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, { "domains": domains }, neocli_path=neocli_path)
	else:
		update_site_config(site, ssl_config, neocli_path=neocli_path)

	make_nginx_conf(neocli_path)
	service('nginx', 'start')
...
def setup_wildcard_ssl(domain, email, neocli_path, exclude_base_domain):
