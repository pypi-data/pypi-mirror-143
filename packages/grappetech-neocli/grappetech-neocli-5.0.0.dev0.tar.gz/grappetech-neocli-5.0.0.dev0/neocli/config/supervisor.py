...
from neocli.utils import get_neocli_name, which
from neocli.neocli import NeoCLI
from neocli.config.common_site_config import update_config, get_gunicorn_workers
...
def generate_supervisor_config(neocli_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective neocli path"""
	if not user:
		user = getpass.getuser()

	config = NeoCLI(neocli_path).conf
	template = neocli.config.env().get_template('supervisor.conf')
	...
	conf_path = os.path.join(neocli_path, 'config', 'supervisor.conf')
	...
	update_config({'restart_supervisor_on_update': True}, neocli_path=neocli_path)
	update_config({'restart_systemd_on_update': False}, neocli_path=neocli_path)
...
def update_supervisord_config(user=None, yes=False):
	"""From neocli v5.x, we're moving to supervisor running as user"""
	import configparser
