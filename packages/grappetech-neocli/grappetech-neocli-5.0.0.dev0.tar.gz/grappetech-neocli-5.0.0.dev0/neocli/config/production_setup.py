...
from neocli.config.supervisor import generate_supervisor_config, update_supervisord_config
from neocli.config.systemd import generate_systemd_config
from neocli.neocli import NeoCLI
from neocli.utils import exec_cmd, which, get_neocli_name, get_cmd_output, log
from neocli.utils.system import fix_prod_setup_perms
from neocli.exceptions import CommandFailedError
...
def setup_production_prerequisites():
	...
	if not which("fail2ban-client"):
		exec_cmd("neocli setup role fail2ban")
	if not which("nginx"):
		exec_cmd("neocli setup role nginx")
	if not which("supervisord"):
		exec_cmd("neocli setup role supervisor")


def setup_production(user, neocli_path='.', yes=False):
	...
	if conf.get('restart_systemd_on_update'):
		...
		generate_systemd_config(neocli_path=neocli_path, user=user, yes=yes)
	else:
		print("Setting Up supervisor...")
		update_supervisord_config(user=user, yes=yes)
		generate_supervisor_config(neocli_path=neocli_path, user=user, yes=yes)

	print("Setting Up NGINX...")
	make_nginx_conf(neocli_path=neocli_path, yes=yes)
	fix_prod_setup_perms(neocli_path, neo_user=user)
	...
	if conf.get('restart_supervisor_on_update'):
		...
		supervisor_conf = os.path.join(get_supervisor_confdir(), f'{neocli_name}.{supervisor_conf_extn}')

		# Check if symlink exists, If not then create it.
		if not os.path.islink(supervisor_conf):
			os.symlink(os.path.abspath(os.path.join(neocli_path, 'config', 'supervisor.conf')), supervisor_conf)

	if not os.path.islink(nginx_conf):
		os.symlink(os.path.abspath(os.path.join(neocli_path, 'config', 'nginx.conf')), nginx_conf)
...
def disable_production(neocli_path='.'):
	neocli_name = get_neocli_name(neocli_path)
	conf = NeoCLI(neocli_path).conf
