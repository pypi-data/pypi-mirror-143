...
tmp_neocli_repo = os.path.join('/', 'tmp', '.neocli')
...
def check_environment():
	...
	if message:
		log("NeoCLI's CLI needs these to be defined!", level=3)
...
def check_system_package_managers():
	if 'Darwin' in os.uname():
		if not shutil.which('brew'):
			raise Exception('''
			Please install brew package manager before proceeding with neocli setup. Please run following
...
def import_with_install(package):
	# copied from https://discuss.erpneo.com/u/nikunj_patel
	# https://discuss.erpneo.com/t/easy-install-setup-guide-for-erpneo-installation-on-ubuntu-20-04-lts-with-some-modification-of-course/62375/5
...
def install_prerequisites():
	# pre-requisites for neocli repo cloning
	run_os_command({
		'apt-get': [
			'sudo apt-get update',
...
def install_neocli(args):
	# clone neocli repo
	if not args.run_travis:
		clone_neocli_repo(args)
	...
	extra_vars.update(neo_user=args.user)

	extra_vars.update(user_directory=get_user_home_directory(args.user))

	if os.path.exists(tmp_neocli_repo):
		repo_path = tmp_neocli_repo
	else:
		repo_path = os.path.join(os.path.expanduser('~'), 'neocli')
	...
	if args.version <= 10:
		neo_branch = "{0}.x.x".format(args.version)
		erpneo_branch = "{0}.x.x".format(args.version)
	else:
		neo_branch = "version-{0}".format(args.version)
		erpneo_branch = "version-{0}".format(args.version)

	# Allow override of neo_branch and erpneo_branch, regardless of args.version (which always has a default set)
	if args.neo_branch:
		neo_branch = args.neo_branch
	if args.erpneo_branch:
		erpneo_branch = args.erpneo_branch

	extra_vars.update(neo_branch=neo_branch)
	extra_vars.update(erpneo_branch=erpneo_branch)

	neocli_name = 'grappetech-neocli' if not args.neocli_name else args.neocli_name
	extra_vars.update(neocli_name=neocli_name)

	# Will install ERP Neo production setup by default
	if args.without_erpneo:
		log("Initializing neocli {neocli_name}:\n\tNeo Branch: {neo_branch}\n\tERP Neo will not be installed due to --without-erpneo".format(neocli_name=neocli_name, neo_branch=neo_branch))
	else:
		log("Initializing neocli {neocli_name}:\n\tNeo Branch: {neo_branch}\n\tERP Neo Branch: {erpneo_branch}".format(neocli_name=neocli_name, neo_branch=neo_branch, erpneo_branch=erpneo_branch))
	run_playbook('site.yml', sudo=True, extra_vars=extra_vars)

	if os.path.exists(tmp_neocli_repo):
		shutil.rmtree(tmp_neocli_repo)


def clone_neocli_repo(args):
	'''Clones the neocli repository in the user folder'''
	branch = args.neocli_branch or 'develop'
	repo_url = args.repo_url or 'https://github.com/grappetech/neocli'

	if os.path.exists(tmp_neocli_repo):
		log('Not cloning already existing NeoCLI repository at {tmp_neocli_repo}'.format(tmp_neocli_repo=tmp_neocli_repo))
		return 0
	elif args.without_neocli_setup:
		clone_path = os.path.join(os.path.expanduser('~'), 'neocli')
		log('--without-neocli-setup specified, clone path is: {clone_path}'.format(clone_path=clone_path))
	else:
		clone_path = tmp_neocli_repo
		# Not logging repo_url to avoid accidental credential leak in case credential is embedded in URL
		log('Cloning neocli repository branch {branch} into {clone_path}'.format(branch=branch, clone_path=clone_path))

	success = run_os_command(
		{'git': 'git clone --quiet {repo_url} {neocli_repo} --depth 1 --branch {branch}'.format(
			repo_url=repo_url, neocli_repo=clone_path, branch=branch)}
	)
...
def get_passwords(args):
	...
	and creates passwords.txt in the neocli user's home directory
	"""
	log("Input MySQL and Neo Administrator passwords:")
	ignore_prompt = args.run_travis or args.without_neocli_setup
...
def run_playbook(playbook_name, sudo=False, extra_vars=None):
	...
	if os.path.exists(tmp_neocli_repo):
		...
		cwd = os.path.join(os.path.expanduser('~'), 'neocli')

	playbooks_locations = [os.path.join(cwd, 'neocli', 'playbooks'), os.path.join(cwd, 'playbooks')]
...
def parse_commandline_args():
	...
	parser = argparse.ArgumentParser(description='Neo Installer')
	...
	args_group.add_argument('--production', dest='production', action='store_true', default=False, help='Setup Production environment for neocli')
	parser.add_argument('--site', dest='site', action='store', default='site1.local', help='Specify name for your first ERP Neo site')
	...
	parser.add_argument('--user', dest='user', help='Install grappetech-neocli for this user')
	parser.add_argument('--neocli-branch', dest='neocli_branch', help='Clone a particular branch of neocli repository')
	parser.add_argument('--repo-url', dest='repo_url', help='Clone neocli from the given url')
	parser.add_argument('--neo-repo-url', dest='neo_repo_url', action='store', default='https://github.com/grappetech/neo', help='Clone neo from the given url')
	parser.add_argument('--neo-branch', dest='neo_branch', action='store', help='Clone a particular branch of neo')
	parser.add_argument('--erpneo-repo-url', dest='erpneo_repo_url', action='store', default='https://github.com/grappetech/erpneo', help='Clone erpneo from the given url')
	parser.add_argument('--erpneo-branch', dest='erpneo_branch', action='store', help='Clone a particular branch of erpneo')
	parser.add_argument('--without-erpneo', dest='without_erpneo', action='store_true', default=False, help='Prevent fetching ERP Neo')
	# direct provision to install versions
	parser.add_argument('--version', dest='version', action='store', default=13, type=int, help='Clone particular version of neo and erpneo')
	...
	parser.add_argument('--without-neocli-setup', dest='without_neocli_setup', action='store_true', default=False, help=argparse.SUPPRESS)
	# whether to overwrite an existing neocli
	parser.add_argument('--overwrite', dest='overwrite', action='store_true', default=False, help='Whether to overwrite an existing neocli')
	# set passwords
	...
	parser.add_argument('--neocli-name', dest='neocli_name', help='Create neocli with specified name. Default name is grappetech-neocli')
...
if __name__ == '__main__':
	...
	log("NeoCLI + Neo + ERP Neo has been successfully installed!")
