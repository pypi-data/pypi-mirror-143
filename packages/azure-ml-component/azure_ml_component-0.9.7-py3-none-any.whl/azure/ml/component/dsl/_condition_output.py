...
def _condition_output(
        *,
        condition: Union[Output, PipelineExpression],
        input_a: Output = None,
        input_b: Output = None
) -> Component:
    """
...
def _validate_parameters(condition, input_a, input_b):
    if not isinstance(condition, (Output, PipelineExpression)):
        raise UserErrorException(
            f"'condition' of dsl.condition_output must be an instance of "
            f"{Output} or {PipelineExpression}, got {type(condition)}.")
    ...
    if condition._owner is not None:
