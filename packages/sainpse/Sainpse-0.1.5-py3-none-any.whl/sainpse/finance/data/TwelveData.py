...
class TwelveData():
    ...
    def getRealTime(self,lookback=15):
        ...
        cols = ["open","high","low","close","percent_b","slow_k","slow_d","apo","supertrend","trange","ultosc"]
        data = data[cols]
        obs  = data.tail(lookback).values
        obs  = obs.reshape(lookback*len(cols),)
