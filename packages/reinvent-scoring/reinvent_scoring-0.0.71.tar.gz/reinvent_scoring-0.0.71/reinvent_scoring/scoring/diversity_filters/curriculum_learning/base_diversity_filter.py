...
class BaseDiversityFilter(abc.ABC):
    ...
    def _compose_loggable_data(self, dto: UpdateLoggableDataDTO):
        likelihood = dto.likelihood if f'{dto.likelihood}|' else ''
        input = dto.input if f'{dto.input}|' else ''
        output = dto.output if f'{dto.output}' else ''
        loggable_data = f'{likelihood}{input}{output}'
