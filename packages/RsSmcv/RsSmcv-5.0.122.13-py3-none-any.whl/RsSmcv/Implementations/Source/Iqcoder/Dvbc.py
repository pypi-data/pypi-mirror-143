...
class Dvbc:
	...
	def get_input_py(self) -> enums.CodingInputSignalInputAsi:
		...
		Snippet: value: enums.CodingInputSignalInputAsi = driver.source.iqcoder.dvbc.get_input_py() \n
		No command help available \n
			:return: ipart_nput_sfe: No help available
		"""
		response = self._core.io.query_str('SOURce:IQCoder:DVBC:INPut?')
	...
	def set_input_py(self, ipart_nput_sfe: enums.CodingInputSignalInputAsi) -> None:
		"""SCPI: [SOURce]:[IQCoder]:DVBC:INPut \n
		Snippet: driver.source.iqcoder.dvbc.set_input_py(ipart_nput_sfe = enums.CodingInputSignalInputAsi.ASI1) \n
		No command help available \n
		...
		self._core.io.write(f'SOURce:IQCoder:DVBC:INPut {param}')
