...
class Ontime:
	...
	def set(self, ontime: int, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.dab.trigger.output.ontime.set(ontime = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DAB:TRIGger:OUTPut{output_cmd_val}:ONTime {param}')

	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:DAB:TRIGger:OUTPut<CH>:ONTime \n
		Snippet: value: int = driver.source.bb.dab.trigger.output.ontime.get(output = repcap.Output.Default) \n
		No command help available \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: ontime: No help available"""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DAB:TRIGger:OUTPut{output_cmd_val}:ONTime?')
