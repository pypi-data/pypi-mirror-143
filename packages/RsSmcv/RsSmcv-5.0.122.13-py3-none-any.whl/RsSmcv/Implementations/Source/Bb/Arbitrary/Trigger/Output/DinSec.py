...
class DinSec:
	...
	def get(self, output=repcap.Output.Default) -> float:
		...
		Snippet: value: float = driver.source.bb.arbitrary.trigger.output.dinSec.get(output = repcap.Output.Default) \n
		Queries the marker delay in microseconds.
		You can define a marker delay in samples with [:SOURce<hw>]:BB:ARBitrary:TRIGger:OUTPut<ch>:DELay. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: delay_in_seconds: float Range: 0 to 16777215"""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ARBitrary:TRIGger:OUTPut{output_cmd_val}:DINSec?')
