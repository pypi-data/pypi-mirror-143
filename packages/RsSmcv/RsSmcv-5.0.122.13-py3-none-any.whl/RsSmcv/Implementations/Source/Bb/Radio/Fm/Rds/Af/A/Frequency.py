...
class Frequency:
	...
	def set(self, af_freq_a: float, index=repcap.Index.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:AF:A:FREQuency<CH> \n
		Snippet: driver.source.bb.radio.fm.rds.af.a.frequency.set(af_freq_a = 1.0, index = repcap.Index.Default) \n
		Sets the alternative frequencies in AF method A. \n
			:param af_freq_a: float Range: 87.6 to 107.9
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
		"""
		param = Conversions.decimal_value_to_str(af_freq_a)
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		self._core.io.write(f'SOURce<HwInstance>:BB:RADio:FM:RDS:AF:A:FREQuency{index_cmd_val} {param}')

	def get(self, index=repcap.Index.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:AF:A:FREQuency<CH> \n
		Snippet: value: float = driver.source.bb.radio.fm.rds.af.a.frequency.get(index = repcap.Index.Default) \n
		Sets the alternative frequencies in AF method A. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
			:return: af_freq_a: float Range: 87.6 to 107.9"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:RADio:FM:RDS:AF:A:FREQuency{index_cmd_val}?')
