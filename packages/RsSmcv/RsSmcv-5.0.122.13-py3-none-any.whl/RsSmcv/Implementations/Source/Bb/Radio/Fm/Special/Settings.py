...
class Settings:
	...
	def get_state(self) -> bool:
		...
		Snippet: value: bool = driver.source.bb.radio.fm.special.settings.get_state() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:RADio:FM:SPECial:SETTings:STATe?')
		return Conversions.str_to_bool(response)

	def set_state(self, special_settings: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:[SPECial]:SETTings:[STATe] \n
		Snippet: driver.source.bb.radio.fm.special.settings.set_state(special_settings = False) \n
		Enables/disables special settings. The setting allows you to switch between standard-compliant and user-defined channel
		coding. \n
			:param special_settings: 1| ON| 0| OFF
		"""
		param = Conversions.bool_to_str(special_settings)
		self._core.io.write(f'SOURce<HwInstance>:BB:RADio:FM:SPECial:SETTings:STATe {param}')
