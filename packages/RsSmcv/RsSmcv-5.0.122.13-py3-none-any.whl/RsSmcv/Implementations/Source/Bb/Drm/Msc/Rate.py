...
class Rate:
	...
	def get(self, profile=repcap.Profile.Default) -> enums.DrmCodingCoderate:
		...
		Snippet: value: enums.DrmCodingCoderate = driver.source.bb.drm.msc.rate.get(profile = repcap.Profile.Default) \n
		Queries the overall code rate used in each of the protection profiles. \n
			...
			:return: drm_msc_rate: R025| R033| R040| R041| R045| R048| R050| R055| R057| R058| R060| R062| R066| R071| R072| R078| INV R0xy 0xy constitutes a code rate of 0.xy INV Invalid code rate"""
		profile_cmd_val = self._cmd_group.get_repcap_cmd_value(profile, repcap.Profile)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DRM:MSC:RATE{profile_cmd_val}?')
	...
	def clone(self) -> 'Rate':
		...
		new_group = Rate(self._core, self._cmd_group.parent)
