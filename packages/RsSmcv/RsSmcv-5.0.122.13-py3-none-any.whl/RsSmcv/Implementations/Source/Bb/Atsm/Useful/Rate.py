...
class Rate:
	...
	def get_max(self) -> float:
		...
		Snippet: value: float = driver.source.bb.atsm.useful.rate.get_max() \n
		...
		value at the TS input interface, that is necessary for the modulator. \n
			:return: max_usefull: float Range: 0 to 999999999
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ATSM:USEFul:RATE:MAX?')
		return Conversions.str_to_float(response)

	def get_value(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:ATSM:USEFul:[RATE] \n
		Snippet: value: float = driver.source.bb.atsm.useful.rate.get_value() \n
		...
		the installed input interface. \n
			:return: usefull_data: float Range: 0 to 999999999
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ATSM:USEFul:RATE?')
		return Conversions.str_to_float(response)
