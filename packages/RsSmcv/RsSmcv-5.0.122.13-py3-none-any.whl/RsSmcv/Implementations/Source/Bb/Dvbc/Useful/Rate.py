...
class Rate:
	...
	def get_max(self) -> float:
		...
		Snippet: value: float = driver.source.bb.dvbc.useful.rate.get_max() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:DVBC:USEFul:RATE:MAX?')
		return Conversions.str_to_float(response)

	def get_value(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:DVBC:USEFul:[RATE] \n
		Snippet: value: float = driver.source.bb.dvbc.useful.rate.get_value() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:DVBC:USEFul:RATE?')
		return Conversions.str_to_float(response)
