...
class Polarity:
	...
	def set(self, polarity: enums.NormalInverted, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.dm.external.polarity.set(polarity = enums.NormalInverted.INVerted, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:DM:EXTernal:POLarity{channel_cmd_val} {param}')

	# noinspection PyTypeChecker
	def get(self, channel=repcap.Channel.Default) -> enums.NormalInverted:
		"""SCPI: [SOURce<HW>]:DM:EXTernal:POLarity<CH> \n
		Snippet: value: enums.NormalInverted = driver.source.dm.external.polarity.get(channel = repcap.Channel.Default) \n
		No command help available \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Polarity')
			:return: polarity: No help available"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:DM:EXTernal:POLarity{channel_cmd_val}?')
		return Conversions.str_to_scalar_enum(response, enums.NormalInverted)

	def clone(self) -> 'Polarity':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Polarity(self._core, self._cmd_group.parent)
