...
class File:
	...
	def day(self):
		...
		if not hasattr(self, '_day'):
			from .Day import Day
			self._day = Day(self._core, self._cmd_group)
		return self._day
	...
	def month(self):
		...
		if not hasattr(self, '_month'):
			...
			self._month = Month(self._core, self._cmd_group)
		return self._month
	...
	def prefix(self):
		...
		if not hasattr(self, '_prefix'):
			...
			self._prefix = Prefix(self._core, self._cmd_group)
		return self._prefix
	...
	def year(self):
		...
		if not hasattr(self, '_year'):
			...
			self._year = Year(self._core, self._cmd_group)
		return self._year
	...
	def get_number(self) -> int:
		...
		count starts at 0. The R&S SMCV100B searches the specified output directory for the highest number in the stored files.
