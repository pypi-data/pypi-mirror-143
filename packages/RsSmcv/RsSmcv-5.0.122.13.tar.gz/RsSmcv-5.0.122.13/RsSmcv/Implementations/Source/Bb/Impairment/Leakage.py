...
class Leakage:
	...
	def get_icomponent(self) -> float:
		...
		Snippet: value: float = driver.source.bb.impairment.leakage.get_icomponent() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:IMPairment:LEAKage:I?')
		return Conversions.str_to_float(response)

	def set_icomponent(self, ipart: float) -> None:
		"""SCPI: [SOURce<HW>]:BB:IMPairment:LEAKage:I \n
		Snippet: driver.source.bb.impairment.leakage.set_icomponent(ipart = 1.0) \n
		Determines the leakage amplitude of the I or Q signal component of the corresponding stream \n
			:param ipart: float Range: -10 to 10
		"""
		param = Conversions.decimal_value_to_str(ipart)
		self._core.io.write(f'SOURce<HwInstance>:BB:IMPairment:LEAKage:I {param}')

	def get_qcomponent(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:IMPairment:LEAKage:Q \n
		Snippet: value: float = driver.source.bb.impairment.leakage.get_qcomponent() \n
		Determines the leakage amplitude of the I or Q signal component of the corresponding stream \n
			:return: qpart: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:IMPairment:LEAKage:Q?')
		return Conversions.str_to_float(response)

	def set_qcomponent(self, qpart: float) -> None:
		"""SCPI: [SOURce<HW>]:BB:IMPairment:LEAKage:Q \n
		Snippet: driver.source.bb.impairment.leakage.set_qcomponent(qpart = 1.0) \n
		Determines the leakage amplitude of the I or Q signal component of the corresponding stream \n
			:param qpart: float Range: -10 to 10
		"""
		param = Conversions.decimal_value_to_str(qpart)
		self._core.io.write(f'SOURce<HwInstance>:BB:IMPairment:LEAKage:Q {param}')
