...
class Frequency:
	...
	def set(self, eon_afb_freq: float, index=repcap.Index.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:EON:AF:B:FREQuency<CH> \n
		Snippet: driver.source.bb.radio.fm.rds.eon.af.b.frequency.set(eon_afb_freq = 1.0, index = repcap.Index.Default) \n
		Sets the alternative frequencies in EON for AF mapped frequencies method. \n
			:param eon_afb_freq: float Range: 87.6 to 107.9, Unit: MHz
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
		"""
		param = Conversions.decimal_value_to_str(eon_afb_freq)
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		self._core.io.write(f'SOURce<HwInstance>:BB:RADio:FM:RDS:EON:AF:B:FREQuency{index_cmd_val} {param}')

	def get(self, index=repcap.Index.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:EON:AF:B:FREQuency<CH> \n
		Snippet: value: float = driver.source.bb.radio.fm.rds.eon.af.b.frequency.get(index = repcap.Index.Default) \n
		Sets the alternative frequencies in EON for AF mapped frequencies method. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
			:return: eon_afb_freq: float Range: 87.6 to 107.9, Unit: MHz"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:RADio:FM:RDS:EON:AF:B:FREQuency{index_cmd_val}?')
