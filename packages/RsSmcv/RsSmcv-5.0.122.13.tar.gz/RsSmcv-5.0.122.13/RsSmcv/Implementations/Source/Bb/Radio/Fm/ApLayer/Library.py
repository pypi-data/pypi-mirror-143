...
class Library:
	...
	def get_catalog(self) -> List[str]:
		...
		Snippet: value: List[str] = driver.source.bb.radio.fm.apLayer.library.get_catalog() \n
		...
		a specific directory. \n
			:return: tx_audio_bc_fm_wav_cat_nam: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:RADio:FM:APLayer:LIBRary:CATalog?')
		return Conversions.str_to_str_list(response)

	def get_select(self) -> str:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:APLayer:LIBRary:SELect \n
		Snippet: value: str = driver.source.bb.radio.fm.apLayer.library.get_select() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:RADio:FM:APLayer:LIBRary:SELect?')
		return trim_str_response(response)

	def set_select(self, sel: str) -> None:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:APLayer:LIBRary:SELect \n
		Snippet: driver.source.bb.radio.fm.apLayer.library.set_select(sel = '1') \n
		Selects the audio file. If no file of the specified name exists, an error message is displayed. You can select files with
		the file extension *.wv and *.wav. Refer to 'Accessing Files in the Default or Specified Directory' for general
		information on file handling in the default and in a specific directory. \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:RADio:FM:APLayer:LIBRary:SELect {param}')
