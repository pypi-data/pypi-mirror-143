...
class Frequency:
	...
	def set(self, af_list_3_freq: float, index=repcap.Index.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:AF:B:LIST3:FREQuency<CH> \n
		Snippet: driver.source.bb.radio.fm.rds.af.b.list3.frequency.set(af_list_3_freq = 1.0, index = repcap.Index.Default) \n
		Sets an alternative frequency of a list in AF method B. \n
			:param af_list_3_freq: float Range: 87.6 to 107.9, Unit: MHz
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
		"""
		param = Conversions.decimal_value_to_str(af_list_3_freq)
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		self._core.io.write(f'SOURce<HwInstance>:BB:RADio:FM:RDS:AF:B:LIST3:FREQuency{index_cmd_val} {param}')

	def get(self, index=repcap.Index.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:RADio:FM:RDS:AF:B:LIST3:FREQuency<CH> \n
		Snippet: value: float = driver.source.bb.radio.fm.rds.af.b.list3.frequency.get(index = repcap.Index.Default) \n
		Sets an alternative frequency of a list in AF method B. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Frequency')
			:return: af_list_3_freq: No help available"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:RADio:FM:RDS:AF:B:LIST3:FREQuency{index_cmd_val}?')
