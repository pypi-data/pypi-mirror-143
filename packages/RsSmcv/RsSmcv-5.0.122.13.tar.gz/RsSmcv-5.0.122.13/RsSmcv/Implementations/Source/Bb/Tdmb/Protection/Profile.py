...
class Profile:
	...
	# noinspection PyTypeChecker
	def get(self, profile=repcap.Profile.Default) -> enums.TdmbInputSignalProtectionProfile:
		"""SCPI: [SOURce<HW>]:BB:TDMB:PROTection:PROFile<CH> \n
		Snippet: value: enums.TdmbInputSignalProtectionProfile = driver.source.bb.tdmb.protection.profile.get(profile = repcap.Profile.Default) \n
		Queries the protection profile. \n
			...
			:return: prot_profile: UEP| EEP UEP Unequal error protection EEP Equal error protection"""
		profile_cmd_val = self._cmd_group.get_repcap_cmd_value(profile, repcap.Profile)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDMB:PROTection:PROFile{profile_cmd_val}?')
		return Conversions.str_to_scalar_enum(response, enums.TdmbInputSignalProtectionProfile)
