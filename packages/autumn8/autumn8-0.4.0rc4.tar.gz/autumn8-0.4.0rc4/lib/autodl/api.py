...
import requests
from lib.autodl.api_creds import retrieve_api_creds
from lib.autodl.cli_environment import CliEnvironment
from lib.common.config.s3 import init_s3
...
def require_ok_response(response):
    if response.status_code == 403:
        raise Exception(
            f"Received response {response.status_code}:\n{response.text}\n\nUser not authenticated; please run `autodl-cli login` to authorize your CLI"
        )
    if response.status_code != 200:
        raise Exception(f"Received response {response.status_code}:\n{response.text}")
...
def fetch_user_data(environment: CliEnvironment):
    ...
    try:
        ...
        raise Exception(
            f"API key is missing! To configure API access, please visit {autodl_host}/profile and generate an API key, then run `autodl-cli login`"
        )
...
def async_estimation(environment, organization_id, model_id):
    autodl_host = environment.value["host"]
    new_url = url_with_params(
