...
from worker import test_task, estimation_task
...
class NewModelInfo(BaseModel):
  name: str
  s3_file_url: str if os.environ.get('NEXT_PUBLIC_FEATURE_S3_UPLOADS') == 'true' else Optional[str] = None
  framework: str
  quantization: str
...
def check_if_all_chunks(src_filename, from_onspecta, user_id, model_name, model_id, total):
  file_extension = Path(src_filename).suffix
  if from_onspecta:
    dest_relpath = '/mlmodel_files/onspecta/' + model_name + file_extension
  else:
    dest_relpath = '/mlmodel_files/' + str(user_id) + '/' + model_name + '_' + str(model_id) + file_extension
...
def save_mlmodel_file(src, src_filename, from_onspecta, user_id, model_name, model_id, chunk = None):
  file_extension = Path(src_filename).suffix
  if from_onspecta:
    dest_relpath = '/mlmodel_files/onspecta/' + model_name + file_extension
  else:
...
async def new_model(model_info: NewModelInfo):
  ...
  if model_info.framework != "TENSORFLOW" and model_info.framework != "PYTORCH":
  ...
  if model_info.framework is not None:
    ...
    raise HTTPException(status_code=400, detail="Missing framework in request body")

  if model_info.quantization is not None:
    fields['quantization'] = model_info.quantization
  else:
    raise HTTPException(status_code=400, detail="Missing quantization in request body")

  if model_info.from_onspecta is not None:
    fields['from_onspecta'] = model_info.from_onspecta
  else:
    raise HTTPException(status_code=400, detail="Missing from_onspecta in request body")
  ...
  if settings.is_domain_task_supported(model_info.domain, model_info.task) == False:
  ...
  if model_info.channels is not None:
    ...
    if model_info.domain == "Computer Vision":
      raise HTTPException(status_code=400, detail="Missing channels for Computer Vision network")

  if model_info.height is not None:
    fields['height'] = model_info.height
  else:
    if model_info.domain == "Computer Vision":
      raise HTTPException(status_code=400, detail="Missing height for Computer Vision network")

  if model_info.width is not None:
    fields['width'] = model_info.width
  else:
    if model_info.domain == "Computer Vision":
      raise HTTPException(status_code=400, detail="Missing width for Computer Vision network")

  if model_info.batch_size is not None:
    fields['batch_size'] = model_info.batch_size
  else:
    raise HTTPException(status_code=400, detail="Missing batch_size in rquest body")
...
async def upload_model_file(model_id:int, model_file: UploadFile = File(...)):
  ...
  dest_relpath = save_mlmodel_file(model_file.file, model_file.filename, mlmodel.from_onspecta, test_user_id, mlmodel.name, model_id)
  fields = { 'local_relpath': dest_relpath }
  mlmodel_model.update(model_id, **fields)
  return { 'message': 'uploaded file', 'model_relpath': dest_relpath, 'model_id':model_id }

@router.put(lab_api_base + "/model/file/chunk")
async def upload_model_file_chunk(model_id:int, chunk:int, total:int, model_file: UploadFile = File(...)):
  mlmodel = mlmodel_model.get(model_id)
  save_mlmodel_file(model_file.file, model_file.filename, mlmodel.from_onspecta, test_user_id, mlmodel.name, model_id, chunk)
  dest_relpath = check_if_all_chunks(model_file.filename, mlmodel.from_onspecta, test_user_id, mlmodel.name, model_id, total)
...
async def update_model_info(model_id:int, model_info: UpdateModelInfo):
  ...
  if ((model_info.top1 is not None) or
      (model_info.top5 is not None)):
    mlmodel_model.add_or_update_accuracy(model_id, model_info.top1, model_info.top5)
...
async def get_prediction(model_id: int, input_file: UploadFile = File(...)):
    ...
    print(input)
    print(autodl.loadModel(workdir + model.local_relpath)(torch.from_numpy(input)))
...
def fake_data_generator(model, model_id, threads, enable_dls, hw):
    ...
    for provider in settings.cloud_instances:
      for instance in provider['instance_types']:
        instance_config = get_instance_config(model, model_id, threads, enable_dls, hw, provider, instance)
        ...
        mlmodel_model.add_or_update_metrics(model_id, run_threads, enable_dls, instance['hw'], latency, throughput)
...
def perform_estimation(model_id, threads, enable_dls, hw):
  ...
  def update(model_id, pred_cnt, total, run_threads, enable_dls, hw, latency, throughput):
      mlmodel_model.add_or_update_metrics(model_id, run_threads, enable_dls, instance['hw'], latency, throughput)
  ...
  model_params = [model_id, model.framework, workdir + model.local_relpath, width, height, channels, quantization, bs]
  try:
      __perform_estimation(model_params, threads, enable_dls, hw, raiseError, update)
  except:
...
def render_instance_estimations(metrics):
  ...
  for entry in metrics:
    instances_list = settings.find_instances_with_config(entry['hw'], entry['num_threads'])
    if len(instances_list) > 0:
      for instance in instances_list:
        ...
        instance_metrics['cost_performance'] = (entry['throughput'] * 60 * 60) / usd_per_hr
...
async def get_estimation(
  ...
  return ({"message": "Estimation", "model_id": model_id, "estimation": instance_estimations})
...
def schedule_new_estimation_task(model, threads, enable_dls, hw):
  ...
  channels = model.channels
  bs = model.batch_size
  task = estimation_task.apply_async(args=(model.id, model.framework, model_filepath, model.width, model.height,
                     model.channels, model.quantization, model.batch_size, threads, enable_dls, hw))
  fields = {}
...
async def get_async_estimation(
    ...
    if task_result.status != 'PROGRESS':
      if task_result.status == 'SUCCESS':
        for estimation in task_result.result['estimations']:
          mlmodel_model.add_or_update_metrics(model_id, estimation[0], estimation[1], estimation[2], estimation[3], estimation[4])
      else:
         raise "Unhandled task status attained: {}".format(task_result.status)
  ...
  return ({"message": "Async Estimation", "model_id": model_id, "task_id": task.id})

@router.get(lab_api_base + "/supported_hardwares")
async def get_procs():
  return ({"message": "Get hardware supported", 'hardware': settings.supported_hardwares})

@router.get(lab_api_base + "/supported_frameworks")
async def get_frameworks():
  return ({"message": "Get ML frameworks supported", 'frameworks': settings.supported_frameworks})

@router.get(lab_api_base + "/supported_quants")
async def get_quants():
  return ({"message": "Get Quantizations modes supported", 'quants': settings.supported_quants})

@router.get(lab_api_base + "/supported_domains")
async def get_domains():
  return ({"message": "Get domains and tasks supported", 'domains': settings.supported_domains})
