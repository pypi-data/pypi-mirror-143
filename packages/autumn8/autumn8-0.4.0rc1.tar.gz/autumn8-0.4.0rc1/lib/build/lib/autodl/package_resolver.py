...
def package(model, interns = [], externs = [], max_search_depth = 5):
    ...
    package_name = ''.join(random.choices(string.ascii_lowercase, k = package_name_length))
    ...
    for i in range(0, max_search_depth):
        try:
            ...
            success = True
            break
        except torch.package.package_exporter.PackagingError as e:
            for node in e.dependency_graph.nodes:
                if 'error' in e.dependency_graph.nodes[node]:
...
If module was incorrectly labeled please pass it in autodl.attach_model. Example:
autodl.attach_model(model, input, interns = ["mymodule"], externs=["yaml"])
...
autodl.attach_model(model, input, max_search_depth = 10)""".format(interns, externs))
    requirements = {}
