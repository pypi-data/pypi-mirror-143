...
def analyze_pytorch_source(model_file_path, model_script_args=[]):
    ...
    args = sys.argv
    sys.argv = [args[0]]
    importlib.import_module(model_file_path[:-len(PYTHON_FILE_EXTENSION)]) # TODO relative imports not working
    ...
    if (len(models) < 1):
        ...
        raise ClickException("The provided model file does not contain any models attached with `autodl.attach_model`.\n"
            + "Please refer to our documentation to read on how to attached your models."
        )
    if (len(models) > 1):
        print ("WARNING: Our current CLI implementation does not support multiple models in one file yet. Please contact us and file an inquiry to add this feature if you need it.")

    model, dummy_input, interns, externs, max_search_depth = models[-1]
    model_file_bytes = autodl.export_pytorch_model_repr(model, dummy_input, interns, externs, max_search_depth)
...
def infer_tensorflow_protobuf_input_shape(model_file_path):
    ...
    inferred_batch_size, inferred_width, inferred_height, inferred_channels = None, None, None, None
    ...
    for node in graph_nodes:
        if node.op == "Placeholder":
            ...
            if len(shape_proto.shape.dim) > 0:
                [inferred_batch_size, inferred_width, inferred_height, inferred_channels] = dim_sizes

    return inferred_batch_size, inferred_width, inferred_height, inferred_channels
...
def analyze_model_file(model_file_path, model_script_args=[]):
    ...
    model_file, inferred_model_name, framework, inferred_quantization, inferred_channels, inferred_width, inferred_height, inferred_batch_size = output
    ...
    if extension in ['.py']:
        framework = 'PYTORCH'
        model_file_bytes, dummy_input = analyze_pytorch_source(model_file_path, model_script_args)
        ...
        [inferred_batch_size, inferred_channels, inferred_width, inferred_height] = dummy_input.shape
        ...
        [inferred_batch_size, inferred_width, inferred_height, inferred_channels] = infer_tensorflow_protobuf_input_shape(model_file_path)

    elif extension in ['.tflite']: # TODO - add this extension also to the UI autofiller
        framework = 'TFLITE'
        raise ClickException(f"Files with '{extension}' are not supported yet")

    elif extension in ['.pt', '.pth']:
        raise ClickException(f"Files with '{extension}' are not supported - please contact us at support@autumn8.ai to add the support.")

    else:
        raise ClickException(f"Files with '{extension}' are not supported - please contact us at support@autumn8.ai to add the support.")


    return model_file, inferred_model_name, framework, inferred_quantization, inferred_channels, inferred_width, inferred_height, inferred_batch_size
