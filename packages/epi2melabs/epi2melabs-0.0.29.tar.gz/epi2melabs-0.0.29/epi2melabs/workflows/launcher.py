...
class WorkflowLauncher(object):
    ...
    def _start_instance(
        ...
        command = (
            f'{self.invoker} -i {instance.id} -w {workflow_target} '
            f'-p {params_file} -wd {work_dir} -l {nf_logfile} '
            f'-s {nf_std_out} -d {database} -n {self.nextflow}')
        ...
        try:
            proc = subprocess.Popen(
                command.split(' '), start_new_session=True, stdout=stdout,
                stderr=stderr, close_fds=True, cwd=self.base_dir, env=env)
