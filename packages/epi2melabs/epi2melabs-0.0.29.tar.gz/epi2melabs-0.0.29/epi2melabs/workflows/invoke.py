...
def invoke(
    ...
    if not os.path.isfile(workflow):
        pull_command = f'{nextflow} pull {workflow}'
    ...
    if revision:
        ...
        run_command = run_command + f' -r {revision}'
    ...
    try:
        ...
        if pull_command:
            ...
            proc = subprocess.Popen(
                pull_command.split(' '), stdout=stdout, stderr=stderr)
            if proc.wait():
                logging.info(
        ...
        proc = subprocess.Popen(
            run_command.split(' '), stdout=stdout, stderr=stderr)
