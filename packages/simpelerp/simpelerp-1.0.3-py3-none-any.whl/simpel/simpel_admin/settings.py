...
ADMIN_DEFAULTS: Dict[str, Any] = {
    "APP_INDEX_EXCLUDES": [],
    "INDEX_TITLE": os.getenv("INDEX_TITLE", "Simpel Administration"),
    "SITE_TITLE": os.getenv("SITE_TITLE", "Simpel Administration"),
    "SITE_HEADER": os.getenv("SITE_HEADER", "Simpel Administration"),
    ...
    "PRINT_VIEW_CLASS": "simpel.simpel_admin.mixins.PDFPrintView",
}
...
def import_from_string(val, setting_name):
    ...
    try:
        ...
        msg = "Could not import '%s' for ADMIN setting '%s'. %s: %s." % (
            val,
            setting_name,
...
class AppSettings:
    ...
    def __init__(self, user_settings=None, defaults=None, import_strings=None):
        ...
        self.defaults = defaults or ADMIN_DEFAULTS
    ...
    def user_settings(self):
        if not hasattr(self, "_user_settings"):
            self._user_settings = getattr(settings, "ADMIN", {})
        return self._user_settings

    def __getattr__(self, attr):
        if attr not in self.defaults:
            raise AttributeError("Invalid ADMIN settings: '%s'" % attr)
...
admin_settings = AppSettings(None, ADMIN_DEFAULTS, IMPORT_STRINGS)


def reload_admin_settings(*args, **kwargs):
    setting = kwargs["setting"]
    if setting == "ADMIN":
        admin_settings.reload()


setting_changed.connect(reload_admin_settings)
