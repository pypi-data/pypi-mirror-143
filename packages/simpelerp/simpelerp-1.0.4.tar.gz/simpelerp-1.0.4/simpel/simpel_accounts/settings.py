...
SIMPEL_ACCOUNTS_DEFAULTS: Dict[str, Any] = {
    "NAMES": {
        "ASSET": "Asset",  # Asset
        "CASH": "Cash",
...
def import_from_string(val, setting_name):
    ...
    try:
        ...
        msg = "Could not import '%s' for SIMPEL_ACCOUNTS setting '%s'. %s: %s." % (
            val,
            setting_name,
...
class AppSettings:
    """
    This module is largely inspired by django-rest-framework settings.
    This module provides the `simpel_accounts_settings` object, that is used to access
    app settings, checking for user settings first, then falling
    back to the defaults.
    """
    ...
    def __init__(self, user_settings=None, defaults=None, import_strings=None):
        ...
        self.defaults = defaults or SIMPEL_ACCOUNTS_DEFAULTS
    ...
    def user_settings(self):
        if not hasattr(self, "_user_settings"):
            self._user_settings = getattr(settings, "SIMPEL_ACCOUNTS", {})
        return self._user_settings

    def __getattr__(self, attr):
        if attr not in self.defaults:
            raise AttributeError("Invalid SIMPEL_ACCOUNTS settings: '%s'" % attr)
...
simpel_accounts_settings = AppSettings(None, SIMPEL_ACCOUNTS_DEFAULTS, IMPORT_STRINGS)


def reload_simpel_accounts_settings(*args, **kwargs):
    setting = kwargs["setting"]
    if setting == "SIMPEL_ACCOUNTS":
        simpel_accounts_settings.reload()


setting_changed.connect(reload_simpel_accounts_settings)
