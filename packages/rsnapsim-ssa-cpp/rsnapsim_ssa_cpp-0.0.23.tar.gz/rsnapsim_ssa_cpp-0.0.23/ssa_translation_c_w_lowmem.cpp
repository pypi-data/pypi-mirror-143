...
void translationSSA_lowmem(double* kelong, double* t_array, int Nt, double kbind, double kcompl, int* SSA_intensity, int N, int FRAP,
	int Inhibitor, double inhibit_time, double inhibit_time_stop,  int seed, double* SSA_ribtimes, int* watched_ribs,
	int ribtimesize,  int cNt, int* col_result, double* col_t, int* col_x, int colNp, int* x0, int r_footprint,
	int* SSA_probe, int Ncolor, int* flags, double kon, double koff, double* k_probe,int* probe_loc,int* n_probes, int rib_max, double photobleaching_rate, int photobleaching)
{
		...
		if (leaky){
			...
			if (NR > old_NR) {
				for (int k = 0; k < total_probes * 2; k += 2) {
					if (photobleach){
						if (unif(rng) < exp(-photobleaching_rate * t)) {
							//std::cout << exp(-photobleaching_rate * t) << std::endl;
							leaky_probe_matrix(Ncolor * (NR - 1) + probe_loc[k], probe_loc[k + 1]) = 1;
						}
					}
					else {
