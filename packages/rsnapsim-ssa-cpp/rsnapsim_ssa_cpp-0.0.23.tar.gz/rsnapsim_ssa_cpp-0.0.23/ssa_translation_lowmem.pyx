...
cdef extern from "ssa_translation_c_w_replicon.h":
    void translationSSA_replicon(int *riblocations, double *kelong, double *t_array,int Nt, double kbind, double kcompl, int* SSA_intensity,int N, int FRAP, int Inhibitor, double inhibit_time,double inhibit_time_stop,  int seed, double *SSA_ribtimes,int *nribs,int ribtimesize, int cNt, int* col_result,double* col_t, int* col_x, int colNp, int* x0, int r_footprint,int* SSA_probe,int Ncolor, int* flags, double kon, double koff,double* k_probe,int* probe_loc,int* n_probes, int rib_max,double photobleaching_rate, int photobleaching,int *protease_locations, double *protease_rates, int protease_location_count)
...
def run_SSA_replicon(np.ndarray[int, ndim=2, mode="c"] rib_locations not None, np.ndarray[int, ndim=2, mode="c"] intensity not None,np.ndarray[double, ndim=1, mode="c"] ribtimes not None, np.ndarray[int, ndim=1, mode="c"] coltimes not None,np.ndarray[int, ndim=1, mode="c"] col_x not None, np.ndarray[double, ndim=1, mode="c"] col_t not None,np.ndarray[double, ndim=1, mode="c"] kelong not None, np.ndarray[double, ndim=1, mode="c"] t_array not None,double kbind, double kcompl, int FRAP, int Inhibitor, double inhibit_time, double inhibit_time_stop,int seed, np.ndarray[int, ndim=1, mode="c"] nribs not None,  np.ndarray[int, ndim=1, mode="c"] x0 not None,int r_footprint, np.ndarray[int, ndim=2, mode="c"] probevec not None, int Ncolor, double kon, double koff,np.ndarray[double, ndim=1, mode="c"] k_probe not None,np.ndarray[int, ndim=2, mode="c"] probeloc not None,np.ndarray[int, ndim=1, mode="c"] flags not None,  double photobleaching_rate, int photobleaching,np.ndarray[int, ndim=1, mode="c"] protease_locations not None,np.ndarray[double, ndim=1, mode="c"] protease_rates not None):
    """
    wrapper interface between C++ / cython / python
    HIGH MEMORY VERSION 
    Records X ribosome tensor for the user, this costs a lot of memory!
    ...
    Nprotease_locations = protease_locations.shape[0]
    ...
    #call the c++
    translationSSA_replicon(&rib_locations[0,0], &kelong[0], &t_array[0], Nt, kbind, kcompl,&intensity[0,0], N, FRAP, Inhibitor, inhibit_time,inhibit_time_stop, seed,&ribtimes[0],&nribs[0],ribtimesize,coltimesize,&coltimes[0], &col_t[0],&col_x[0],colNp, &x0[0], r_footprint, &probevec[0,0],Ncolor, &flags[0], kon, koff, &k_probe[0], &probeloc_indexes[0,0],&n_probes[0],rib_max, photobleaching_rate, photobleaching, &protease_locations[0], &protease_rates[0], Nprotease_locations)
