...
void translationSSA_replicon(int* riblocations, double* kelong, double* t_array, int Nt, double kbind, double kcompl, int* SSA_intensity, int N,
	int FRAP, int Inhibitor, double inhibit_time, double inhibit_time_stop, int seed, double* SSA_ribtimes, int* watched_ribs, int ribtimesize,
	int cNt, int* col_result, double* col_t, int* col_x, int colNp, int* x0, int r_footprint, int* SSA_probe, int Ncolor, int* flags, double kon,
	double koff, double* k_probe,int* probe_loc,int* n_probes, int rib_max, double photobleaching_rate, int photobleaching,
	int* protease_locations, double* protease_rates, int protease_location_count)
{
	...
	*/
	...
	MatrixXi passed_probes(Ncolor, N_rib);
	passed_probes.setZero(Ncolor, N_rib);
...
//	N = 10;
	...
	Eigen::Map<Eigen::MatrixXi> Ribosome_locations(riblocations,Nt,N_rib);
    ...
    while( t < tf)
    {
			...
			if (NR> old_NR){
				for (int k=0; k < total_probes*2; k+=2){
					...
					if ( r1 > 1-k_probe[probe_loc[k]]  ){
						...
						if (photobleach) {
							if (unif(rng) < exp(-photobleaching_rate * t)) {
		...
		Eigen::VectorXd wn;
		if (bursting) {
			wn.setZero(NR + 2 + NR*protease_location_count);
		}
		else {
			wn.setZero(NR + 1 + NR*protease_location_count);
		}
...
//       std::cout  << X << std::endl;
		...
		if (NR !=0){
    		for (int i = 0; i < NR; i++){
        		for (int j = 0; j < protease_location_count; j++){
            		if(X(0,i)  > protease_locations[j]){
                		//std::cout << "X =" << X(0,i) << "--------" << std::endl;
                		//std::cout << "adding protease rate =" << protease_rates[j] << "--------" << std::endl;
                		wn(NR+1 + protease_location_count*i+j ) = protease_rates[j];
		...
		//check edge case where the system picks a binding reaction that happens after inhibition, make sure this doesnt go through
		if ((inhibitor_pres) && (Inhibit_condition == 1)) {
			if (t >= inhibit_time) {
				//std::cout << "X =" << X.topLeftCorner(1,10) << "--------" << std::endl;
				//std::cout << "NR =" << NR << "---------" << std::endl;
				//std::cout << "picked bind reaction that happens aver inhibit =" << Sn << "--------" << std::endl;
				Sn(NR, NR) = 0;
				//std::cout << "picked bind reaction that happens aver inhibit =" << Sn << "--------" << std::endl;
			}
		}

		// wipe all probes if FRAP
		if (FRAP_pres) {
			//std::cout << "--------n_probes=" << n_probes << "--------" << std::endl;
			if ((t >= inhibit_time) && (t < inhibit_time_stop)) {
				for (int j = 0; j < Ncolor; j++) {
					for (int i = 0; i < NR; i++) {
						if (leaky_probe_matrix.block(Ncolor * i + j, 0, 1, X(0, i) - 1).sum() != 0) {
							leaky_probe_matrix.block(Ncolor * i + j, 0, 1, X(0, i) - 1).setZero();
						}
					
						//std::cout << "--------probe_mat=" << leaky_probe_matrix.block(Ncolor * i + j, 0, 1, X(0, i) - 1) << "--------" << std::endl;
					}
				}
			}
        ...
        while( (it<=Nt-1) && (t>t_array[it])) {
            ...
            Ribosome_locations.row(it) = X.row(0);
        ...
        if (ind <=  NR + 1 + bursting) {
    		//std::cout << "non protease reaction " << std::endl;
    		if (bursting){
    			if (ind == NR+2){
    		...
    		if ( (Sn.row(ind-1).sum() < 0) && (ind != NR+2)){
    			...
    			//std::cout << "removing row: " << leaky_probe_matrix.block(0,0,NR+3,25)  << std::endl;
    			passed_probes.block(0, 0, NR-1, Ncolor) = passed_probes.block(1, 0, NR, Ncolor); //ncolor nrib
    		}
    	}
    	
    	else{
    		//std::cout << "protease reaction " << std::endl;
    		
    		//std::cout << "location_of_cut " << location_of_cut <<std::endl;
    		
    		location_of_cut = protease_locations[(ind-1 - NR-1) % protease_location_count];
    		which_R_cut = (ind-1-NR-1) / protease_location_count;
    		
    		//std::cout << "location_of_cut " << location_of_cut <<std::endl;
    		//std::cout << "which_R_cut " << which_R_cut <<  std::endl;
    		// set all previous probes to zero if the protease cut here!
    		
    		//std::cout << "sum before blank " << leaky_probe_matrix.block(which_R_cut*Ncolor,0,Ncolor,location_of_cut).sum() <<  std::endl;
    		leaky_probe_matrix.block(which_R_cut*Ncolor,0,Ncolor,location_of_cut).setZero(); 
    		//std::cout << "sum after blank " << leaky_probe_matrix.block(which_R_cut*Ncolor,0,Ncolor,location_of_cut).sum() <<  std::endl;
