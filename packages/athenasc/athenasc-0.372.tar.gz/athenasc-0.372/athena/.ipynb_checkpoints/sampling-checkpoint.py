...
class Sampling:
    ...
    def sample(self, ncells=10000, pop_fp=None, cache=True):
        ...
        # cells_meta = self.clean_cells_metadata(cells_meta)
    ...
    def clean_cells_metadata(self, cell_meta):
        cells_meta['grna'] = cells_meta['sim_label'].apply(lambda x: "_".join(x.split('_')[0:2]))
        cells_meta['target_gene'] = cells_meta['sim_label'].apply(lambda x: x.split('-grna')[0])
        
        if self.crispr_type == 'knockout':
            cells_meta['is_cell_perturbed'] = cells_meta['sim_label'].apply(lambda x: x.split('_')[-1])
            cells_meta.loc[cells_meta.target_gene == self.ctrl_label, 'is_cell_perturbed'] = self.ctrl_label
        else:
            cells_meta['is_cell_perturbed'] = 'PRT'
            cells_meta.loc[cells_meta.target_gene == self.ctrl_label, 'is_cell_perturbed'] = self.ctrl_label
    ...
    def downsampling(self, sim_counts_cpm, lib_sizes):
        ...
        for index in tqdm(range(len(lib_sizes))):
            lib_size = lib_sizes[index]
