...
class CLIInterface:
    ...
    @staticmethod
    def _validate_yarn_version(temp_dir_path: Text):
        yarn_version_command = ["yarn", "--version"]
        std_out, std_err = execute_subprocess_popen_command(yarn_version_command, cwd=temp_dir_path)
        if std_out:
            yarn_valid = True if std_out[0].startswith("3") else False
        else:
            yarn_valid = False
    ...
    def _switch_yarn_version(self, temp_dir_path: Text):
        ...
        yarn_switch_version_partial_command = ["yarn", "set", "version"]
        possible_versions = [
            "stable",
            "3.2.0",
            "3.1.0",
            ...
            "https://repo.yarnpkg.com/3.1.0/packages/yarnpkg-cli/bin/yarn.js",
        ...
        for i in possible_versions:
            self.logger.debug(f"Trying yarn version: {i}")
            yarn_switch_version_command = yarn_switch_version_partial_command.copy()
            yarn_switch_version_command.append(i)

            std_out, std_err = execute_subprocess_popen_command(yarn_switch_version_command, cwd=temp_dir_path)
            self.logger.debug("".join(std_out))
            self.logger.debug("".join(std_err))

            # validating that yarn has switch versions successfully
            yarn_valid = self._validate_yarn_version(temp_dir_path=temp_dir_path)
            if yarn_valid:
                switched = True
                break
    ...
    def preprocess_package_resolution(self, command: CLIParsedCommand) -> Tuple[ManifestData, ManifestData]:
        ...
        try:
            ...
            self.logger.point_fail("Failed to resolve dependencies.")
