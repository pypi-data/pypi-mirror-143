...
class Titan(object):
    ...
    def update_conflicting_results(self, results_paths, threads=60, max_workers=4):
        ...
        r_chunks1 = r_chunks[r_chunks['_merge'] == 'conflict'].copy()
        ...
        if not r_chunks1.empty:
            ...
            if max_workers <= 1:
                ...
                for chunk in chunks1:
                    p1 = tu.processing.update_compare_results(chunk, self.preprocessed_path)
                    updated_paths.append(p1)
            else:
                with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=mp.get_context("spawn")) as executor:
                    futures = []
                    for chunk in chunks1:
                        f = executor.submit(tu.processing.update_compare_results, chunk, self.preprocessed_path)
                        futures.append(f)
                    runs = concurrent.futures.wait(futures)
