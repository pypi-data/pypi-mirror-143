...
from aiida.engine import CalcJob, CalcInfo
...
class PbeSeiCalculation(CalcJob):
    ...
    def prepare_for_submission(self, folder) -> CalcInfo:
        ...
        calc_info.local_copy_list = [(self.inputs.parameters.uuid, self.inputs.parameters.filename, 'parameters.dat')]
        calc_info.local_copy_list = [(self.inputs.InitialSeiDistribution.uuid, self.inputs.InitialSeiDistribution.filename, 'InitialSEIDistribution.dat')]
        calc_info.local_copy_list = [(self.inputs.PybammData.uuid, self.inputs.PybammData.filename, 'Trajectory.pkl')]
