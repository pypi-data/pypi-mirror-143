...
class PbeSeiCalculation(CalcJob):
    ...
    def prepare_for_submission(self, folder) -> CalcInfo:
        ...
        calcinfo.local_copy_list = [(self.inputs.parameters.uuid, self.inputs.parameters.filename, 'parameters.txt'), (self.inputs.InitialSeiDistribution.uuid, self.inputs.InitialSeiDistribution.filename, 'InitialSEIDistribution.txt'), (self.inputs.PybammData.uuid, self.inputs.PybammData.filename, 'Trajectory.pkl')]
