...
class MonitorConfig(ConfigObject):
    ...
    def includes_file(self, filepath):
        for pp in self.global_config.exclude_path_patterns:
            if re.search(pp, filepath):
                log.info(
                    '%s excluded by %s: matches global exclude_path_patterns %r',
                    filepath,
                    self.name,
                    pp,
                )
...
class GlobalConfig(ConfigObject):
    def __init__(self, monitors=None, exclude_path_patterns=None):
        exclude_path_patterns = exclude_path_patterns if exclude_path_patterns is not None else []
        if not isinstance(exclude_path_patterns, list):
            raise BadConfig('exclude_path_patterns is not a list')
        if not all(isinstance(pp, str) for pp in exclude_path_patterns):
            raise BadConfig('exclude_path_patterns is not a list of strings')
        ...
        for name, mon_con in monitors.items():
            try:
                self.monitors[name] = clean_monitor_config(mon_con, name=name, global_config=self)
...
def clean_monitor_config(monitor_config, **kwargs):
    if not isinstance(monitor_config, dict):
        raise BadConfig('Not a dictionary')

    return MonitorConfig(**monitor_config, **kwargs)
