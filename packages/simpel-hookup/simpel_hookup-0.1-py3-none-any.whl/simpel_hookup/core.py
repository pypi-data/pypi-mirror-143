...
_simpel_hookup = {}
_searched_for_simpel_hookup = False
...
def register(hook_name, fn=None, order=0):
    ...
    for name in hook_names:
        if name not in _simpel_hookup:
            _simpel_hookup[name] = []
        _simpel_hookup[name].append((fn, order))


def search_for_simpel_hookup():
    global _searched_for_simpel_hookup
    if not _searched_for_simpel_hookup:
        list(get_app_submodules("hooks"))
        _searched_for_simpel_hookup = True


def get_hooks(hook_name=None):
    """Return the hooks function sorted by their order."""
    search_for_simpel_hookup()
    if hook_name is None:
        return _simpel_hookup
    hooks = _simpel_hookup.get(hook_name, [])
