...
def decomposition_to_dataset(df, coef_dict, features, spend_df, medias, name_target_column, name_prediction_column):
    columns = features
    coef_array = list(coef_dict.values())
    ...
    all_decomp_dict = {}
    all_decomp_dict[name_target_column] = df[name_target_column]
    all_decomp_dict[name_prediction_column] = df[name_prediction_column]

    for x in range(len(coef_array) - 1):
        coef_dict[columns[x]] = coef_array[x]
        all_decomp_dict[columns[x]] = result[columns[x]] * coef_array[x]

    coef_dict['intercept'] = coef_array[-1]

    all_decomp_dict['intercept'] = coef_array[-1]
    df_all_decomp = pd.DataFrame.from_dict(all_decomp_dict)
    ...
    decomp_df = pd.DataFrame(all_decomp_df[result.keys()].sum(axis=0))
