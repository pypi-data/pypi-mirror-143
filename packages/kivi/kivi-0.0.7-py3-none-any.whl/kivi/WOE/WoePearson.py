...
class Pearson(WOE):
    ...
    def __init__(self, df, targetName='target', R=1, ForceBin=3, MaxBin=20):
        ...
        :param MaxBin: 最大分箱数
        """
        super().__init__(variables=None, target=None, bins=None, all_feature=True)
        self.df = df
    ...
    def continuousBin(self, variables, target):
        """
        ...
        self.justMiss, self.notMiss = self.findMissData(variables, target)
        ...
        self.woe_iv_res(BucketGroup)
        return self.res

    def charBin(self, variables, target):
        """
    ...
    def WoeIV(self, ):
        ...
        for col in tqdm_notebook(cols):
            if np.issubdtype(df[col], np.number) and len(pd.Series.unique(df[col])) > 2:
                conv = self.continuousBin(df[col], target)
            else:
                conv = self.charBin(df[col], target)
            conv["var_name"] = col
            if len(woe) == 0:
                woe = conv
            else:
