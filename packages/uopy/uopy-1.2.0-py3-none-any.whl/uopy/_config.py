...
class _Config:
    ...
    '''
    Update the connection config and pooling config.
    '''
    def update(self, conn_pool_config):
        _connection_config.update({k: v for k, v in conn_pool_config.items() if k in _connection_config})
        _pooling_config.update({k: v for k, v in conn_pool_config.items() if k in _pooling_config})
        _eval_config(_connection_config)
        _eval_config(_pooling_config)
