...
class RepositoryImpl(Repository):
    def __init__(self) -> None:
        self.cache = {}
    def attach(self, key: str, obj: Entity) -> None:
        self.cache[key] = obj
    ...
    def find_by_helper(
        ...
        if x is None or x.is_expired:
            x_do = find_method(by.get_value())
            x = self.converter.to_entity(x_do)
            self.attach(key, x)
        return x

    def find_helper(self, find_method, key, converter=None)-> Union[List[Entity], Entity]:
        if converter is None:
            converter = self.converter
        x = self.find_from_cache(key)
        if x is None or x.is_expired:
...
class ErrorFactory:
    @staticmethod
    def make(error_code: ReturnCode):
        error_code = error_code.get_value()
