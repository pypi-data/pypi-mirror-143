...
def _get_workspace_dataplane(
    ...
    if cloud.lower() not in _AML_HOSTNAME_ID_DICT.keys():
        raise ValueError(
            "Valid cloud names are {}.".format(str.join(", ", [v for v in _AML_HOSTNAME_ID_DICT.keys()])))

    domain = _AML_HOSTNAME_ID_DICT[cloud.lower()]
