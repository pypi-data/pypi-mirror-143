...
class InteractiveLoginAuthentication(AbstractAuthentication):
    ...
    def __init__(self, force=False, tenant_id=None, cloud=None):
        ...
        self._cli_found = True
        self._use_adal_auth = False
        try:
            from azure.cli.core import __version__ as ver
        except Exception:
            self._cli_found = False
        self._fallback_az_cli = False
    ...
    def _get_arm_token_using_interactive_auth(self, force_reload=False, resource=None):
        ...
        if self._fallback_az_cli and self._cli_found:
            from azure.cli.core import __version__ as ver
            from azure.cli.core._session import ACCOUNT, CONFIG, SESSION
            from azure.cli.core._environment import get_config_dir
            from azure.cli.core._profile import Profile
            if parse_version('2.30.0') > parse_version(ver):
                self._use_adal_auth = True
            if self._use_adal_auth:
        ...
        arm_token = _get_arm_token_with_refresh(
            profile_object, cloud_type, ACCOUNT, CONFIG, SESSION, get_config_dir(), force_reload=force_reload,
            resource=resource, use_adal_auth=self._use_adal_auth
        )
...
class AzureCliAuthentication(AbstractAuthentication):
    ...
    def _get_arm_token(self):
        ...
        return _get_arm_token_with_refresh(
            profile_object, cloud_type, ACCOUNT, CONFIG, SESSION, get_config_dir(), use_adal_auth=self._use_adal_auth
        )
    ...
    def _get_graph_token(self):
        ...
        return _get_arm_token_with_refresh(
            ...
            get_config_dir(),
            resource=self._cloud_type.endpoints.active_directory_graph_resource_id,
            use_adal_auth=self._use_adal_auth
        )
...
class ArmTokenAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids(self):
        ...
        return _get_subscription_ids_via_client(auth_object, self._cloud_type.endpoints.resource_manager), token_tenant_id
...
class ServicePrincipalAuthentication(AbstractAuthentication):
    ...
    def _get_sp_credential_object(self):
        ...
        sp_credentials = ServicePrincipalCredential(
            sp_auth, authority='{0}/{1}'.format(login_endpoint, self._tenant_id)
        )
...
class MsiAuthentication(AbstractAuthentication):
    ...
    def _get_resource_token(self, resource):
        ...
        msi_auth = MSIAuthenticationWrapper(resource=resource, **self._identity_config)
        msi_auth.set_token()
    ...
    def _get_arm_token(self):
        ...
        # retrieves the AAD token through MSI
        from azureml._vendor.azure_cli_core.auth.adal_authentication import MSIAuthenticationWrapper
        msi_auth = MSIAuthenticationWrapper(**self._identity_config)
        msi_auth.set_token()
        return msi_auth.token['access_token']
    ...
    def _get_graph_token(self):
        ...
        from azureml._vendor.azure_cli_core.auth.adal_authentication import MSIAuthenticationWrapper
        resource = self._cloud_type.endpoints.active_directory_graph_resource_id
        msi_auth = MSIAuthenticationWrapper(resource, **self._identity_config)
        msi_auth.set_token()
        return msi_auth.token['access_token']
...
class TokenAuthentication(AbstractAuthentication):
    ...
    def _get_all_subscription_ids(self):
        ...
        return _get_subscription_ids_via_client(auth_object,
                                                self._cloud_type.endpoints.resource_manager), token_tenant_id


def _get_subscription_ids_via_client(auth_obj, resource_manager='https://management.azure.com'):
    """Return a list of subscriptions that are accessible through this authentication.
    ...
    result = []
    subscription_client = SubscriptionClient(auth_obj, base_url=resource_manager)
...
def _get_arm_token_with_refresh(profile_object, cloud_type, account_object, config_object, session_object,
                                config_directory, force_reload=False, resource=None, use_adal_auth=False):
    ...
    if not use_adal_auth:
        ...
        try:
            ...
            if user_type == "user":
                ...
                if (token_dict["expiresOn"] - time.time()) < _TOKEN_REFRESH_THRESHOLD_SEC:
                    # profile_object.get_raw_token is being used as per the suggestion
                    # https://github.com/Azure/azure-cli/issues/15805#issuecomment-722914070
                    access_token = profile_object.get_raw_token(resource=resource)[0][1]
                    return access_token
