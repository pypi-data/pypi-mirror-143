...
class AbstractDataset(object):
    ...
    def get_partition_key_values(self, partition_keys=None):
        ...
        if not self.partition_keys or len(self.partition_keys) == 0:
            raise UserErrorException("get_partition_key_values is not available to a dataset that has no "
                                     "partition keys")
        ...
        partition_key_values = pd[partition_keys].to_dict(orient='records') if pd.shape[0] != 0 else []
        ...
        with open(local_yaml_path, "r") as stream:
            try:
                ...
                msg_args = {
                    ...
                    "RsflowYaml": yaml.dump(mltable_yml)
                }
                activity_data = get_engine_api().convert_rsflow_to_dataflow_json(msg_args)

                # this only works when all paths is cloud path in long form datastore uri
                # pending for relative path handling logic
                from azureml.data.tabular_dataset import TabularDataset
                from azureml.data.file_dataset import FileDataset
                if _is_tabular(mltable_yml):
                    return TabularDataset._create(
                        definition=Dataflow._dataflow_from_activity_data(activity_data, get_engine_api()))
                else:
    ...
    def _validate_args(path: str, workspace):
        if path is None or len(path) == 0:
            raise UserErrorException("path cant be empty")
    ...
    def _register(self, workspace, name, description=None, tags=None, create_new_version=False):
        def request():
            return _restclient(workspace).dataset.register(
                ...
                workspace.resource_group,
                workspace.name,
                dataset_dto=_dataset_to_dto(self, name, description, tags, create_new_version),
                if_exists_ok=create_new_version,
                update_definition_if_exists=create_new_version,
                custom_headers=self._get_telemetry_headers())
    ...
    def __repr__(self):
        ...
        if is_dataprep_installed():
            ...
            if step_type == 'Microsoft.DPrep.GetDatastoreFilesBlock':
                ...
                source = [
                    '(\'{}\', \'{}\')'.format(si['arguments']['datastoreName'], si['resourceidentifier'])
