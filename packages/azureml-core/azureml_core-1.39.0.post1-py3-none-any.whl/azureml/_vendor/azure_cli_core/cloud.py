...
from azureml._vendor.azure_cli_core.profiles import API_PROFILES
from azureml._vendor.azure_cli_core._config import GLOBAL_CONFIG_DIR
from azureml._vendor.azure_cli_core.util import urlretrieve
...
class CloudEndpointNotSetException(AzureMLException):
    pass


class CloudSuffixNotSetException(AzureMLException):
    pass
...
def get_clouds(cli_ctx):
    ...
    for section in config.sections():
        ...
        if c.profile not in API_PROFILES:
            raise AzureMLException('Profile {} does not exist or is not supported.'.format(c.profile))
...
def get_cloud_or_default(cli_ctx=None, cloud_name=None):
    """
    This function returns Cloud Instance given a cloud name as a parameter or an environment variable
    'AZUREML_CURRENT_CLOUD' containing the cloud name. If both parameters are not provided, a default
    Cloud instance of Azure Public Cloud is returned.
    :param cloud_name: The name of the target cloud. Can be one of "AzureCloud", "AzureChinaCloud",
            "AzureUSGovernment", or AzureGermanCloud. If no cloud is specified, any configured default from
            the Azure CLI is used. If no default is found, "AzureCloud" is used.
    :type cloud_name: str
    :param cli_ctx: CLI context
    :type cli_ctx: azureml._vendor.azure_cli_core.AzCli
    ...
    if os.getenv(AZUREML_CLOUD_ENV_NAME):
        return get_cloud(cli_ctx, os.getenv(AZUREML_CLOUD_ENV_NAME))
    default_cloud = get_default_cloud_name()
    return get_cloud(cli_ctx, default_cloud)
...
def _set_active_subscription(cli_ctx, cloud_name):
    from azureml._vendor.azure_cli_core._profile import (Profile, _ENVIRONMENT_NAME, _SUBSCRIPTION_ID,
