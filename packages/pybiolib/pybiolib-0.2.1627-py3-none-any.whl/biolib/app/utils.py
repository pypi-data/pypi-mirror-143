...
def run_job(job, module_input_serialized: bytes, is_run_from_cli=False):
    host = '127.0.0.1'
    ...
    if is_run_from_cli:
        # Setting to None to have it write to current process output
        output_destination = None
    elif utils.IS_RUNNING_IN_NOTEBOOK:
    ...
    if utils.IS_RUNNING_IN_NOTEBOOK:
        logger.debug("Running in Notebook...")
        logger.debug("Starting output streaming thread...")
        threading.Thread(
            target=stream_process_output,
            args=(compute_node_process,)
        ).start()
