...
class MultiPartUploader:
    ...
    def upload(self, payload_iterator: Iterator[bytes], payload_size_in_bytes: int) -> None:
        ...
        if self._start_multipart_upload_request:
            ...
            logger.debug('Multipart upload started')
            if not start_multipart_upload.ok:
        ...
        with multiprocessing.Pool(processes=min(16, multiprocessing.cpu_count() - 1)) as process_pool:
            ...
            for response in process_pool.imap(self._upload_chunk, iterator_with_index):
                ...
                logger_no_user_data.info(
                    f'Uploaded {chunk_byte_length} bytes, the approximate progress is {approx_rounded_progress}%'
                )
        ...
        for index in range(3):
            try:
                ...
                if not complete_upload_response.ok:
                    raise Exception(
                        f'Failed to complete multipart upload got response status code '
                        f'{complete_upload_response.status_code}'
                    )
                return
