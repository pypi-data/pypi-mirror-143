...
def _sympy_compare(m, other):
    equation1 = m.sympify(symbolic_lr=False, include_weights=False)
    equation2 = other.sympify(symbolic_lr=False, include_weights=False)
    return equation1 - equation2 == 0
...
def _canonical_compare(m, other):
    ...
    if plen != olen:
        ...
        for i in range(plen):
            p = _canonize_program(p, i)
        for i in range(olen):
            other = _canonize_program(other, i)
...
def _canonize_program(prog, ix: int):
    """Order only the immediate children lexically of a binary node at ix"""
    prog = prog.copy()
    if prog.arity_at(ix) != 2:
        return prog

    child1 = ix + 1
    child2 = prog.find_end(child1)
    if prog._codes[child1] <= prog._codes[child2]:
        return prog

    end2 = prog.find_end(child2)
    st1 = prog._codes[child1:child2]
    st2 = prog._codes[child2:end2]
    prog._codes = prog._codes[:child1] + st2 + st1 + prog._codes[end2:]
    return prog
