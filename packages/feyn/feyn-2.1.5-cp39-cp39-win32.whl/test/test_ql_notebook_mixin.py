...
from .config import _unittest_config_resolver
...
class TestModel(unittest.TestCase):
    ...
    def setUp(self):
        config = _unittest_config_resolver(None)
        self.ql = ClassUnderTest(config)

    def test_expand_auto_run(self):

        with self.subTest("Test that it runs and produces code lines"):
            code_lines = self.ql.expand_auto_run()
        ...
        with self.subTest("Test that it assigns positional arguments given"):
            df = pd.DataFrame()
            output = 'output_value'
            code_lines = self.ql.expand_auto_run(df, output)
            ...
            ql_expected = "ql = feyn.connect_qlattice()\n"
            data_expected = "data = df\n"
            output_expected = "output_name = output\n"

            self.assertEqual(ql_expected, code_lines[parameter_start], "self should be reassigned to ql")
            self.assertEqual(data_expected, code_lines[parameter_start+1], "Data should be assigned to outer variable name")
            self.assertEqual(output_expected, code_lines[parameter_start+2], "output_name should be assigned to outer variable name")

        with self.subTest("Test that it assigns any positional argument given inline (except DataFrames)"):
            code_lines = self.ql.expand_auto_run(pd.DataFrame(), "test_output_value", 42)
            ...
            self.assertEqual(ql_expected, code_lines[parameter_start], "self should be reassigned to ql")
            self.assertEqual(data_expected, code_lines[parameter_start+1], "Data should be assigned to None")
            self.assertEqual(output_expected, code_lines[parameter_start+2], "output_name should be assigned to its value")
            self.assertEqual(kind_expected, code_lines[parameter_start+3], "kind should be assigned to its value")

        with self.subTest("Test that it assigns kwargs arguments"):
            current_models = []
            crit = 'test_criterion'
            code_lines = self.ql.expand_auto_run(n_epochs=42, criterion=crit, starting_models=current_models)
            ...
            for line in code_lines:
                if line.startswith('n_epochs ='):
                    self.assertEqual(n_epochs_expected, line, "n_epochs should be assigned")
                if line.startswith('criterion ='):
                    self.assertEqual(criterion_expected, line, "criterion should be assigned")
                if line.startswith('starting_models ='):
                    self.assertEqual(starting_models_expected, line, "starting_models should be assigned")

        with self.subTest("Test that it has its limitations due to frame magic"):
            unrelated_variable = 420
            code_lines = self.ql.expand_auto_run(unrelated_variable)
            ...
            self.assertEqual(data_expected, code_lines[parameter_start+1], "Data gets assigned to unrelated variable name")

        with self.subTest("Test that it strips function declaration, self, return and docs"):
            code_lines = self.ql.expand_auto_run()
            ...
            docs = self.ql.auto_run.__doc__.split('\n')
        ...
        with self.subTest("Test that important assignments happen and aren't stripped"):
            ...
            for line in code_lines:
                if "best = feyn.get_diverse_models" in line:
                    count += 1
            self.assertEqual(2, count, "best should be assigned to get_diverse_models (not stripped by return stripping)")

    @pytest.mark.integration
    def test_expand_auto_run_evaluates(self):
        train = pd.DataFrame({'x': [0, 1, 2, 3, 4, 5], 'y': [0, 1, 2, 3, 4, 5]})
        code_lines = self.ql.expand_auto_run(train, 'y', n_epochs=1, max_complexity=5)
