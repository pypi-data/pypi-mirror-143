...
def main(argv=None):    
    ...
    parser.add_argument("-p_sources","--path_sources",dest='PATH_SOURCES',help="path to sources matrix. Ex: ./matrix_100.pa.txt",required=True)
    parser.add_argument("-p_sink","--path_sink",dest='PATH_SINK',help="Path to sink vector, the output of kmtricks filter Ex: ./kmtricks_output/matrices/100.vec",required=True)
    parser.add_argument("-p_missing", "--path_missing_kmers",dest='PATH_MISSING_KMERS',help="Path to missing kmers, the output of kmtricks filter after using kmtricks aggregate Ex: ./kmtricks_output/count/{sink}_missing.txt",required=True)
    ...
    partition=dd.read_table(p_sources,header=None,sep=" ",\
