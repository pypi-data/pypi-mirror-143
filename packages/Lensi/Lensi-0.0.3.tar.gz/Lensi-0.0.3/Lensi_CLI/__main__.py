...
def DownloadFile(download_url,app_source,DeleteWhenInstalled):
...
class Lensi(object):
    def __init__(self) -> None:
        try:
            ...
            global DeleteWhenInstalled
            DeleteWhenInstalled = Lensi_config.get("Lensi","DeleteWhenInstalled")
        except:
            ...
            qq_num = Lensi_config.getint("Lensi", "qq_num")
            baoku_num = Lensi_config.getint("Lensi", "360_num")
            DeleteWhenInstalled = Lensi_config.getboolean("Lensi","DeleteWhenInstalled") 
    ...
    def install_s(self,app_name,app_source):
        if app_source == "qq" or app_source == "360":
            web_install_normal(app_name)
        elif app_source == "hippo":
            web_install_hippo(app_name)
    ...
    def install(self,app_name):
        try:
            ...
            DownloadFile(download_url,"qq",DeleteWhenInstalled)
    
    def search_s(self,source,app_name,limmit_num):#搜索大函数
        source = str(source)
        if source == "-360" or source == "360":
            pprint(web_360_search(app_name,limmit_num))
        elif source == "-Q" or source == "qq":
            pprint(web_qq_search(app_name,limmit_num))
        elif source == "-H" or source == "Hippo":
    ...
    def set(self,options,le_set):
        ...
        elif options == "DeleteWhenInstalled":
            Lensi_config.set("Lensi", "DeleteWhenInstalled",le_set)
