...
+--------------------------------------------+                                                                
            ...
            if a_i < -eps or l_i < -eps:
                ...
                if entry == "Cash":
                    if not self._bankrupt:
                        ...
                        self.owner.file_bankruptcy(event="negative cash")
    ...
    def leverage(self) -> float:
        """
        computes the financial leverage value (debt-to-equity ratio)
    ...
    def plot(self):
        ...
        colors = []
        greens = ["honeydew", "lightgreen", "seagreen", "darkgreen", "darkslategray"]
        ...
        fig = matplotlib_barplot(df, xlabel="", ylabel="", color=colors, title=title, stacked=True, fmt="{0:.2f}",
                                legend="off", size=(5, 5), show=False)
        plt.tight_layout()

        plt.legend(handles=legend_patches, bbox_to_anchor=(1.25, 1))
