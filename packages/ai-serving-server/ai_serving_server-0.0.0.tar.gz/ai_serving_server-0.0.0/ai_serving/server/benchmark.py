...
class BenchmarkClient(threading.Thread):
    ...
    def run(self):
        try:
            from ai_serving.client import AIClient
        except ImportError:
            raise ImportError('AIClient module is not available, it is required for benchmarking.'
                              'Please use "pip install -U bert-serving-client" to install it.')
        with AIClient(port=self.port, port_out=self.port_out,
