...
def set_logger(context, logger_dir=None, logger_name=None, verbose=False, error_log=False, max_bytes=500*1024*1024, backup_count=10):
    ...
    logger_name = logger_name if logger_name else 'AIServer'
    if verbose:
...
def get_args_parser():
    ...
    parser = argparse.ArgumentParser(description='Start a AIServer for serving')
    ...
    group1.add_argument('-tmp_folder', type=str, default='tmp_ai',
...
def get_benchmark_parser():
    ...
    parser.description = 'Benchmark AIServer locally'
...
def get_shutdown_parser():
    ...
    parser.description = 'Shutting down a AIServer instance running on a specific port'

    parser.add_argument('-ip', type=str, default='localhost',
                        help='the ip address that a AIServer is running on')
    parser.add_argument('-port', '-port_in', '-port_data', type=int, required=True,
                        help='the port that a AIServer is running on')
