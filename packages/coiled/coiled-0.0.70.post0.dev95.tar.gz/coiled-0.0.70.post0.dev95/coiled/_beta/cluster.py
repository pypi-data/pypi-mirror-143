...
from .states import ClusterStateEnum, ProcessStateEnum, flatten_log_states, log_states
...
class ClusterBeta(Cluster, Generic[IsAsynchronous]):
    ...
    worker_options
        ...
        to ``{}``.
    worker_vm_types
        List of instance types that you would like workers to use, this list
        can have up to five items. You can use the command
    ...
    worker_memory
        ...
        from the memory that you specify.
    scheduler_class
    ...
    scheduler_options
        ...
        to ``{}``.
    scheduler_vm_types
        List of instance types that you would like the scheduler to use, this
        list can have up to five items. You can use the command
        ``coiled.list_instance_types()`` to se a list of allowed types.
    scheduler_cpu
        Number of CPUs allocated for the scheduler.
    scheduler_memory
        Amount of memory to allocate for the scheduler, Coiled will use a +/-10%
        buffer from the memory what you specify.
    asynchronous
    ...
    def __init__(
        ...
        *,
        ...
        if self.worker_cpu or self.worker_memory:
            self.worker_vm_types = get_instance_type_from_cpu_memory(
                self.worker_cpu, self.worker_memory
            )
        else:
        ...
        if self.scheduler_cpu or self.scheduler_memory:
            self.scheduler_vm_types = get_instance_type_from_cpu_memory(
                self.scheduler_cpu, self.scheduler_memory
            )
        else:
    ...
    async def _start(self):
        ...
        if should_create:
            self.cluster_id = await cloud._create_cluster(
                ...
                scheduler_vm_types=self.scheduler_vm_types or default_instance_types,
                worker_vm_types=self.worker_vm_types or default_instance_types,
        ...
        logger.info(
            f"Creating Cluster (name: {self.name}, id: {self.cluster_id}). This might take a few minutes..."
        )
    ...
    async def _wait_until_ready(self) -> None:
        ...
        if self.show_widget:
            if use_rich_widget():
                ...
                widget = RichClusterWidget(n_workers=self._start_n_workers)
                ctx = widget
        ...
        with ctx:
            while True:
                ...
                scheduler_state_reason = scheduler_current_state["reason"]
                ...
                not_ready_worker_current = [
                    current
                    for current in worker_current_states
                    if ProcessStateEnum(current["state"]) != ProcessStateEnum.started
                ]

                if scheduler_state == ProcessStateEnum.started:
                    scheduler_ready = True
                    scheduler_reason_not_ready = ""
                else:
                    scheduler_ready = False
                    scheduler_reason_not_ready = (
                        f"Scheduler not ready. State: {scheduler_state}. "
                        f"Reason: {scheduler_state_reason}."
                    )

                if len(ready_worker_current) >= self.wait_for_workers:
                    workers_ready = True
                    workers_reason_not_ready = ""
                else:
                    workers_ready = False
                    workers_sample = not_ready_worker_current[:3]
                    state_reason_as_string = "\n".join(
                        f"State: {w['state']}. Reason: {w['reason']}"
                        for w in workers_sample
                    )
                    workers_reason_not_ready = (
                        f"Only {len(ready_worker_current)} workers ready "
                        f"(not {self.wait_for_workers} as requested). "
                        f"Some not ready worker states and reasons are: \n{state_reason_as_string}"
                    )
                ...
                if scheduler_ready and workers_ready:
                    ...
                    if cluster_state in (
                        ...
                        raise ClusterCreationError(
                            f"Cluster status is {cluster_state.value} (reason: {reason})",
                            cluster_id=self.cluster_id,
                        )
                        ...
                        raise ClusterCreationError(
                            "User-specified timeout expired: " + reason_not_ready,
                            cluster_id=self.cluster_id,
                        )
                    else:
    ...
    async def _update_cluster_status_logs(self):
        ...
        if states:
            if not self.show_widget or EXECUTION_CONTEXT == "terminal":
                log_states(states, only_errors=(not self.show_state_updates))
