...
class RichClusterWidget:
    ...
    def update(self, cluster_details: Mapping[str, Any], *args, **kwargs) -> None:
        ...
        if EXECUTION_CONTEXT == "ipython_terminal":
            self.live.refresh()
        else:
            # rich needs ipywidgets to work as intended in jupyter
            # this is a hack to avoid needing ipywidgets
            # maybe it's okay to depend on ipywidgets after https://github.com/dask/distributed/issues/5914
            # is fixed
            #
            # Clearing output to avoid rendering the widget a million times.
            # That will (annoyingly) clear any other output the user has from this cell, but...
            # seems like an okay hack for the moment.
    ...
    def _get_renderable(self) -> rich.console.RenderableType:
        ...
        config = dedent(
            f"""
            [bold green]Region:[/bold green] {region}
        ...
        layout.split(
            rich.layout.Layout(
                rich.panel.Panel(
                    rich.align.Align.center("[bold green frame]Coiled Cluster"),
                ),
                name="header",
                size=3,
            ),
