...
class Cloud(Generic[IsAsynchronous]):
    ...
    async def _list_instance_types(
        ...
        if backend:
            return {
                f"{name}": i
                for name, i in all_instance_types.items()
    ...
    @delete_docstring
    def set_gcp_credentials(self, gcp_credentials: dict, account: Optional[str] = None):
        return self._sync(
            self._set_gcp_credentials, account=account, gcp_credentials=gcp_credentials
        )

    @track_context
    async def _set_gcp_credentials(
        self, gcp_credentials: dict, account: Optional[str] = None
    ):
        account = account or self.default_account
        response = await self._do_request(
            "GET",
