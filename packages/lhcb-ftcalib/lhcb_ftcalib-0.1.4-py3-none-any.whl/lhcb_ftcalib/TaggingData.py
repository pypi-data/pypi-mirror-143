...
class FTCWarning:
    def __init__(self, warntype, msg):
...
class TaggingData:
    ...
    def _compute_calibrated_statistics(self, func, minimizer):
        warnings = []
        if not minimizer.accurate:
            warnings.append(FTCWarning("MinimizerWarning", "Minimization did not converge!"))

        if isinstance(minimizer, iminuit.Minuit):
            self._params_nominal = [minimizer.values[n] for n in func.param_names]
            self._params_uncerts = [minimizer.errors[n] for n in func.param_names]
            self._covariance_matrix = minimizer.covariance.tolist()
        else:
            self._params_nominal = minimizer.values
            self._params_uncerts = minimizer.errors
            self._covariance_matrix = minimizer.covariance

        self._full_data.eval("omega = 0.5", inplace=True)

        # Convert to delta parameter convention, then evaluate using p's only
        params_ps = p_conversion_matrix(func.npar) @ self._params_nominal

        self._full_data.loc[self._full_data.tagged, "omega"] = func.eval_ignore_delta(params_ps[:func.npar],
                                                                                      eta     = self._full_data.eta[self._full_data.tagged],
    ...
    def tagging_efficiency(self, calibrated, inselection=True):
        r"""
    ...
    @functools.lru_cache
    def dilution_squared(self, calibrated, inselection=True):
        ...
        if calibrated:
            if inselection:
                ...
                grad_calib = self.func.gradient(self._params_nominal, self._tagdata.omega, self._tagdata.cdec, self.avg_eta)
                grad_mean_D_sq = -4 * np.sum(grad_calib * D * np.array(self._tagdata.weight), axis=1) / self.cal_Nwts
                cal_error = np.sqrt(grad_mean_D_sq @ self._covariance_matrix @ grad_mean_D_sq.T)
            else:
                ...
                grad_calib = self.func.gradient(self._params_nominal, self._full_data[self._full_data.tagged].omega, self._full_data[self._full_data.tagged].cdec, self.avg_eta)
    ...
    @functools.lru_cache(maxsize=4)
    def tagging_power(self, calibrated, inselection=True):
        r"""
        ...
        :return type: tuple
        """
        D_sq = self.dilution_squared(calibrated, inselection)
        tag_eff = self.tagging_efficiency(calibrated, inselection)

        if calibrated:
            return D_sq[0] * tag_eff[0], np.sqrt(tag_eff[0]**2 * D_sq[1]**2 + D_sq[0]**2 * tag_eff[1]**2), np.sqrt(tag_eff[0]**2 * D_sq[2]**2 + D_sq[0]**2 * tag_eff[1]**2)
        else:
            return D_sq[0] * tag_eff[0], np.sqrt(tag_eff[0]**2 * D_sq[1]**2 + D_sq[0]**2 * tag_eff[1]**2)

    @functools.lru_cache(maxsize=4)
    def mistag_rate(self, calibrated, inselection=True):
    ...
    @functools.lru_cache(maxsize=4)
    def effective_mistag(self, calibrated, inselection=True):
        ...
        if calibrated:
            return 0.5 * (1 - np.sqrt(D_sq[0])), D_sq[1] / (4 * np.sqrt(D_sq[0])), D_sq[2] / (4 * np.sqrt(D_sq[0]))
        else:
            return 0.5 * (1 - np.sqrt(D_sq[0])), D_sq[1] / (4 * np.sqrt(D_sq[0]))
