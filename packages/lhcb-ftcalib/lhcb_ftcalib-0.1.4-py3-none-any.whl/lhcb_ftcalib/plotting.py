...
def calibration_lineshape(tagger, eta_plot):
    """ Returns the lineshape of the calibration curve
        ...
        :return type: list
    """
    return tagger.func.eval_plotting(tagger.stats._params_nominal, eta_plot, tagger.stats._tagdata.decay_flav, tagger.stats.avg_eta)
...
def confidence_bands_lineshape(tagger, eta_plot, dec_plot):
    ...
    J = tagger.func.gradient(tagger.stats._params_nominal, eta_plot, dec_plot, tagger.stats.avg_eta).T
...
def print_fit_setup(ax, position, tagger, uformat="{:L}"):
    ...
    cov = tagger.stats._covariance_matrix
    noms  = conv_mat @ tagger.stats._params_nominal
    uncts = np.sqrt(np.diag(conv_mat @ np.array(cov) @ conv_mat.T))

    param = ["$" + p + "=" + f"{n} ± {s}" + "$" for n, s, p in zip(noms, uncts, param)]
