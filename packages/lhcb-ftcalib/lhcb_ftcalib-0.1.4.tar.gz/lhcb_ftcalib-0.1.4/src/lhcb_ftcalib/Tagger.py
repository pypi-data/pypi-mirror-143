...
class Tagger(TaggerBase):
    ...
    def __nll(self, params):
        ...
        omega = self.func.eval(params, data.eta, data.prod_flav, self.stats.avg_eta)
    ...
    def __nll_oscil(self, params):
        ...
        data = self.stats._tagdata  # This is not a copy
        omega_given = self.func.eval(params, data.eta,      data.prod_flav, self.stats.avg_eta)  # Omega based on predicted production flavour
        omega_oscil = self.func.eval(params, data.eta, -1 * data.prod_flav, self.stats.avg_eta)  # Omega for opposite prod flavour
    ...
    def __nll_oscil_grad(self, params):
        ...
        omega_given = self.func.eval(params, data.eta,      data.prod_flav, self.stats.avg_eta)
        omega_oscil = self.func.eval(params, data.eta, -1 * data.prod_flav, self.stats.avg_eta)
        ...
        for i in range(self.func.npar * 2):
            correct_terms  =      osc_dilution_correct  * self.func.derivative(i, params, data.eta[correct_tags], -1 * data.decay_flav[correct_tags], self.stats.avg_eta)
            correct_terms -= (1 - osc_dilution_correct) * self.func.derivative(i, params, data.eta[correct_tags],      data.decay_flav[correct_tags], self.stats.avg_eta)

            wrong_terms  = (1 - osc_dilution_wrong) * self.func.derivative(i, params, data.eta[wrong_tags],      data.decay_flav[wrong_tags], self.stats.avg_eta)
            wrong_terms -=      osc_dilution_wrong  * self.func.derivative(i, params, data.eta[wrong_tags], -1 * data.decay_flav[wrong_tags], self.stats.avg_eta)
