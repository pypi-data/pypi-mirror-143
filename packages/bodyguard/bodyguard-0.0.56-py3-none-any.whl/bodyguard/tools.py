...
# Main
#------------------------------------------------------------------------------
def generate_random_masks(n_obs, n_split, test_pct):    
    """
    ...
    if n_split==0:
        ...
        masks = [np.random.permutation(n_obs)[0:int(n_obs*test_pct)]]
    elif n_split>1:
        masks = np.array_split(ary=np.random.permutation(n_obs), indices_or_sections=n_split)
