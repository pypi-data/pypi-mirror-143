...
def expand_df(df, by, non_observed=None):
    ...
    if any(not isinstance(x, str) for x in by):
        raise Exception("All by-arguments must be strings. Some of them are not at the moment.")
    ...
    if non_observed is not None:
        ...
        for k,v in non_observed.items():            
            ...
            # Add key to df to be inserted
            df_mergeable = pd.DataFrame(data={k:v,
                                              KEY_COL:KEY_VAL})
            ...
            if k in mergeables.keys():
                ...
                mergeables.append(df_mergeable.drop_duplicates())    
