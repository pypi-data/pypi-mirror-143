...
class LocalObject( Mapping ):
    ...
    def reload( self ):
        self.__init__( self.path, parent = self.__parent )
...
class LocalAsset( LocalObject ):
    ...
    def __init__( self, path, parent ):
        ...
        file = self._parse_path( self.meta[ 'file' ] )
        if (
            ...
            ( not file.startswith( windows_long_file_prefix ) )
                ...
                # convert path to long form on windows if needed
...
class LocalCollection():
    ...
    def find( self, search = {} ):
        ...
        """
        return filter( search, self.__objects )
    ...
    def insert_one( self, path, properties = {} ):
        ...
        # incorporate new object
        obj = (
            LocalContainer( _id, parent = self.root )
            if self.kind == 'container' else
            LocalAsset( _id, self.root )
        )
