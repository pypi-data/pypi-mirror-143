...
class DescargaCfdi(models.Model):
    ...
    def habilitar_consulta_status_descarga(self):
        if self.metodo_descarga == self.WEBSERVICE:
            return bool(self.numero_solicitud)
        
        return bool(self.status == self.ST_PENDIENTE)

    def solicitar_descarga_scrapping(self, rfc, ciec_password, forzar_descarga=False):
        from django.conf import settings
        import requests

        api_url = getattr(settings, "SAT_SCRAPPER_URL", None)
        ...
        try:
            ...
            data = {
                "ciecPass": ciec_password,
                "externalId": self.id, 
                "from": self.fecha_inicio.isoformat(),
                "to": self.fecha_final.isoformat(),
                "rfc": rfc,
                "downloadType": tipo_descarga, 
                "downloadRequestType": self.tipo_solicitud, 
            }
            ...
            if api_response.ok:
                ...
                if json_response.get("downloadUrl"):
                    self.status = self.ST_FINALIZADO
                    self.archivos = [json_response.get("downloadUrl")]
                    self.save()
        ...
        if self.metodo_descarga == self.WEBSERVICE:
            if self.pac == 1:
                if self.consulta_object:
                    return self.consulta_object["mensaje"]
    ...
    def solicitar_descarga(self, rfc=None, ciec_password=None):
        if self.metodo_descarga == self.SCRAPPING:
            ...
            return self.solicitar_descarga_scrapping(
                rfc=rfc, 
        ...
        descarga = SolicitaDescarga(self.fiel)
    ...
    def solicitar_status_descarga(self, rfc=None, ciec_password=None):
        ...
        if self.metodo_descarga == self.SCRAPPING:
            self.solicitar_descarga_scrapping(
                rfc=rfc, 
                ciec_password=ciec_password,
            )
