...
class LineageItem:
    ...
    name: Optional[str] = ''  # name for dashboards
    level: int  # upstream/downstream distance from current resource
    type: str  # indicate resouce type
    group_name: Optional[str] = ''  # group name for dashboards
    description: Optional[str]
    badges: Optional[List[Badge]] = None
    usage: Optional[int] = None  # statistic to sort lineage items by
    upstream_entities: Optional[List[str]] = []  # list of upstream table/col/task keys
    downstream_entities: Optional[List[str]] = []  # list of downstream table/col/task keys
