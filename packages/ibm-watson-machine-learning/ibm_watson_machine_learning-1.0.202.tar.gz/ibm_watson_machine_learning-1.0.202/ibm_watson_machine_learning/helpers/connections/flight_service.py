...
class FlightConnection:
    ...
    def _get_data(self,
        ...
        try:
                ...
                if row_size != 0:  # append batches only when we have data
                    if not self.stop_reading:
                        ...
                        if self.enable_subsampling:
                            ...
                            self.q.put_nowait((thread_number, data))
            ...
            self.q.put_nowait((thread_number, 0))  # finish this thread

        except StopIteration:
            # Reading of batch finished, result commited."
            self.q.put_nowait((thread_number, 0))
        ...
        def start_reading_threads():
            ...
            if self.enable_subsampling:
                if self.learning_type == 'regression':
                    subsampling_thread = threading.Thread(target=self.regression_random_sampling, args=(self.label,))

                else:
                    subsampling_thread = threading.Thread(target=self.stratified_subsampling, args=(self.label,))
            ...
            self.q.put_nowait((-1, -1))  # send close queue message (stop reading logical batches)
    ...
    def _get_max_rows(self, _type: str) -> int:
        ...
        while True:
            if self.row_size == -1:
                raise TypeError("Data is not of CSV/parquet type. Try to use a binary read mode.")
    ...
    def create_logical_batch(self, timeout: int = 60 * 10, _type: str = 'logical') -> 'pd.DataFrame':
        ...
        threads_finished = 0
        msg = "Data is not of CSV/parquet type. Try to use a binary read mode."
        ...
        while True:
            try:
                ...
                thread_number, data = self.q.get(timeout=timeout)  # wait max 10 min
                self.q.task_done()
    ...
    def _select_source_command(self, infer_schema: bool = True) -> List[str]:
        ...
        if self.data_location is not None and self.data_location['location'].get('userfs'):
            command['interaction_properties'].update({'userfs': True})
