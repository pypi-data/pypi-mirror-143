...
@multi_build.command()
@click.option('--versions', '-v', multiple=True, default=["3.7", "3.8", "3.9", "3.10"], show_default=True, help="Python version to setup conda environment with.")
def setup(versions: Tuple[str, ...]):
    ...
    for version in versions:
        ...
        if env_name not in envs:
            ...
            create_conda_env(env_name, [f"python={version}", "pybind11", "pybind11-stubgen", "numpy"])
        else:
            click.secho(f"{env_name} already exists. Skipping.", fg="yellow")


@multi_build.command()
@click.option('--versions', '-v', multiple=True, default=["3.7", "3.8", "3.9", "3.10"], show_default=True, help="Python version to remove conda environment with.")
...
@multi_build.command()
@click.option('--versions', '-v', multiple=True, default=["3.7", "3.8", "3.9", "3.10"], show_default=True, help="Python version to build project with.")
...
def run(ctx: click.Context, versions: Tuple[str, ...], aggregate_output: Path, aggregate: bool):
    ...
    if not target_envs.issubset(envs):
        raise click.BadParameter(f"Missing environments: {target_envs - envs}, please run \"slxpy build --setup-builder\" first.")
