...
from pyvisflow.utils.data_gen import get_project_root
...
class App(BoxContainer):
    def __init__(self) -> None:
        ...
        def on_before_data_fn(data: pd.DataFrame):
            if isinstance(data, pd.DataFrame):
                return self.df_manager.mark_data(data)
    ...
    def add_srcipt(self, *names: str):
        ...
        self._scripts.extend(names)

    def pretty_json(self):
        return json.dumps(self.create_config().dict(), indent=2)
    ...
    def to_html(self, file):
        ...
        config = self.create_config().json()
