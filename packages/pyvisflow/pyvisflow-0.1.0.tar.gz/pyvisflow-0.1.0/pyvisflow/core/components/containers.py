...
from pyvisflow.core.props import BoolTypePropInfo, NamePropInfo
...
class BoxContainer(Container):
    ...
    def dataTable(self, dataframe: pd.DataFrame, page_size=10):
        cp = DataTable(self._on_before_data_fn(dataframe), page_size)
        self.add_child(cp)
        return cp
    ...
    def echart(self, data: pd.DataFrame):
        cp = EChart(self._on_before_data_fn(data))
        self.add_child(cp)
        return cp
