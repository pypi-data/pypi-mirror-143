...
class Tft(Config):
    ...
    def prepare_data(self, normalization=True):
        ...
        #Add time idx----------------------------------------------------------------------------------------
        if frequency=='2M':
            data_range=pd.date_range(start=Qprep.timestamp.min(), end=Qprep.timestamp.max(), freq=frequency)
            data_range= data_range+pd.offsets.MonthEnd(n=Qprep.timestamp.max().month%2)if suffix=='' else data_range+pd.offsets.MonthBegin(n=Qprep.timestamp.max().month%2)
        else:
        ...
        return data1, scalers, Qprep, suffix, known, unknown, group_ids, test_data, n_features, item_location, categorical
