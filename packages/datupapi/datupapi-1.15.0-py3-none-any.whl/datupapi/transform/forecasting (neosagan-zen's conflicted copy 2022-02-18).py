...
class Forecasting(Config):
    ...
    #def compute_interval_per_backtests(self, df_forecast, q_backtest, backtest_names=None, datalake_path_=None, use_location=False, location=None):
    #    """
    ...
    #            if use_location:
    #                df_back = df_back.loc[df_back['Location'] == location, :]
    #            for item in df_back['Item'].unique():
    #                mask_back = df_back['Item'] == item
    #                mask_fore = df_forecast['Item'] == item
    #                interval_dict.update({item: interval for interval in df_back.loc[mask_back, 'SuggestedInterval']})
    #                df_forecast.loc[mask_fore, 'SuggIntervalBacktest' + self.backtest_ids[back_ix]] = \
    #                df_forecast.loc[mask_fore, 'Item'].map(interval_dict)
    #        sugg_interval_cols = ['SuggIntervalBacktest' + b for b in self.backtest_ids]
    ...
    def compute_next_suggested_interval_forecast(self, df_forecast):
        ...
        try:
            if len(forecast_types) == 5:
                forecast_types_trim = [self.forecast_types[0],self.forecast_types[1],self.forecast_types[3],self.forecast_types[5],self.forecast_types[6]]
...
#    def compute_naive_forecast(self, df_forecast, num_periods=io.forecast_horizon, naive_type='roll'):
#        """
#        Return a dataframe attaching the back suggested interval and forecast per date and item record
#
#        :param df_forecast: Original dataframe including all items' forecasts
...
#                if naive_type == 'last':
#                    target = df_forecast[mask]['Target'].shift(-num_periods).fillna(0)
#                    df_forecast.loc[mask, 'ForecastNaive'] = target
#                elif naive_type == 'roll':
#                    target = pd.Series(reversed(list(df_forecast.loc[mask, 'Target'].values)))
#                    rolling = np.flipud(np.round(target.rolling(window=num_periods, min_periods=1).mean().values))
#                    df_forecast.loc[mask, 'ForecastNaive'] = np.roll(rolling, -1)
#                else:
        ...
        try:
          ...
          if naive_type=="roll":
            for i in range(df_forecast.Item.nunique()):
              rolling =np.roll((np.round(pd.Series(target[num_periods*(self.backtests+1)*i:num_periods*(self.backtests+1)*(i+1)]).rolling(window=num_periods, min_periods=1).mean().values)),1)
              rolling_list=np.append(rolling_list,rolling)
          elif naive_type=="last":
            for i in range(df_forecast.Item.nunique()):
              rolling=pd.Series(target[num_periods*(self.backtests+1)*i:num_periods*(self.backtests+1)*(i+1)]).shift(num_periods).fillna(0).values
              rolling_list=np.append(rolling_list,rolling)
          else:
            self.logger.debug(f'Invalid naive forecast option. Please check options.')
    ...
    def concat_forecast_horizons(self, model_type=None, forecast_horizons=None):
        ...
        try:
            df_fcst_s = self.io.download_csv(q_name='Qfcst',
                                             datalake_path=os.path.join(self.multiforecast_path, model_type + str(forecast_horizons[0]) + self.dataset_frequency.lower(), 'output'),
                                             date_cols=['Date'])
            if len(forecast_horizons) == 2:
                df_fcst_m = self.io.download_csv(q_name='Qfcst',
                                                 datalake_path=os.path.join(self.multiforecast_path, model_type + str(forecast_horizons[1]) + self.dataset_frequency.lower(), 'output'),
                                                 date_cols=['Date'])
                ...
                df_fcst_l = self.io.download_csv(q_name='Qfcst',
                                                 datalake_path=os.path.join(self.multiforecast_path, model_type + str(forecast_horizons[2]) + self.dataset_frequency.lower(), 'output'),
                                                 date_cols=['Date'])
            else:
            ...
            if len(forecast_horizons) == 2:
                if self.io.dataset_frequency == 'M':
                    ...
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1] - forecast_horizons[0])]
                    ...
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(weeks=forecast_horizons[1] - forecast_horizons[0])]
                    ...
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(days=forecast_horizons[1] - forecast_horizons[0])]
                else:
                ...
                if self.io.dataset_frequency == 'M':
                    ...
                    df_fcst_s = df_fcst_s[df_fcst_s['Date'] > df_fcst_s['Date'].max() - pd.DateOffset(months=forecast_horizons[0])]
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1] - forecast_horizons[0])]
                    df_fcst_l = df_fcst_l[df_fcst_l['Date'] > df_fcst_l['Date'].max() - pd.DateOffset(months=forecast_horizons[2] - forecast_horizons[1])]
                elif self.io.dataset_frequency == 'W':
                    df_bckt_s = df_fcst_s[df_fcst_s['Date'] <= df_fcst_s['Date'].max() - pd.DateOffset(weeks=forecast_horizons[0])]
                    df_bckt_m = df_fcst_m[df_fcst_m['Date'] < df_bckt_s['Date'].min()]
                    df_bckt_l = df_fcst_l[df_fcst_l['Date'] < df_bckt_m['Date'].min()]
                    df_fcst_s = df_fcst_s[df_fcst_s['Date'] > df_fcst_s['Date'].max() - pd.DateOffset(weeks=forecast_horizons[0])]
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(weeks=forecast_horizons[1] - forecast_horizons[0])]
                    df_fcst_l = df_fcst_l[df_fcst_l['Date'] > df_fcst_l['Date'].max() - pd.DateOffset(weeks=forecast_horizons[2] - forecast_horizons[1])]
                elif self.io.dataset_frequency == 'D':
                    df_bckt_s = df_fcst_s[df_fcst_s['Date'] <= df_fcst_s['Date'].max() - pd.DateOffset(days=forecast_horizons[0])]
                    df_bckt_m = df_fcst_m[df_fcst_m['Date'] < df_bckt_s['Date'].min()]
                    df_bckt_l = df_fcst_l[df_fcst_l['Date'] < df_bckt_m['Date'].min()]
                    df_fcst_s = df_fcst_s[df_fcst_s['Date'] > df_fcst_s['Date'].max() - pd.DateOffset(days=forecast_horizons[0])]
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(days=forecast_horizons[1] - forecast_horizons[0])]
                    df_fcst_l = df_fcst_l[df_fcst_l['Date'] > df_fcst_l['Date'].max() - pd.DateOffset(days=forecast_horizons[2] - forecast_horizons[1])]
                else:
                    self.logger.debug(f'No valid dataset frequency. Please check dataset_frequency in config.yml file.')
        ...
        try:
            ...
            if len(forecast_horizons) == 2:
                if self.io.dataset_frequency == 'M':
                    ...
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1]/4 - forecast_horizons[0]/4)]
                elif self.io.dataset_frequency == 'D':
                    ...
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1]/30 - forecast_horizons[0]/30)]
                else:
                    self.logger.debug(f'No valid dataset frequency. Please check dataset_frequency in config.yml file.')
                ...
                if self.io.dataset_frequency == 'M':
                    ...
                    df_fcst_s = df_fcst_s[df_fcst_s['Date'] > df_fcst_s['Date'].max() - pd.DateOffset(months=forecast_horizons[0]/4)]
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1]/4 - forecast_horizons[0]/4)]
                    df_fcst_l = df_fcst_l[df_fcst_l['Date'] > df_fcst_l['Date'].max() - pd.DateOffset(months=forecast_horizons[2]/4 - forecast_horizons[1]/4)]
                elif self.io.dataset_frequency == 'D':
                    df_bckt_s = df_fcst_s[df_fcst_s['Date'] <= df_fcst_s['Date'].max() - pd.DateOffset(months=forecast_horizons[0]/30)]
                    df_bckt_m = df_fcst_m[df_fcst_m['Date'] < df_bckt_s['Date'].min()]
                    df_bckt_l = df_fcst_l[df_fcst_l['Date'] < df_bckt_m['Date'].min()]
                    df_fcst_s = df_fcst_s[df_fcst_s['Date'] > df_fcst_s['Date'].max() - pd.DateOffset(months=forecast_horizons[0]/30)]
                    df_fcst_m = df_fcst_m[df_fcst_m['Date'] > df_fcst_m['Date'].max() - pd.DateOffset(months=forecast_horizons[1]/30 - forecast_horizons[0]/30)]
                    df_fcst_l = df_fcst_l[df_fcst_l['Date'] > df_fcst_l['Date'].max() - pd.DateOffset(months=forecast_horizons[2]/30 - forecast_horizons[1]/30)]
                else:
                    self.logger.debug(f'No valid dataset frequency. Please check dataset_frequency in config.yml file.')
                df_fcst = pd.concat([df_fcst_l, df_fcst_m, df_fcst_s, df_bckt_s, df_bckt_m, df_bckt_l], axis='rows') \
                            .sort_values(['Date', 'Item'], ascending=False)
