...
def ftl_to_stub(ftl_path: str, locale: str = "en-US", root_class_name: str = "TranslatorRunner") -> str:
...
def _ftl_bundle_to_raw_items(ftl_path: str, locale: str, **kwargs) -> Dict[str, str]:
    raw_ftl = FluentBundle(locale=locale, resources=[
        FtlResource.from_file(ftl_path)
    ], **kwargs).__dict__.get("_compiled_messages")
    if raw_ftl:
