...
from pyvss.const import (
    ...
    VSS_API_WEBDAV_SERVER, __version__ as product_version)
...
class VssManager:
    ...
    def get_vskey_stor(self, **kwargs):
        ...
        opts = dict(
            ...
            webdav_hostname=os.environ.get(
                'VSS_API_WEBDAV_SERVER', VSS_API_WEBDAV_SERVER
            ),
        )
        opts.update(kwargs)
    ...
    def cancel_retirement_request(self, request_id: int):
        ...
        rv = self.request(
            f'/request/retirement/{request_id}/cancel', method=self.PUT,
        )
        if rv:
            return rv.get('data')
        return rv
    ...
    def confirm_retirement_request(self, request_id: int):
        ...
        """
        rv = self.request(
            f'/request/retirement/{request_id}/confirm', method=self.PUT,
        )
        if rv:
            return rv.get('data')
        return rv
    ...
    def create_vm_snapshot(
        ...
        if date_time is None:
            date_time = datetime.datetime.now().strftime(DATETIME_FMT)
        date_time_v = datetime.datetime.strptime(date_time, DATETIME_FMT)
        if self.debug:
            print(date_time_v)
        # payload
        ...
        rv = self.request(
            f'/vm/{vm_id}/snapshot', method=self.POST, payload=payload,
        )
        return rv.get('data')
    ...
    def update_vm_disk_scsi(self, vm_id, unit, bus_number, **kwargs):
        ...
        payload.update(kwargs)
        rv = self.request(
            f'/vm/{vm_id}/disk/{unit}/scsi', method=self.PUT, payload=payload,
        )
        return rv.get('data')
    ...
    def update_vm_disk_capacity(self, vm_id, unit, value_gb, **kwargs):
        ...
        payload.update(kwargs)
        rv = self.request(
            f'/vm/{vm_id}/disk/{unit}', method=self.PUT, payload=payload,
        )
        return rv.get('data')
    ...
    def delete_vm_disks(self, vm_id, units, **kwargs):
        ...
        payload.update(kwargs)
        rv = self.request(
            f'/vm/{vm_id}/disk', method=self.DELETE, payload=payload,
        )
        return rv.get('data')
    ...
    def get_retirement_datetime_spec(date_time: str) -> Dict:
        ...
        :return: object
        """
        _ = datetime.datetime.strptime(date_time, DATETIME_FMT)
