...
def get_label(f):   
    ...
    if ("Neg" in filename) or ("Control" in filename) or ('CONTROL' in filename):
        return "Control"
    elif ("Pos" in filename) or ("Covid" in filename) or ('COVID' in filename):
        return "Covid"
...
def preprocess_all_files(path_to_measurements, file_list=[], parse_time=True, parse_filename=True, concat_files=True, rename_column=True, no_repeat=False, fillna=True, offset=False, has_label=True):
    ...
    if isinstance(path_to_measurements, str):
