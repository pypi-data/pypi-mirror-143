...
def dataframe_to_sql(dataframe, table_name: str, append: bool=False, flavor: str=None):
    ...
    if flavor == "bigquery":
        dtype_to_sql = {"int64": "INT64", "float64": "NUMERIC"}
        str_type = "STRING"
    else:
        dtype_to_sql = {"int64": "INTEGER", "float64": "DOUBLE PRECISION"}
        str_type = "VARCHAR"
    cols = list(dataframe.columns)
    types = []
    for col, dtype in zip(cols, dataframe.dtypes):
        sql_type = dtype_to_sql.get(str(dtype)) or f"{str_type}({dataframe[col].str.len().max()+3:d})"
