...
class Monochrome(Component, Settings):
    ...
    configurables = [
        ...
        ('TNR_kernel_size', {
            'name': 'kernel size in pixels', 'float': (3, 30, 1),
            'help': ''})
        ]

    save_settings = [
        'gradient', 'white', 'black', 'p1', 'lift', 'fracbin', # 'TNR_param',
    ...
    def luminance(self, *args):
        ...
        # if self.TNR_param > .5 and self._background is not None:
        #     bkg = (self._background - black) / (self.white - black)
        #     bkg[bkg > 1] = 1
        #     bkg[bkg < 0] = 0
