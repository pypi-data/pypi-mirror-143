...
class PipeChecker(unittest.TestCase):
    ...
    def runTest(self):
        ...
        check_fixtures_data = checker_r.json().get('data', [])
        error_check_fixtures_data = checker_r.json().get('error', None)
        ...
        for i, (current_data_e, check_fixtures_data_e) in enumerate(zip(current_data, check_fixtures_data)):
            ...
            for x in current_data_e.keys():
                if type(current_data_e[x]) == float:
                    ...
                    self.assertLessEqual(d / current_data_e[x], 0.01, f"Failed on index {i}, key {x}. old value: {current_data_e[x]}, new value {check_fixtures_data_e[x]}, {current_data_e} vs {check_fixtures_data_e}")
                else:
                    self.assertEqual(
                        current_data_e[x], check_fixtures_data_e[x], f"Failed on index {i}, key {x}")


async def check_pipe(pipe, host, token, populate, cl, limit=50, only_response_times=False):
    ...
    params = {
        'q': f"select distinct(url) endpoint_url from tinybird.pipe_stats_rt where url like '%/{pipe['name']}.%' and url is not null limit {limit} FORMAT JSON"
    }
    ...
    pipe_top_requests = [{'endpoint_url': f"{host}{x['endpoint_url']}"} for x in r.json().get('data', [])]
...
async def new_pipe(p, tb_client: TinyB, replace=False, check=True, populate=False, subset=None, wait_populate=False, skip_tokens=False, tag='', skip_table_checks=False, ignore_sql_errors=False, only_response_times=False, timeout=None, run_tests=False, as_standard=False):  # noqa: C901
    exists = False
    materialized = False
...
async def exec_file(r, tb_client, override, check, debug, populate, subset, wait_populate, tag, skip_table_checks=False, ignore_sql_errors=False, skip_confirmation=False, only_response_times=False, timeout=None, run_tests=False, as_standard=False):
    if debug:
            ...
            for x in file_deps:
                if x not in INTERNAL_TABLES:
                    ...
                    if ds:
                        ...
                        try:
                            to_run[ds_fn]['deps'] = set(to_run[ds_fn].get('deps', []) + prev.get('deps', []) + [fn.replace(deps_tag, '')])
                        except ValueError:
                            pass
...
async def folder_push(tb_client: TinyB, tag='', filenames=None, dry_run=False, check=False, push_deps=False,  # noqa: C901
                      debug=False, force=False, folder='.', populate=False, subset=None, upload_fixtures=False, wait=False,
                      skip_table_checks=False, ignore_sql_errors=False, skip_confirmation=None, only_response_times=False,
                      workspace_map=None, workspace_lib_paths=None, no_versions=False, timeout=None, run_tests=None, as_standard=False):
            ...
            if name in to_run:
                if not dry_run:
                    if name not in remote_resource_names or resource_versions.get(name.replace(".", "_"), '') != latest_datasource_versions.get(name, '') or force or run_tests:
                        ...
                        try:
                            await exec_file(to_run[name], tb_client, force, check, debug, populate, subset, wait, tag,
                                            skip_table_checks, ignore_sql_errors, skip_confirmation, only_response_times, timeout, run_tests, as_standard)
                            if not run_tests:
