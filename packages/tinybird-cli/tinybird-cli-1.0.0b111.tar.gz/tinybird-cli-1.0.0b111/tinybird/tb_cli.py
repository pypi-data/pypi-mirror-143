...
from tinybird.client import TinyB, AuthException, AuthNoTokenException, CanNotBeDeletedException, DoesNotExistException, OperationCanNotBePerformed
...
@click.option('-l', '--limit', type=click.IntRange(10, 100), default=50, required=False, help="Number of requests to validate")
@click.pass_context
@coro
async def push(ctx, prefix, filenames, dry_run, check, push_deps, debug, force, populate, subset, fixtures, wait, skip_table_checks, yes, only_response_times, workspace_map, workspace, no_versions, timeout, limit: int):
...
@cli.command(name="materialize", short_help="Given a Pipe datafile and a node name it generates the target Data Source and materialized Pipe ready to be pushed")
@click.argument('filename', type=click.Path(exists=True))
@click.argument('node_name')
@click.argument('target_datasource', default=None, required=False)
@click.option('--force', is_flag=True, default=False, help="Override existing local files and/or pipe")
@click.option('--prefix', default='', help="Use prefix for all the resources")
@click.option('--push-deps', is_flag=True, default=False, help="Push dependencies, disabled by default")
...
@click.pass_context
@coro
async def materialize(ctx, filename, node_name, force, prefix, push_deps, workspace_map, workspace, no_versions, target_datasource=None):
    """Analyze the `node_name` SQL query to generate the `.datasource` and `.pipe` files needed to push a new materialize view.

    Syntax: tb materialize path/to/pipe.pipe node_name
    """
    try:
        ...
        pipe_name, _ = filename.rsplit('.', 1)
        pipe = to_run[pipe_name.split('/')[-1]]
        node_name = node_name.lower().strip()
        for node in pipe['nodes']:
            if node['params']['name'].lower() == node_name:
                datasource_name = target_datasource or node['params'].get('datasource', None) or f'mv_{pipe["name"]}_{node_name}'
                analysis = await cl.analyze_pipe_node(pipe['name'], node_name, datasource_name)
                datafile = analysis['analysis']['datasource']['datafile']

                base = Path('pipes')
                if not base.exists():
                    base = Path()
                pipe_file_name = f"{pipe['name']}_gen.pipe"
                f_pipe = base / pipe_file_name
                if not f_pipe.exists() or force:
                    with open(f'{f_pipe}', 'w') as file:
                        sql = analysis['analysis']['pipe']['datafile']
                        file.write(sql)
                else:
                    click.echo(FeedbackManager.error_exception(error=f'File {f_pipe} already exists, use --force to override'))
                    return
                ...
                if not f.exists() or force:
                    with open(f'{f}', 'w') as file:
                        file.write(datafile)
                    click.echo(FeedbackManager.success_generated_matview(node_name=node_name, file=f))
                    click.echo(FeedbackManager.success_generated_pipe(file=f_pipe))
                else:
                    click.echo(FeedbackManager.error_exception(error=f'File {f} already exists, use --force to override'))
    except Exception as e:
        click.echo(FeedbackManager.error_exception(error=str(e)))
        ...
        try:
            await cl.pipe_delete(pipe['name'])
        except Exception:
            pass
...
@click.pass_context
@coro
async def pipe_ls(ctx: click.Context, prefix: str, match: str):
    """List pipes"""
...
@click.option('--workspace', nargs=2, type=str, multiple=True, help="add a workspace path to the list of external workspaces, usage: --workspace name path/to/folder")
@click.option('--no-versions', is_flag=True, default=False, help="when set, resource dependency versions are not used, it pushes the dependencies as-is")
@click.option('-l', '--limit', type=click.IntRange(10, 100), default=10, required=False, help="Number of requests used to sample per each group")
...
@click.pass_context
@coro
async def regression_test(ctx, prefix, filenames, debug, only_response_times, workspace_map, workspace, no_versions, limit: int, match: List[str]):
