...
from classiq_interface.executor.optimizer_preferences import CombinatorialOptimizer
...
class CombinatorialOptimization:
    ...
    def solve_classically(self) -> ExecutionData:
        return asyncio.run(self.solve_classically_async())

    async def solve_classically_async(self) -> ExecutionData:
        """Async version of `solve`"""
        wrapper = ApiWrapper()
        result = await wrapper.call_combinatorial_optimization_solve_classically_task(
            problem=self._problem
        )
    ...
    def get_operator(self) -> operator.PauliOperator:
