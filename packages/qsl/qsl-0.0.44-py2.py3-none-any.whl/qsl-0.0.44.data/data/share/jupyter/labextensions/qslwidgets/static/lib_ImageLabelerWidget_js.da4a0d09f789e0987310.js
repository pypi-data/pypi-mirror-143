...
const Widget = ({ model }) => {
    ...
    // @ts-ignore
    const [fixedLayout, setFixedLayout] = hooks_1.useModelState('fixedLayout', model);
    // @ts-ignore
    ...
    return (react_1.default.createElement(react_image_labeler_1.default, { src: url, config: config, labels: labels || {}, metadata: metadata, style: { padding: 16 }, callbacks: {
            ...
            onUnignore: buttons['unignore']
                ? () => setAction('unignore')
                : undefined,
        }, preload: preload, options: { progress, mode, fixedLayout, maxCanvasSize, showNavigation } }));
};
