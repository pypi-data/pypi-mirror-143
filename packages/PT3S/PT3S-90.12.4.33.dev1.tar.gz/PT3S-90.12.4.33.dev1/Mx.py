...
class Mx():
            ...
            while fPos < file_size:   
                ...
                try:
                    ...
                    if mxsVecsH5StorePtr != None:                                                        
                                ...
                                if h5Dump and ((not StatTimeTminTmaxInVecsOnly) or (cSNAPSHOTTYPE=='STAT')):                                  
                                    try:                                            
                                ...
                                elif ((cSNAPSHOTTYPE in ['TMIN','TMAX']) or (cSNAPSHOTTYPE=='TIME' and time == firstTime)):     
                                # kein "normaler" store vecs in H5
                                # d.h. z.B. es gibt den Zeitstempel schon oder Zeitstempel sollen nicht ins vecs weil das dann zu groess wird
                                # die angegebenen gelesenen "Zeiten" werden hier nach dfVecAggs verarbeitet 
                                        h5DumpLog="{:s} - H5Dump:  NO (key /{!s:>20s})".format(h5DumpLogSST,h5Key)  
                    ...
                    if  warnLogLevel:
                        logger.warning("{:s}TimeNr. {:>6d} read and processed finally={!s:s} Values (without TIMESTAMP): {:d} - {:s}.".format(logStr
                                                                    ,recsReadFromFile+1
                                                                    ,time_read_finally
    ...
    def getVecAggsResultsForObjectType(self
        ,Sir3sVecIDReExp='^KNOT~\*~\*~\*~PH'
        #'^KNOT~\*~\*~\*~PH$' - genau 1 Kanal des ObjektTyps
        #'^KNOT~\*~\*~\*~PH' - ggf. mehrere
        #'^KNOT' - alle
    ):
        """
        # Sir3sIDReExp definiert einen oder mehrere oder alle Kanaele eines OBJTYPEs 
        ...
        # zukuenftig:
        # lesen der verlangten Kanaele von MXS
