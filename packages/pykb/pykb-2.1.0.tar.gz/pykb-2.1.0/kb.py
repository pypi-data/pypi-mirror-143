    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                time.sleep(1.0 / EVENT_POLLING_RATE)
                continue
...
class KB:
    def __init__(
        ...
        if not self.embedded:
            if not host or not port:
                raise KbError(
                    "No host and/or port specified to connect to the knowledge base."
                )
            self._channels = {}
            self._asyncore_thread = threading.Thread(
                target=asyncore.loop, kwargs={"timeout": 0.1, "map": self._channels}
            )
            self._client = RemoteKBClient(
                self._internal_events, self._channels, host, port, sock
            )
        ...
        self._callbackexecutor = EventCallbackExecutor(
            self._internal_events, self.events, self._registered_callbacks
        )
    ...
    def add_method(self, m):
        def innermethod(*args, **kwargs):
            kblogger.debug(
                "Sending <%s(%s,%s)> request to server."
                % (
                    m,
        ...
        innermethod.__doc__ = (
            "This method is a proxy for the knowledge server %s method." % m
        )
        # special cases for the server's methods we want to override
        if m == "subscribe":
            innermethod.__name__ = "server_subscribe"
        elif m == "close":
            innermethod.__name__ = "server_close"
        else:
            innermethod.__name__ = m
        ...
        if type == "NEW_INSTANCE" and not var:
            ...
            if len(vars) > 1:
                raise AttributeError(
                    "You must specify which variable must be returned "
    ...
    def __getitem__(self, *args):
        ...
        # List of patterns
            ...
            return self.find(
                list(allvars), ["%s %s %s" % p for p in patterns], None, models
            )
    ...
    def __init__(self, defaultontology=None):
        ...
        if not EmbeddedKBClient.kb:
            ...
            EmbeddedKBClient.kb_thread = threading.Thread(
                target=self.process, kwargs={"defaultontology": defaultontology}
            )
            ...
            if defaultontology:
                kblogger.warn(
                    "The embedded knowledge base has already been "
                    + "initialized. I will ignore default ontology <%s>."
                    % defaultontology
                )
...
class RemoteKBClient(asynchat.async_chat):
    ...
    def __init__(
        self, event_queue, map, host="localhost", port=DEFAULT_PORT, sock=None
    ):
    ...
    def decode(self, raw):
        ...
        if parts[0] == "ok":
            ...
            return "error", "%s: %s" % (
                parts[1],
                parts[2] if len(parts) == 3 else "[no error msg]",
            )
        else:
            raise KbError(
                "Got an unexpected message status from the knowledge base: %s"
                % parts[0]
            )
