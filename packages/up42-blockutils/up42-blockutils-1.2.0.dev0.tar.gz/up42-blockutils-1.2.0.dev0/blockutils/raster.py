...
def to_cog(path_to_image: Path, profile: str = "deflate", **options) -> bool:
    ...
    with _handle_alpha_colorinterp(path_to_image=path_to_image):
...
@contextmanager
def _handle_alpha_colorinterp(path_to_image: Path) -> None:
    alpha_index = None
    with rio.open(path_to_image, 'r+') as src: 
        if ColorInterp.alpha in src.colorinterp:
            ...
            colorinterp_list = list(src.colorinterp)
            colorinterp_list[alpha_index] = ColorInterp.undefined

            logger.info(
                "Dropping alpha band color interpretation before cog_translate, which can cause data corruption."
            )
            src.colorinterp = colorinterp_list
    
    yield
    
    with rio.open(path_to_image, 'r+') as src:
        if alpha_index is not None:
            colorinterp_list[alpha_index] = ColorInterp.alpha

            logger.info(
                "Adding alpha band color interpretation back."
            )
            src.colorinterp = colorinterp_list