...
class DatasetGenerator:
    ...
    def __init__(self,
        ...
        if self._transforms is not None and self._transforms_callback is None:
            ...
            if not isinstance(self._transforms, list):
                logger.warning('"transforms" inputs is not a list, but a "transforms_callback" was not given! using default callback for list')
            self._transforms_callback = self.default_transforms_callback
    ...
    def default_transforms_callback(transforms, image, target, annotation_type):
        """
        Default function to run for transformation on each batch

        :param transforms: the transformations to perform. for the default should be list
        :param image: image ndarray
        :param target: annotation. can be box, polygon or segmentation
...
def torchvision_transforms_callback(transforms, image, target, annotation_type):
    import torchvision
    assert isinstance(transforms, torchvision.transforms.Compose), 'mismatch between transforms and transform_callback'
    for t in transforms.transforms:
        image = t(image)
    return image, target


def imgaug_transforms_callback(transforms, image, target, annotation_type):
    # handle imgaug calls
    import imgaug
    assert isinstance(transforms,
                      imgaug.augmenters.meta.Sequential), 'mismatch between transforms and transform_callback'
