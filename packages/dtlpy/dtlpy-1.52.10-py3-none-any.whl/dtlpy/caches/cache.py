...
class CacheManger:
    ...
    def delete(self, key: CacheKey):
        ...
        for i in range(1, self._max_level + 1):
            ...
            # self.delete_all_child(master_id=key.entity_id)
            key.object_type = '*'
            list_keys = self.cache_levels[i].list(key=key.get_key())
            for k in list_keys:
                val = self.cache_levels[i].get(key=k)
                e_type, e_id, e_obj = val.split('\\')
                self.delete(key=CacheKey(entity_type=e_id, entity_id=e_id, object_type=e_obj))

    def delete_all_child(self, master_id):
        """
        Delete all the children's of the master key

        :param str master_id: master id
        """
        for i in range(1, self._max_level + 1):
            key_to_delete = CacheKey(master_id=master_id,
                                     master_type='*',
                                     entity_type='*',
                                     entity_id='*',
                                     object_type='*').get()
            to_delete_list = self.cache_levels[i].get(key_to_delete)
            for to_delete in to_delete_list:
                # check if the value is entity or binary
                if isinstance(to_delete, dict):
                    self.cache_levels[i].delete(CacheKey(entity_id=to_delete['id'], object_type='*').get())
                    self.delete_all_child(master_id=to_delete['id'])
                else:
                    if os.path.isfile(os.path.normpath(to_delete)):
                        try:
                            os.remove(os.path.normpath(to_delete))
                        except OSError as e:
    ...
    def write_stream(self,
        ...
        self.set(key=key, value=filepath)
    ...
    def write(self, list_entities_json):
        ...
        for entity_json in list_entities_json:
            ...
            if 'metadata' in entity_json:
                entity_json['metadata']['cache'] = 'hit'
