...
class ProductSupplier(metaclass=PoolMeta):
    ...
    def compute_supply_date(self, date=None):
        ...
        if product and product.type == 'kit':
            ...
            for component in product.components:
                lead_time = max(
                    lead_time, component.product.lead_time_used)
            if not date:
                date = Date.today()
            return date + lead_time
        return super().compute_supply_date(date=date)
    ...
    def compute_purchase_date(self, date):
        ...
        if product.type == 'kit':
            lead_time = timedelta(0)
            for component in product.components:
                lead_time = max(lead_time, component.product.lead_time_used)
            if not date:
                date = Date.today()
            return date - lead_time
        return super().compute_purchase_date(date=date)
