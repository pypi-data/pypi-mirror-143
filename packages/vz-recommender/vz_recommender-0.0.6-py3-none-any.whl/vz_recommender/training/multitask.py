...
from ..models.multitask import MultiTaskTPlustT, MultiTaskTxT
from .callbacks_base import (AttnVisCallbackBase, MLflowCallbackBase,
                             TensorboardCallbackBase)


class MTWandbLogger(WandbLogger):
    def __init__(self, main_logger, task1_logger, loss_logger):
        super().__init__()
...
class OrderDataset(Dataset):
    ...
    def __init__(self, ctx_cols, seq_col, vl_col, candidate_col, history_col, label_cols,
                 from_s3=True, local_data_dir=None, local_lookup_dirs=None, parts_num=9999, data_file_format="json", lookup_file_format="json"):
    ...
    def get_feature_num(self, col):
        ...
        if col == self.ctx_cols[0]:
            return self.ctx_np[:, 0].astype(int).max() + 1
        elif col == self.ctx_cols[1]:
            return self.ctx_np[:, 1].astype(int).max() + 1
        elif col == self.ctx_cols[2]:
            return self.ctx_np[:, 2].astype(int).max() + 1
        elif col == self.ctx_cols[3]:
            return self.ctx_np[:, 3].astype(int).max() + 1
        elif col == self.ctx_cols[4]:
            return self.ctx_np[:, 4].astype(int).max() + 1
        elif col == self.seq_col:
            return max(int(self.seq_np.reshape(-1).max()), int(self.candidate_np.reshape(-1).max())) + 1
        else:
...
class PLMultiTaskRecommender(LightningModule):
    def __init__(self, task1_criterion, optimizer, learning_rate,
        ...
        if multi_task_txt_model == "MultiTaskTPlusT":
            self.multi_task = MultiTaskTPlustT(**multi_task_txt_kwargs)
        elif multi_task_txt_model == "MultiTaskTxT":
    ...
    def forward_step(self, batch):
        (ctx, seq, vl, candidate), label_pos = batch
        out = self(
            ctx_in=ctx,
            seq_in=seq,
            vl_in=vl,
        ...
        return {"loss1": loss1.detach(), "loss": loss,
    ...
    def write_logs(self, phase):
        log_str = "{} metrics Epoch {} Time elapsed {:.2f} - "\
            ...
            self.logger.loss_logger.get("loss"), self.logger.loss_logger.get("loss1"),
