...
class OrderDataset(Dataset):
    ...
    def get_feature_num(self, col):
        ...
        elif col == self.ctx_cols[5]:
            return self.ctx_np[:, 5].astype(int).max() + 1
        elif col == self.seq_col:
            return max(int(self.seq_np.reshape(-1).max()), int(self.candidate_np.reshape(-1).max())) + 1
        elif col == self.label_cols[0]:
            return self.label_np[:, 0].astype(int).max()
        else:
...
class PLMultiTaskRecommender(LightningModule):
    def __init__(self, task1_criterion, task2_criterion, optimizer, learning_rate,
        ...
        elif multi_task_txt_model == "MultiTaskTPlustT":
