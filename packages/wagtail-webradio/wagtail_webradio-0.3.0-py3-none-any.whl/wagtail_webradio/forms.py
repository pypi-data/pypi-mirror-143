...
class PodcastAdminForm(WagtailAdminModelForm):
    ...
    The `sound_url` value will be validated on client side by loading it as an
    Audio object. This will trigger the `is_sound_url_valid` field and set the
    `duration` field if the audio could be loaded.
    """

    is_sound_url_valid = forms.BooleanField(
    ...
    def clean(self):
        ...
        if (
            not cleaned_data.get('is_sound_url_valid')
            ...
            self.add_error(
                ...
                ValidationError(
                    _(
                        "Unable to retrieve an audio file at this URL. Check "
