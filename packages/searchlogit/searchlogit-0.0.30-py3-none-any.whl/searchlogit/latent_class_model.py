...
class LatentClassModel(MultinomialLogit):
    ...
    def optimal_class_fit(self, X, y, varnames=None, alts=None, isvars=None,
        ...
        while curr_bic < prev_bic and num_classes > 1:  # lowest BIC
            ...
            model.fit(X, y, varnames, alts, isvars, num_classes,
                                            ...
                                            ids,
                                            weights, avail,
                                            transvars, transformation,
                                            base_alt,
                                            fit_intercept, init_coeff,
                                            maxiter, random_state, ftol,
                                            gtol, grad, hess, panels, verbose,
    ...
    def _get_class_X_idx(self, class_num, **kwargs):
            ...
            X_class_idx = [np.where(np.char.find(self.varnames, param) != -1)[0]
    ...
    def _expectation_maximisation_algorithm(self, X, y, avail=None, weights=None,
        ...
        if class_betas is None:
            ...
            # class_betas = [np.random.rand(self._get_betas_length(i))/10 for i in range(self.num_classes)] # beta vectors for each class
            class_betas = [np.random.normal(0, .1, self._get_betas_length(i)) for i in range(self.num_classes)]
        if class_thetas is None:
            class_thetas = np.stack([np.random.normal(0, .1, len(self._get_member_X_idx(i))) for i in range(1, self.num_classes)], axis=0)  # class member ship probability
        ...
        while not converged and iter_num < max_iter:
            ...
            for class_i in range(1, self.num_classes):
                ...
                # self.transvars = np.array(self.class_params_spec[class_i])[class_fxtransidxs[class_i]] # TODO!
            ...
            for s in range(0, self.num_classes):
                ...
                # self.transvars = np.array(self.class_params_spec[s])[class_fxtransidxs[s]] # TODO!
        ...
        optimisation_result = {'x': x, 
                               'success': opt_res['success'],  # TODO? valid?
