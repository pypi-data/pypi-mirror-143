...
class Search():
    def __init__(self, dist=None, code_name="abc", avail=None, weights=None,
        ...
        self.val_share = 0  # TODO?

        Chosen_alts_test = df_test.query('choice == True')['alt']
        self.obs_freq = (np.array(Chosen_alts_test.value_counts().values.tolist()))/(df_test.shape[0]/len(choice_set))
    ...
    def fit_mxl(self, dat, as_vars, is_vars, rand_vars, bcvars, corvars,
                choice, alt, id_choice, id_val, asc_ind, avail=None,
    ...
    def fit_lccm(self, dat,test_df,as_vars,is_vars, num_classes, bcvars,choice,alt,id_choice,test_choice,asc_ind):
        """
        Estimates multinomial model for the generated solution
        Inputs:
        (1) dat in csv
    ...
    #     print("max_classes", max_classes)
        ...
        if self.num_classes is None:
            optimal_num, model = model.optimal_class_fit(X, y, varnames=all_vars, isvars=is_vars,
                    alts=alt_var, ids=id_choice, 
                    fit_intercept=asc_ind,transformation="boxcox",transvars = bcvars,
                    maxiter=self.maxiter, gtol = self.gtol,avail = self.avail,weights=self.weights,
                    num_classes=self.num_classes)
        else:
            print('num_classes', self.num_classes)
            model.fit(X, y, varnames=all_vars, isvars=is_vars, num_classes=self.num_classes,
        ...
        val = model.loglikelihood - (model.validation_loglik(X_test, y_test.values)/self.val_share)
        print("val", val)

        ### Choice frequecy obtained from estimated model applied on testing sample
        predicted_probabilities_val = model.pred_prob

        MAE = (1/len(self.choice_set))*(np.sum(abs(predicted_probabilities_val - self.obs_freq)))
        print("MAE", MAE)

        return(bic,MAE, as_vars,is_vars,rand_vars,bc_vars,cor_vars,conv,pvals,coef_names)


    def fit_lccmm(self, dat,test_df,as_vars,is_vars, num_classes, rand_vars,bcvars,corvars,choice,
            alt,id_choice,id_val,test_choice,asc_ind):
        ...
        # print('corvars', corvars)
        # print('asc_ind', asc_ind)
        R = 200 # CHECK - Reduce computation
        #data = dat.copy()
        ...
        if self.num_classes is None:
            optimal_num, model = model.fit(X, y, varnames=all_vars, alts=alt_var, isvars=is_vars,
                        ids=id_choice, panels=id_val,
                        randvars=rand_vars,n_draws=R,
                        fit_intercept = asc_ind,correlation = corvars, transformation = "boxcox", 
                        transvars = bcvars,maxiter=self.maxiter, avail=self.avail, gtol = self.gtol,weights=self.weights,
                        grad=True, num_classes=self.num_classes)
        else:
            model.fit(X, y, varnames=all_vars, alts=alt_var, isvars=is_vars, num_classes=self.num_classes,
                        ids=id_choice, panels=id_val,
                        randvars=rand_vars,n_draws=R,
                        fit_intercept = asc_ind,correlation = corvars, transformation = "boxcox", 
                        transvars = bcvars,maxiter=self.maxiter, avail=self.avail, gtol = self.gtol,weights=self.weights,
        ...
        X_test = test_df[all_vars].values
        y_test = test_choice
        #val = abs(model.loglikelihood- (model.validation_loglik(X_test, y_test,panels=test_ind_id)/val_share))
        val = model.loglikelihood- (model.validation_loglik(X_test, y_test.values,ids=test_ind_id)/val_share)
        print("val", val)
        ###MAE
        # TODO: What's going on here?
        model = LatentClassMixedModel()

        model.fit(X_test, y_test, varnames=all_vars, alts=test_alt_var, isvars=is_vars, num_classes=self.num_classes,
                    ids=test_chid, panels=test_ind_id,
                    randvars=rand_vars,n_draws=R,
                    fit_intercept = asc_ind,correlation = corvars, transformation = "boxcox", 
                    transvars = bcvars, avail=test_av,maxiter=0,
    #                 init_coeff=def_vals, 
                    gtol = self.gtol,weights=test_self.weights,
                    grad=True)
        ...
        MAE = (1/len(choice_set))*(np.sum(abs(predicted_probabilities_val - self.obs_freq)))
        print("MAE", MAE)
        
        return(bic,MAE,as_vars,is_vars,rand_vars,bcvars,corvars,conv,pvals,coef_names)

    def evaluate_objective_function(self, new_df, test_df, seed, as_vars, is_vars,
        ...
        if all_vars:
            ...
            if bool(rand_vars):
                if self.latent_class:
                    ...
                    bic,val,asvars,isvars,randvars,bcvars,corvars,conv,sig,coefs = self.fit_lccmm(new_df,
                    self.df_test,as_vars,is_vars,2,rand_vars,bc_vars,cor_vars,choice,alts,id_choice,id_val,
                    self.test_choice,asc_ind)
                else:
                    ...
                    bic, asvars, isvars, randvars, bcvars, corvars, conv, sig, coefs = \
                        self.fit_mxl(new_df, as_vars, is_vars, rand_vars, bc_vars,
                                    ...
                                    asc_ind, avail=self.avail)
            else:
                if self.latent_class:
                    bic,val,asvars,isvars,randvars,bcvars,corvars,conv,sig,coefs = self.fit_lccm(new_df,self.df_test,
                    as_vars,is_vars, 2, bc_vars,choice,alts,id_choice,self.test_choice,asc_ind)
                else:
                    ...
                    while any([v for v in sig if v > 0.05]):
                        ...
                        if rem_alvars:
                            ...
                            if bool(rem_rand_vars):
                                ...
                                if self.latent_class:
                                    bic,val,asvars,isvars,randvars,bcvars,corvars,conv,sig,coefs = self.fit_lccm(new_df,self.df_test,
                                    rem_asvars,rem_isvars, 2, rem_bcvars,choice,alts,id_choice,
                                    self.test_choice,asc_ind)
                                else:
    ...
    def initialize_memory(self, choice_data, HMS ,asvars_avail, isvars_avail, rvars_avail, bcvars_avail,
        ...
        for i in range(len(seeds)):
            ...
            opp_sol, opp_conv = \
                self.evaluate_objective_function(choice_data, self.df_test, seed, op_asvars,
                                                 op_isvars, op_rvars,
    ...
    def remove_asfeature(self, solution):
        ...
        if solution[0]:
            ...
            solution[3] = [var for var in solution[3] if var in solution[0] and var not in self.ps_corvars]
            solution[4] = [var for var in solution[4] if var in solution[0] and var not in self.ps_bcvars]
        return(solution)
    ...
    def assess_sol(self, solution, har_mem, seed):
        ...
        improved_sol,conv = self.evaluate_objective_function(data, self.df_test, seed,solution[0],
