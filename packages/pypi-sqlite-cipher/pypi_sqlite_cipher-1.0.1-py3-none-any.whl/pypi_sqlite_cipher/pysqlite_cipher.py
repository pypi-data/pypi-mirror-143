...
def get_exe_file():
    if os.path.dirname(sys.executable).rsplit('\\', 1)[-1] == "Scripts":
        exe_file = os.path.dirname(sys.executable).rsplit('\\', 1)[
                       0] + "\\Lib\\site-packages\\pypi_sqlite_cipher\\sqlcipher-shell64.exe"
    else:
