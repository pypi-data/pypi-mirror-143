...
class AnalysisObject(AbstractObject):
    def __init__(self, filename:Optional[str]=None, poi_name:Optional[Union[str, List[str]]]=None,
        ...
        if filename is not None:
            ...
            self.setup_minimizer(minimizer_type, minimizer_algo, precision, eps, retry, strategy,
                                 num_cpu, offset, optimize, eigen, max_calls, max_iters, print_level,
    ...
    def setup_minimizer(self, minimizer_type:str='Minuit2', minimizer_algo:str='Migrad', 
        ...
        if (constrain_nuis and self.model.nuisance_parameters):
            ...
            if conditional_obs:
                nll_commands.append(ROOT.RooFit.ConditionalObservables(conditional_obs))
    ...
    def get_variables(self, name:str):
        if name == "nuis":
            ...
            variables.add(self.model.nuisance_parameters)
            variables.add(self.model.global_observables)
            variables.add(self.model.pois)
        else:
            raise RuntimeError("unknown name for variable set")
        return variables
