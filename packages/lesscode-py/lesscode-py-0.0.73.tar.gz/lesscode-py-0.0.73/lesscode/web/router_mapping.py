    ...
    def wrapper(cls):
        ...
        for item in res:
            ...
            if tornado.options.options.rms_register_enable:
                redis = RedisHelper("rms_redis").get_connection(sync=True)
                key = f"upms:url_info:{full_url}"
                ...
                if flag == 0:
                    ...
                    if not client_res.get("data"):
                        ...
                        client_res = get(url=f"{tornado.options.options.rms_register_server}/upms/client/fetchall",
                                         params={"client_name": client_name}).json()
                    client_id = client_res.get("data")[0].get("client_id")
                    resource_res = get(
...
def GetMapping(url: str, client_name: str = None, title: str = None, en_name: str = None, access: str = 0):
    """
    ...
    def wrapper(func):
        ...
        RouterMapping.instance().dynamicMethods.append(
            (func.__qualname__.replace('.' + func.__name__, ''), func, url, client_name, title, en_name, access))

    return wrapper


def PostMapping(url: str, client_name: str = None, title: str = None, en_name: str = None, access: str = 0):
    """
    用于类名、处理方法、url 的元祖集合注册处理，暂时GetMapping与PostMapping实现保持一致，预留入口为后期扩展提供支持。
    :param client_name:
    :param access:
