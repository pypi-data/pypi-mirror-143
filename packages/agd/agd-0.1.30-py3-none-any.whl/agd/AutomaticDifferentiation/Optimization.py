...
def newton_minimize(f,x0,fargs=tuple(),
	...
	- δ_atol,δ_rtol,δ_ntol,
	"""
	if verbosity==0: kiter_print=[-1]
	...
	print(f"Initialization, objective {y[0]}")
...
#		print(x,d,"x,d,iter=",i)
		...
		if not δ > -δtol and s>step_min: 
			while s>step_min:
				...
				print(f"Warning minimal step size {s} deteriorates objective")
			y = vgd(x)
		...
		if not np.isfinite(v): 
			print("Infinite objective value, aborting.")
			exit=True
		...
		if i==kiter_print_next or exit:
			kiter_print_next = next(kiter_print)
			print(f"Iteration {i}, Newton step {s}, objective {v}.")
