...
class Node:
    def __init__(self, data, is_ts=False):
        ...
        self.children = []  # list; nodes in order of function arguments
        self.is_ts = is_ts  # bool; if data of this node is time-series const
    ...
    def chg_child(self, old_child, new_child):
...
class SyntaxTree:
    ...
    def __build(self):
        ...
        while children_stack:
            ...
            if depth < max_depth and (method == 'full' or random.random() < self.build_preference[0]):
                # add function
                data = random.choice(self.function_set)
                node = Node(data)
                parent_stack[-1].add_child(node)
    ...
    def __get_subnode(self, ex_terminal=False, ex_ts_const=False, for_cross=False):
        if ex_terminal:
            # exclude terminal
            candidates = [node for node in self.nodes if isinstance(node.data, Function)]
        elif ex_ts_const:
        ...
        if for_cross:
            ...
            probs = np.array(([9 if isinstance(node.data, Function) else 1 for node in candidates]))
            probs = np.cumsum(probs / probs.sum())
            return candidates[np.searchsorted(probs, random.random())]
        else:
            return random.choice(candidates)

    def execute(self, X:pd.DataFrame):
        """
        Execute the program according to X
        @param X: training data
        """
        outcome = np.hstack([np.full(self.ttl_shift, np.nan), self.nodes[0](X)[self.ttl_shift:]])
        if self.transformer is not None:
            outcome = self.transformer(X, outcome, **self.transformer_kwargs)
        return outcome

    def fitness(self, X:pd.DataFrame, benchmark:pd.Series):
        """
    ...
    def point_mutate(self):
        ...
        if isinstance(mutation_node.data, Function):
            ...
            function_candidates = [function for function in self.function_set
                                   if function.arity == mutation_node.data.arity
                                   and function.is_ts == mutation_node.data.is_ts
                                   and function.fixed_params == mutation_node.data.fixed_params]
