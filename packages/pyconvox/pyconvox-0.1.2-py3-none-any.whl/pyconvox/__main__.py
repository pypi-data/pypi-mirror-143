        ...
        if args.app in apps:
            ...
            while option not in 'Y,y,N,n'.split(','):
                ...
                if option == 'Y' or option == 'y':
                    stream = os.system(f"convox env set {' '.join([ vars for vars in args.envvars])} -a {args.app} -p -w")
                elif option == 'N' or option == 'n':
                    stream = os.system(f"convox env set {' '.join([ vars for vars in args.envvars])} -a {args.app}")
                else:
...
def Scale(args,apps,parser):
    if args.app:
        if args.app in apps:
            stream = os.system(f"convox scale -a {args.app}")
        else:
            print("Wrong Application Name!!..\nPlease provide application name from the below")
            for index,appname in enumerate(apps):
                print(f"{index+1}. {appname}")
...
def PS(args,apps,parser):
    if args.app:
        if args.app in apps:
            stream = os.system(f"convox ps -a {args.app}")
        else:
            print("Wrong Application Name!!..\nPlease provide application name from the below")
            for index,appname in enumerate(apps):
                print(f"{index+1}. {appname}")
...
def Releases(args,apps,parser):
    if args.app:
        if args.app in apps:
            stream = os.system(f"convox releases -a {args.app}")
        else:
            print("Wrong Application Name!!..\nPlease provide application name from the below")
            for index,appname in enumerate(apps):
                print(f"{index+1}. {appname}")
...
def Logs(args,apps,parser):
    if args.app:
        if args.app in apps:
            stream = os.system(f"convox logs -a {args.app}")
        else:
            print("Wrong Application Name!!..\nPlease provide application name from the below")
            for index,appname in enumerate(apps):
                print(f"{index+1}. {appname}")
...
def Instances(args,apps,parser):
        stream = os.system(f"convox instances")
