...
def export_dataset(objects_f: list, type_f: str, path_f: str):
    ...
    elif type_f == "tensorflowtfrecord":
        try:
            from all_label_converter.cores.tensorflowtfrecord import TFRecordExport
            TFRecordExport(objs=objects_f, folder_path=path_f)
            logging.info("Success! Exporting Tensorflow TFRecord!")
        except Exception as error:
            logging.error("There is problem with export type Tensorflow TFRecord!")
            logging.error(str(error))
            logging.error("The process has been terminated. Unsuccessful!")
