...
import haohan_unittest.test


class Test_TestProgram(haohan_unittest.TestCase):

    def test_discovery_from_dotted_path(self):
        loader = haohan_unittest.TestLoader()

        tests = [self]
        expectedPath = os.path.abspath(os.path.dirname(haohan_unittest.test.__file__))
        ...
        suite = loader.discover('haohan_unittest.test')
    ...
    def testNoExit(self):
        ...
        oldParseArgs = haohan_unittest.TestProgram.parseArgs
        def restoreParseArgs():
            haohan_unittest.TestProgram.parseArgs = oldParseArgs
        haohan_unittest.TestProgram.parseArgs = lambda *args: None
        self.addCleanup(restoreParseArgs)

        def removeTest():
            del haohan_unittest.TestProgram.test
        haohan_unittest.TestProgram.test = test
        self.addCleanup(removeTest)

        program = haohan_unittest.TestProgram(testRunner=runner, exit=False, verbosity=2)

        self.assertEqual(program.result, result)
        self.assertEqual(runner.test, test)
        self.assertEqual(program.verbosity, 2)

    class FooBar(haohan_unittest.TestCase):
        def testPass(self):
            assert True
        def testFail(self):
            assert False

    class FooBarLoader(haohan_unittest.TestLoader):
    ...
    def test_defaultTest_with_string(self):
        ...
        sys.argv = ['faketest']
        runner = FakeRunner()
        program = haohan_unittest.TestProgram(testRunner=runner, exit=False,
                                              defaultTest='haohan_unittest.test',
                                              testLoader=self.FooBarLoader())
        sys.argv = old_argv
        self.assertEqual(('haohan_unittest.test',), program.testNames)
    ...
    def test_defaultTest_with_iterable(self):
        ...
        sys.argv = ['faketest']
        runner = FakeRunner()
        program = haohan_unittest.TestProgram(
            testRunner=runner, exit=False,
            defaultTest=['haohan_unittest.test', 'haohan_unittest.test2'],
            testLoader=self.FooBarLoader())
        sys.argv = old_argv
        self.assertEqual(['haohan_unittest.test', 'haohan_unittest.test2'],
                          program.testNames)

    def test_NonExit(self):
        program = haohan_unittest.main(exit=False,
                                       argv=["foobar"],
                                       testRunner=haohan_unittest.TextTestRunner(stream=io.StringIO()),
                                       testLoader=self.FooBarLoader())
...
class InitialisableProgram(haohan_unittest.TestProgram):
    exit = False
    result = None
    verbosity = 1
    defaultTest = None
    tb_locals = False
    testRunner = None
    testLoader = haohan_unittest.defaultTestLoader
    module = '__main__'
    progName = 'test'
    test = 'test'
...
class TestCommandLineArgs(haohan_unittest.TestCase):
    ...
    def testWarning(self):
        ...
        # see #10535
        class FakeTP(haohan_unittest.TestProgram):
    ...
    def testCatchBreakInstallsHandler(self):
        module = sys.modules['haohan_unittest.main']
    ...
    def testSelectedTestNamesFunctionalTest(self):
        def run_unittest(args):
            p = subprocess.Popen([sys.executable, '-m', 'haohan_unittest'] + args,
