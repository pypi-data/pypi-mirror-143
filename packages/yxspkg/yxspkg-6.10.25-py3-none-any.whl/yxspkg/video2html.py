    ...
    for i in p.glob('*'):
        ...
        if not encrypt:
            ...
            if di.suffix in suffixs2:
                ...
                os.remove(i)
def generate_poster_all(dirname,time_interval,shuffle,delete_all_links,encrypt):
    global root_path 
...
def main(dirname,time_interval,shuffle=False,delete_all_links=False,no_encrypt=False,endir=False,dedir=False,fix_ffmpeg=True):
    ...
    if dirname:
        if endir or dedir:
            ...
            add_new_dirs = generate_poster_all(dirname,time_interval,shuffle,delete_all_links,encrypt)
            if add_new_dirs != 0 and encrypt:
                generate_poster_all(dirname,time_interval,shuffle,delete_all_links,encrypt)
