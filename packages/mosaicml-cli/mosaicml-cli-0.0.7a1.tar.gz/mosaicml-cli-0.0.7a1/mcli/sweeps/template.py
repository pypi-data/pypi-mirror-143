...
from mcli.utils.utils_types import delete_nested_key
...
class TemplateConfigFactory():
    ...
    def build_model_config(self) -> LocalRunConfig:
        ...
        if self.model is not None:
            ...
            # TODO (TL): Remove this when json merge overwrites are available (HEK-83)
            # Composer YAMLs have TQDM logger configured, which is unsuitable for cluster workloads but makes sense
            # from the library's perspective.
            assert isinstance(parameters, dict)
            delete_nested_key(parameters, ['loggers', 'tqdm'])
