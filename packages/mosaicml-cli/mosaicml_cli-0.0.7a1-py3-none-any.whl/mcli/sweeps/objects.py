...
from mcli.submit.kubernetes.mcli_job_future import MCLIJob as MCLIJobFuture
...
def submit_run(
    ...
    if config.feature_enabled(config.FeatureFlag.USE_PLATFORMS_FUTURE):
        ...
        platform, instance_type = registry.get_mcli_platform_and_instance_type(instance_str=local_run_config.instance,)
        # END TODO(HEK-323):
        runner.submit(
            job=mcli_job,
            platform=platform,
        ...
        mcli_job = MCLIJob(
            ...
            container_image=local_run_config.image,
            parameters=parameters,
            sweep=local_run_config.sweep,
        )
        runner = Runner()
        assert local_run_config.instance is not None
        runner.submit(
            mcli_job,
            local_run_config.instance,
            priority_class=priority_class,
        )
...
def resolve_full_run_config(
    ...
    # Add WandB data - prioritize values here over incoming wandb configs
    # TODO (TL): Refactor so that additional logging data is only added if WandB is used (HEK-106)
    wandb_config_factory = WandBStorage(local_config)
    partial_config = wandb_config_factory.build_partial_config()
    local_config = LocalRunConfig.merge(partial_config, local_config, 'overlay')
    assert isinstance(local_config, LocalRunConfig)
...
class WandBStorage(object):
    """ A Collection to help prepopulate yamls with WandB """

    def __init__(self, local_config: LocalRunConfig):
        self.config = local_config

    def build_partial_config(self) -> LocalRunConfig:
        parameters = {'loggers': {'wandb': {}}}
        root = parameters['loggers']['wandb']
        project = self.config.project
        if not project:
            algo_str = '-'.join(list(self.config.algorithm)) if self.config.algorithm else 'baseline'
            project = f'sweep-{self.config.model}-{algo_str}'
        root['project'] = project
        if self.config.name:
            root['name'] = self.config.name
        if self.config.algorithm:
            algo = self.config.algorithm
            root['tags'] = ','.join(algo if isinstance(algo, list) else [algo])
        ...
        metadata = {
            ...
            'commit': self.config.git_branch,
            'version': self.config.version,
            'run_type': str(self.config.run_type),
        }

        if self.config.sweep:
            metadata['sweep'] = self.config.sweep

        root['extra_init_params'] = {'config': {'metadata': metadata}}
        return LocalRunConfig(name=None, parameters=parameters)
