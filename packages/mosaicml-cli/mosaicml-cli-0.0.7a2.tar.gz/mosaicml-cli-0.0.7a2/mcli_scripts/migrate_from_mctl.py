...
def main() -> int:
    ...
    with Yaspin() as sp:
        ...
        source_platforms = []
        for name in ('colo-research-01', 'r1z1'):
            ...
            with MCLIPlatform.use(platform):
                ...
                if not migrated['docker']:
                    ...
                    if docker_secret_name in known_mcli_secrets:
                        ...
                        mcli_config.secrets.append(mcli_docker_secret)
                ...
                if not migrated['ssh']:
                    ...
                    if new_name in known_mcli_secrets:
                        ...
                        mcli_ssh_secret.mount_path = cluster_info.get('ssh_secrets_mount_path',
                                                                      '/etc/secret/ssh-privatekey')
                        mcli_config.secrets.append(mcli_ssh_secret)
                ...
                if not migrated['wandb']:
                    ...
                    if new_name in known_mcli_secrets:
                        ...
                        mcli_wandb_secret.value = wandb_secret['data']['wandb_api_key']
                        mcli_wandb_secret.env_key = 'WANDB_API_KEY'
                        mcli_config.secrets.append(mcli_wandb_secret)
        ...
        with sp.hidden():
            for platform in track(mcli_config.platforms):
                for secret in mcli_config.secrets:
