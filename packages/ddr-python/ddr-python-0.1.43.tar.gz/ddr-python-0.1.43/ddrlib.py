...
def ddr_show_parameter_all(device, access_type, genie_parser_name, show_template, pcount, par1, par2, par3, debug_flag=False):
        ...
        Usage::
              ...
              ddr_show_parameter_all(device, 'cli', 'ShowInterface', 'show interface {0}', 1, 'GigabitEthernet0/0', 'none', 'none', 'GigabitEthernet0/0', debug_flag=True)
        ...
        #
          if str(access_type) == 'cli':
            debug_msg(debug_flag, f" ddr_show_parameter_all: cli python command: {str(command)}")
            response = cli.cli(command)
