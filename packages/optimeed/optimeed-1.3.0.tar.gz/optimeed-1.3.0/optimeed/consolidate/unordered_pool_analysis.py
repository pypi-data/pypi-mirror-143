...
class Unordered_pool_analysis(Option_class):
    ...
    def __init__(self, theReferenceDevice, theAttributes, theCharacterization, filename_collection, autosave=False, number_of_cores=1):
        super().__init__()
    ...
    def run(self):
        ...
        # Simulates
        nb_processes = min(cpu_count(), min(self.get_optionValue(self.NUMBER_OF_CORES),len(listOfDevices)))
        if nb_processes <= 1:
            for device in listOfDevices:
                theCollection_out.add_data(self.evaluate(device))
        else:
