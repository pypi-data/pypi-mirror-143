...
def logformat(argv=None) -> None:
    ...
    try:
        for line in fileinput.input(files=(args.file,)):
            ...
            try:
                line = format_json(line, always=args.color)
            except (AssertionError, KeyError, ValueError):
                pass
            parts = f"{prefix}{line}".split(" ", maxsplit=2)
            if len(parts) > 2 and ansi_re.sub("", parts[1]) in parts[0]:
                parts.pop(1)
...
def format_json(line: str, always: bool = False) -> str:
