...
def create_random_string(count=28):
    """
    功能描述: 生成指定长度的随机字符串
    :param count: int  字符串的长度
    :return:
    """
    random_string = "".join(random.sample("0123456789abcdefghijklmnpqrstuvwxyzABCDEFGHIJKLMNPQRSTUVWXYZ", k=count))
...
def get_all_list(func):
    ...
    def wrapper(self, *args, **kwargs):
        ...
        if "totalRows" in response["data"]:
            pages = math.ceil(response["data"]["totalRows"] / 10)
        else:
            pages = math.ceil(response["data"]["total"] / 10)
        for i in range(1, pages + 1):
            if i > 1:
                response = func(self, page=i, **kwargs)
            result.extend(response["data"]["list"])
        # debug(f"{func.__name__}:{result}")
        return result
...
if __name__ == "__main__":
    print(get_date_time_relative(years=0, months=0, days=-9, hours=1, format="%y-%m-%d %H:%M:%S"))
