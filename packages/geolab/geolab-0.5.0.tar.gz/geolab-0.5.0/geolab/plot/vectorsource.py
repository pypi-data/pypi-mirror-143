...
class Vectors:
    ...
    def _make_data(self):
        if type(self._vectors.shape) is list:
    ...
    def _make_glyph(self):
        ...
        if self._glyph_type == '3D-arrow':
            ...
            if self._shaft_radius is not None:
                g_src.shaft_radius = self._shaft_radius
            if self._tip_length is not None:
                g_src.tip_length = self._tip_length
            if self._tip_radius is not None:
