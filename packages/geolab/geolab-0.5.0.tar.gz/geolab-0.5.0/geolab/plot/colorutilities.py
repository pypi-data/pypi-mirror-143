...
def color_library():
    library = {
        ...
        'pastel_blue': (106, 149, 228),

        'pastel_yellow': (255, 238, 166),

        'pastel_green': (124, 207, 169),

        'pastel_pink': (255, 109, 180),
...
def palette_library():
    library = {
        ...
        'w_rainbow': [(255, 84, 82), (255, 157, 82), (255, 217, 129),
                      (72, 207, 202), (0, 45, 78), (72, 64, 130), (182, 63, 134)],

        '70s': [(0, 45, 78), (72, 64, 130), (182, 63, 134),
                (255, 84, 82), (255, 157, 82), (255, 217, 129)],
...
def turbo_lut_table():
    ...
    lut = np.round(255 * np.array(turbo_colormap_data))
    lut = np.column_stack((lut, 255 * np.ones(256))).astype('i')
    return lut


def wsurf_lut_table():
    scale = palette_library()['70s']
    cmap = interpolate_colors(scale)
    cmap = np.column_stack((cmap, 255 * np.ones(len(cmap)))).astype('i')
    return cmap
...
def rgbm_lut_table(expansion=2):
    X = np.linspace(0, 1, 64)
    increase = 255 * (X ** (expansion ** (-1)))
    decrease = increase[::-1]
    one = 255 * np.ones(64)
...
def interpolate_colors(colors, number_of_colors=256):
    colors = np.array(colors)
    N = number_of_colors
    C = N // (len(colors)-1)
    n = np.repeat(C, len(colors)-1) + 1
    R = N % (len(colors)-1)
    n[:R-1] += 1
    R = []
    G = []
    ...
    for i in range(len(colors)-1):
        R.append(np.linspace(colors[i, 0], colors[i+1, 0], n[i])[:-1])
        G.append(np.linspace(colors[i, 1], colors[i+1, 1], n[i])[:-1])
        B.append(np.linspace(colors[i, 2], colors[i+1, 2], n[i])[:-1])
    R.append(colors[-1, 0])
    G.append(colors[-1, 1])
    B.append(colors[-1, 2])
    RGB = np.column_stack((np.hstack(R), np.hstack(G), np.hstack(B)))
    if np.max(RGB) > 1:
        RGB = np.round(RGB).astype('i')
    return RGB


def wheel_map(colors, theta, r):
    colors = np.array(colors)
    #colors = np.vstack((colors, colors[0]))
    cmap = interpolate_colors(colors, len(theta))
    theta -= np.min(theta)
    theta = np.round(theta / 360 * (len(theta) - 1)).astype('i')
    r = np.abs(r / np.max(np.abs(r)))
    white = 1
    if np.max(colors) > 1:
        white = 255
    r = np.column_stack((r, r, r))
    lut = r * cmap[theta] + (1-r) * white
    lut = np.column_stack((lut, white * np.ones(len(lut))))
    if np.max(colors) > 1:
        lut = np.round(lut).astype('i')
    return lut
...
def rgba_lut(color, opacity=1, expansion=2):
    if isinstance(color, np.ndarray):
        ...
        lut = bwr_lut_table(expansion)
    elif color == 'rgbm':
        lut = rgbm_lut_table(expansion)
    elif color == 'rgb_e':
        lut = rgb_lut_table(expansion)
    else:
        try:
        ...
        if isinstance(opacity, int):
            opacity = [opacity]
        if len(color) != len(opacity):
            opacity = np.repeat(opacity[0], len(color))
        rgb = []
        for key in color:
        ...
        col = rgb[-1]
        alpha = opacity[-1]
        lut[:, 0] = col[0] * np.ones([256])
        lut[:, 1] = col[1] * np.ones([256])
        lut[:, 2] = col[2] * np.ones([256])
        ...
        for i in range(L - 1):
            col = rgb[i]
            alpha = opacity[i]
            lut[n:n + N, 0] = col[0] * np.ones([N])
            lut[n:n + N, 1] = col[1] * np.ones([N])
            lut[n:n + N, 2] = col[2] * np.ones([N])
...
if __name__ == '__main__':
    ...
    plot.plot_points([[0, 0, 0]], color='pastel_blue', glossy=0.8)

    plot.plot_points([[0, 0, .2]], color='pastel_pink', glossy=0.8)

    plot.plot_points([[0, 0, .4]], color='pastel_yellow', glossy=0.8)

    plot.plot_points([[0, 0, .6]], color='pastel_green', glossy=0.8)
