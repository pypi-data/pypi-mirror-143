...
class Certificate:
    ...
    def parse(msg):
        ...
        for lg_node in msg.findall('LicenseGroups'):

            row_id = _int_content(lg_node, 'RowID')

            features = []
            xpath_fmt = "Features[RowID='{}']"
    ...
    @staticmethod
    def find_cert(certs, xml):
        """
        Locate a certificate from an XML description
        """
        lic = ET.fromstring(xml)
        gid = lic.find('param[@name="groupID"]')
        if gid is not None:
            gid = int(gid.get('value'))
        if gid in certs:
            return certs[gid]

        requirements = []
        for feature in lic.find('featureset'):
            if feature.get('kind') == 'Int':
                name = feature.get('name')
                value = int(feature.get('value'))
                if value > 1:
        ...
        valid = [cert for cert in certs.values() if cert.meets(requirements)]
        ...
        raise RuntimeError("Unable to determine license certificate")
    ...
    def meets(self, requirements):
        ...
        if isinstance(requirements, dict):      # pylint: disable=no-else-return
    ...
    def meet(self, key, req):
        ...
        if value is None:
            ...
            if len(req) > 2 and req.startswith("/") and req.endswith("/"):
                meets = re.search(req[1:-1], value) is not None
            elif len(req) > 3 and  req.startswith("/") and req.endswith("/i"):
                meets = re.search(req[1:-2], value, re.IGNORECASE) is not None
            else:
                meets = value == req
