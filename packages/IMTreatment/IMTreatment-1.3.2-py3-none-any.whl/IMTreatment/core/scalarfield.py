...
class ScalarField(fld.Field):
    ...
    def import_from_arrays(self, axe_x, axe_y, values, mask=None,
                           ...
                           dtype=float, dontchecknans=False,
    ...
    def get_profile_between(self, pt1, pt2, interp='linear', resolution=1):
        ...
        pt1, pt2 : 2x1 array of floats
            Position of the points between which we want a profile.
        interp : string in [‘linear’, ‘cubic’, ‘quintic’]
            The kind of spline interpolation to use. Default is ‘linear’.
        resolution: float
            Resolution of the resulting profile.
            1 (default) leads to a step size similar to the original
            scalar field.
            A value of X will increase the resolution by a factor X.
        ...
        return prof.Profile(t, vals, unit_x=self.unit_x,
                            unit_y=self.unit_values)
    ...
    def get_interpolator(self, interp="linear"):
        ...
        if np.any(np.isnan(self.values)):
            warnings.warn("Interpolating with NaN values present."
                          " Interpolation can give wrong/strange results")
