...
def _download_datacenter_args(params, indir, session):
    ...
    for param in params:
        if isinstance(param["value"], str) and param["value"].startswith("dc://"):
            ...
            _ = ds.download(dl_dir)
            # If the dataset only contains a single file, pass that. Otherwise pass the containing dir
            files = glob(os.path.join(dl_dir, "*"))
...
def pmws_wrapper(token, execid, pythonexe):
    ...
    try:
        ...
        job._upload("output", outdir, appname=appname)
