...
class ProjectFlow(object):
    def __init__(self, project_dir=None):
        ...
        if hb.path_exists(hb.BASE_DATA_DIR):
            self.base_data_dir = hb.BASE_DATA_DIR
        else:
            self.base_data_dir = None

        if hb.path_exists(hb.EXTERNAL_BULK_DATA_DIR):
            self.external_bulk_data_dir = hb.EXTERNAL_BULK_DATA_DIR
        else:
    ...
    def DataRef(self, input_path, graft_dir=None, verbose=False):
        ...
        return hb.DataRef(input_path, p=self, graft_dir=graft_dir, verbose=verbose)
...
class DataRef(str):
    """History: the used to be the p.Path(input_path_string) class but it failed. I tried hard to subclass str so that I could override __str__() etc and give the correct, existant path to other libraries.
