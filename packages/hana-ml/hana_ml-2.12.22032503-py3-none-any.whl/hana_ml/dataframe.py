...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def to_datetime(self, cols):
        """
        Convert target columns to the specified date format. Default format is "YYYY-MM-DD HH24:MI:SS".
                        ...
                        for t_param in param:
                            ...
                            if trans_func.upper() == 'GEOHASH_HIERARCHY':  # Needs special handling
                                ...
                                if order_by is not None:
                                    temp_trans = temp_trans + ' OVER({}ORDER BY {}) AS "{}" '.format(partition_by, quotename(order_by), temp_trans.replace('"', ''))
                                else:
...
def create_dataframe_from_pandas(
    ...
    force : bool, optional
        ...
        Defaults to False.
    replace : bool, optional
        If replace is True, NULL will replaced by 0.
