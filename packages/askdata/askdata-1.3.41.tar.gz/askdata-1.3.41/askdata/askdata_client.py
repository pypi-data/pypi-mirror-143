...
class Agent(Insight, Channel, Catalog, Dataset):
    ...
    def get_card_slug_from_feed(self, feed_slug):
        ...
        channel_id = self.get_channel(feed_slug)

        url = self._base_url+"/smartfeed/"+channel_id+"/feed?page=0&limit=100&agentId="+self._agentId
        ...
        for card in cards:
            if card["id"] is not "_system-highlight":
