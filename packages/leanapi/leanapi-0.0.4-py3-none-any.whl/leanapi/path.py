...
T = TypeVar("T")
LEAN_CLASS_KEY = "__leanapi_class__"


def controller(path: str, router: APIRouter) -> Callable[[Type[T]], Type[T]]:
    def decorator(cls: Type[T]) -> Type[T]:
        return _controller(router, path, cls)

    return decorator


def _controller(router: APIRouter, path: str, cls: Type[T]) -> Type[T]:
    """
    ...
    router.include_router(cbv_router, prefix=f"/{path}")
    return cls


def _init_controller(cls: Type[Any]) -> None:
    """
    ...
    * The `__signature__` attribute is updated to indicate to FastAPI what arguments should be passed to the initializer
    """
    if getattr(cls, LEAN_CLASS_KEY, False):
        return
    ...
    setattr(cls, LEAN_CLASS_KEY, True)


def _update_controller_route_endpoint_signature(cls: Type[Any], route: Union[Route, WebSocketRoute]) -> None:
    """
