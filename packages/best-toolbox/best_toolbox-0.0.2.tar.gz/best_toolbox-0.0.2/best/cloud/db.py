...
class SystemStateLoader(DatabaseHandler):
    ...
    def load_stimulation_annotations(self, patient_id, start_utc, end_utc):
        self._sql_connection = self._engine.connect()
        if np.log10(start_utc) < 10: start_uutc = int(np.round(start_utc * 1e6))
        if np.log10(end_utc) < 10: stop_uutc = int(np.round(end_utc * 1e6))
        ...
        if df_data.__len__() > 0:
            ...
            return pd.DataFrame(columns=['frequency', 'amplitude', 'start', 'end', 'duration'])
