...
from nawah.utils import (_expand_attr, _extn_list, _set_none_value,
                         _update_attr_values, validate_doc)
...
class BaseModule:
    ...
    async def read(
        ...
        results = await Data.read(
            ...
            attrs=self.attrs,
            query=query,
            extn_attrs=_extn_list(
                extns=self.extns, skip_events=skip_events, query=query
            ),
        )
    ...
    async def watch(
        ...
        async for results in Data.watch(
            ...
            attrs=self.attrs,
            query=query,
            # [TODO] Revisit as part of Watch Workflow
    ...
    async def update(
        ...
        # [DOC] update soft action is to only return the new created doc _id.
        if Event.SOFT in skip_events:
            read_results = await self.read(
