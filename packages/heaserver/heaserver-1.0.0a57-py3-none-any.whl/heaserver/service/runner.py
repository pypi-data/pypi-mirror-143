...
from aiohttp_remotes import setup as aiohttp_remote_setup, XForwardedRelaxed
import configparser
import os.path
from heaserver.service import appproperty, appfactory
from heaserver.service import wstl
from typing import Callable, Iterable, Optional, Union, AsyncIterator, AsyncGenerator
...
class Configuration:
    ...
    """
    def __init__(self,
                 base_url: Union[str, URL] = DEFAULT_BASE_URL,
...
def start(db: Callable[[web.Application, Optional[configparser.ConfigParser]], Database] = None,
    ...
    object, and then it calls AioHTTP's aiohttp.web.run_app() method with it to launch the service. It sets the
    application and request properties described in the documentation for get_application().

    :param db: a database object from the heaserver.server.db package, if database connectivity is needed. Sets the
    appproperty.HEA_DB application property to this object.
    ...
    """
    app = get_application(db=db, wstl_builder_factory=wstl_builder_factory, cleanup_ctx=cleanup_ctx, config=config)
    web.run_app(app, port=config.port if config else DEFAULT_PORT)


def get_application(db: Callable[[web.Application, Optional[configparser.ConfigParser]], Database] = None,
                    wstl_builder_factory: Optional[Callable[[], wstl.RuntimeWeSTLDocumentBuilder]] = None,
                    cleanup_ctx: Optional[Iterable[Callable[[web.Application], AsyncIterator[None]]]] = None,
                    config: Optional[Configuration] = None,
                    testing=False) -> web.Application:
    """
    ...
    if db:
        async def _db(app: web.Application) -> AsyncGenerator:
            app[appproperty.HEA_DB] = db(app, config.parsed_config)
            yield
        app.cleanup_ctx.append(_db)

    async def _hea_registry(app: web.Application) -> AsyncGenerator:
        app[appproperty.HEA_REGISTRY] = config.parsed_config['DEFAULT'].get('Registry', DEFAULT_REGISTRY_URL)
        yield
    app.cleanup_ctx.append(_hea_registry)

    async def _hea_component(app: web.Application) -> AsyncGenerator:
        app[appproperty.HEA_COMPONENT] = str(config.base_url)
        yield
    ...
    async def _hea_wstl_builder_factory(app: web.Application) -> AsyncGenerator:
        if wstl_builder_factory is None:
            ...
            wstl_builder_factory_ = wstl.builder_factory(resource=None)
        else:
