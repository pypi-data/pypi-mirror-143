...
from heaserver.service.docker import get_exposed_port, start_microservice_container, DockerContainer
...
def setup_test_environment(stack: ExitStack,
                           ...
                           registry_docker_image: Optional[str] = None,
                           other_docker_images: Optional[Iterable[DockerContainer]] = None) -> str:
    ...
    if registry_docker_image is None:
        ...
        if other_docker_images is None:
            other_docker_images_: Iterable[DockerContainer] = []
        else:
            other_docker_images_ = other_docker_images
        registry_url = start_microservice_container(DockerContainer(image=registry_docker_image, port=8080, check_path='/components'), mongo_, stack)
