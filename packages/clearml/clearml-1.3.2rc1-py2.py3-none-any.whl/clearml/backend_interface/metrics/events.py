...
class MetricsEventAdapter(object):
    ...
    should_report_nan_warning = True
    should_report_inf_warning = True
    ...
    def _convert_np_nan_inf(cls, val):
        if np.isnan(val):
            if cls.should_report_nan_warning:
                ...
                cls.should_report_nan_warning = False
            return cls.default_nan_value
        if np.isinf(val):
            if cls.should_report_inf_warning:
                LoggerRoot.get_base_logger().info(
                ...
                cls.should_report_inf_warning = False
