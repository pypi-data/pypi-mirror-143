# This file was generated by mettle.genes.db.GenPy3 [ver 2.1] on Tue Mar 22 16:49:01 2022
#

import copy
import datetime
import uuid
import time
import mettle.lib
import mettle.io
import mettle.db

class iMsgStatusCntByReq(mettle.io.ISerializable):

    def __init__(self,
                 msgreq_id: int = 0):
        """
        Constructor.

        :param msgreq_id: int
        """
        self.msgreq_id = msgreq_id

    def clear(self):
        """
        Clears all member variables for this database record.
        """
        self.msgreq_id = 0

    def _name(self) -> str:
        """
        Name of the record.

        :return: The name.
        """
        return 'iMsgStatusCntByReq'

    def _serialize(self, _w: mettle.io.IWriter, _oname: str = None):
        """
        Serialize record to a stream.

        :param _w:
        :param _oname:
        """
        if _oname == None:
            _oname = self._name()

        _w.write_start(_oname)
        _w.write_int64("msgreq_id", self.msgreq_id)
        _w.write_end(_oname)

    def _deserialize(self, _r: mettle.io.IReader, _oname: str = None):
        """
        Deserialize record from a stream.

        :param _r:
        :param _oname:
        """
        if _oname == None:
            _oname = self._name()

        _r.read_start(_oname)
        self.msgreq_id = _r.read_int64("msgreq_id")
        _r.read_end(_oname)

    def _copy_from(self, rec: "iMsgStatusCntByReq"):
        """
        Copies the column from the rec into this record.

        :param rec: The source record.
        """
        self.msgreq_id = rec.msgreq_id

    @staticmethod
    def _cache_davs(dvc=None):
        """
        Cache the DAV into the dav cache (or create one) and return it.

        :param dvc: (mettle.lib.DavCache), target cache, if None a new one is created and returned.
        """
        if dvc is None:
            dvc = mettle.lib.DavCache()

        dvc.add_targ("id", mettle.lib.Dav(mettle.lib.Dav.eDavType.Min, 1))
        dvc.add_targ("id", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))
        dvc.add_targ("msgreq_id", mettle.lib.Dav(mettle.lib.Dav.eDavType.NotNull))

        return dvc

    def _get_davs(self, dvc=None):
        iMsgStatusCntByReq._cache_davs(dvc)

    def __repr__(self) -> str:
        return '<Msg [msgreq_id:%r]>' % (
            self.msgreq_id)


    class List(list, mettle.io.ISerializable):
        """
        List Class
        """

        def _name(self) -> str:
            """
            Name of the list.

            :return: The name
            """
            return 'iMsgStatusCntByReq.List'

        def _serialize(self, _w: mettle.io.IWriter, _oname: str = None):
            """
            Serialize the list to a stream.

            :param _w:
            :param _oname:
            """
            if _oname == None:
                _oname = self._name()

            _w.write_start_list(_oname, len(self))

            for _rec in self:
                _rec._serialize(_w)

            _w.write_end(_oname)

        def _deserialize(self, _r: mettle.io.IReader, _oname: str = None):
            """
            Deserialize the list from a stream.

            :param _r:
            :param _oname:
            """
            if _oname == None:
                _oname = self._name()

            _cnt = _r.read_start_list(_oname)

            while _cnt >= 1:
                _rec  = iMsgStatusCntByReq()
                _cnt -= 1
                _rec._deserialize(_r)
                self.append(_rec)

            _r.read_end(_oname)

        def __repr__(self) -> str:
            return '<iMsgStatusCntByReq.List[%d]>' % (len(self))

        def _get_davs(self, dvc=None):
            return iMsgStatusCntByReq._cache_davs(dvc)

