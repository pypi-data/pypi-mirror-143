# This file was generated by mettle.genes.db.GenPy3 [ver 2.1] on Tue Mar 22 16:49:01 2022
#  Target Database = postgresql
#
import copy
import datetime
import uuid
import time
import mettle.lib
import mettle.io
import mettle.db

from bs_loco.db.tables.imsg_req_update_status import iMsgReqUpdateStatus

class dMsgReqUpdateStatus:

    def __init__(self, dbcon: mettle.db.IAConnect):
        """
        Constructor.

        :param dbcon: Mettle database connection object.
        """
        self._dbcon   = dbcon
        self._dbstmnt = None
        self.irec     = iMsgReqUpdateStatus()

    def __del__(self):
        """
        Destructor.
        """
        del self._dbstmnt

    def __enter__(self):
        """
        With statement enter.
        """
        return self

    def __exit__(self, type, value, traceback):
        """
        With statement exit.
        """
        self._destroy()

    def _destroy(self):
        if self._dbstmnt == None:
            return

        del self._dbstmnt
        self._dbstmnt = None

    async def exec_deft(self,
                        id: int,
                        status: str,
                        time_proc: datetime.datetime,
                        modified_by: str) -> "dMsgReqUpdateStatus":
        """
        Execute the query by setting all the inputs.

        :param id: int
        :param status: str
        :param time_proc: datetime.datetime
        :param modified_by: str
        :return: Self for convenience.
        """
        self.irec.id = id
        self.irec.status = status
        self.irec.time_proc = time_proc
        self.irec.modified_by = modified_by

        return await self.exec()

    async def exec(self, irec: iMsgReqUpdateStatus = None) -> "dMsgReqUpdateStatus":
        """
        Execute the query, optionally passing in the input rec.

        :param irec:
        :return: Self for convenience.
        """
        if irec:
            self.irec._copy_from(irec)

        self._destroy()

        self._dbstmnt = await self._dbcon.statement("MsgReqUpdateStatus", self._dbcon.STMNT_TYPE_CUD)

        self._dbstmnt.sql("""update
  loco.msgreq m
set
  status      = :status,
  time_proc   = :time_proc,
  modified_by = :modified_by,
  tm_stamp    = now()
where
  m.id      = :id and
  m.status  in ('P', 'B')""")

        self._dbstmnt.bind_in("id", self.irec.id, int, 8)
        self._dbstmnt.bind_in("status", self.irec.status, str, 1)
        self._dbstmnt.bind_in("time_proc", self.irec.time_proc, datetime.datetime)
        self._dbstmnt.bind_in("modified_by", self.irec.modified_by, str, 128)

        await self._dbcon.execute(self._dbstmnt)

        if irec:
            irec._copy_from(self.irec)

        return self

