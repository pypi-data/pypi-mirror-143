...
class dNoDict:
    ...
    async def lock_one_with(self, _rec: tNoDict, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
    ...
    async def lock_one_by_key(self, _key: tNoDictKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        with dNoDictLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle("LockOne failed to return a record (dNoDict)")
                    ...
                    if _dex == 0:
                        raise mettle.lib.xMettle("Could not lock NoDict record after %d attempt(s) at %d mili second intervals!" % (
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_with(self, _rec: tNoDict) -> bool:
        """
    ...
    async def try_select_one_by_key(self, _key: tNoDictKey) -> bool:
        """
        ...
        """
        with dNoDictSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dNoDict":
        """
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle("SelectOne failed to return a record (dNoDict)")

        return self

    async def select_one_with(self, _rec: tNoDict) -> "dNoDict":
        """
    ...
    async def select_one_by_key(self, _key: tNoDictKey) -> "dNoDict":
        """
        Attempt to select the record with it's primary key.
    ...
    async def update(self, irec: tNoDict = None):
        """
        ...
        with dNoDictUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tNoDict = None):
        """
        ...
        with dNoDictInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tNoDictKey = None):
        """
        ...
        with dNoDictDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
