...
class dMsgReqRes:
    ...
    async def lock_one_with(self, _rec: tMsgReqRes, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
    ...
    async def lock_one_by_key(self, _key: tMsgReqResKey, _lock4update: mettle.db.DBLock, _expected: bool = True) -> bool:
        """
        ...
        :return: True if selected, False if not found and _expected is set to False
        """
        with dMsgReqResLockOne(self._dbcon) as _qry:
            ...
            while _dex >= 0:
                try:
                    ...
                    if not await _qry.fetch():
                        ...
                        raise mettle.lib.xMettle("LockOne failed to return a record (dMsgReqRes)")
                    ...
                    if _dex == 0:
                        raise mettle.lib.xMettle("Could not lock MsgReqRes record after %d attempt(s) at %d mili second intervals!" % (
_lock4update.retrys, _lock4update.mili_seconds), err_code = mettle.lib.xMettle.eCode.DBLockNoWaitFailed)
    ...
    async def try_select_one_with(self, _rec: tMsgReqRes) -> bool:
        """
    ...
    async def try_select_one_by_key(self, _key: tMsgReqResKey) -> bool:
        """
        ...
        """
        with dMsgReqResSelectOne(self._dbcon) as _qry:
            await _qry.exec(_key)
            if not await _qry.fetch():
                return False
    ...
    async def select_one(self) -> "dMsgReqRes":
        """
        ...
        if not await self.try_select_one():
            raise mettle.lib.xMettle("SelectOne failed to return a record (dMsgReqRes)")

        return self

    async def select_one_with(self, _rec: tMsgReqRes) -> "dMsgReqRes":
        """
    ...
    async def select_one_by_key(self, _key: tMsgReqResKey) -> "dMsgReqRes":
        """
        Attempt to select the record with it's primary key.
    ...
    async def update_deft(self,
        ...
        self.rec.corr_id_list = corr_id_list
    ...
    async def update(self, irec: tMsgReqRes = None):
        """
        ...
        with dMsgReqResUpdate(self._dbcon) as _qry:
    ...
    async def insert(self, irec: tMsgReqRes = None):
        """
        ...
        with dMsgReqResInsert(self._dbcon) as _qry:
            await _qry.exec(self.rec)
    ...
    async def delete_one(self, irec: tMsgReqResKey = None):
        """
        ...
        with dMsgReqResDeleteOne(self._dbcon) as _qry:
            await _qry.exec(_key)
