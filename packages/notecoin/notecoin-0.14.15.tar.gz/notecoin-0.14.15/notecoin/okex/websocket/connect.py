...
class BaseConnect:
    def __init__(self, url, channels, channel_listen=None, api_key=None, secret_key=None, passphrase=None,
                 private=False):
        self.url = url
        ...
        self.channel_listen = channel_listen
    ...
    def run(self):
        ...
        while True:
            if self.channel_listen is not None:
                channels = self.channel_listen()
                if len(list(set(self.channels).intersection(set(channels)))) != len(channels):
                    self.channels = channels
