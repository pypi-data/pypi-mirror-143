...
class OfficeRegistrator(Base):
    ...
    def __repr__(self):
        return '<OfficeRegistrator {} (id={})>'.format(self, self.id)
...
class OfficeRegistratorFacultyLink(Base):
    ...
    def __repr__(self):
        return '<OfficeRegistratorFacultyLink {}>'.format(self)
...
class Dean(Base):
    ...
    def __repr__(self):
        return '<Dean {} (id={})>'.format(self, self.id)
...
class TeacherChairLink(Base):
    ...
    def __repr__(self):
        return '<TeacherChairLink {}>'.format(self)
...
class AdviserStudent(Base):
    ...
    # Эдвайзер
    adviser_id = Column('advicer_id', ForeignKey('univer_advicer.advicer_id'), primary_key=True)
    adviser = relationship('Adviser', backref=backref('students', lazy='dynamic'))

    # Студент
    student_id = Column(ForeignKey('univer_students.students_id'), primary_key=True)
    student = relationship('Student', backref=backref('advisers', lazy='dynamic'))

    def __repr__(self):
        return '<AdviserStudent {}>'.format(self)

    def __str__(self):
        return f'{adviser}: {student}'
