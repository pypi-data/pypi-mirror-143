...
def snps_to_aa(vep,genotype,map_to_pdb_file,database='pdb'): 
	...
	map_to_pdb = pd.read_csv(io.BytesIO(map_to_pdb_file), encoding='utf8', \
...
def map_PDB_structure(vep_mapping):
	...
	for pdb in set(vep_mapping['structure'].values):
		try:
			pdbl=PDBList()
			pdbl.retrieve_pdb_file(pdb, pdir='proteins/pdb', file_format='pdb',obsolete=False)
			structure = parser.get_structure(pdb, "proteins/pdb/pdb%s.ent"%pdb)
...
def map_alphafold_structure(vep_mapping):
	...
	af_struct_dir='proteins/alphafold'
	...
	for uniprot in vep_mapping['structure'].unique():
		try:
			...
			urllib.request.urlretrieve(url, "proteins/alphafold/%s.pdb"%uniprot)
			structure = parser.get_structure(uniprot, "proteins/alphafold/%s.pdb"%uniprot)
