...
def run_pokemon():
	...
	# default files
	cwd = os.getcwd()
	map_to_pdb_file = pkgutil.get_data('PokemonAnalysis','ref/pdb_chain_uniprot.csv')
	pwm_file = pkgutil.get_data('PokemonAnalysis','ref/blosum62')
	pwm = pd.read_csv(io.BytesIO(pwm_file),encoding='utf8',index_col=0,delim_whitespace=True)
