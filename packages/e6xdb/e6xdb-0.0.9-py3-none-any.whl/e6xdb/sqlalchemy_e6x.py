...
from sqlalchemy.engine import default, Engine, Connection, URL
...
class E6xDialect(default.DefaultDialect):
    ...
    def get_view_names(self, connection, schema=None, **kw):
        ...
        return client.getTables("default")
    ...
    def _get_table_columns(self, connection, table):
        try:
            ...
            client = cursor.connection
            buffer = client.getColumns("default", table)
            buffer = BytesIO(buffer)
            dis = DataInputStream(buffer)
            columns_info = get_query_columns_info(dis)
            rows = list()
            for i in columns_info:
                row = dict()
                row["col_name"] = i.split(":")[0].lower()
                row["data_type"] = i.split(":")[1].lower()
                rows.append(row)
...
if __name__ == '__main__':
    dialect = E6xdbE6xDialect()
    from urllib.parse import urlparse

    args = dialect.create_connect_args(urlparse("e6xdb://admin:admin@localhost:9000/?schema=./input/tpcds_330mb"))
    connection = dialect.dbapi.connect(args)
    cursor = None
    if isinstance(connection, Engine):
        cursor = connection.raw_connection().connection.cursor()
    elif isinstance(connection, Connection):
