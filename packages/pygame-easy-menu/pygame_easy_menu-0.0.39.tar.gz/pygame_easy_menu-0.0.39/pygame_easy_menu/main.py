...
class ScrollableBox(sprite):
    ...
    def __init__(self, name, size, manager: Menu_Manager,path:str=None,width_cursor=20,speed=5, isactive=True, layer=0):
        ...
        if not path:
            self.cursor = py.Surface((width_cursor,self.rect.height - self.get_max()))
        else:
    ...
    def add_sprite(self,func):
            ...
            if sprite in _sprite.__class__.__bases__ or type(_sprite)==sprite:
                ...
                self.cursor = py.Surface((20,self.rect.height - self.get_max()))
            else:
