...
class ScrollableBox(sprite,py.sprite.Group):

    def __init__(self, name, size, manager: Menu_Manager, isactive=True, layer=0):
        super().__init__()
        self.name = name
        self.layer = layer
        self.isactive = isactive
        ...
        self.initial_size = Vector2(self.image.get_width(),self.image.get_height())
        super(py.sprite.Group,self).__init__()

    def add_sprite(self,sp:sprite):
        self.add(sp)

    def update(self, *args, **kwargs):
        super(py.sprite.Group,self).update(*args, **kwargs)
