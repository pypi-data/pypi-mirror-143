...
class pagos_data_cipher:
    ...
    def encrypt_object(self, data):
        ...
        if (self.PAYLOAD_FIELD in encrypted_data):
            encrypted_field = cipher.encrypt(pad(encrypted_data[self.PAYLOAD_FIELD].encode('utf8'), self.BLOCK_SIZE))
            encrypted_data[self.PAYLOAD_FIELD] = binascii.hexlify(encrypted_field).decode()
            encrypted_data[self.PAYLOAD_FIELD] = json.dump(encrypted_data[self.PAYLOAD_FIELD])
    ...
    def decrypt_object(self, data):
        ...
        if (self.PAYLOAD_FIELD in decrypted_data):
            ...
            decrypted_data[self.PAYLOAD_FIELD] = json.loads(decrypted_field.decode())
