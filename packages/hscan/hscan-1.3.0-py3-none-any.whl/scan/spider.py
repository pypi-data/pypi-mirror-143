...
class Spider(object):
    ...
    # async def on_success(self, redis_conn=None, spider_id=None, count=1):
    #     """
    #     处理成功计数
    #     :param redis_conn:
    ...
    #     :return:
    #     """
    #     if not redis_conn and not self.redis_conn:
    #         await logger.error('没有redis连接信息，不能计数')
    #         return
    #     try:
    #         # 10分钟一组
    ...
    #         if redis_conn:
    #             await redis_conn.incrby(count_key, count)
    #         else:
    #             await self.redis_conn.incrby(count_key, count)
    #     except Exception as e:
    #         await logger.error(f'处理成功回调异常:{spider_id} {e}')

    async def on_success(self, mongo_db=None, spider_id=None, count=1):
        """
        处理成功计数
        :param mongo_db:
        :param spider_id: spider_name-local_ip-container_id
        ...
        if not mongo_db and not self.mongo_db:
            await logger.error('没有mongodb连接信息，不能计数')
            return
        try:
            # 10分钟一组
            tick = date_to_char('m')[:-1] + '0'
            ...
            try:
                db_data = await mongo_db.get_collection('spider_statistics').find_one({'_id': _id})
                if db_data:
                    await mongo_db.get_collection('spider_statistics').update_one(
                        {'_id': _id}, {'$inc': {'count': count}})
                else:
                    await mongo_db.get_collection('spider_statistics').insert_one(
                        {'_id': _id, 'sid': spider_id, 'date': tick, 'count': count})
    ...
    async def on_error(self, e, mongo_conn=None, spider_name=None, desc=None, url=None, headers=None):
        """
        处理失败日志记录
        :param mongo_conn:
        :param spider_name:
        ...
        if not self.mongo_db and not mongo_conn:
            await logger.error('没有mongodb连接信息，不能保存日志')
            return
        try:
            save_data = {
                "ip": self.local_ip,
            ...
            if mongo_conn:
                await mongo_conn.get_collection('spider_error_logs').insert_one(save_data)
            else:
                await self.mongo_db.get_collection('spider_error_logs').insert_one(save_data)
        except Exception as e:
            await logger.error(f'处理失败回调异常:{spider_name}-{self.local_ip}-{self.container_id} {e}')
    ...
    async def queue_task(self):
        ...
        while self.status:
            try:
                res = await self.rabbitmq.consume(task_queue, arguments=arguments)
                if res:
