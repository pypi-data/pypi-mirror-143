...
class Scraper:
    ...
    def xPath(self, path, element=None, pos=0, dataType=None, prop=None, explode=None, condition=None, substring=None, transform=None, replace=None, join=None, timeout=0):
        ...
        for item in el:
            try:
                if prop == WebScraper.Properties.TEXT:
                    response.append(self.on_get_text(item))
                elif prop == WebScraper.Properties.HTML:
        ...
        if '[' in dataType: # in the case dataType is a datetime with format (DATE[%Y-%m-%d %H:%M:%S])
    ...
    def exists(self, path: str, element: any, timeout: int=0):
    ...
    def enter_text(self, path, text, element=None, clear=False):
        return self.on_enter_text(path=path, text=text, element=element, clear=clear)
