...
class Selenium(Scraper):
    ...
    def on_xPath(self, path, element=None, timeout=0):
        ...
        try:
            ...
            if el != False:
                return (self.driver if element is None else element).find_elements_by_xpath(path)
            else:
                return None
    ...
    def on_enter_text(self, path, text, clear=False, element=None):
        element = self.on_xPath(path, element)
        if element is not None:
            if type(element) is list:
                element = element[0]
    ...
    def on_select(self, path, option, element=None):
        element = WebDriverWait(self.driver if element is None else element, 10).until(
            EC.presence_of_element_located((By.XPATH, path))
        )

        option = self.on_xPath(element=element, path=option, prop='text()')
