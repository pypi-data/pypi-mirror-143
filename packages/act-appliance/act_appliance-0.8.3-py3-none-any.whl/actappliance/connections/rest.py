...
class ApplianceRest(ApplianceConnection):
    ...
    def connect(self):
        ...
        # cert_paths = (self.connect_kwargs.get('cert'), self.connect_kwargs.get('key'))
        # print(cert_paths)
        #
        # result = self.s.post("https://" + self.system + "/actifio/user/login", verify=False, params=self.connect_kwargs, cert=cert_paths,timeout=request
