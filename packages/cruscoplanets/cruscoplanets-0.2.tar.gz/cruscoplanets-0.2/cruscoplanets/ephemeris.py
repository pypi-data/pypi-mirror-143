...
class Ephemeris:
	...
	def __init__(self, event: dates.Event):
		...
		#First we add to swe.EPHE_PATH the path of the directory where Eris' files are stored. The static variable isPathInitialized assures that this operation is done just one time.
		if not self.__class__.isPathInitialized:
			path_to_initialize = swe.EPHE_PATH + os.pathsep + self.__class__.SWISSEPH_FILES_DIR
			swe.set_ephe_path(path_to_initialize)
			self.__class__.isPathInitialized = True
