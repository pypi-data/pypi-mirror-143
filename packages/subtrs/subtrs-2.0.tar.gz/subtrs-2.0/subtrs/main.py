...
class Subtitles:
    ...
    def write_new_subs(self):
        ...
        for index, lang in enumerate(self.dest, 1):
            ...
            self.trs_jobs(index, cap_file, lang)
            with open(new_captions, 'w') as new_subs:
                bar = Bar('Processing', max=len(self.subs_file),
    ...
    def trs_jobs(self, index, file, lang):
        '''Printing the current job'''
        print(f"[{index}/{len(self.dest)}] Subtitles "
              f"'{file}' translate to {googletrans.LANGUAGES.get(lang)}")
