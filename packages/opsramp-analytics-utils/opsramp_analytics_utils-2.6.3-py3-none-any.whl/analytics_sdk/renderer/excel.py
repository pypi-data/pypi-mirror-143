...
class ExcelRenderer:
    ...
    global res
    res = ''
    def __init__(self, analysis_run, tenantId, integrationId):
        """
        :param analysis_run: AnalysisRun
        """
        self.wb = Workbook()
        ...
        url = BASE_API_URL + f'/analyticsapps/api/v7/tenants/{tenantId}/{integrationId}/runs/{analysis_run}/summary'
    ...
    def generate_summary(self, res, tenantId):
        """
        add summary sheet
        """
        ws = self.wb.active
        ...
        cell = ws['C7']
        # params = self.analysis_run.params
        start_date = res['genStime']
        end_date = res['genEtime']
        #cell.value = f'{start_date} - {end_date}'
        cell.value = f"{datetime.fromtimestamp(start_date / 1000).strftime('%B %d %Y %H:%M:%S')} - {datetime.fromtimestamp(end_date / 1000).strftime('%B %d %Y %H:%M:%S')}"
        #cell.value = f'{start_date.strftime("%B %d, %Y")} - {end_date.strftime("%B %d, %Y")}'
        cell.font = Font(color="000000")
        cell.fill = PatternFill("solid", fgColor="eeeeee")
    ...
    def render(self):
        ...
        self.generate_summary(res, tenantId)
        # run_data = self.analysis_run.get_result().get('excel-data', {})
