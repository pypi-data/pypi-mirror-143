...
def _get_reference_edges(variables, statistics, dtypes, histogram_edges, bins=10, **kwargs):
    edges = []
    ...
    if os.path.isfile("artifacts/input/data_stats.json"):
        ...
        for var in variables:
            # if reference edges are provided by the user, use these instead of computing
            if histogram_edges and var in histogram_edges:
                edges.append(histogram_edges[var])
...
def _list_mapper(obj, mapper):
    return [mapper[item] if item in mapper else item for item in obj] if mapper else obj
...
def _parse_scoring_stats(features_df: DataFrame,
    ...
    for var_type in ["features", "predictors"]:
        for name, value in data_stats[var_type].items():
            ...
            if "statistics" in value and "histogram" in value["statistics"] and "edges" in value["statistics"][
                "histogram"]:
    ...
    data_stats = _capture_stats(df=features_df,
                                ...
                                importance_dict=importance_dict if importance_dict is not None else importance,
...
def _capture_stats(df: DataFrame,
    ...
    if histogram_edges:
        histogram_edges = {k.lower(): v for k, v in histogram_edges.items()}
    ...
    if targets and not set(targets).issubset(df_columns):
        raise ValueError(f"Dataframe with columns ({df.columns}) does not contain targets: {features}")
    ...
    if len(continuous_vars) > 0:
        continuous_vars_mapped = _list_mapper(continuous_vars, mapper)
        stats = valib.Statistics(data=df, columns=continuous_vars_mapped, stats_options="all")
        stats = stats.result.to_pandas().reset_index()
        stats["xcol"] = stats["xcol"].str.lower()

        reference_edges = _get_reference_edges(continuous_vars, stats, dtypes, histogram_edges)
        hist = valib.Histogram(data=df, columns=continuous_vars_mapped,
    ...
    if len(categorical_vars) > 0:
        categorical_vars_mapped = _list_mapper(categorical_vars, mapper)
        frequencies = valib.Frequency(data=df, columns=categorical_vars_mapped)
    ...
    def add_var_metadata(variable, var_type, group_label):
        mapped_var = mapper.get(variable, variable)
    ...
    for var in features:
        add_var_metadata(var, "features", feature_group)

    for var in targets:
        add_var_metadata(var, "predictors", target_group)
...
def record_training_stats(df: DataFrame,
    ...
    if "context" in kwargs:
        ...
        data_stats_filename = os.path.join(context.artefact_output_path, "data_stats.json")
...
def record_evaluation_stats(features_df: DataFrame,
    ...
    :param mapper: (optional) Dictionary with the column names from the model as keys and the new names from the provided dataframes as values. E.g. {"target": "prediction"}
    :return data_stats: Dict of statistics
    """
    ...
    if "context" in kwargs:
        context = kwargs.get("context")
        output_data_stats_filename = os.path.join(context.artefact_output_path, "data_stats.json")
        input_data_stats_filename = os.path.join(context.artefact_input_path, "data_stats.json")
