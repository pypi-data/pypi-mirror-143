...
class DllMode:
    ...
    def get_his_value(self, tag, start_time: datetime = None, end_time: datetime = None, time_interval=10):
        """
        待验证
        ...
        :return:
        """
        tag_name = ctypes_str2char_array(tag)  # 名字
        ...
        insert_time = timedelta(seconds=10)
        start_time_long = int(time.mktime(start_time.timetuple()))  # 将时间转为UNIX时间
        end_time_long = int(time.mktime(end_time.timetuple()))
        # insert_time_long = int(time.mktime(insert_time))
        start_time_c = ctypes.c_ulong(start_time_long)  #
        end_time_c = ctypes.c_ulong(end_time_long)
        insert_time = ctypes.c_ulong(10)
        flag = ctypes.c_long(0)  # 标记，不知道什么意思

        data_size = (end_time_long - start_time_long) // time_interval+1
        # time_interval = ctypes.c_long(time_interval)
        value_double_arr = (ctypes.c_double * data_size)()
        value2_arr = (ctypes.c_long * data_size)()
        ...
        for i in range(data_size):
            data_size_arr[i] = ctypes.c_int32(data_size)
