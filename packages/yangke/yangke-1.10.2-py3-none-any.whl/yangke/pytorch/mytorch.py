...
class DataFitterNet(torch.nn.Module):
    def __init__(self, settings, trial=None):
        ...
        for layer in cfg.get("layers"):  # 从配置中拿到神经网络各层的信息，进而构建神经网络
            ...
            elif _type.lower() == "tanh":
                layer_dict[f"layer{i}"] = nn.Tanh()
                _cell_num = _cell_num_last_layer
            elif _type.lower() == "softmax":
                layer_dict[f"layer{i}"] = nn.Softmax(dim=0)
                _cell_num = _cell_num_last_layer
            elif _type == "dropout":
        ...
        elif settings.get_settings("networks.optimizer.type") == "sgd":
            self.optimizer = torch.optim.SGD(self.parameters(), lr=self.lr, weight_decay=weight_decay)
        elif settings.get_settings("networks.optimizer.type") == "nadam":
            self.optimizer = torch.optim.NAdam(self.parameters(), lr=self.lr, weight_decay=weight_decay)
        elif settings.get_settings("networks.optimizer.type") == "adadelta":  # 无需设置学习率
            self.optimizer = torch.optim.Adadelta(self.parameters(), lr=self.lr, weight_decay=weight_decay)
        else:
            self.optimizer = torch.optim.Adam(self.parameters(), lr=self.lr, weight_decay=weight_decay)
    ...
    def load_yk(path):
        if not os.path.exists(path):
            logger.warning(f"加载的模型文件不存在：{path}")
            return None
    ...
    def train_loop(self, data_loader):
        ...
        for batch, (x, y) in enumerate(pbar):
            ...
            loss.backward()
            self.optimizer.step()
            pred, y = self.standard_reverse(pred), self.standard_reverse(y)
        ...
        logger.debug(f"训练集预测结果({self.y_title[0]}): 平均损失为{loss:>8f}，平均相对误差为{relative_err:>8f}")
        return loss

    def test_loop(self, data_loader):
        num_batches = len(data_loader)
        size = len(data_loader.dataset)
        test_loss = relative_err = max_err = 0
        self.eval()
        with torch.no_grad():
    ...
    def start_train(self, train_set, test_set, writer=None):
        ...
        for t in range(self.epochs):
            ...
            logger.info(f"最大相对误差为{max_err.item():>8f}，平均相对误差为：{current_err:>8f}")
            # ------------------------- 损失曲线图 ---------------------------------
            ...
            # ------------------------- 权重直方图 ---------------------------------
            try:
                if current_err < 0.01 and max_err < 0.06 and t >= 10:
...
class OptunaModel:
    ...
    def optimize(self):
        study_name = "model_study5"
        sampler = self.settings.get_settings("optuna.sampler")
    ...
    def objective(self, trial):
        ...
        trial.set_user_attr('max_err', float(max_err))
        return loss
