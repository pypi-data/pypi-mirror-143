...
def min(operands):
    return sequantial_calculation(lambda data : np.nanmin([*data]), operands)

def max(operands):
    return sequantial_calculation(lambda data : np.nanmax([*data]), operands)
...
class WitData:
    ...
    def spread(self, target_type):
        ...
        new_dates = np.array([])
        new_data = np.array([])
        new_stock_codes = np.array(self.stock_codes.copy())
