...
def plus(operands):
    return sequantial_calculation(lambda data : reduce(lambda x, y : y if x is None else x if y is None else (x + y), data), operands)

def multiply(operands):
    return sequantial_calculation(lambda data : reduce(lambda x, y : x * y, data), operands)

def minus(operand1, operand2):
    return two_elements_calculation(lambda data1, data2 :  -data2 if data1 is None else -data1 if data2 is None else (data1 - data2), operand1, operand2)
