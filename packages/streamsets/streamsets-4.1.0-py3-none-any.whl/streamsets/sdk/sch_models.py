...
from .utils import (DEFAULT_PROVISIONING_SPEC, MutableKwargs, SeekableList, TRANSFORMER_DEFAULT_EXECUTION_MODE, Version,
                    build_tag_from_raw_tag, format_sch_log, get_params, get_topology_nodes, METERING_METRIC_LOOKUP,
    ...
    def _get_all_results_from_api(self, **kwargs):
        ...
        Returns:
            A :obj:`collections.namedtuple`: of
                ...
                kwargs (:obj:`dict`): a dict of local variables not used in this function.
        """
        return ModelCollectionResults(SeekableList(Organization(organization,
...
class ApiCredentialBuilder:
    ...
    def build(self, name, generate_auth_token):
        """Build the ApiCredential.

        Args:
            name (:obj:`str`): ApiCredential name.
            generate_auth_token (:obj:`bool`): Flag to indicate if auth token needs to be generated.

        Returns:
            An instance of :py:class:`streamsets.sdk.sch_models.ApiCredential`.
        """
        self._api_credential.update({'label': name,
                                     'generateAuthToken': generate_auth_token,
                                     'active': True})
        return ApiCredential(api_credential=self._api_credential, control_hub=self._control_hub)
...
class ApiCredentials(ModelCollection):
    ...
    def _get_all_results_from_api(self, **kwargs):
        ...
        Returns:
            A :obj:`collections.namedtuple`: of
                ...
                kwargs (:obj:`dict`): a dict of local variables not used in this function.
        """
        return ModelCollectionResults(SeekableList(ApiCredential(api_credential, control_hub=self._control_hub)
                                                   for api_credential in
                                                   self._control_hub.api_client
                                                   .get_api_user_credentials(self._control_hub.organization)
                                                   .response.json()),
                                      kwargs)
...
class LegacyDeployments(ModelCollection):
    ...
    def _get_all_results_from_api(self, id=None, organization=None, **kwargs):
        ...
        Returns:
            A :obj:`collections.namedtuple`: of
                results (:py:class:`streamsets.sdk.utils.SeekableList`): a SeekableList of
                :py:class:`streamsets.sdk.sch_models.LegacyDeployment` instances and
                kwargs (:obj:`dict`): a dict of local variables not used in this function.
        """
        ...
        if id is not None:
            try:
                result = SeekableList([LegacyDeployment(self._control_hub.api_client
                                                        .get_legacy_deployment(deployment_id=id).response.json(),
                                                        self._control_hub)])
            except requests.exceptions.HTTPError:
            ...
            deployment_ids = [deployment['id'] for deployment in self._control_hub.api_client
                              ...
                              .response.json()]
            result = SeekableList([LegacyDeployment(self._control_hub.api_client
                                                    .get_legacy_deployment(deployment_id=deployment_id).response.json(),
...
class Environments(ModelCollection):
    ...
    def _get_all_results_from_api(self, id=None, **kwargs):
        ...
        if id is not None:
            try:
                environment = self._control_hub.api_client.get_environment(id).response.json()
                result = SeekableList([EnvironmentFactory.create_environment(environment, self._control_hub)])
            except requests.exceptions.HTTPError:
                raise ValueError('Environment (id={}) not found'.format(id))
        else:
            result = SeekableList(EnvironmentFactory.create_environment(environment, control_hub=self._control_hub)
                                  for environment in
                                  self._control_hub.api_client
...
class EnvironmetTags(ModelCollection):
    """Collection of :py:class:`streamsets.sdk.sch_models.Tag` instances.
    ...
    def _get_all_results_from_api(self, organization=None, parent_id=None, **kwargs):
        ...
        result = SeekableList(Tag(label)
                              for label in
                              self._control_hub.api_client
                              .get_all_environment_tags(organization=organization,
                                                        parent_id=parent_id,
                                                        offset=kwargs_unioned['offset'],
                                                        len=kwargs_unioned['len'],
                                                        order=kwargs_unioned['order']).response.json()['data'])
...
class EnvironmentBuilder:
    ...
    def build(self, environment_name, environment_type, allow_nightly_engine_builds=False, environment_tags=None):
        """Define the environment.

        Args:
            environment_name (:obj:`str`): environment name.
            environment_type (:obj:`str`): Type of environment.
        ...
        """
        self._environment.update({'name': environment_name, 'type': environment_type,
                                  'allowSnapshotEngineVersions': allow_nightly_engine_builds})
        environment = EnvironmentFactory.create_environment(self._environment, self._control_hub)
        if environment_tags:
            environment.tags = environment_tags
        return environment


class EnvironmentFactory:
    """A factory class that is used to create objects of specific Environment."""
    @staticmethod
    def create_environment(environment, control_hub):
        """A factory method that is used to create objects of specific Environment.
        ...
        if environment['type'] == 'SELF':
            environment = SelfManagedEnvironment(environment=environment, control_hub=control_hub)
        elif environment['type'] == 'AWS':
            environment = AWSEnvironment(environment=environment, control_hub=control_hub)
        elif environment['type'] == 'AZURE':
            environment = AzureEnvironment(environment=environment, control_hub=control_hub)
        elif environment['type'] == 'GCP':
            environment = GCPEnvironment(environment=environment, control_hub=control_hub)
        else:
...
class Deployment(BaseModel, ABC):
    ...
    def registered_engines(self):
        command = self._control_hub.api_client.get_deployment_registered_engines(self.deployment_id)
        engines = command.response.json()['data']
        if len(engines) < 1:
            logger.debug('No registered engines found')
            return SeekableList()
        executor_type = engines[0]['executorType']
        if executor_type == 'COLLECTOR':
            return SeekableList(DataCollector(engine, self._control_hub) for engine in engines)
        elif executor_type == 'TRANSFORMER':
            return SeekableList(Transformer(engine, self._control_hub) for engine in engines)
...
class DeploymentBuilder:
    ...
    def build(self, deployment_name, deployment_type, engine_type, engine_version,
              environment, deployment_tags=None, engine_build=None, scala_binary_version=None):
        """Define the deployment.

        Args:
            deployment_name (:obj:`str`): deployment name.
            deployment_type (:obj:`str`): Type of deployment.
        ...
        deployment = DeploymentFactory.create_deployment(self._deployment, self._control_hub)
...
class DeploymentFactory:
    """A factory class that is used to create objects of specific Deployment."""

    @staticmethod
    def create_deployment(deployment, control_hub):
        """A factory method that is used to create objects of specific Deployment.
        ...
        if deployment['type'] == 'SELF':
            deployment = SelfManagedDeployment(deployment=deployment, control_hub=control_hub)
        elif deployment['type'] == 'AZURE_VM':
            deployment = AzureVMDeployment(deployment=deployment, control_hub=control_hub)
        elif deployment['type'] == 'EC2':
            deployment = EC2Deployment(deployment=deployment, control_hub=control_hub)
        elif deployment['type'] == 'GCE':
            deployment = GCEDeployment(deployment=deployment, control_hub=control_hub)
        else:
...
class Deployments(ModelCollection):
    ...
    def _get_all_results_from_api(self, id=None, deployment_type=None, environment_id=None,
                                  tag=None, engine_type=None, **kwargs):
        """Args offset, len, order_by, order, tag, with_total_count, state, status, type are not exposed
        directly as arguments because of their limited use by normal users but, could still be specified just like any
        other args with the help of kwargs.
        ...
        if id is not None:
            try:
                deployment = self._control_hub.api_client.get_deployment(id).response.json()
                result = SeekableList([DeploymentFactory.create_deployment(deployment, self._control_hub)])
            except requests.exceptions.HTTPError:
                raise ValueError('Deployment (id={}) not found'.format(id))
        else:
            result = SeekableList(DeploymentFactory.create_deployment(deployment, control_hub=self._control_hub)
                                  for deployment in
                                  self._control_hub.api_client
...
class DeploymentEngineConfigurations(ModelCollection):
    ...
    def _get_all_results_from_api(self, id=None, engine_type='DC', disabled_filter='ONLY_ALLOWED', **kwargs):
        ...
        Returns:
            A :obj:`collections.namedtuple`: of
                ...
                kwargs (:obj:`dict`): a dict of local variables not used in this function.
        """
        kwargs_defaults = {'offset': 0, 'len': -1, 'order_by': 'CREATE_TIME', 'order': 'ASC',
                           'with_total_count': False}
        kwargs_instance = MutableKwargs(kwargs_defaults, kwargs)
        kwargs_unioned = kwargs_instance.union()
        if id is not None:
            try:
                engine_config = self._control_hub.api_client.get_engine_version(id).response.json()
                result = SeekableList([DeploymentEngineConfiguration(engine_config, self._control_hub)])
            except requests.exceptions.HTTPError:
                raise ValueError('DeploymentEngineConfiguration (id={}) not found'.format(id))
        else:
            result = SeekableList(DeploymentEngineConfiguration(engine_config, control_hub=self._control_hub, )
                                  for engine_config in
                                  self._control_hub.api_client
...
class DeploymentEngineAdvancedConfiguration():
    ...
    def __init__(self, deployment_engine_advanced_config, deployment=None):
        ...
        self._file_mappings = {'log4j': ('sdc-log4j.properties' if self._engine_type == 'DC'
                                         else 'transformer-log4j.properties'),
                               'security_policy': ('sdc-security.policy' if self._engine_type == 'DC'
