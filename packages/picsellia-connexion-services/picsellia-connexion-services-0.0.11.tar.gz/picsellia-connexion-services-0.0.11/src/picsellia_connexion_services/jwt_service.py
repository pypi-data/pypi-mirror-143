...
class JwtServiceConnexion(AbstractServiceConnexion):
    ...
    def __init__(self, host : str, jwt_identifier_data: dict,  authorization_key: str = "Bearer", login_path: str = '/login') -> None:
    ...
    def generate_jwt(self):
        ...
        if response.status_code == 200:
            try:
                ...
                raise UnauthorizedError("Cannot parse response from external service. Please contact support.")
        else:
            raise UnauthorizedError("Unauthorized action on the deployment services")
    
    def _build_headers(self):
        return { "Authorization": "{} {}".format(self.authorization_key, self.jwt)}
    ...
    def patch(self, path: str, data: dict = None, params: dict = None):
        url = urljoin(self.host, path)
        return requests.patch(url=url, data=data, headers=self._build_headers(), params=params)
