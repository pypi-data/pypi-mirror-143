...
class MentatNetmngrScript(mentat.script.fetcher.FetcherScript):
    ...
    def _process_whois_data(whois_file_data, whois_file_type):
        ...
        for network_data in whois_file_data:
            ...
            # all severities with ',' as the delimiter.
            if not 'abuse_group' in nwr:
                continue
            ...
            if not nwrkey in processed_data[abuse_group]:
        ...
        for abuse_group in wi_file_data:
            ...
            for severity in ['low', 'medium', 'high', 'critical']:
                if not all(x == emails[severity][0] for x in emails[severity]):
                    self.logger.error("Abuse group %s has inconsistent emails across networks", abuse_group)
            ...
            if index == -1:
                ...
                if status_only:
                    ...
                    if group.networks[index].rank != net.get('rank', None):
                        self.logger.debug("Rank: '%s' -> '%s'", group.networks[index].rank, net.get('rank', None))
                    if group.networks[index].source != net['source']:
                        self.logger.debug("Source: '%s' -> '%s'", group.networks[index].source, net['source'])
                    if group.networks[index].netname != net['netname']:
                        self.logger.debug("Netname: '%s' -> '%s'", group.networks[index].netname, net['netname'])
                    if group.networks[index].is_base != net['is_base']:
                        self.logger.debug("is_base: '%s' -> '%s'", str(group.networks[index].is_base), str(net['is_base']))
                    continue
    ...
    def _get_index_of_network(network, netlist):
        ...
        if isinstance(network, NetworkModel):
            for index, net in enumerate(netlist):
                if network.network == net['network'] and (('netname' in net and network.netname == net['netname']) or network.source == net['source']):
                    return index
        else:
            for index, net in enumerate(netlist):
                if net.network == network['network'] and (('netname' in network and net.netname == network['netname']) or net.source == network['source']):
                    return index
        return -1
    ...
    def _network_has_changed(original_network, new_network):
        ...
        if isinstance(original_network, NetworkModel):
            if isinstance(new_network, NetworkModel):
                ...
                return original_network.rank != new_network.get('rank', None) or original_network.is_base != new_network.get('is_base')
        else:
            if isinstance(new_network, NetworkModel):
                return original_network.get('rank', None) != new_network.rank or original_network.get('is_base') != new_network.is_base
            else:
                return original_network.get('rank', None) != new_network.get('rank', None) or original_network.get('is_base') != new_network.get('is_base')
