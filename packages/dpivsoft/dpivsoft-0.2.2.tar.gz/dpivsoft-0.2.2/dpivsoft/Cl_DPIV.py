...
def initialization(width, height, thr):
    ...
    data1 = np.array((width, height,
        ...
        peak_ratio, Parameters.gaussian_filter)).astype(np.int32)
    ...
    if (Parameters.gaussian_filter):
        ...
        kernel = DPIV.gaussian_kernel(Parameters.gaussian_filter)
        ...
        GPU.img2_g = temp_manager.array([height, width],
            ...
            GPU.subImg1_2, GPU.u_index_1, GPU.v_index_1,
            GPU.img1_g])
        #img_g = np.float32(np.zeros([height,width]))
        #GPU.img1_g = thr.empty_like(img_g)
        #GPU.img2_g = thr.empty_like(img_g)
