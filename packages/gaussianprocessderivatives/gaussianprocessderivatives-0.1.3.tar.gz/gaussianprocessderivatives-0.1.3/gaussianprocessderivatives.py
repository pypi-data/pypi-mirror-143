"""
    ...
    Atfer running g.predict, the mean of the fitted GP is g.f and its variance is g.fvar.
...
class gaussianprocess:
    def __init__(self, lthbounds, x, y, merrors=None, warnings=True):
        ...
        Arguments
        --
        lthbounds: a dictionary of pairs of the bounds on the hyperparameters
        ...
        merrors: if specified, a 1-d array of the measurement errors (as
        variances)
        v"""
        ...
        if x.size == 0 or y.size == 0:
            ...
            if warnings and len(x) > 1.0e3:
                print(
                    "GP Warning: large data set -",
                    len(x),
                    "datapoints - may slow optimisation and give memory issues",
                )
    ...
    def covfn(self):
        raise NotImplementedError(
            " No covariance function specified in class %s" % self.__class__.__name__
        )
    ...
    def d1covfn(self):
        raise NotImplementedError(
            " No first derivative of the covariance function specified in class %s"
    ...
    def d1d2covfn(self):
        raise NotImplementedError(
            " No second derivative of the covariance function specified in class %s"
            % self.__class__.__name__
        )
    ...
    def findhyperparameters(
        ...
        quiet: if False, print warning if an optimum hyperparameter is at a bound
        ...
        for i in range(noruns):
            ...
            for k in range(noinits):
                lth = [np.random.uniform(b[j][0], b[j][1])
            ...
            while linalgerror < linalgmax:
                try:
                    ...
                    if optmethod == "tnc":
                        ...
                        lthf[i, :], lmlml[i], dout = fmin_l_bfgs_b(
                            self.nlml,
                            lth,
                            fprime=self.jacnlml,
                            bounds=b,
            ...
            if success[i] != 1 or np.any(np.isnan(lthf)):
                print(
                    " Warning: optimization of hyperparameters failed at run "
                    + str(i + 1)
                )
        ...
        if np.any(success == 1):
            ...
            for i in range(len(b)):
                if lthb[i] == b[i][1] or lthb[i] == b[i][0]:
                    if not quiet:
                        print(" Warning: hparam["
                              + str(i) + "] is at a boundary")
                        print(
                            "\thparam["
                            + str(i)
            ...
            print("Optimization of hyperparameters failed")
    ...
    def sampleprior(self, size=1, lth=False):
        ...
        if np.any(lth):
            ...
            lth = np.log(
                np.power(
                    10, [np.random.uniform(b[i][0], b[i][1])
                         for i in range(len(b))]
                )
            )
    ...
    def predict(self, xnew, derivs=0, addnoise=False, merrorsnew=False):
        ...
        addnoise: if True, add measurement noise to the predicted variance
        merrorsnew: if set and addnoise is True, the expected measurements errors at xnew (should not be set if xnew= x)
        """
                ...
                if addnoise:
                    ...
                    if np.any(self.merrors):
                        if xold:
                            ...
                            raise gaussianprocessException(
                                " Measurement errors for new x values must be specified."
                            )
                    else:
    ...
    def batchpredict(
        ...
        if len(xnew) < maxlen:
            ...
            for i in range(1, len(bs)):
                self.predict(xnew[bs[i - 1]: bs[i]],
                             merrorsnew, derivs, addnoise)
    ...
    def sketch(self, datasymbol="o", derivs=0, GPcolor="blue", nostds=2):
        ...
        plt.fill_between(
            xnew, f - nostds * sd, f + nostds * sd, facecolor=GPcolor, alpha=0.2
        )
    ...
    def d12d22covfn(self, x, xp, lth):
        ...
        return (
            ...
            * (
                ...
                * x ** 3
                * np.sin((2 * (x - xp)) / th[2])
                + 32 * th[1] * th[2] * th[3] * xp
                * np.sin((2 * (x - xp)) / th[2])
                - 24 * th[1] ** 3 * th[2] * th[3]
                * xp * np.sin((2 * (x - xp)) / th[2])
                + 24
                * th[1]
                * th[2] ** 3
                * th[3] ** 2
                ...
                * np.sin((2 * (x - xp)) / th[2])
                - 48 * th[1] ** 2 * th[2] * th[3]
                * x * np.sin((4 * (x - xp)) / th[2])
                + 48 * th[1] ** 2 * th[2] * th[3]
                * xp * np.sin((4 * (x - xp)) / th[2])
                - 8 * th[1] ** 3 * th[2] * th[3]
                * x * np.sin((6 * (x - xp)) / th[2])
                + 8 * th[1] ** 3 * th[2] * th[3]
                * xp * np.sin((6 * (x - xp)) / th[2])
            )
            / th[2] ** 4,
            False,
        )
