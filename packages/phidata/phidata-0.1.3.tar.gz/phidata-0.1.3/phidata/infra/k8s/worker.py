...
class K8sWorker:
    ...
    def read_resources(
        ...
        k8s_resources: Dict[str, List[K8sResourceType]] = OrderedDict()
        for rg_name, rg in self.k8s_resources.items():
            filtered_resources: Optional[
                List[K8sResourceType]
            ] = filter_and_flatten_k8s_resource_groups(
                k8s_resource_groups={rg_name: rg},
                name_filter=name_filter,
                type_filter=type_filter,
                sort_order="delete",
            )
            if filtered_resources is not None and len(filtered_resources) > 0:
                k8s_resources[rg_name] = filtered_resources
    ...
    #                 resource.update(k8s_client=self.k8s_api_client, namespace=_active_ns)
