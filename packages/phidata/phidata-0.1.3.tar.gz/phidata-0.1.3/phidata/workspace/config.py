...
class WorkspaceConfigArgs(InfraArgs):
    ...
    # default infra for phi ws up/down
...
class WorkspaceConfig(InfraConfig):
    def __init__(
        ...
        aws_config_file: Optional[str] = None,
        aws_shared_credentials_file: Optional[str] = None,
        # path to important directories relative to the workspace root
        ...
        products_dir_clean = products_dir if products_dir else f"{name}/products"
        notebooks_dir_clean = notebooks_dir if notebooks_dir else f"{name}/notebooks"
        workspace_config_dir_clean = (
            workspace_config_dir if workspace_config_dir else f"{name}/workspace"
        )
    ...
    @property
    def default_infra(self) -> Optional[str]:
        return self.args.default_infra if self.args else None
