...
def fix_nans(data_array: Dict[str, Any]):
    """
    Utilities to deal with `NaN` and `inf`. The dataframes are temporarily
    "fixed" on the client side as they are sent to the server side. On the
    server side, they are "unfixed" before they are consumed.

    The server side code to deal with this is in executor-service/helpers/util.py.
    """
    for data in data_array:
        for key, val in data.items():
