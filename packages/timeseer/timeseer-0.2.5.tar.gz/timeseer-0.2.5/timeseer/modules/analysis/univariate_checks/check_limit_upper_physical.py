"""Upper limit set in the metadata should not be exceeded.

<p>This check identifies whether the upper limit, if defined, is crossed
...
points where the corresponding value is higher than the given limit.
Imagine that 100 points are analyzed in a given time-frame
and there are 10 points whose value is above the given limit.
The score for this check in that case would be
90% = 1 - 10 / 100. Which means that 90% of all points lie below the limit.</p>
...
_CHECK_NAME = "Out-of-bounds (upper, physical)"
_EVENT_FRAME_NAME = "Out of bounds (upper, physical)"
...
def _get_active_points(df: pd.DataFrame, metadata: timeseer.Metadata):
    return df["value"] > metadata.get_field(fields.LimitHighPhysical)
...
def _is_valid_input(
    ...
    if analysis_input.metadata.get_field(fields.LimitHighPhysical) is None:
        return False
...
def run(analysis_input: timeseer.AnalysisInput) -> timeseer.AnalysisResult:
    ...
    pct_active, frames = _run_limit_check(
