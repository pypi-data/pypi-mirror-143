...
_CHECK_NAME = "Out-of-bounds (upper, physical, accuracy)"
_EVENT_FRAME_NAME = "Out of bounds (upper, physical, accuracy)"
...
def _get_active_points(df: pd.DataFrame, metadata: timeseer.Metadata):
    limit_high = metadata.get_field(fields.LimitHighPhysical)
...
def run(analysis_input: timeseer.AnalysisInput) -> timeseer.AnalysisResult:
    ...
    if not _is_input_valid(analysis_input, median_archival_step):
