...
def prescriptions(model, input_df, scored_df, actionable_fields, dv_description, field_labels=None, returned_insights=5):
    ...
    #Remove "by step_jump" and make to increase relative to current score
    change_score['insight'] = np.where(change_score['binary'] == False,
                                       change_score['direction']+" \'"+change_score['feature_label']+f"\' will increase {dv_description} by "+np.where(change_score['effect_pct'] > 1, np.round((change_score['effect_pct']+1),1).astype(str)+"%.", (change_score['effect_pct']*100).astype(int).astype(str)+"%."),
                                       "Changing the value of \'"+change_score['feature_label']+"\' to "+np.where(change_score['direction']=='Increasing', '\'Yes\'', '\'No\'')+f" will increase {dv_description} by "+np.where(change_score['effect_pct'] > 1, np.round((change_score['effect_pct']+1),1).astype(str)+"x.", (change_score['effect_pct']*100).astype(int).astype(str)+"%.")
                                       )     
