...
class InfluxDBConnection:
    ...
    def handle_timeout(self):
        self.maybe_print_stats(self.PRINT_TIME_INTERVAL)
...
class DBStats:
    SUMMARY_FORMAT_STR = "{: >10} msgs {: >10} bytes in {: >10} bytes out {: >10} Msg/s in {: >10} B/s in {: >10} B/s out"
    def __init__(self):
        self.messages_in = 0
        self.bytes_in = 0
        self.bytes_out = 0
        self.messages_in = self.messages_in
        self.bytes_in = self.bytes_in
        self.bytes_out = self.bytes_out
    
    def compute_stats(self, now, time_interval):
        ...
        return (self.messages_in, self.bytes_in, self.bytes_out, mips, bips, bops)

    def maybe_print_stats(self, time_interval):
        now = time.time()
        if now > self.last_time + time_interval:
            self.last_time = now
            self.compute_stats(now, time_interval)
            (mi, bi, bo, mips, bips, bops) = self.compute_stats(time_interval)
            print(SUMMARY_FORMAT_STR.format(mi, bi, bo, mips, bips, bops))
