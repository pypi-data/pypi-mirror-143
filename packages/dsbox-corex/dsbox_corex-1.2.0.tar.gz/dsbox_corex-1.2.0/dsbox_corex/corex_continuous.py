...
class CorexContinuous(UnsupervisedLearnerPrimitiveBase[Input, Output, CorexContinuous_Params, CorexContinuous_Hyperparams]):  #(Primitive):
    ...
    def __init__(self, *, hyperparams : CorexContinuous_Hyperparams, random_seed : int =  0) -> None: #, docker_containers: typing.Dict[str, DockerContainer] = None
    ...
    def _fit_transform(self, inputs : Input, timeout: float = None, iterations : int = None) -> Sequence[Output]:
        ...
        if not hasattr(self, 'model') or self.model is None:
            ...
            sys.stdout = null
            self.model = corex_cont.Corex(n_hidden= self.n_hidden, max_iter = self.max_iter, seed = self.random_seed)
            sys.stdout = _stdout
