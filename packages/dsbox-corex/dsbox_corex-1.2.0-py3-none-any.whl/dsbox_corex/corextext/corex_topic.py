...
class Corex(object):
    ...
    def __init__(self, n_hidden=2, max_iter=200, eps=1e-5, seed=None, verbose=False, count='binarize',
        ...
        self.tree = tree
        self.random_state = np.random.RandomState(seed)  # Set seed for deterministic results
    ...
    def label(self, p_y_given_x):
        ...
        return p_y_given_x #(p_y_given_x > 0.5).astype(bool)
    ...
    def fit_transform(self, X, anchors=None, anchor_strength=1, words=None):
        ...
        p_y_given_x = self.random_state.random((self.n_samples, self.n_hidden))
        if anchors is not None:
    ...
    def initialize_parameters(self, X, words):
        ...
        if self.n_hidden > 1:
            self.alpha = self.random_state.random((self.n_hidden, self.n_visible))
    ...
    def calculate_alpha(self, X, p_y_given_x, theta, log_p_y, tcs):
        ...
        if self.n_hidden == 1:
            ...
            for i in np.where((mis == maxmis).sum(axis=0))[0]:  # Break ties for the largest MI
                mis[:, i] += 1e-10 * self.random_state.random(self.n_hidden)
