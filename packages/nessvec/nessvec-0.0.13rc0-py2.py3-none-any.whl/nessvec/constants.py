...
HOME_DIR = Path.home()
SHARED_DATA_DIR_NAMES = ['.nlpia2-data', 'nessvec-data']
DATA_DIR_NAME = SHARED_DATA_DIR_NAMES[-1]
DATA_DIR = HOME_DIR / DATA_DIR_NAME
for DATA_DIR_NAME in SHARED_DATA_DIR_NAMES:
    DATA_DIR = HOME_DIR / DATA_DIR_NAME
    if DATA_DIR.is_dir():
        break
...
if not (DATA_DIR / 'constants' / 'LANG_ISO.csv').is_file():
    (DATA_DIR / 'constants').mkdir(parents=True, exist_ok=True)
    try:
        shutil.copytree(str(PACKAGE_DIR / 'data'), str(DATA_DIR), dirs_exist_ok=True)
