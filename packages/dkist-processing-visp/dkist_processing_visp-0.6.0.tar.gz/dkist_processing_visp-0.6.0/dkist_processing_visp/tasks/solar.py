...
class SolarCalibration(
    ...
    def run(self) -> None:
        ...
        for beam in range(1, self.constants.num_beams + 1):
            for modstate in range(1, self.constants.num_modstates + 1):
                ...
                with self.apm_processing_step(f"Reshifting characteristic spectra for {apm_str}"):
    ...
    def reshift_characteristic_spectra(
        ...
        Note: This function re-fits the shifts instead of using the pre-computed shifts, which seems a little weird, but is
        ...
        -------
        np.ndarray
            Re-shifted characteristic spectra array
        """
