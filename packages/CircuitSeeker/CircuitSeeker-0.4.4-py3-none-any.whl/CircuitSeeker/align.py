...
def distributed_piecewise_alignment_pipeline(
    ...
    **kwargs,
    ...
    ut.numpy_to_zarr(fix_p, zarr_blocks, fix_zarr_path)
    ut.numpy_to_zarr(mov_p, zarr_blocks, mov_zarr_path)
    if fix_mask is not None:
        ut.numpy_to_zarr(fm_p, zarr_blocks, fix_mask_zarr_path)
    if mov_mask is not None:
        ut.numpy_to_zarr(mm_p, zarr_blocks, mov_mask_zarr_path)

    # construct start/stop indices for each block (includes overlap)
    slc = tuple(slice(None, x, y) for x, y in zip(fix_p.shape, blocksize))
    starts = np.moveaxis(np.array(np.mgrid[slc]), 0, -1) - overlaps
    stops = starts + blocksize + 2 * overlaps
    starts = np.maximum(0, starts)  # no boundary overlap
    indices = np.concatenate((starts, stops), axis=-1).astype(int)
    indices_da = da.from_array(indices, chunks=(1,)*fix_p.ndim + (2*fix_p.ndim,))
    ...
    def align_single_block(coords, block_info=None):

        # squeeze the coords and convert to tuple of slices
        coords = coords.squeeze()
        d = len(coords) // 2
        coords = tuple(slice(x, y) for x, y in zip(coords[:d], coords[d:]))

        # read the chunks
        fix = zarr.open(fix_zarr_path, 'r')[coords]
        mov = zarr.open(mov_zarr_path, 'r')[coords]
        fix_mask, mov_mask = None, None
        if os.path.isdir(fix_mask_zarr_path):
            fix_mask = zarr.open(fix_mask_zarr_path, 'r')[coords]
        if os.path.isdir(mov_mask_zarr_path):
            mov_mask = zarr.open(mov_mask_zarr_path, 'r')[coords]
    ...
    # align all chunks
    fields = da.map_blocks(
        align_single_block, indices_da,
        dtype=object,
        drop_axis=[len(indices_da.shape)-1],
        chunks=[1,1,1],
