...
from ts_t1_validator.utils import \
    is_number, is_int
        ...
        if self.impression_pacing_type is not ImpressionPacingTypeEnum.UNDEFINED:
            ...
            if self.impression_pacing_type in [ImpressionPacingTypeEnum.ASAP,
                ...
                if self.input_type is InputTypeEnum.FIXED:
                    if not is_int(self.impression_pacing_amount):
                        raise ValidationException("for input_type='fixed' impression_pacing_amount must be an integer")
                elif self.input_type is InputTypeEnum.PERCENTAGE:
                    if not is_number(self.impression_pacing_amount):
                        raise ValidationException("for input_type='percentage' impression_pacing_amount must be a float")
        else:
            if self.impression_pacing_interval is not PacingIntervalEnum.UNDEFINED:
