...
def sntsmkf(pairs:list =[["She has ready.","She is ready."], ["It are ok.","It is ok."]], dskhost:str="dsk.jukuu.com", asdsk:bool=False , diffmerge:bool=False, use_cache=True):
	...
	try:
		...
		sntdic  = dict(pairs)
		input	= [ mkf_input(i,snts[i],sntdic[snts[i]], [t.text for t in doc], [t.text for t in spacy.getdoc(sntdic[snts[i]], use_cache) ], doc, diffmerge)  for i, doc in enumerate(docs)]
