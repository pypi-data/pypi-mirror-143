...
@app.get('/')
def home(): 
	return HTMLResponse(content=f"<h2>docfts,  dsk,mkf,bs, fts </h2><a href='/docs'> docs </a> | <a href='/redoc'> redoc </a><br>uvicorn uvirun:app --port 80 --host 0.0.0.0 --reload <br> <br> 2022.3.20")
...
'''
	db.conn.execute('insert	into dsk (title,	body) VALUES (?,?)', (title, body))
	did = db.conn.execute('SELECT LAST_INSERT_ROWID()').fetchone()[0]
'''