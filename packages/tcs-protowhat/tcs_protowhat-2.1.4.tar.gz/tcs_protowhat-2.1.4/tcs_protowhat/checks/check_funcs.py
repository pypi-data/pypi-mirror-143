...
from tcs_protowhat.Feedback import Feedback

MSG_CHECK_FALLBACK = "Ваше решение неверно. Пробовать снова!"
DEFAULT_MISSING_MSG = "Не удалось найти {index}{node_name}."
DEFAULT_APPEND_MSG = "Проверьте {index}{node_name}. "
...
def has_code(
    state,
    text,
    incorrect_msg="Проверьте {ast_path}. Ожидалось найти {text}.",
    fixed=False,
):
...
def has_equal_ast(
    ...
    if not has_custom_message:
        if should_append_msg:  # Remove the ast_path mention because we are prepending.
            ...
            incorrect_msg = "Проверьте {ast_path}. {extra}"
    ...
    _msg = incorrect_msg.format(
        ...
        extra="Контестер ожидал найти `{}`.".format(sol_str)
        if sol_str
