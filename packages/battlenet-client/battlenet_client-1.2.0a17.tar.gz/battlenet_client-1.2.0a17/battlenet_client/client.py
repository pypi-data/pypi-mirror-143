...
class BattleNetClient(OAuth2Session):
    ...
    def get(self, uri, locale, namespace, **kwargs):
        ...
        kwargs['params'] = {'locale': localize(locale), 'namespace': namespace}
    ...
    def post(self, uri, locale, namespace, **kwargs):
        ...
        for _ in range(5):
            try:
                raw_data = super().post(url, headers=headers, params=params)
                raw_data.raise_for_status()
            except requests.Timeout:
                sleep(2.5)
            except requests.exceptions.HTTPError as error:
