...
class Cache:
    def __init__(self, host=None, port=None, user=None, passwd=None, db_name=None):
        ...
        if not passwd:
            passwd = config('CACHE_PASS')

        if not db_name:
            db_name = config('CACHE_DB')

        try:
            self._db_client = connect(host=host, port=port, user=user, password=passwd, dbname=db_name)
        except OperationalError as error:
            print(f"Error: {error}")
        else:
            self._cursor = self._db_client.cursor(cursor_factory=extras.DictCursor)
    ...
    def insert_cache(self, game, release, api_name, category, api_id, data, locale):
        ...
        insert_query = '''INSERT INTO cache (category, locale, cache, api_id, last_updated) SELECT
            (categories.id, %s, %s, %s, NOW()) FROM
