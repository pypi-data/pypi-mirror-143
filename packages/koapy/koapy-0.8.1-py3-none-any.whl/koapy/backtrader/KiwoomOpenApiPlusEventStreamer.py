...
class KiwoomOpenApiPlusPriceEventChannel(Logging):
    ...
    def initialize(self):
        ...
        request.initialize_request.fid_list.extend(
            self._fid_list
    ...
    def register_code(self, code):
        ...
        request.register_request.code_list.extend(
            code_list
        )  # pylint: disable=no-member
    ...
    def is_chejan_response(self, response):
        ...
        if name == "OnReceiveChejanData" and gubun == "0":
            ...
            limit_order_created = (
                ...
                and status in ["접수"]
                and data["819"] in ["0"]
            )  # 취소 확인 뒤에 오는 원주문 이벤트는 819 가 1 로 들어오는 것 같음
        ...
        with self._lock:
            if self._stub not in self._price_event_channels_by_stub:
                self._price_event_channels_by_stub[
                    self._stub
                ] = KiwoomOpenApiPlusPriceEventChannel(self._stub)
