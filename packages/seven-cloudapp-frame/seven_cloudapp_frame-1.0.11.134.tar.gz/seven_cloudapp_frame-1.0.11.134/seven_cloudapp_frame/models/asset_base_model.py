...
@LastEditTime: 2022-02-28 11:52:22
...
class AssetBaseModel():
    ...
    def _delete_asset_dependency_key(self, act_id, user_id, delay_delete_time=0.1):
        """
        :description: 删除资产依赖建
        :param act_id: 活动标识
        :param user_id: 用户标识
    ...
    def update_user_asset(self, app_id, act_id, module_id, user_id, open_id, user_nick, asset_type, asset_value, asset_object_id, source_type, source_object_id, source_object_name, log_title, only_id="",handler_name="",request_code="", info_json={}):
        ...
        user_asset_model = UserAssetModel(db_transaction=db_transaction, sub_table=SevenHelper.get_sub_table(act_id, sub_table_config.get("user_asset_num", 0)), context=self.context)
        asset_log_model = AssetLogModel(db_transaction=db_transaction, sub_table=SevenHelper.get_sub_table(act_id, sub_table_config.get("asset_log_num", 0)), context=self.context)
        asset_only_model = AssetOnlyModel(db_transaction=db_transaction, sub_table=SevenHelper.get_sub_table(act_id, sub_table_config.get("asset_only_num", 0)), context=self.context)
    ...
    def get_user_asset_list(self, app_id, act_id, user_ids, asset_type=0):
        ...
        sub_table_config = config.get_value("sub_table_config",{})
        user_asset_model = UserAssetModel(sub_table=SevenHelper.get_sub_table(act_id, sub_table_config.get("user_asset_num", 0)), context=self.context)
    ...
    def get_asset_log_list(self, app_id, act_id, asset_type, page_size=20, page_index=0, user_id=0, asset_object_id="", start_date="", end_date="", user_nick="", open_id="", source_type=0, source_object_id=None, field="*", is_cache=True, operate_type=-1):
        ...
        sub_table_config = config.get_value("sub_table_config", {})
        asset_log_model = AssetLogModel(sub_table=SevenHelper.get_sub_table(act_id, sub_table_config.get("asset_log_num", 0)), context=self.context)
        if is_cache:
