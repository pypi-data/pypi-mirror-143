...
class IDSMonitorThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while self.running:
            ...
            logging.info(f"Reading IDS data")
            alert_counters = IdsManagerUtil.read_ids_data(self.latest_ts)
            logging.info(f"Reading IDS data done")
            record = alert_counters.to_kafka_record(ip = self.ip)
            logging.info(f"Pushing IDS data to Kafka: {record}")
            self.producer.produce("ids_log", record)
            logging.info(f"Pushed to kafka successfully")
            self.latest_ts = time.time()
