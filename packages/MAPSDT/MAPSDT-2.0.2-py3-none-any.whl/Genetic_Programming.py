...
def GPFE(continuousAttr, attribute, config):
    ...
    def init_population(_init_size, _continuousAttr, _attribute, _data):
        ...
        while True:
            _chromosome_list, operation = [], ['+', '-', '*', '/']
            chromosome_in_population = 20
            while len(_chromosome_list) != chromosome_in_population:
                _chr = data_load.Chromosome()
                selectATTR = np.random.choice(_continuousAttr, 4, replace=True)
                selectOP = np.random.choice(operation, 3, replace=True)
                _chr.string.extend(selectATTR[:2])
                _chr.string.extend(selectOP[0])
                _chr.string.extend(selectATTR[2:4])
                _chr.string.extend(selectOP[1:])
                _chr.combine()
    ...
    def crossover_mutate(_population_list, _max_generation, forest, _data, test_data, _continuousAttr, config):
        ...
        while generation <= _max_generation:
            ...
            for i in range(int(len(parent)/2)):
                ...
                child1.population = parent[i*2].population.population[:int(chromosome_in_population/2)] + parent[i*2+1].population.population[int(chromosome_in_population/2):]
                child2.population = parent[i*2+1].population.population[:int(chromosome_in_population/2)] + parent[i*2].population.population[int(chromosome_in_population/2):]
            ...
            for _population in new_parent:
                for i in range(int(len(_population.population)/2)):
                    ...
                    if mutate_probability > 0.95:
                        _operation = ['+', '-', '*', '/']
                        while True:
                            selectOP = np.random.choice(_operation, 1, replace=True)[0]
                            ...
                            _population.population[i*2].string[2] = selectOP[0]
                            _population.population[i*2].combine()
                            try:
                                list(filter(lambda obj: eval(_population.population[i*2].expression), _data))
                                break
                            except:
                                continue
                    else:
                    ...
                    if mutate_probability > 0.95:
                        ...
                        while True:
                            ...
                            _population.population[i*2+1].string[2] = selectOP[0]
                            _population.population[i*2+1].combine()
                            try:
                                list(filter(lambda obj: eval(_population.population[i*2+1].expression), _data))
                                break
                            except:
                                continue
                    else:
            ...
            if config['best_tree'].fitness_value <= sorted(forest, key=lambda x: x.fitness_value, reverse=True)[0].fitness_value:
