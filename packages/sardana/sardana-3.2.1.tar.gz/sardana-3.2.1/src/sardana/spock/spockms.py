...
def _is_h5_file_valid(fname):
    ...
    try:
        ...
        try:
            h5py.File(fname, "r", libver='latest', swmr=True)
        except IOError as e:
            is_valid = False
    return is_valid
...
class GUIViewer(BaseGUIViewer):
    ...
    def show_scan(self, scan_nb=None, scan_history_info=None, directory_map=None):
            ...
            for scan in reversed(scan_history_info):
                ...
                if not isinstance(scan_file, str):
                    ...
                    for fname in scan_files:
                        if _is_h5_file_valid(os.path.join(scan_dir, fname)):
                            scan_file = fname
                            break
                    if scan_file is None:
                        print("Cannot plot scan:")
                        print("ScanFile is not HDF5 or can not be opened")
                        return
                else:
                    if not _is_h5_file_valid(os.path.join(scan_dir, scan_file)):
                        print("Cannot plot scan:")
                        print("ScanFile is not HDF5 or can not be opened")
                        return
                break
        ...
        try:
            ...
            title = file_model.getNodeFromIndex(title_index)[()]
            try:
                title = title.decode()
            except AttributeError:
                # in h5py < 3 this is already a str
                pass
            windowTitle += " - " + title
