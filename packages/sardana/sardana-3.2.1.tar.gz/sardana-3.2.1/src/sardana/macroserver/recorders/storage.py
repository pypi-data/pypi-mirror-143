...
class SPEC_FileRecorder(BaseFileRecorder):
    ...
    def _addCustomData(self, value, name, spec_custom_fmt=None, **kwargs):
        '''
        The custom data will be added as a comment line in the format::

        #UVAR name value

        Optinally ``spec_custom_fmt`` *kwarg* can change this format to something else.
        ...
        if spec_custom_fmt is None:
            ...
            spec_custom_fmt = getattr(sardanacustomsettings,
                                      'SPEC_CUSTOM_DATA_FORMAT',
                                      '#UVAR {name} {value}\n')

        if not spec_custom_fmt.endswith('\n'):
            spec_custom_fmt += '\n'
