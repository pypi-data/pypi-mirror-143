...
from localstack.aws.api.ec2 import(CreateImageRequest,CreateImageResult,DescribeImagesRequest,DescribeImagesResult,DescribeInstancesRequest,DescribeInstancesResult,Image,ImageState,ImageTypeValues,Instance,InstanceState,InstanceStateChange,InstanceStateName,InstanceType,Reservation,RunInstancesRequest,StartInstancesRequest,StartInstancesResult,StopInstancesRequest,StopInstancesResult,TerminateInstancesRequest,TerminateInstancesResult)
...
class DockerVmManager(VmManager):
 ...
 def container_name_from_instance_id(instance_id:str,verify:bool=True)->str:
  ...
  valid_containers=[_["name"]for _ in DOCKER_CLIENT.list_containers()]
  if verify and container_name not in valid_containers:
 ...
 def instance_state(self,instance_id:str)->InstanceState:
  container_name=self.container_name_from_instance_id(instance_id)
 ...
 def run_instances(self,context:RequestContext,run_instances_request:RunInstancesRequest)->Reservation:
  ...
  count=run_instances_request.get("MaxCount",1)
  user_data=run_instances_request.get("UserData")
  docker_image=self.image_from_ami_id(ami_id)
  image_obj=DOCKER_CLIENT.inspect_image(docker_image,pull=False)
  ...
  for launch_index in range(count):
   instance_id=f"i-{short_uid()}"
   ...
   try:
    ...
    instances.append(Instance(InstanceId=instance_id,ImageId=ami_id,State=self.instance_state(instance_id),Architecture=image_obj["Architecture"],PlatformDetails=image_obj["Os"],LaunchTime=datetime.datetime.utcnow().isoformat(),AmiLaunchIndex=launch_index,InstanceType=InstanceType.t2_micro))
  ...
  return Reservation(Groups=[],Instances=instances,ReservationId=f"r-{short_uid()}")
 def describe_instances(self,context:RequestContext,describe_instances_request:DescribeInstancesRequest)->DescribeInstancesResult:
  filter_instance_ids=describe_instances_request.get("InstanceIds",[])
  filter_container_names=[self.container_name_from_instance_id(_)for _ in filter_instance_ids]
  container_names=[_["name"]for _ in DOCKER_CLIENT.list_containers()if _["name"].startswith(DOCKER_PREFIX)]
  instances:List[Instance]=[]
  for container_name in container_names:
   if filter_container_names and container_name not in filter_container_names:
    continue
   instance_id=self.instance_id_from_container_name(container_name)
   container_obj=DOCKER_CLIENT.inspect_container(container_name)
   image_id=container_obj.get("Image")
   image_obj=DOCKER_CLIENT.inspect_image(image_id,pull=False)
   image=next(filter(lambda x:x.startswith(DOCKER_PREFIX),image_obj["RepoTags"]))
   instances.append(Instance(InstanceId=instance_id,ImageId=self.ami_id_from_image(image),State=self.instance_state(instance_id),Architecture=image_obj["Architecture"],PlatformDetails=container_obj["Platform"],LaunchTime=container_obj["Created"],AmiLaunchIndex=0,InstanceType=InstanceType.t2_micro))
  return DescribeInstancesResult(Reservations=[Reservation(Groups=[],Instances=instances,ReservationId=f"r-{short_uid()}")])
 ...
 def describe_images(self,context:RequestContext,describe_images_request:DescribeImagesRequest)->DescribeImagesResult:
  filter_ami_ids=describe_images_request.get("ImageIds",[])
  filter_docker_images=[self.image_from_ami_id(ami_id,verify=True)for ami_id in filter_ami_ids]
  images=[]
  for docker_image in DOCKER_CLIENT.get_docker_image_names():
   if docker_image.startswith(DOCKER_PREFIX):
    ...
    if filter_docker_images and docker_image not in filter_docker_images:
     continue
    images.append(Image(ImageId=self.ami_id_from_image(docker_image),Name=self.ami_name_from_image(docker_image),Architecture=image_obj.get("Architecture"),CreationDate=image_obj.get("Created"),ImageType=ImageTypeValues.machine,Public=True,State=ImageState.available,Hypervisor="docker"))
  return DescribeImagesResult(Images=images)
