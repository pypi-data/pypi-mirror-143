...
from .helpers import api_collection_id, uuid, content_hash, get_in
...
class MetabaseIO:
    ...
    def export_json(self, collection):
        ...
        items = self.get_snippets(source["id"], items) + items
        result = {
            "items": items,
        }
    ...
    def get_snippets(self, collection_id, items):
        result = []
        for snip_id in self.get_snippet_ids(items):
            snippet = self.client.get("native-query-snippet", snip_id)
            snippet = Trimmer.trim_data(snippet, Trimmer.keep_snippet_keys)
    ...
    def add_items(
        ...
        if only_model == "all":
            ...
            self.add_items(items, collection_id, mapper, overwrite, "snippet", result)
            ...
            for item in items:
                ...
                elif item["model"] == "snippet" and only_model == "snippet":
                    if item in mapper and not mapper.has_changed(item):
                        log_item_io("⏭️", item)
                        result.append(item)
                    else:
                        ...
                        if item in mapper:
                            item["id"] = mapper[item]
                            upserted_snippet = self.client.upsert_snippet(item)
                        else:
                            upserted_snippet = self.client.add_snippet(item)
                            mapper[item] = upserted_snippet["id"]

                        result.append(upserted_snippet)
    ...
    def get_snippet_ids(self, items, result=None):
        """
        Search items for cards (questions), recursively scanning collections, and
        return a set of snippet ids the cards are using.
        """
        if result is None:
            result = set()
        ...
        for item in items:
            if item["model"] == "collection":
                self.get_snippet_ids(item["items"], result)
            if item["model"] == "card":
                tags = get_in(item, ["dataset_query", "native", "template-tags"])
                if tags:
                    for id in [tag['snippet-id'] for tag in tags.values() if tag['type'] == 'snippet']:
                        result.add(id)
        return result
        ...
        for table in db["tables"]:
            ...
            for field in table["fields"]:
                ...
                if (
                    ...
                    f_field["values"] = self.client.get("field", field["id"], "values", [table, field])[
                        "values"
                    ]
...
class Mapper:
    ...
    def versioned_mapping(self, item):
        return {
            "name": item["name"],
            "uuid": uuid(item),
    ...
    def add_items(self, items, datamodel, result=None):
        ...
        for item in items:
            result[item["model"] + "s"][item["id"]] = self.versioned_mapping(item)
            if item["model"] == "collection":
        ...
        if isinstance(expression, list):
            if len(expression) == 2 and expression[0] == "field-id":
                ...
                if segment_id not in mappings["segments"]:
                    if segment_id in datamodel["segments"]:
                        segment = datamodel["segments"][segment_id]
                        mappings["segments"][segment_id] = self.versioned_mapping(segment)
                    else:
                        metabase_io_log.error("Could not find segment id {} in data model. It is needed by expression: {}".format(segment_id, expression))
...
#                         if tag['type'] == 'snippet':
#                             # Here you could process the card to snippet reference on export.
#                             # Now it's not necessary as we export all snippets earlier, and the id mapping is handled by Mapper class.
    ...
    def existing_items(self, collection_id):
        ...
        dest_items["snippets"] = self.client.get("native-query-snippet")

        for model in ["collections", "cards", "dashboards", "segments", "snippets"]:
    ...
    def __getitem__(self, item):
        try:
            ...
            raise KeyError("No item at key self.mappings['{}s'][{}]".format(item['model'], item['id'])) from error
    ...
    def deref_table(self, table_id):
        if str(table_id).startswith("card__"):
            return "card__" + str(self.mappings["cards"][int(table_id[6:])])
        else:
    ...
    def deref_json_key(self, key):
        ...
        # The separator must be "," not ", " so metabase considers such key!
        # Bad:
        # "[\"name\", \"action_rate\"]": {....}
        # Good:
        # "[\"name\",\"click_rate\"]": {....}
        return json.dumps(keyobj, separators=(',', ': '))
    ...
    def deref_column_settings(self, col_settings, for_card):
        if col_settings is not None and "click_behavior" in col_settings:
            ...
            if click_behavior["type"] == "link":
                ...
                if click_behavior["linkType"] != "url":
                    if click_behavior["targetId"] in self.mappings[objectType]:
                        ...
                        metabase_io_log.error("Could not map {} {}, try to re-import".format(objectType, click_behavior["targetId"]))
    ...
    def deref_snippet(self, snippet_tag):
        snippet_tag['snippet-id'] = self[{"model": "snippet", "id": snippet_tag["snippet-id"]}]
...
def broken_dashboards(items, broken=set(), source_items=None):
    ...
    for dash in filter(lambda i: i["model"] == "dashboard", items):
        for card in dash["ordered_cards"]:
            if is_virtual_card(card):
                continue
            if find_card(source_items, card["card_id"]) is None:
                broken.add((dash["id"], dash["name"], card["card_id"]))

    for col in filter(lambda i: i["model"] == "collection", items):
        broken_dashboards(col["items"], broken, source_items)
...
class Trimmer:
    ...
    keep_snippet_keys = ["id", "name", "description", "content"]
    ...
    def trim_data(item, keep_keys=None):
        ...
        native_query = get_in(trimmed, ["dataset_query", "native", "query"])
        if native_query:
            trimmed["dataset_query"]["native"]["query"] = re.sub(r' +\n', "\n", native_query, re.MULTILINE)
        return trimmed
