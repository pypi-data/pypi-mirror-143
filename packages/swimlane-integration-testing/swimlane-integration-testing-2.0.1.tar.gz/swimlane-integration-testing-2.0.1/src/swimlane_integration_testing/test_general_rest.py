...
def get_test_config(task, test):
    ...
    """
    file_path = f'{DATA_DIR}/{task}/{test}/config.json'
    config = {}
...
class TestStandardRest:
    ...
    def test_kwargs(self, *args, **kwargs):
        ...
        """
        schema = json.loads(open(f'{DATA_DIR}/{self.task}/schemas/kwargs.json').read())
        validate(
    ...
    def test_session_headers(self, test, *args, **kwargs):
        ...
        if not schema:
            ...
            schema = json.loads(open(f'{DATA_DIR}/headers.json').read())
            if not schema:
                raise ValueError(
