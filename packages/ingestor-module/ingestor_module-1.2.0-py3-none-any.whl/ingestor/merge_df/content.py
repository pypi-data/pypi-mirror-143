...
class FinalDfController:
    ...
    def build_content_profile_final_df(df_content, df_country, df_actor, df_tag, df_homepage, df_content_core,
        ...
        if not pd.isna(df_content[CONTENT_ID][0]):
            content_df = CommonUtils.fetch_prepare_content_id(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_created_on(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_title(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_synopsis(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_synopsis_en(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_type(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_status(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_year(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_status(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_rating(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_is_free(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_is_original(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_is_exclusive(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_is_branded(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_is_geo_block(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_duration(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_start_date(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_end_date(content_df, df_content)
            content_df = CommonUtils.fetch_prepare_modified_on(content_df, df_content)

            content_df = CommonUtils.fetch_prepare_country(content_df, df_country)
