...
class DatabaseHandler:
    ...
    def read_events(self):
        try:
            ...
            self.db_trk["start_time"] = pd.to_datetime(self.db_trk["start_time"], errors='ignore', format =  "%Y-%m-%d %H:%M:%S" )
            self.db_trk["end_time"] = pd.to_datetime(self.db_trk["end_time"],errors='ignore', format =  "%Y-%m-%d %H:%M:%S" )
            #print(self.db_trk.info())
            return(self.db_trk)
