...
@app.command()
def switch(db_path: str = typer.Option(
        str(database.DEFAULT_DB_FILE_PATH),
        "--db-path",
        "-db",
        prompt="time tracker database location?"
    ...
    typer.echo(f"Default database was switched to: {db_path}")
...
@app.command()
def summary(column_names: List[str], today: bool =  typer.Option(False, "--today", "-t", help = "filter for today's date")
    ...
    summary = tracker.summary(ls_column_names, today)
    typer.echo("\n")
