...
class RegistrUser(QDialog):
    ...
    def __init__(self, database, server):
        ...
        self.setFixedSize(200, 183)
        self.setModal(True)
        self.setAttribute(Qt.WA_DeleteOnClose)

        self.setAutoFillBackground(True)
        palette = self.palette()
        palette.setColor(self.backgroundRole(), QColor(215, 220, 255))
        self.setPalette(palette)

        self.label_username = QLabel('Введите имя пользователя:', self)
        self.label_username.move(10, 10)
        self.label_username.setFixedSize(180, 15)
    ...
    def save_data(self):
        '''
        Проверка правильности ввода и сохранение в базу нового пользователя.
        '''
        if not self.client_name.text():
            ...
            # Генерируем хэш пароля, в качестве соли будем используем логин в
            # нижнем регистре.
