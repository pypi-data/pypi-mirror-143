...
def test_date_interval_trees_finds_the_right_interval(data, intervals):
    ...
    interval = Range.new_open_right(picked_cell.lowerbound, picked_cell.upperbound)
...
@given(st.data(), many_float_ranges(5, outer=FLOAT_OUTER_RANGE))
def test_float_interval_trees_finds_the_right_interval_issue3_regression(data, intervals):
    cells = [Cell.from_range(r, i) for i, r in enumerate(intervals)]
...
@given(
    st.data(),
    st.lists(
        date_ranges(outer=MORE_THAN_ONE_LIFETIME, kinds=st.just(Range.new_open_right)),
        min_size=1,
        max_size=10,
    ),
)
