...
class ModelLine(Line):  # pragma: no cover
    ...
    def __getstate__(self):
        if self.instances:
            envdata = {EnvironmentData.from_instance(instance) for instance in self.instances}
            assert len(envdata) == 1, envdata
            env = tuple(envdata)[0]
            state = {
                "pos": self.pos,
                "instances": [
                    ModelInstance(env, instance._name, instance.id) for instance in self.instances
                ],
            }
            return state
        else:
    ...
    def _resolve(self):
        if len(self):
            ...
            if isinstance(result, ModelInstance):
                ...
                self.instances = tuple(env[m._name].browse(m.id) for m in self.instances)
...
class CyclicInteger(int):
    ...
    def __new__(cls, x, l):
        if l <= 0:
            raise TypeError("Cyclic integers are only defined over strictly positive modulus.")
