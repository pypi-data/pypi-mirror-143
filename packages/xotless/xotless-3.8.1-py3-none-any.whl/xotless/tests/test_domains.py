...
kinds_with_samples = st.sampled_from((Range.new_open_left, Range.new_open_right, Range.new_closed))


@st.composite
def _sorted_points(draw, many=2):
    assert many >= 2
    points = list(sorted(draw(st.sets(st.integers(), min_size=many, max_size=many))))
    if draw(st.booleans()):
        points[0] = -Infinity
    if draw(st.booleans()):
        points[-1] = Infinity
...
def test_range_intersection_with_a_common_Excluded_boundary(points, Bound1, Bound2):
    a, b, c = sorted(points)
...
def test_float_ranges_generation_regression_job_117109(data):
    _test_float_ranges_generation(data, Range.new_open(0, 1), kinds=st.just(Range.new_closed))
...
class TestRange(DomainCase):
    ...
    def add(left, right):  # pragma: no cover
        if left and right:
            assert right.lowerbound in left or right.lowerbound == left.upperbound
            return Range.new_open_right(left.lowerbound, max(left.upperbound, right.upperbound))
        elif left:
            return left
        else:
            return right
    ...
    @given(float_ranges(kinds=kinds_with_samples) | date_ranges(kinds=kinds_with_samples))
    def test_sample_in_range(self, range_):
        self.assertIn(range_.sample, range_, f"Sample {range_.sample} is not in {range_}")

    @given(float_ranges() | date_ranges())
    def test_range_kinds(self, range_):
        if isinstance(range_._lower, Included) and isinstance(range_._upper, Excluded):
            self.assertTrue(range_.kind == RangeKind.OPEN_RIGHT)
        elif isinstance(range_._lower, Included) and isinstance(range_._upper, Included):
            self.assertTrue(range_.kind == RangeKind.CLOSED)
        elif isinstance(range_._lower, Excluded) and isinstance(range_._upper, Excluded):
            self.assertTrue(range_.kind == RangeKind.OPEN)
        elif isinstance(range_._lower, Excluded) and isinstance(range_._upper, Included):
...
class IntervalTest(DomainCase):
    ...
    @given(st.lists(float_ranges(), max_size=10) | st.lists(date_ranges(), max_size=10))
    ...
    def test_float_interval_diff_produces_disjoint_ranges_non_empty_diff(self, r1, data):
        last_range = r1[-1]
    ...
    def _test_interval_diff_produces_sorted_ranges(self, r1, r2):
        ...
        i2 = IntervalSet(r2)
        idiff = i1.diff(i2)
        assert idiff.ranges == tuple(sorted(idiff.ranges, key=operator.attrgetter("lowerbound")))
    ...
    def test_date_interval_diff_produces_sorted_ranges_non_emptydiff(self, r1, data):
        last_range = r1[-1]
    ...
    def _test_interval_diff_produces_non_toucheing_ranges(self, r1, r2):
        ...
        i2 = IntervalSet(r2)
        idiff = i1.diff(i2)
        assert not idiff.ranges or idiff.ranges == idiff.ranges[0].join(*idiff.ranges[1:])
    ...
    @settings(deadline=None)
    @given(st.lists(float_ranges(), min_size=2, max_size=10), st.data())
    def test_float_interval_diff_produces_non_toucheing_ranges_non_emptydiff(self, r1, data):
        last_range = r1[-1]
        next_range = _build_next_range(last_range, 1, data)
        r2 = r1 + [next_range]
        self._test_interval_diff_produces_non_toucheing_ranges(r1, r2)
