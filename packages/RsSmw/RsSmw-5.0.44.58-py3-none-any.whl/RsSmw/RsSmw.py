...
class RsSmw:
	"""12153 total commands, 27 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMM/SMA/SMCV, SupportedIdnPatterns = SMW/SMBV/SMM/SMA/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMW200A,100001,5.0.44.0058'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '5.0.44.0058'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.0.44.0058'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmw version failed. Current version: '5.0.44.0058', minimum required version: '{min_version}'")
	...
	def bert(self):
		...
		if not hasattr(self, '_bert'):
			...
			self._bert = Bert(self._core, self._cmd_group)
		return self._bert
	...
	def bler(self):
		...
		if not hasattr(self, '_bler'):
			...
			self._bler = Bler(self._core, self._cmd_group)
		return self._bler

	@property
	def calibration(self):
		"""calibration commands group. 10 Sub-classes, 2 commands."""
	...
	@property
	def device(self):
		"""device commands group. 1 Sub-classes, 4 commands."""
	...
	def instruments(self):
		"""instruments commands group. 6 Sub-classes, 5 commands."""
		if not hasattr(self, '_instruments'):
			from .Implementations.Instruments import Instruments
			self._instruments = Instruments(self._core, self._cmd_group)
	...
	def sconfiguration(self):
		"""sconfiguration commands group. 7 Sub-classes, 5 commands."""
	...
	def service(self):
		...
		if not hasattr(self, '_service'):
			from .Implementations.Service import Service
	...
	@property
	def source(self):
		"""source commands group. 29 Sub-classes, 1 commands."""
	...
	@property
	def system(self):
		"""system commands group. 34 Sub-classes, 30 commands."""
	...
	@property
	def test(self):
		"""test commands group. 13 Sub-classes, 5 commands."""
