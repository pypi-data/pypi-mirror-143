...
class Data:
	...
	def get_dselection(self) -> str:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBT:[HP]:DATA:DSELection \n
		Snippet: value: str = driver.source.bb.dvb.dvbt.hp.data.get_dselection() \n
		...
		*.gts, *.ts, or *.trp. Refer to 'Accessing Files in the Default or Specified Directory' for general information on file
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:DVB:DVBT:HP:DATA:DSELection?')
		return trim_str_response(response)

	def set_dselection(self, dselection: str) -> None:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBT:[HP]:DATA:DSELection \n
		Snippet: driver.source.bb.dvb.dvbt.hp.data.set_dselection(dselection = '1') \n
		Selects an existing TS file from the default directory or from the specific directory. TS files are files with extension
		*.gts, *.ts, or *.trp. Refer to 'Accessing Files in the Default or Specified Directory' for general information on file
		handling in the default and in a specific directory. \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DVB:DVBT:HP:DATA:DSELection {param}')

	# noinspection PyTypeChecker
	def get_value(self) -> enums.DvbDataSour:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBT:[HP]:DATA \n
		Snippet: value: enums.DvbDataSour = driver.source.bb.dvb.dvbt.hp.data.get_value() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:DVB:DVBT:HP:DATA?')
		return Conversions.str_to_scalar_enum(response, enums.DvbDataSour)

	def set_value(self, data: enums.DvbDataSour) -> None:
		"""SCPI: [SOURce<HW>]:BB:DVB:DVBT:[HP]:DATA \n
		Snippet: driver.source.bb.dvb.dvbt.hp.data.set_value(data = enums.DvbDataSour.DLISt) \n
		Selects the data source to be used. \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DVB:DVBT:HP:DATA {param}')
