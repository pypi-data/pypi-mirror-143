...
class Zpt:
	...
	def get(self, cellNull=repcap.CellNull.Default, indexNull=repcap.IndexNull.Default) -> List[str]:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:CSIS:[CELL<CH0>]:[ZPRS<ST0>]:ZPT \n
		Snippet: value: List[str] = driver.source.bb.v5G.downlink.csis.cell.zprs.zpt.get(cellNull = repcap.CellNull.Default, indexNull = repcap.IndexNull.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:V5G:DL:CSIS:CELL{cellNull_cmd_val}:ZPRS{indexNull_cmd_val}:ZPT?')
