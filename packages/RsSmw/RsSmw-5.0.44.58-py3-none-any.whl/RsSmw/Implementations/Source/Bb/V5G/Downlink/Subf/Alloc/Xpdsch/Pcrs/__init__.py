...
class Pcrs:
	...
	@property
	def apConf(self):
		"""apConf commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_apConf'):
			from .ApConf import ApConf
			self._apConf = ApConf(self._core, self._cmd_group)
		return self._apConf
	...
	def nidpcrs(self):
		...
		if not hasattr(self, '_nidpcrs'):
			from .Nidpcrs import Nidpcrs
			self._nidpcrs = Nidpcrs(self._core, self._cmd_group)
	...
	def rpower(self):
		...
		if not hasattr(self, '_rpower'):
			from .Rpower import Rpower
			self._rpower = Rpower(self._core, self._cmd_group)
		return self._rpower
	...
	def clone(self) -> 'Pcrs':
		...
		new_group = Pcrs(self._core, self._cmd_group.parent)
