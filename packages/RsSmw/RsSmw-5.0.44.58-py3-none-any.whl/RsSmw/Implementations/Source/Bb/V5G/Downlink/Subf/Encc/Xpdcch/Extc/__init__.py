...
class Extc:
	"""Extc commands group definition. 51 total commands, 9 Subgroups, 2 group commands"""
	...
	@property
	def item(self):
		"""item commands group. 11 Sub-classes, 0 commands."""
	...
	def sitem(self):
		...
		if not hasattr(self, '_sitem'):
			from .Sitem import Sitem
			self._sitem = Sitem(self._core, self._cmd_group)
		return self._sitem
	...
	def solve(self):
		...
		if not hasattr(self, '_solve'):
			...
			self._solve = Solve(self._core, self._cmd_group)
		return self._solve
	...
	def uitems(self):
		...
		if not hasattr(self, '_uitems'):
			from .Uitems import Uitems
			self._uitems = Uitems(self._core, self._cmd_group)
		return self._uitems
	...
	def delete(self, subframeNull=repcap.SubframeNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:[SUBF<ST0>]:ENCC:XPDCch:EXTC:DELete \n
		Snippet: driver.source.bb.v5G.downlink.subf.encc.xpdcch.extc.delete(subframeNull = repcap.SubframeNull.Default) \n
		Deletes the selected row. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
		"""
		subframeNull_cmd_val = self._cmd_group.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		self._core.io.write(f'SOURce<HwInstance>:BB:V5G:DL:SUBF{subframeNull_cmd_val}:ENCC:XPDCch:EXTC:DELete')

	def delete_with_opc(self, subframeNull=repcap.SubframeNull.Default, opc_timeout_ms: int = -1) -> None:
		subframeNull_cmd_val = self._cmd_group.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:[SUBF<ST0>]:ENCC:XPDCch:EXTC:DELete \n
		Snippet: driver.source.bb.v5G.downlink.subf.encc.xpdcch.extc.delete_with_opc(subframeNull = repcap.SubframeNull.Default) \n
		Deletes the selected row. \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:V5G:DL:SUBF{subframeNull_cmd_val}:ENCC:XPDCch:EXTC:DELete', opc_timeout_ms)

	def reset(self, subframeNull=repcap.SubframeNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:[SUBF<ST0>]:ENCC:XPDCch:EXTC:RESet \n
		Snippet: driver.source.bb.v5G.downlink.subf.encc.xpdcch.extc.reset(subframeNull = repcap.SubframeNull.Default) \n
		Resets the table. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
		"""
		subframeNull_cmd_val = self._cmd_group.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		self._core.io.write(f'SOURce<HwInstance>:BB:V5G:DL:SUBF{subframeNull_cmd_val}:ENCC:XPDCch:EXTC:RESet')

	def reset_with_opc(self, subframeNull=repcap.SubframeNull.Default, opc_timeout_ms: int = -1) -> None:
		subframeNull_cmd_val = self._cmd_group.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:[SUBF<ST0>]:ENCC:XPDCch:EXTC:RESet \n
		Snippet: driver.source.bb.v5G.downlink.subf.encc.xpdcch.extc.reset_with_opc(subframeNull = repcap.SubframeNull.Default) \n
		Resets the table. \n
		Same as reset, but waits for the operation to complete before continuing further. Use the RsSmw.utilities.opc_timeout_set() to set the timeout value. \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Subf')
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:V5G:DL:SUBF{subframeNull_cmd_val}:ENCC:XPDCch:EXTC:RESet', opc_timeout_ms)

	def clone(self) -> 'Extc':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Extc(self._core, self._cmd_group.parent)
