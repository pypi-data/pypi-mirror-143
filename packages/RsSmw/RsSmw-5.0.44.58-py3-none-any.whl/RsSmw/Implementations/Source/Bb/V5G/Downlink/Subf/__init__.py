...
class Subf:
	"""Subf commands group definition. 123 total commands, 5 Subgroups, 0 group commands
	...
	def alCount(self):
		...
		if not hasattr(self, '_alCount'):
			from .AlCount import AlCount
			self._alCount = AlCount(self._core, self._cmd_group)
		return self._alCount

	@property
	def alloc(self):
		"""alloc commands group. 22 Sub-classes, 0 commands."""
	...
	def encc(self):
		...
		if not hasattr(self, '_encc'):
			from .Encc import Encc
			self._encc = Encc(self._core, self._cmd_group)
		return self._encc
