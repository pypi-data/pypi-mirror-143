...
class Xpdcch:
	"""Xpdcch commands group definition. 67 total commands, 14 Subgroups, 0 group commands"""
	...
	def alRegs(self):
		...
		if not hasattr(self, '_alRegs'):
			from .AlRegs import AlRegs
			self._alRegs = AlRegs(self._core, self._cmd_group)
		return self._alRegs
	...
	def avcces(self):
		...
		if not hasattr(self, '_avcces'):
			from .Avcces import Avcces
			self._avcces = Avcces(self._core, self._cmd_group)
		return self._avcces
	...
	def avRegs(self):
		...
		if not hasattr(self, '_avRegs'):
			from .AvRegs import AvRegs
			self._avRegs = AvRegs(self._core, self._cmd_group)
		return self._avRegs
	...
	def dcRegs(self):
		...
		if not hasattr(self, '_dcRegs'):
			from .DcRegs import DcRegs
			self._dcRegs = DcRegs(self._core, self._cmd_group)
		return self._dcRegs
	...
	def dregs(self):
		...
		if not hasattr(self, '_dregs'):
			...
			self._dregs = Dregs(self._core, self._cmd_group)
		return self._dregs
	...
	def extc(self):
		...
		if not hasattr(self, '_extc'):
			from .Extc import Extc
			self._extc = Extc(self._core, self._cmd_group)
		return self._extc
	...
	def noPdcchs(self):
		...
		if not hasattr(self, '_noPdcchs'):
			from .NoPdcchs import NoPdcchs
			self._noPdcchs = NoPdcchs(self._core, self._cmd_group)
	...
	def clone(self) -> 'Xpdcch':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Xpdcch(self._core, self._cmd_group.parent)
