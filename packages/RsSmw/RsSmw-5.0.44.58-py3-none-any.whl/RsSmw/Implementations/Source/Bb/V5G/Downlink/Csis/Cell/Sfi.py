...
class Sfi:
	...
	def set(self, csi_rs_sf_conf: int, cellNull=repcap.CellNull.Default) -> None:
		...
		Snippet: driver.source.bb.v5G.downlink.csis.cell.sfi.set(csi_rs_sf_conf = 1, cellNull = repcap.CellNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:V5G:DL:CSIS:CELL{cellNull_cmd_val}:SFI {param}')

	def get(self, cellNull=repcap.CellNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:CSIS:[CELL<CH0>]:SFI \n
		Snippet: value: int = driver.source.bb.v5G.downlink.csis.cell.sfi.get(cellNull = repcap.CellNull.Default) \n
		No command help available \n
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')
			:return: csi_rs_sf_conf: No help available"""
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:V5G:DL:CSIS:CELL{cellNull_cmd_val}:SFI?')
