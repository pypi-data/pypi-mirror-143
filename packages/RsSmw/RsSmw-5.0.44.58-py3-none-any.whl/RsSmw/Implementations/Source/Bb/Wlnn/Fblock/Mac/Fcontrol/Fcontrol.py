...
class Fcontrol:
	...
	def set(self, fcontrol: List[str], frameBlock=repcap.FrameBlock.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:WLNN:FBLock<CH>:MAC:FCONtrol:FCONtrol \n
		Snippet: driver.source.bb.wlnn.fblock.mac.fcontrol.fcontrol.set(fcontrol = ['raw1', 'raw2', 'raw3'], frameBlock = repcap.FrameBlock.Default) \n
	...
	def get(self, frameBlock=repcap.FrameBlock.Default) -> List[str]:
		"""SCPI: [SOURce<HW>]:BB:WLNN:FBLock<CH>:MAC:FCONtrol:FCONtrol \n
		Snippet: value: List[str] = driver.source.bb.wlnn.fblock.mac.fcontrol.fcontrol.get(frameBlock = repcap.FrameBlock.Default) \n
		No command help available \n
			:param frameBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fblock')
