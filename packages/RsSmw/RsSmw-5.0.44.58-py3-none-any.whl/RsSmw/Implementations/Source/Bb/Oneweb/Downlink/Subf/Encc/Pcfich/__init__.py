...
class Pcfich:
	"""Pcfich commands group definition. 4 total commands, 4 Subgroups, 0 group commands"""
	...
	def cfi(self):
		...
		if not hasattr(self, '_cfi'):
			from .Cfi import Cfi
			self._cfi = Cfi(self._core, self._cmd_group)
		return self._cfi
	...
	def cregion(self):
		...
		if not hasattr(self, '_cregion'):
			...
			self._cregion = Cregion(self._core, self._cmd_group)
	...
	def clone(self) -> 'Pcfich':
		...
		new_group = Pcfich(self._core, self._cmd_group.parent)
