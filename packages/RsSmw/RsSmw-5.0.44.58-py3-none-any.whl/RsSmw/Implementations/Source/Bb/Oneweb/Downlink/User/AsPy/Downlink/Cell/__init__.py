...
class Cell:
	"""Cell commands group definition. 26 total commands, 13 Subgroups, 2 group commands
	...
	def fmcs(self):
		...
		if not hasattr(self, '_fmcs'):
			...
			self._fmcs = Fmcs(self._core, self._cmd_group)
		return self._fmcs
	...
	def mcsMode(self):
		...
		if not hasattr(self, '_mcsMode'):
			from .McsMode import McsMode
			self._mcsMode = McsMode(self._core, self._cmd_group)
	...
	def rvcSequence(self):
		...
		if not hasattr(self, '_rvcSequence'):
			from .RvcSequence import RvcSequence
			self._rvcSequence = RvcSequence(self._core, self._cmd_group)
	...
	def selement(self):
		...
		if not hasattr(self, '_selement'):
			...
			self._selement = Selement(self._core, self._cmd_group)
	...
	def seqElem(self):
		...
		if not hasattr(self, '_seqElem'):
			from .SeqElem import SeqElem
			self._seqElem = SeqElem(self._core, self._cmd_group)
	...
	def tcr(self):
		...
		if not hasattr(self, '_tcr'):
			from .Tcr import Tcr
			self._tcr = Tcr(self._core, self._cmd_group)
		return self._tcr
	...
	def tmod(self):
		...
		if not hasattr(self, '_tmod'):
			...
			self._tmod = Tmod(self._core, self._cmd_group)
		return self._tmod
	...
	def urlCounter(self):
		...
		if not hasattr(self, '_urlCounter'):
			from .UrlCounter import UrlCounter
			self._urlCounter = UrlCounter(self._core, self._cmd_group)
		return self._urlCounter

	@property
	def usPattern(self):
		"""usPattern commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_usPattern'):
			from .UsPattern import UsPattern
			self._usPattern = UsPattern(self._core, self._cmd_group)
		return self._usPattern

	@property
	def usubframe(self):
		"""usubframe commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_usubframe'):
			from .Usubframe import Usubframe
			self._usubframe = Usubframe(self._core, self._cmd_group)
		return self._usubframe

	def delete(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:AS:DL:CELL<ST0>:DELete \n
		Snippet: driver.source.bb.oneweb.downlink.user.asPy.downlink.cell.delete(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:AS:DL:CELL{cellNull_cmd_val}:DELete')

	def delete_with_opc(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default, opc_timeout_ms: int = -1) -> None:
		userIx_cmd_val = self._cmd_group.get_repcap_cmd_value(userIx, repcap.UserIx)
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:AS:DL:CELL<ST0>:DELete \n
		Snippet: driver.source.bb.oneweb.downlink.user.asPy.downlink.cell.delete_with_opc(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		No command help available \n
		...
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:AS:DL:CELL{cellNull_cmd_val}:DELete', opc_timeout_ms)

	def reset(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:AS:DL:CELL<ST0>:RESet \n
		Snippet: driver.source.bb.oneweb.downlink.user.asPy.downlink.cell.reset(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		No command help available \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')
		"""
		userIx_cmd_val = self._cmd_group.get_repcap_cmd_value(userIx, repcap.UserIx)
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:AS:DL:CELL{cellNull_cmd_val}:RESet')

	def reset_with_opc(self, userIx=repcap.UserIx.Default, cellNull=repcap.CellNull.Default, opc_timeout_ms: int = -1) -> None:
		userIx_cmd_val = self._cmd_group.get_repcap_cmd_value(userIx, repcap.UserIx)
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:AS:DL:CELL<ST0>:RESet \n
		Snippet: driver.source.bb.oneweb.downlink.user.asPy.downlink.cell.reset_with_opc(userIx = repcap.UserIx.Default, cellNull = repcap.CellNull.Default) \n
		No command help available \n
		Same as reset, but waits for the operation to complete before continuing further. Use the RsSmw.utilities.opc_timeout_set() to set the timeout value. \n
			...
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')
			:param opc_timeout_ms: Maximum time to wait in milliseconds, valid only for this call."""
		self._core.io.write_with_opc(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:AS:DL:CELL{cellNull_cmd_val}:RESet', opc_timeout_ms)
