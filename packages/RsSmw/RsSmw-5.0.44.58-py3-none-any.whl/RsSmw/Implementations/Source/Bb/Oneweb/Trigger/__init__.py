...
class Trigger:
	"""Trigger commands group definition. 27 total commands, 6 Subgroups, 5 group commands"""
	...
	def get_rmode(self) -> enums.TrigRunMode:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:RMODe \n
		Snippet: value: enums.TrigRunMode = driver.source.bb.oneweb.trigger.get_rmode() \n
		Queries the signal generation status. \n
			:return: trig_run_mode: STOP| RUN
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ONEWeb:TRIGger:RMODe?')
		return Conversions.str_to_scalar_enum(response, enums.TrigRunMode)

	def get_slength(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SLENgth \n
		Snippet: value: int = driver.source.bb.oneweb.trigger.get_slength() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ONEWeb:TRIGger:SLENgth?')
		return Conversions.str_to_int(response)

	def set_slength(self, trig_seq_len: int) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SLENgth \n
		Snippet: driver.source.bb.oneweb.trigger.set_slength(trig_seq_len = 1) \n
		Defines the length of the signal sequence that is output in the SINGle trigger mode. \n
			:param trig_seq_len: integer Range: 1 to 4294967295
		"""
		param = Conversions.decimal_value_to_str(trig_seq_len)
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:TRIGger:SLENgth {param}')

	# noinspection PyTypeChecker
	def get_sl_unit(self) -> enums.Unit:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SLUNit \n
		Snippet: value: enums.Unit = driver.source.bb.oneweb.trigger.get_sl_unit() \n
		Defines the unit for the entry of the signal sequence length, generated after the trigger event. \n
			:return: seq_len_unit: SEQuence| FRAMe| SUBFrame| SLOT| SAMPle FRAMe Single frame SEQuence Single sequence. SUBFrame Single subframe. SLOT Single slot SAMPle Selected number of samples.
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ONEWeb:TRIGger:SLUNit?')
		return Conversions.str_to_scalar_enum(response, enums.Unit)

	def set_sl_unit(self, seq_len_unit: enums.Unit) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SLUNit \n
		Snippet: driver.source.bb.oneweb.trigger.set_sl_unit(seq_len_unit = enums.Unit.FRAMe) \n
		Defines the unit for the entry of the signal sequence length, generated after the trigger event. \n
			:param seq_len_unit: SEQuence| FRAMe| SUBFrame| SLOT| SAMPle FRAMe Single frame SEQuence Single sequence. SUBFrame Single subframe. SLOT Single slot SAMPle Selected number of samples.
		"""
		param = Conversions.enum_scalar_to_str(seq_len_unit, enums.Unit)
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:TRIGger:SLUNit {param}')

	# noinspection PyTypeChecker
	def get_source(self) -> enums.TriggerSourceC:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SOURce \n
		Snippet: value: enums.TriggerSourceC = driver.source.bb.oneweb.trigger.get_source() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ONEWeb:TRIGger:SOURce?')
	...
	def set_source(self, trig_sour: enums.TriggerSourceC) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:TRIGger:SOURce \n
		Snippet: driver.source.bb.oneweb.trigger.set_source(trig_sour = enums.TriggerSourceC.BBSY) \n
			INTRO_CMD_HELP: Selects the trigger signal source and determines the way the triggering is executed. Provided are: \n
			- Internal triggering by a command (INTernal)
			- External trigger signal via one of the local or global connectors
			Table Header:  \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:TRIGger:SOURce {param}')

	# noinspection PyTypeChecker
	def get_sequence(self) -> enums.DmTrigMode:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:[TRIGger]:SEQuence \n
		Snippet: value: enums.DmTrigMode = driver.source.bb.oneweb.trigger.get_sequence() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ONEWeb:TRIGger:SEQuence?')
		return Conversions.str_to_scalar_enum(response, enums.DmTrigMode)

	def set_sequence(self, trig_mode: enums.DmTrigMode) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:[TRIGger]:SEQuence \n
		Snippet: driver.source.bb.oneweb.trigger.set_sequence(trig_mode = enums.DmTrigMode.AAUTo) \n
			INTRO_CMD_HELP: Selects the trigger mode: \n
			- AUTO = auto
			- RETRigger = retrigger
			- AAUTo = armed auto
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:TRIGger:SEQuence {param}')
