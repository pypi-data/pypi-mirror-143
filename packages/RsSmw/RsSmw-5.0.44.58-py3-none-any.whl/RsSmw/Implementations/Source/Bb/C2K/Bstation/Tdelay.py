...
class Tdelay:
	...
	def set(self, tdelay: int, baseStation=repcap.BaseStation.Default) -> None:
		...
		Snippet: driver.source.bb.c2K.bstation.tdelay.set(tdelay = 1, baseStation = repcap.BaseStation.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:C2K:BSTation{baseStation_cmd_val}:TDELay {param}')

	def get(self, baseStation=repcap.BaseStation.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:C2K:BSTation<ST>:TDELay \n
		Snippet: value: int = driver.source.bb.c2K.bstation.tdelay.get(baseStation = repcap.BaseStation.Default) \n
		Sets the time shift of the selected base station compared to base station 1. \n
			:param baseStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Bstation')
			:return: tdelay: integer Range: 0 to dynamic, Unit: chip"""
		baseStation_cmd_val = self._cmd_group.get_repcap_cmd_value(baseStation, repcap.BaseStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:C2K:BSTation{baseStation_cmd_val}:TDELay?')
