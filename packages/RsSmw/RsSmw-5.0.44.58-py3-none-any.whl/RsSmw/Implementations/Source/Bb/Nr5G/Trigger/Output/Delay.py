...
class Delay:
	...
	def set(self, mark_delay: float, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.nr5G.trigger.output.delay.set(mark_delay = 1.0, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:TRIGger:OUTPut{output_cmd_val}:DELay {param}')
