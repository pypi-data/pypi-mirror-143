...
class State:
	...
	def set(self, state: bool, iqConnector=repcap.IqConnector.Default) -> None:
		"""SCPI: [SOURce]:BB:IMPairment:BBMM<CH>:STATe \n
		Snippet: driver.source.bb.impairment.bbmm.state.set(state = False, iqConnector = repcap.IqConnector.Default) \n
		...
		self._core.io.write(f'SOURce:BB:IMPairment:BBMM{iqConnector_cmd_val}:STATe {param}')

	def get(self, iqConnector=repcap.IqConnector.Default) -> bool:
		"""SCPI: [SOURce]:BB:IMPairment:BBMM<CH>:STATe \n
		Snippet: value: bool = driver.source.bb.impairment.bbmm.state.get(iqConnector = repcap.IqConnector.Default) \n
		Activates the impairment or correction values LEAKage, QUADrature and IQRatio for the corresponding stream. \n
			:param iqConnector: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Bbmm')
			:return: state: 1| ON| 0| OFF"""
		iqConnector_cmd_val = self._cmd_group.get_repcap_cmd_value(iqConnector, repcap.IqConnector)
		response = self._core.io.query_str(f'SOURce:BB:IMPairment:BBMM{iqConnector_cmd_val}:STATe?')
