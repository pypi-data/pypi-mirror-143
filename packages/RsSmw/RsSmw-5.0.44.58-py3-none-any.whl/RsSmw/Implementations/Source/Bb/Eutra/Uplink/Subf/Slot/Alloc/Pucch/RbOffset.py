...
class RbOffset:
	...
	def get(self, subframeNull=repcap.SubframeNull.Default, slotNull=repcap.SlotNull.Default, allocationNull=repcap.AllocationNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:[SUBF<ST0>]:SLOT<USER0>:ALLoc<CH0>:PUCCh:RBOFfset \n
		Snippet: value: int = driver.source.bb.eutra.uplink.subf.slot.alloc.pucch.rbOffset.get(subframeNull = repcap.SubframeNull.Default, slotNull = repcap.SlotNull.Default, allocationNull = repcap.AllocationNull.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:SUBF{subframeNull_cmd_val}:SLOT{slotNull_cmd_val}:ALLoc{allocationNull_cmd_val}:PUCCh:RBOFfset?')
