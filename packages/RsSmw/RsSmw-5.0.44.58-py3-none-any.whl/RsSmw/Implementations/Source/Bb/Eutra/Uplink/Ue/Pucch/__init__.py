...
class Pucch:
	...
	def f4Naport(self):
		...
		if not hasattr(self, '_f4Naport'):
			from .F4Naport import F4Naport
			self._f4Naport = F4Naport(self._core, self._cmd_group)
	...
	def f5Naport(self):
		...
		if not hasattr(self, '_f5Naport'):
			from .F5Naport import F5Naport
			self._f5Naport = F5Naport(self._core, self._cmd_group)
