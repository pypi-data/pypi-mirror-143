...
class Cosine:
	...
	def get_cofs(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:FILTer:PARameter:COSine:COFS \n
		Snippet: value: float = driver.source.bb.eutra.filterPy.parameter.cosine.get_cofs() \n
		Sets the filter parameter. \n
			:return: cofs: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:EUTRa:FILTer:PARameter:COSine:COFS?')
		return Conversions.str_to_float(response)

	def set_cofs(self, cofs: float) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:FILTer:PARameter:COSine:COFS \n
		Snippet: driver.source.bb.eutra.filterPy.parameter.cosine.set_cofs(cofs = 1.0) \n
		Sets the filter parameter. \n
			:param cofs: float Range: -1 to 1
		"""
		param = Conversions.decimal_value_to_str(cofs)
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:FILTer:PARameter:COSine:COFS {param}')

	def get_value(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:FILTer:PARameter:COSine \n
		Snippet: value: float = driver.source.bb.eutra.filterPy.parameter.cosine.get_value() \n
		Sets the filter parameter.
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:EUTRa:FILTer:PARameter:COSine?')
		return Conversions.str_to_float(response)

	def set_value(self, cosine: float) -> None:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:FILTer:PARameter:COSine \n
		Snippet: driver.source.bb.eutra.filterPy.parameter.cosine.set_value(cosine = 1.0) \n
		Sets the filter parameter.
			Table Header: Filter type / Parameter / Parameter name / Min / Max / Increment / Default \n
			- APCO25 / Rolloff factor / <Apco25> / 0.05 / 0.99 / 0.01 / 0.2
			- COSine / Rolloff factor / <Cosine> / 0 / 1 / 0.01 / 0.1
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:FILTer:PARameter:COSine {param}')
