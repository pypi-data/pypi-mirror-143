...
class Dmrs:
	...
	def set(self, ulca_n_1_dmrs: int, cellNull=repcap.CellNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.uplink.ca.cell.dmrs.set(ulca_n_1_dmrs = 1, cellNull = repcap.CellNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:CA:CELL{cellNull_cmd_val}:DMRS {param}')

	def get(self, cellNull=repcap.CellNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:CA:CELL<CH0>:DMRS \n
		Snippet: value: int = driver.source.bb.eutra.uplink.ca.cell.dmrs.get(cellNull = repcap.CellNull.Default) \n
		Sets the parameter n(1) _DMRS per component carrier. \n
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')
			:return: ulca_n_1_dmrs: integer Range: 0 to 11"""
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:UL:CA:CELL{cellNull_cmd_val}:DMRS?')
