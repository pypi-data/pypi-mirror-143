...
class Pattern:
	...
	def set(self, emtc_ack_nack_pat: List[str], bitcount: int, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.uplink.ue.emtc.trans.pucch.harq.pattern.set(emtc_ack_nack_pat = ['raw1', 'raw2', 'raw3'], bitcount = 1, userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:UL:UE{userEquipment_cmd_val}:EMTC:TRANs{transmission_cmd_val}:PUCCh:HARQ:PATTern {param}'.rstrip())
