...
class Plength:
	...
	def set(self, payload_length: int, commandBlock=repcap.CommandBlock.Default) -> None:
		...
		Snippet: driver.source.bb.nfc.cblock.fpGeneric.plength.set(payload_length = 1, commandBlock = repcap.CommandBlock.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NFC:CBLock{commandBlock_cmd_val}:FPGeneric:PLENgth {param}')

	def get(self, commandBlock=repcap.CommandBlock.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:NFC:CBLock<CH>:FPGeneric:PLENgth \n
		Snippet: value: int = driver.source.bb.nfc.cblock.fpGeneric.plength.get(commandBlock = repcap.CommandBlock.Default) \n
		Sets the length of a standard frame. \n
			:param commandBlock: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cblock')
			:return: payload_length: No help available"""
		commandBlock_cmd_val = self._cmd_group.get_repcap_cmd_value(commandBlock, repcap.CommandBlock)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NFC:CBLock{commandBlock_cmd_val}:FPGeneric:PLENgth?')
