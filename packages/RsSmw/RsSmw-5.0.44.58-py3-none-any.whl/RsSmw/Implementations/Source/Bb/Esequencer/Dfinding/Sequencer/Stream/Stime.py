...
class Stime:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:DFINding:[SEQuencer<ST>]:STReam:STIMe \n
		Snippet: value: float = driver.source.bb.esequencer.dfinding.sequencer.stream.stime.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:DFINding:SEQuencer{sequencer_cmd_val}:STReam:STIMe?')
