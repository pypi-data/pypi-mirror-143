...
class Comment:
	...
	def get(self, baseband=repcap.Baseband.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:PSEQuencer:[BB<ST>]:COMMent \n
		Snippet: value: str = driver.source.bb.esequencer.psequencer.bb.comment.get(baseband = repcap.Baseband.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:PSEQuencer:BB{baseband_cmd_val}:COMMent?')
