...
class ReShift:
	...
	def set(self, shift: int, output=repcap.Output.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:WLAD:TRIGger:OUTPut<CH>:RESHift \n
		Snippet: driver.source.bb.wlad.trigger.output.reShift.set(shift = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:WLAD:TRIGger:OUTPut{output_cmd_val}:RESHift {param}')

	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:WLAD:TRIGger:OUTPut<CH>:RESHift \n
		Snippet: value: int = driver.source.bb.wlad.trigger.output.reShift.get(output = repcap.Output.Default) \n
		Shifts the rising edge of the marker the specified number of samples. Negative values result in a shift back of the
		marker edge. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: shift: integer Range: -1000 to 1000"""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:WLAD:TRIGger:OUTPut{output_cmd_val}:RESHift?')
