...
class Gain:
	...
	def set(self, gain: float, terminal=repcap.Terminal.Default) -> None:
		...
		Snippet: driver.source.bb.evdo.terminal.dchannel.gain.set(gain = 1.0, terminal = repcap.Terminal.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EVDO:TERMinal{terminal_cmd_val}:DCHannel:GAIN {param}')

	def get(self, terminal=repcap.Terminal.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:EVDO:TERMinal<ST>:DCHannel:GAIN \n
		Snippet: value: float = driver.source.bb.evdo.terminal.dchannel.gain.get(terminal = repcap.Terminal.Default) \n
		(enabled for an access terminal working in access mode) Sets the gain in dB of the data channel relative to the pilot
		channel power. \n
			:param terminal: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Terminal')
			:return: gain: float Range: -80 to 30"""
		terminal_cmd_val = self._cmd_group.get_repcap_cmd_value(terminal, repcap.Terminal)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EVDO:TERMinal{terminal_cmd_val}:DCHannel:GAIN?')
