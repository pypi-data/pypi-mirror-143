...
class Mbytes:
	...
	def get(self, vdbTransmitter=repcap.VdbTransmitter.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GBAS:VDB<CH>:SCH:M4T:MBYTes \n
		Snippet: value: int = driver.source.bb.gbas.vdb.sch.m4T.mbytes.get(vdbTransmitter = repcap.VdbTransmitter.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GBAS:VDB{vdbTransmitter_cmd_val}:SCH:M4T:MBYTes?')
