...
class Notch:
	...
	def get_clock(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:ARBitrary:NOTCh:CLOCk \n
		Snippet: value: int = driver.source.bb.arbitrary.notch.get_clock() \n
		Queries the current clock frequency of the waveform signal. \n
			:return: chbw: integer Range: 400 to 2000E6
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ARBitrary:NOTCh:CLOCk?')
		return Conversions.str_to_int(response)

	def get_count(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:ARBitrary:NOTCh:COUNt \n
		Snippet: value: int = driver.source.bb.arbitrary.notch.get_count() \n
		Sets the number of notches. \n
			:return: id_pdb_notched_num: integer Range: 1 to 25
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ARBitrary:NOTCh:COUNt?')
		return Conversions.str_to_int(response)

	def set_count(self, id_pdb_notched_num: int) -> None:
		"""SCPI: [SOURce<HW>]:BB:ARBitrary:NOTCh:COUNt \n
		Snippet: driver.source.bb.arbitrary.notch.set_count(id_pdb_notched_num = 1) \n
		Sets the number of notches. \n
			:param id_pdb_notched_num: integer Range: 1 to 25
		"""
		param = Conversions.decimal_value_to_str(id_pdb_notched_num)
		self._core.io.write(f'SOURce<HwInstance>:BB:ARBitrary:NOTCh:COUNt {param}')

	def get_value(self) -> bool:
		"""SCPI: [SOURce<HW>]:BB:ARBitrary:NOTCh \n
		Snippet: value: bool = driver.source.bb.arbitrary.notch.get_value() \n
		Enables or disables the notch filter. \n
			:return: notch_state: 1| ON| 0| OFF
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:ARBitrary:NOTCh?')
		return Conversions.str_to_bool(response)

	def set_value(self, notch_state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:ARBitrary:NOTCh \n
		Snippet: driver.source.bb.arbitrary.notch.set_value(notch_state = False) \n
		Enables or disables the notch filter. \n
			:param notch_state: 1| ON| 0| OFF
		"""
		param = Conversions.bool_to_str(notch_state)
		self._core.io.write(f'SOURce<HwInstance>:BB:ARBitrary:NOTCh {param}')

	def clone(self) -> 'Notch':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Notch(self._core, self._cmd_group.parent)
