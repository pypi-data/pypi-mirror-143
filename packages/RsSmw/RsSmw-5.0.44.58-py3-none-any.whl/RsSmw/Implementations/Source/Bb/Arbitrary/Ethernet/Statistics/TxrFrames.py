...
class TxrFrames:
	"""TxrFrames commands group definition. 1 total commands, 0 Subgroups, 1 group commands
	Repeated Capability: Index, default value after init: Index.Nr1"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("txrFrames", core, parent)
	...
	def get(self, index=repcap.Index.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.arbitrary.ethernet.statistics.txrFrames.get(index = repcap.Index.Default) \n
		No command help available \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'TxrFrames')
			:return: reply_bytes: No help available"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ARBitrary:ETHernet:STATistics:TXRFrames{index_cmd_val}?')
		return Conversions.str_to_int(response)

	def clone(self) -> 'TxrFrames':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = TxrFrames(self._core, self._cmd_group.parent)
