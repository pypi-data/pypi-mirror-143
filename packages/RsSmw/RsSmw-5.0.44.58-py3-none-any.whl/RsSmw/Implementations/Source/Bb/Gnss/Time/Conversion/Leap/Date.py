...
class Date:
	...
	def set(self, year: int, month: int, day: int) -> None:
		...
		Snippet: driver.source.bb.gnss.time.conversion.leap.date.set(year = 1, month = 1, day = 1) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:LEAP:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:LEAP:DATE \n
		Snippet: value: DateStruct = driver.source.bb.gnss.time.conversion.leap.date.get() \n
		Defines the date of the next UTC time correction. \n
			:return: structure: for return value, see the help for DateStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:LEAP:DATE?', self.__class__.DateStruct())
