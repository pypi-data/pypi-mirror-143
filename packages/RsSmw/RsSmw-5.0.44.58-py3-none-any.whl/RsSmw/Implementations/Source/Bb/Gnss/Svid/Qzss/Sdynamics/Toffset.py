...
class Toffset:
	...
	def set(self, time_offset: float, satelliteSvid=repcap.SatelliteSvid.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.svid.qzss.sdynamics.toffset.set(time_offset = 1.0, satelliteSvid = repcap.SatelliteSvid.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:QZSS:SDYNamics:TOFFset {param}')

	def get(self, satelliteSvid=repcap.SatelliteSvid.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:QZSS:SDYNamics:TOFFset \n
		Snippet: value: float = driver.source.bb.gnss.svid.qzss.sdynamics.toffset.get(satelliteSvid = repcap.SatelliteSvid.Default) \n
		Sets a time delay before the profile is applied. \n
			:param satelliteSvid: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:return: time_offset: float Range: 0 to 9E4"""
		satelliteSvid_cmd_val = self._cmd_group.get_repcap_cmd_value(satelliteSvid, repcap.SatelliteSvid)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:QZSS:SDYNamics:TOFFset?')
