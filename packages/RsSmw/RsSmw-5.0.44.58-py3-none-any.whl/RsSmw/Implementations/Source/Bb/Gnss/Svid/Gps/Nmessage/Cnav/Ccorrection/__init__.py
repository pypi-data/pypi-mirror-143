...
class Ccorrection:
	"""Ccorrection commands group definition. 14 total commands, 4 Subgroups, 0 group commands"""
	...
	@property
	def isc(self):
		"""isc commands group. 4 Sub-classes, 0 commands."""
		if not hasattr(self, '_isc'):
			from .Isc import Isc
			self._isc = Isc(self._core, self._cmd_group)
		return self._isc
	...
	@property
	def tgd(self):
		"""tgd commands group. 1 Sub-classes, 1 commands."""
		if not hasattr(self, '_tgd'):
			from .Tgd import Tgd
			self._tgd = Tgd(self._core, self._cmd_group)
		return self._tgd
