...
class Rf:
	...
	def cndRatio(self):
		...
		if not hasattr(self, '_cndRatio'):
			from .CndRatio import CndRatio
			self._cndRatio = CndRatio(self._core, self._cmd_group)
	...
	def jsRatio(self):
		...
		if not hasattr(self, '_jsRatio'):
			from .JsRatio import JsRatio
			self._jsRatio = JsRatio(self._core, self._cmd_group)
