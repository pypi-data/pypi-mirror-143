...
class Galileo:
	"""Galileo commands group definition. 4 total commands, 0 Subgroups, 4 group commands"""
	...
	def get_active(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GALileo:ACTive \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:ACTive?')
		return Conversions.str_to_int(response)

	def get_available(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GALileo:AVAilable \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:AVAilable?')
		return Conversions.str_to_int(response)

	def get_max(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GALileo:MAX \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:MAX?')
		return Conversions.str_to_int(response)
	...
	def set_max(self, maximum_svs: int) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:MAX {param}')

	def get_min(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:SELection:GALileo:MIN \n
		Snippet: value: int = driver.source.bb.gnss.sv.selection.galileo.get_min() \n
		Sets the minimum and maximum number of satellites per GNSS system that can be included in the satellite constellation. \n
			:return: minimum_svs: No help available
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:MIN?')
		return Conversions.str_to_int(response)
	...
	def set_min(self, minimum_svs: int) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:SELection:GALileo:MIN {param}')
