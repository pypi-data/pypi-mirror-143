...
class Azero:
	...
	def get_unscaled(self) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo:UNSCaled \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo:UNSCaled?')
	...
	def set_unscaled(self, a_0: float) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo:UNSCaled {param}')

	def get_value(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo?')
	...
	def set_value(self, azero: int) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:TIME:CONVersion:GLONass:UTCSu:AZERo {param}')
