...
class Azimuth:
	...
	def get(self, time_basis: enums.TimeBasis, year: int, month: int, day: int, hour: int, minutes: int, seconds: float, week_number: int, time_of_week: float, satelliteSvid=repcap.SatelliteSvid.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RT:SBAS:SVID<CH>:AZIMuth \n
		Snippet: value: float = driver.source.bb.gnss.rt.sbas.svid.azimuth.get(time_basis = enums.TimeBasis.BDT, year = 1, month = 1, day = 1, hour = 1, minutes = 1, seconds = 1.0, week_number = 1, time_of_week = 1.0, satelliteSvid = repcap.SatelliteSvid.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RT:SBAS:SVID{satelliteSvid_cmd_val}:AZIMuth? {param}'.rstrip())
