...
class Svid:
	...
	def set(self, svid: enums.Svid, satelliteSvid=repcap.SatelliteSvid.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GPS:POWer:COPY:SVID \n
		Snippet: driver.source.bb.gnss.svid.gps.power.copy.svid.set(svid = enums.Svid._1, satelliteSvid = repcap.SatelliteSvid.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:GPS:POWer:COPY:SVID {param}')

	# noinspection PyTypeChecker
	def get(self, satelliteSvid=repcap.SatelliteSvid.Default) -> enums.Svid:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GPS:POWer:COPY:SVID \n
		Snippet: value: enums.Svid = driver.source.bb.gnss.svid.gps.power.copy.svid.get(satelliteSvid = repcap.SatelliteSvid.Default) \n
		Sets the SV ID to that the configuration form the current satellite (SVID<ch>) is applied. Both SV IDs belong to the same
		GNSS system. \n
			:param satelliteSvid: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
		...
		satelliteSvid_cmd_val = self._cmd_group.get_repcap_cmd_value(satelliteSvid, repcap.SatelliteSvid)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:GPS:POWer:COPY:SVID?')
