...
class Unscaled:
	...
	def set(self, beta_unscaled: int, betaNull=repcap.BetaNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ATMospheric:GPS:IONospheric:BETA<CH0>:UNSCaled \n
		Snippet: driver.source.bb.gnss.atmospheric.gps.ionospheric.beta.unscaled.set(beta_unscaled = 1, betaNull = repcap.BetaNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ATMospheric:GPS:IONospheric:BETA{betaNull_cmd_val}:UNSCaled {param}')

	def get(self, betaNull=repcap.BetaNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ATMospheric:GPS:IONospheric:BETA<CH0>:UNSCaled \n
		Snippet: value: int = driver.source.bb.gnss.atmospheric.gps.ionospheric.beta.unscaled.get(betaNull = repcap.BetaNull.Default) \n
		No command help available \n
			:param betaNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Beta')
			:return: beta_unscaled: No help available"""
		betaNull_cmd_val = self._cmd_group.get_repcap_cmd_value(betaNull, repcap.BetaNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:ATMospheric:GPS:IONospheric:BETA{betaNull_cmd_val}:UNSCaled?')
