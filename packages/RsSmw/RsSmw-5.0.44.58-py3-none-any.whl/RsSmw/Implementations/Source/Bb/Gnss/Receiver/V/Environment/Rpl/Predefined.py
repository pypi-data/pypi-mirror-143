...
class Predefined:
	...
	def set(self, predefined_env: enums.ObscModelSideBuil, vehicle=repcap.Vehicle.Default) -> None:
		...
		"""
		param = Conversions.enum_scalar_to_str(predefined_env, enums.ObscModelSideBuil)
		vehicle_cmd_val = self._cmd_group.get_repcap_cmd_value(vehicle, repcap.Vehicle)
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:ENVironment:RPL:PREDefined {param}')

	# noinspection PyTypeChecker
	def get(self, vehicle=repcap.Vehicle.Default) -> enums.ObscModelSideBuil:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RECeiver:[V<ST>]:ENVironment:RPL:PREDefined \n
		...
		vehicle_cmd_val = self._cmd_group.get_repcap_cmd_value(vehicle, repcap.Vehicle)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:ENVironment:RPL:PREDefined?')
		return Conversions.str_to_scalar_enum(response, enums.ObscModelSideBuil)
