...
class Date:
	...
	def set(self, year: int, month: int, day: int) -> None:
		...
		Snippet: driver.source.bb.gnss.adGeneration.gps.toaData.date.set(year = 1, month = 1, day = 1) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS:TOAData:DATE {param}'.rstrip())
	...
	def get(self) -> DateStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GPS:TOAData:DATE \n
		Snippet: value: DateStruct = driver.source.bb.gnss.adGeneration.gps.toaData.date.get() \n
		Enabled for UTC or GLONASS timebase ([:SOURce<hw>]:BB:GNSS:ADGeneration:GPS:TOAData:TBASis) . Enters the date for the
		assistance data in DMS format of the Gregorian calendar. \n
			:return: structure: for return value, see the help for DateStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS:TOAData:DATE?', self.__class__.DateStruct())
