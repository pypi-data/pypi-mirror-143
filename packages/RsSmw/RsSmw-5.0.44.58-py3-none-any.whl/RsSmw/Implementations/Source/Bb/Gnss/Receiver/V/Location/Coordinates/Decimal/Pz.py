...
class Pz:
	...
	def set(self, longitude: float, latitude: float, altitude: float, vehicle=repcap.Vehicle.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.receiver.v.location.coordinates.decimal.pz.set(longitude = 1.0, latitude = 1.0, altitude = 1.0, vehicle = repcap.Vehicle.Default) \n
		Defines the coordinates of the geographic location of the GNSS receiver in decimal format. \n
			...
			:param altitude: float Defines the altitude. The altitude value is the height above the reference ellipsoid. Range: -10E3 to 50E6
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:LOCation:COORdinates:DECimal:PZ {param}'.rstrip())
