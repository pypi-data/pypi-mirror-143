...
class Dpower:
	"""Dpower commands group definition. 2 total commands, 2 Subgroups, 0 group commands"""
	...
	@property
	def mpart(self):
		"""mpart commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_mpart'):
			from .Mpart import Mpart
			self._mpart = Mpart(self._core, self._cmd_group)
		return self._mpart
	...
	def clone(self) -> 'Dpower':
		...
		new_group = Dpower(self._core, self._cmd_group.parent)
