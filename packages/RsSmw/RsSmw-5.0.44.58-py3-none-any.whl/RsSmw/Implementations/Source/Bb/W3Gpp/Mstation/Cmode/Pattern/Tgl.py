...
class Tgl:
	...
	def set(self, tgl: int, mobileStation=repcap.MobileStation.Default, patternIx=repcap.PatternIx.Default, transmGapLength=repcap.TransmGapLength.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:CMODe:PATTern<CH>:TGL<DI> \n
		Snippet: driver.source.bb.w3Gpp.mstation.cmode.pattern.tgl.set(tgl = 1, mobileStation = repcap.MobileStation.Default, patternIx = repcap.PatternIx.Default, transmGapLength = repcap.TransmGapLength.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:CMODe:PATTern{patternIx_cmd_val}:TGL{transmGapLength_cmd_val} {param}')

	def get(self, mobileStation=repcap.MobileStation.Default, patternIx=repcap.PatternIx.Default, transmGapLength=repcap.TransmGapLength.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:CMODe:PATTern<CH>:TGL<DI> \n
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.cmode.pattern.tgl.get(mobileStation = repcap.MobileStation.Default, patternIx = repcap.PatternIx.Default, transmGapLength = repcap.TransmGapLength.Default) \n
		Sets the transmission gap lengths. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:param patternIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Pattern')
			:param transmGapLength: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Tgl')
		...
		patternIx_cmd_val = self._cmd_group.get_repcap_cmd_value(patternIx, repcap.PatternIx)
		transmGapLength_cmd_val = self._cmd_group.get_repcap_cmd_value(transmGapLength, repcap.TransmGapLength)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:CMODe:PATTern{patternIx_cmd_val}:TGL{transmGapLength_cmd_val}?')
	...
	def clone(self) -> 'Tgl':
		...
		new_group = Tgl(self._core, self._cmd_group.parent)
