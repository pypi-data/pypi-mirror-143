...
class Cpower:
	...
	def set(self, cpower: float, mobileStation=repcap.MobileStation.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.prach.cpower.set(cpower = 1.0, mobileStation = repcap.MobileStation.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:PRACh:CPOWer {param}')

	def get(self, mobileStation=repcap.MobileStation.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:PRACh:CPOWer \n
		Snippet: value: float = driver.source.bb.w3Gpp.mstation.prach.cpower.get(mobileStation = repcap.MobileStation.Default) \n
		The command defines the power of the control component of the PRACH. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: cpower: float Range: -80 dB to 0 dB"""
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:PRACh:CPOWer?')
