...
class Speriod:
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> float:
		...
		Snippet: value: float = driver.source.bb.w3Gpp.mstation.pcpch.timing.speriod.get(mobileStation = repcap.MobileStation.Default) \n
		Queries the structure length. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: speriod: float"""
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:PCPCh:TIMing:SPERiod?')
