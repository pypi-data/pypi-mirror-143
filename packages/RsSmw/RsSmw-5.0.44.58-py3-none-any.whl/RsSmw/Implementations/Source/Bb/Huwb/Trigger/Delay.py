...
class Delay:
	...
	def get_unit(self) -> enums.TrigDelUnit:
		"""SCPI: [SOURce<HW>]:BB:HUWB:TRIGger:DELay:UNIT \n
		Snippet: value: enums.TrigDelUnit = driver.source.bb.huwb.trigger.delay.get_unit() \n
		Sets the units in that the trigger delay is expressed. \n
			:return: trig_del_unit: SAMPle| TIME
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:HUWB:TRIGger:DELay:UNIT?')
		return Conversions.str_to_scalar_enum(response, enums.TrigDelUnit)

	def set_unit(self, trig_del_unit: enums.TrigDelUnit) -> None:
		"""SCPI: [SOURce<HW>]:BB:HUWB:TRIGger:DELay:UNIT \n
		Snippet: driver.source.bb.huwb.trigger.delay.set_unit(trig_del_unit = enums.TrigDelUnit.SAMPle) \n
		Sets the units in that the trigger delay is expressed. \n
			:param trig_del_unit: SAMPle| TIME
		"""
		param = Conversions.enum_scalar_to_str(trig_del_unit, enums.TrigDelUnit)
		self._core.io.write(f'SOURce<HwInstance>:BB:HUWB:TRIGger:DELay:UNIT {param}')
