...
class Pattern:
	...
	def set(self, pattern: List[str], bitcount: int, cell=repcap.Cell.Default, channel=repcap.Channel.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.up.cell.enh.dch.dtch.data.pattern.set(pattern = ['raw1', 'raw2', 'raw3'], bitcount = 1, cell = repcap.Cell.Default, channel = repcap.Channel.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:ENH:DCH:DTCH{channel_cmd_val}:DATA:PATTern {param}'.rstrip())
