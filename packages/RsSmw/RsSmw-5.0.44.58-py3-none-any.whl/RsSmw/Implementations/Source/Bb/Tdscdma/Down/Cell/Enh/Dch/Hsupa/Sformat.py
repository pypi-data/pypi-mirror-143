...
class Sformat:
	...
	def get(self, cell=repcap.Cell.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:DCH:HSUPA:SFORmat \n
		Snippet: value: str = driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.sformat.get(cell = repcap.Cell.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:SFORmat?')
