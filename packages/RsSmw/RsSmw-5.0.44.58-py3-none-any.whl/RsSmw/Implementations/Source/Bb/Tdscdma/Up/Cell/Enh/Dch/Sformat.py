...
class Sformat:
	...
	def get(self, cell=repcap.Cell.Default) -> str:
		...
		Snippet: value: str = driver.source.bb.tdscdma.up.cell.enh.dch.sformat.get(cell = repcap.Cell.Default) \n
		Queries the slot format of the selected channel. \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: sformat: string"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:ENH:DCH:SFORmat?')
