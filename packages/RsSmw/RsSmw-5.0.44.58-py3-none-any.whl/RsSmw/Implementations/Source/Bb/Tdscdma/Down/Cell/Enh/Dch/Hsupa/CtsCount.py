...
class CtsCount:
	...
	def set(self, cts_count: int, cell=repcap.Cell.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.ctsCount.set(cts_count = 1, cell = repcap.Cell.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:CTSCount {param}')

	def get(self, cell=repcap.Cell.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:DCH:HSUPA:CTSCount \n
		Snippet: value: int = driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.ctsCount.get(cell = repcap.Cell.Default) \n
		Sets the number of physical channels per timeslot. \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: cts_count: integer Range: 1 to 14"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:CTSCount?')
