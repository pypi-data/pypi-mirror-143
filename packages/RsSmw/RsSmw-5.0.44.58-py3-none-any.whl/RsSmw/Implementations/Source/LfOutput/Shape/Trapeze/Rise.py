...
class Rise:
	...
	def set(self, rise: float, lfOutput=repcap.LfOutput.Default) -> None:
		...
		Snippet: driver.source.lfOutput.shape.trapeze.rise.set(rise = 1.0, lfOutput = repcap.LfOutput.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:LFOutput{lfOutput_cmd_val}:SHAPe:TRAPeze:RISE {param}')

	def get(self, lfOutput=repcap.LfOutput.Default) -> float:
		"""SCPI: [SOURce<HW>]:LFOutput<CH>:SHAPe:TRAPeze:RISE \n
		Snippet: value: float = driver.source.lfOutput.shape.trapeze.rise.get(lfOutput = repcap.LfOutput.Default) \n
		Selects the rise time for the trapezoid shape of the LF generator. \n
			:param lfOutput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'LfOutput')
			:return: rise: float Range: 1E-6 to 100"""
		lfOutput_cmd_val = self._cmd_group.get_repcap_cmd_value(lfOutput, repcap.LfOutput)
		response = self._core.io.query_str(f'SOURce<HwInstance>:LFOutput{lfOutput_cmd_val}:SHAPe:TRAPeze:RISE?')
