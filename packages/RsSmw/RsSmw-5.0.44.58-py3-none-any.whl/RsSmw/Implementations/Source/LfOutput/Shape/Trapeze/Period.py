...
class Period:
	...
	def set(self, period: float, lfOutput=repcap.LfOutput.Default) -> None:
		...
		Snippet: driver.source.lfOutput.shape.trapeze.period.set(period = 1.0, lfOutput = repcap.LfOutput.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:LFOutput{lfOutput_cmd_val}:SHAPe:TRAPeze:PERiod {param}')

	def get(self, lfOutput=repcap.LfOutput.Default) -> float:
		"""SCPI: [SOURce<HW>]:LFOutput<CH>:SHAPe:TRAPeze:PERiod \n
		Snippet: value: float = driver.source.lfOutput.shape.trapeze.period.get(lfOutput = repcap.LfOutput.Default) \n
		Sets the period of the generated trapezoid shape. The period determines the repetition frequency of the internal signal. \n
			:param lfOutput: optional repeated capability selector. Default value: Nr1 (settable in the interface 'LfOutput')
			:return: period: float Range: 1E-6 to 100"""
		lfOutput_cmd_val = self._cmd_group.get_repcap_cmd_value(lfOutput, repcap.LfOutput)
		response = self._core.io.query_str(f'SOURce<HwInstance>:LFOutput{lfOutput_cmd_val}:SHAPe:TRAPeze:PERiod?')
