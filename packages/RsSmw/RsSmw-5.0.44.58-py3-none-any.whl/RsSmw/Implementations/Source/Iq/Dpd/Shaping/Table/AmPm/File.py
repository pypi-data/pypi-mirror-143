...
class File:
	...
	def get_catalog(self) -> List[str]:
		...
		Snippet: value: List[str] = driver.source.iq.dpd.shaping.table.amPm.file.get_catalog() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:CATalog?')
		return Conversions.str_to_str_list(response)

	def get_data(self) -> List[float]:
		"""SCPI: [SOURce<HW>]:IQ:DPD:SHAPing:TABLe:AMPM:FILE:DATA \n
		Snippet: value: List[float] = driver.source.iq.dpd.shaping.table.amPm.file.get_data() \n
		...
		"""
		response = self._core.io.query_bin_or_ascii_float_list('SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:DATA?')
		return response

	def set_data(self, dpd_pm_table_data: List[float]) -> None:
		"""SCPI: [SOURce<HW>]:IQ:DPD:SHAPing:TABLe:AMPM:FILE:DATA \n
		Snippet: driver.source.iq.dpd.shaping.table.amPm.file.set_data(dpd_pm_table_data = [1.1, 2.2, 3.3]) \n
		Defines the predistortion function in a raw data format. See also [:SOURce<hw>]:IQ:DPD:SHAPing:TABLe:AMPM:FILE:NEW. \n
			:param dpd_pm_table_data: No help available
		"""
		param = Conversions.list_to_csv_str(dpd_pm_table_data)
		self._core.io.write(f'SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:DATA {param}')
	...
	def set_new(self, ipartd_pi_db_dpd_pm_table_data_new_file: List[float]) -> None:
		...
		Snippet: driver.source.iq.dpd.shaping.table.amPm.file.set_new(ipartd_pi_db_dpd_pm_table_data_new_file = [1.1, 2.2, 3.3]) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:NEW {param}')

	def get_select(self) -> str:
		"""SCPI: [SOURce<HW>]:IQ:DPD:SHAPing:TABLe:AMPM:FILE:[SELect] \n
		Snippet: value: str = driver.source.iq.dpd.shaping.table.amPm.file.get_select() \n
		Selects a file with correction values (extension *.dpd_magn (AM/AM) or *.dpd_phase(AM/FM) ). \n
			:return: filename: string
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:SELect?')
		return trim_str_response(response)

	def set_select(self, filename: str) -> None:
		"""SCPI: [SOURce<HW>]:IQ:DPD:SHAPing:TABLe:AMPM:FILE:[SELect] \n
		Snippet: driver.source.iq.dpd.shaping.table.amPm.file.set_select(filename = '1') \n
		Selects a file with correction values (extension *.dpd_magn (AM/AM) or *.dpd_phase(AM/FM) ). \n
			:param filename: string
		"""
		param = Conversions.value_to_quoted_str(filename)
		self._core.io.write(f'SOURce<HwInstance>:IQ:DPD:SHAPing:TABLe:AMPM:FILE:SELect {param}')
