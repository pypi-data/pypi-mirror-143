...
class Output:
	...
	def afixed(self):
		...
		if not hasattr(self, '_afixed'):
			from .Afixed import Afixed
			self._afixed = Afixed(self._core, self._cmd_group)
		return self._afixed
	...
	def get_amode(self) -> enums.PowAttModeSzu:
		...
		Sets the step attenuator mode at the RF output. Note: The setting [:SOURce<hw>]:POWer:ATTenuation:RFOFf:MODEFATTenuation
