...
class State:
	...
	def get(self, iqConnector=repcap.IqConnector.Default) -> bool:
		"""SCPI: [SOURce]:IQ:OUTPut:DIGital:BBMM<CH>:OFLow:STATe \n
		Snippet: value: bool = driver.source.iq.output.digital.bbmm.oflow.state.get(iqConnector = repcap.IqConnector.Default) \n
		...
		response = self._core.io.query_str(f'SOURce:IQ:OUTPut:DIGital:BBMM{iqConnector_cmd_val}:OFLow:STATe?')
