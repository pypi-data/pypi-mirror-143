...
class Id:
	...
	def set(self, ulca_phy_cell_id: int, cellNull=repcap.CellNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:UL:CA:CELL<CH0>:ID \n
		Snippet: driver.source.bb.oneweb.uplink.ca.cell.id.set(ulca_phy_cell_id = 1, cellNull = repcap.CellNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:UL:CA:CELL{cellNull_cmd_val}:ID {param}')

	def get(self, cellNull=repcap.CellNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:UL:CA:CELL<CH0>:ID \n
		Snippet: value: int = driver.source.bb.oneweb.uplink.ca.cell.id.get(cellNull = repcap.CellNull.Default) \n
		Sets the physical Cell ID of the PCell and the SCells. \n
			:param cellNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Cell')
			:return: ulca_phy_cell_id: integer Range: 0 to 503"""
		cellNull_cmd_val = self._cmd_group.get_repcap_cmd_value(cellNull, repcap.CellNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ONEWeb:UL:CA:CELL{cellNull_cmd_val}:ID?')
