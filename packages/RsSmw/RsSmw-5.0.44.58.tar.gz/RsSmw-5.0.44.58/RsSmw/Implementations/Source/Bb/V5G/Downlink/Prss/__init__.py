...
class Prss:
	...
	def tprs(self):
		...
		if not hasattr(self, '_tprs'):
			from .Tprs import Tprs
			self._tprs = Tprs(self._core, self._cmd_group)
		return self._tprs
	...
	def get_mi_pattern(self) -> List[str]:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:PRSS:MIPattern \n
		Snippet: value: List[str] = driver.source.bb.v5G.downlink.prss.get_mi_pattern() \n
		No command help available \n
	...
	def set_mi_pattern(self, prs_muting_info: List[str]) -> None:
		"""SCPI: [SOURce<HW>]:BB:V5G:DL:PRSS:MIPattern \n
		Snippet: driver.source.bb.v5G.downlink.prss.set_mi_pattern(prs_muting_info = ['raw1', 'raw2', 'raw3']) \n
		No command help available \n
			:param prs_muting_info: No help available
		"""
		param = Conversions.list_to_csv_str(prs_muting_info)
