...
class Foffset:
	...
	def set(self, mark_fall_offs: int, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.nr5G.trigger.output.foffset.set(mark_fall_offs = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:TRIGger:OUTPut{output_cmd_val}:FOFFset {param}')
