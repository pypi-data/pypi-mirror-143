...
class Ontime:
	...
	def set(self, ontime: int, output=repcap.Output.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:DVB:TRIGger:OUTPut<CH>:ONTime \n
		Snippet: driver.source.bb.dvb.trigger.output.ontime.set(ontime = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:DVB:TRIGger:OUTPut{output_cmd_val}:ONTime {param}')

	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:DVB:TRIGger:OUTPut<CH>:ONTime \n
		Snippet: value: int = driver.source.bb.dvb.trigger.output.ontime.get(output = repcap.Output.Default) \n
		Sets the number of samples during which the marker output is on or off. *) If R&S SMW-B9 is installed, the minimum marker
		duration depends on the sample/symbol rate. See 'Marker Minimum Duration'. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: ontime: No help available"""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:DVB:TRIGger:OUTPut{output_cmd_val}:ONTime?')
