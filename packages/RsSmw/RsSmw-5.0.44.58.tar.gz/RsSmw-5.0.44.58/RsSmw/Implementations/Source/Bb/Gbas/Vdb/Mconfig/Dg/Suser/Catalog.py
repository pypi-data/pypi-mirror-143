...
class Catalog:
	...
	def get(self, vdbTransmitter=repcap.VdbTransmitter.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.source.bb.gbas.vdb.mconfig.dg.suser.catalog.get(vdbTransmitter = repcap.VdbTransmitter.Default) \n
		...
		For SCAT-I differential files, files with extension *.rs_scat are listed. \n
			...
			:return: gbas_mc_scat_differ_cat_name_user: No help available"""
		vdbTransmitter_cmd_val = self._cmd_group.get_repcap_cmd_value(vdbTransmitter, repcap.VdbTransmitter)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GBAS:VDB{vdbTransmitter_cmd_val}:MCONfig:DG:SUSer:CATalog?')
