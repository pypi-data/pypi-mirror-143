...
class Dmta:
	"""Dmta commands group definition. 25 total commands, 9 Subgroups, 0 group commands"""
	...
	def bsame(self):
		...
		if not hasattr(self, '_bsame'):
			from .Bsame import Bsame
			self._bsame = Bsame(self._core, self._cmd_group)
		return self._bsame
	...
	def puid(self):
		...
		if not hasattr(self, '_puid'):
			...
			self._puid = Puid(self._core, self._cmd_group)
		return self._puid
	...
	def clone(self) -> 'Dmta':
		...
		new_group = Dmta(self._core, self._cmd_group.parent)
