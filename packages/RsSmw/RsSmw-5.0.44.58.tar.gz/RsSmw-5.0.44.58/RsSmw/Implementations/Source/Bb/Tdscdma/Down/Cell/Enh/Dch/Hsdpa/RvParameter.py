...
class RvParameter:
	...
	def set(self, rv_parameter: int, cell=repcap.Cell.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.enh.dch.hsdpa.rvParameter.set(rv_parameter = 1, cell = repcap.Cell.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSDPA:RVParameter {param}')

	def get(self, cell=repcap.Cell.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:DCH:HSDPA:RVParameter \n
		Snippet: value: int = driver.source.bb.tdscdma.down.cell.enh.dch.hsdpa.rvParameter.get(cell = repcap.Cell.Default) \n
		(for HARQ Mode set to constant ACK) Sets the redundancy version parameter, i.e. indicates which redundancy version of the
		data is sent. \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: rv_parameter: integer Range: 0 to 7"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSDPA:RVParameter?')
