...
class Data:
	...
	def set(self, data: enums.TpcDataSour, mobileStation=repcap.MobileStation.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:DPCCh:TPC:DATA {param}')
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> enums.TpcDataSour:
		...
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:DPCCh:TPC:DATA?')
