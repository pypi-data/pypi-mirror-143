...
class Beidou:
	"""Beidou commands group definition. 6 total commands, 1 Subgroups, 1 group commands"""
	...
	def get_state(self) -> bool:
		...
		Snippet: value: bool = driver.source.bb.gnss.system.beidou.get_state() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SYSTem:BEIDou:STATe?')
		return Conversions.str_to_bool(response)

	def set_state(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SYSTem:BEIDou:[STATe] \n
		Snippet: driver.source.bb.gnss.system.beidou.set_state(state = False) \n
		Defines if satellites from the selected GNSS system are included in the simulated satellites constellation. \n
			:param state: 1| ON| 0| OFF Disabling a GNSS system deactivates all SVID and signals from this system.
		"""
		param = Conversions.bool_to_str(state)
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SYSTem:BEIDou:STATe {param}')

	def clone(self) -> 'Beidou':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = Beidou(self._core, self._cmd_group.parent)
