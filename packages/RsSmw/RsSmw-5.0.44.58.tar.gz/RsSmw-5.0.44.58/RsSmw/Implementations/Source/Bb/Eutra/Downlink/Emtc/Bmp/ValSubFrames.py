...
class ValSubFrames:
	...
	def set(self, valid_sub_frames: bool, subframeNull=repcap.SubframeNull.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.downlink.emtc.bmp.valSubFrames.set(valid_sub_frames = False, subframeNull = repcap.SubframeNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:EUTRa:DL:EMTC:BMP:VALSubframes{subframeNull_cmd_val} {param}')

	def get(self, subframeNull=repcap.SubframeNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:DL:EMTC:BMP:VALSubframes<CH> \n
		Snippet: value: bool = driver.source.bb.eutra.downlink.emtc.bmp.valSubFrames.get(subframeNull = repcap.SubframeNull.Default) \n
		No command help available \n
			:param subframeNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'ValSubFrames')
			:return: valid_sub_frames: No help available"""
		subframeNull_cmd_val = self._cmd_group.get_repcap_cmd_value(subframeNull, repcap.SubframeNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:EUTRa:DL:EMTC:BMP:VALSubframes{subframeNull_cmd_val}?')
	...
	def clone(self) -> 'ValSubFrames':
		...
		new_group = ValSubFrames(self._core, self._cmd_group.parent)
