...
class Trigger:
	"""Trigger commands group definition. 23 total commands, 5 Subgroups, 5 group commands"""
	...
	@property
	def external(self):
		"""external commands group. 2 Sub-classes, 2 commands."""
	...
	def get_rmode(self) -> enums.TrigRunMode:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:RMODe \n
		Snippet: value: enums.TrigRunMode = driver.source.bb.gsm.trigger.get_rmode() \n
		Queries the signal generation status. \n
			:return: rmode: STOP| RUN
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GSM:TRIGger:RMODe?')
		return Conversions.str_to_scalar_enum(response, enums.TrigRunMode)

	def get_slength(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SLENgth \n
		Snippet: value: int = driver.source.bb.gsm.trigger.get_slength() \n
		Defines the length of the signal sequence that is output in the SINGle trigger mode. \n
			:return: slength: integer Range: 1 to max
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GSM:TRIGger:SLENgth?')
		return Conversions.str_to_int(response)

	def set_slength(self, slength: int) -> None:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SLENgth \n
		Snippet: driver.source.bb.gsm.trigger.set_slength(slength = 1) \n
		Defines the length of the signal sequence that is output in the SINGle trigger mode. \n
			:param slength: integer Range: 1 to max
		"""
		param = Conversions.decimal_value_to_str(slength)
		self._core.io.write(f'SOURce<HwInstance>:BB:GSM:TRIGger:SLENgth {param}')

	# noinspection PyTypeChecker
	def get_sl_unit(self) -> enums.UnitSlGsm:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SLUNit \n
		Snippet: value: enums.UnitSlGsm = driver.source.bb.gsm.trigger.get_sl_unit() \n
		Defines the unit for the entry of the signal sequence length. \n
			:return: sl_unit: FRAMe| SYMBol
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GSM:TRIGger:SLUNit?')
		return Conversions.str_to_scalar_enum(response, enums.UnitSlGsm)

	def set_sl_unit(self, sl_unit: enums.UnitSlGsm) -> None:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SLUNit \n
		Snippet: driver.source.bb.gsm.trigger.set_sl_unit(sl_unit = enums.UnitSlGsm.FRAMe) \n
		Defines the unit for the entry of the signal sequence length. \n
			:param sl_unit: FRAMe| SYMBol
		"""
		param = Conversions.enum_scalar_to_str(sl_unit, enums.UnitSlGsm)
		self._core.io.write(f'SOURce<HwInstance>:BB:GSM:TRIGger:SLUNit {param}')

	# noinspection PyTypeChecker
	def get_source(self) -> enums.TriggerSourceC:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SOURce \n
		Snippet: value: enums.TriggerSourceC = driver.source.bb.gsm.trigger.get_source() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GSM:TRIGger:SOURce?')
		return Conversions.str_to_scalar_enum(response, enums.TriggerSourceC)

	def set_source(self, source: enums.TriggerSourceC) -> None:
		"""SCPI: [SOURce<HW>]:BB:GSM:TRIGger:SOURce \n
		Snippet: driver.source.bb.gsm.trigger.set_source(source = enums.TriggerSourceC.BBSY) \n
			INTRO_CMD_HELP: Selects the trigger signal source and determines the way the triggering is executed. Provided are: \n
			- Internal triggering by a command (INTernal)
			- External trigger signal via one of the local or global connectors
			Table Header:  \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GSM:TRIGger:SOURce {param}')

	# noinspection PyTypeChecker
	def get_sequence(self) -> enums.DmTrigMode:
		"""SCPI: [SOURce<HW>]:BB:GSM:[TRIGger]:SEQuence \n
		Snippet: value: enums.DmTrigMode = driver.source.bb.gsm.trigger.get_sequence() \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GSM:TRIGger:SEQuence?')
		return Conversions.str_to_scalar_enum(response, enums.DmTrigMode)

	def set_sequence(self, sequence: enums.DmTrigMode) -> None:
		"""SCPI: [SOURce<HW>]:BB:GSM:[TRIGger]:SEQuence \n
		Snippet: driver.source.bb.gsm.trigger.set_sequence(sequence = enums.DmTrigMode.AAUTo) \n
			INTRO_CMD_HELP: Selects the trigger mode: \n
			- AUTO = auto
			- RETRigger = retrigger
			- AAUTo = armed auto
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GSM:TRIGger:SEQuence {param}')
