...
class Pz:
	...
	def set(self, longitude: float, latitude: float, altitude: float) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:QZSS:LOCation:COORdinates:DECimal:PZ \n
		Snippet: driver.source.bb.gnss.adGeneration.qzss.location.coordinates.decimal.pz.set(longitude = 1.0, latitude = 1.0, altitude = 1.0) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:QZSS:LOCation:COORdinates:DECimal:PZ {param}'.rstrip())
	...
	def get(self) -> PzStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:QZSS:LOCation:COORdinates:DECimal:PZ \n
		Snippet: value: PzStruct = driver.source.bb.gnss.adGeneration.qzss.location.coordinates.decimal.pz.get() \n
		Sets the geographic reference location in decimal format. \n
			:return: structure: for return value, see the help for PzStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:QZSS:LOCation:COORdinates:DECimal:PZ?', self.__class__.PzStruct())
