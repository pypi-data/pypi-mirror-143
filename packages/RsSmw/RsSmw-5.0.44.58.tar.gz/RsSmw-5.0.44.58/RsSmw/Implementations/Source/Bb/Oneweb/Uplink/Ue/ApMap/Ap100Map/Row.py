...
class Row:
	...
	def set(self, ant_port_mapping: bool, userEquipment=repcap.UserEquipment.Default, rowNull=repcap.RowNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:UL:UE<ST>:APMap:AP100Map:ROW<BBID> \n
		Snippet: driver.source.bb.oneweb.uplink.ue.apMap.ap100Map.row.set(ant_port_mapping = False, userEquipment = repcap.UserEquipment.Default, rowNull = repcap.RowNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:UL:UE{userEquipment_cmd_val}:APMap:AP100Map:ROW{rowNull_cmd_val} {param}')

	def get(self, userEquipment=repcap.UserEquipment.Default, rowNull=repcap.RowNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:UL:UE<ST>:APMap:AP100Map:ROW<BBID> \n
		Snippet: value: bool = driver.source.bb.oneweb.uplink.ue.apMap.ap100Map.row.get(userEquipment = repcap.UserEquipment.Default, rowNull = repcap.RowNull.Default) \n
		No command help available \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:param rowNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Row')
		...
		userEquipment_cmd_val = self._cmd_group.get_repcap_cmd_value(userEquipment, repcap.UserEquipment)
		rowNull_cmd_val = self._cmd_group.get_repcap_cmd_value(rowNull, repcap.RowNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ONEWeb:UL:UE{userEquipment_cmd_val}:APMap:AP100Map:ROW{rowNull_cmd_val}?')
