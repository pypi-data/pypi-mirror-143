...
class Roffset:
	...
	def set(self, mark_rise_offs: int, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.nr5G.trigger.output.roffset.set(mark_rise_offs = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:NR5G:TRIGger:OUTPut{output_cmd_val}:ROFFset {param}')
