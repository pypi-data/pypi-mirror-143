...
class Pmodel:
	...
	def set(self, model: enums.ObscPhysModel, vehicle=repcap.Vehicle.Default) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:ENVironment:VOBS:PMODel {param}')
	...
	def get(self, vehicle=repcap.Vehicle.Default) -> enums.ObscPhysModel:
		...
		vehicle_cmd_val = self._cmd_group.get_repcap_cmd_value(vehicle, repcap.Vehicle)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RECeiver:V{vehicle_cmd_val}:ENVironment:VOBS:PMODel?')
