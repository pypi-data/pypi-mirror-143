...
class Modulation:
	...
	def set(self, modulation: enums.ModulationC, cell=repcap.Cell.Default) -> None:
		...
		Snippet: driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.modulation.set(modulation = enums.ModulationC.QAM16, cell = repcap.Cell.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:MODulation {param}')

	# noinspection PyTypeChecker
	def get(self, cell=repcap.Cell.Default) -> enums.ModulationC:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:DCH:HSUPA:MODulation \n
		Snippet: value: enums.ModulationC = driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.modulation.get(cell = repcap.Cell.Default) \n
		Sets the modulation scheme for each HSDPA RMC or HSUPA FRC. The HSUPA FRCs do not support modulation scheme 64QAM. \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: modulation: QPSK| QAM16| QAM64"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:MODulation?')
