...
class Output:
	"""Output commands group definition. 10 total commands, 9 Subgroups, 0 group commands
	...
	def feShift(self):
		...
		if not hasattr(self, '_feShift'):
			from .FeShift import FeShift
			self._feShift = FeShift(self._core, self._cmd_group)
	...
	@property
	def pindex(self):
		"""pindex commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_pindex'):
			from .Pindex import Pindex
			self._pindex = Pindex(self._core, self._cmd_group)
		return self._pindex
	...
	def reShift(self):
		...
		if not hasattr(self, '_reShift'):
			from .ReShift import ReShift
			self._reShift = ReShift(self._core, self._cmd_group)
