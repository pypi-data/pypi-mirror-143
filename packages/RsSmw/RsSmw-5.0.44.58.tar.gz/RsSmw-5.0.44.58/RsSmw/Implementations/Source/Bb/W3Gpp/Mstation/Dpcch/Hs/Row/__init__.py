...
class Row:
	"""Row commands group definition. 11 total commands, 5 Subgroups, 0 group commands
	...
	def poHack(self):
		...
		if not hasattr(self, '_poHack'):
			from .PoHack import PoHack
			self._poHack = PoHack(self._core, self._cmd_group)
		return self._poHack
	...
	def popCqi(self):
		...
		if not hasattr(self, '_popCqi'):
			from .PopCqi import PopCqi
			self._popCqi = PopCqi(self._core, self._cmd_group)
		return self._popCqi
