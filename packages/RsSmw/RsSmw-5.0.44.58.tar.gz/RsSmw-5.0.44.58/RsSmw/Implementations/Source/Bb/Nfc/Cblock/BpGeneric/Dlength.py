...
class Dlength:
	...
	def get(self, data_length: int, commandBlock=repcap.CommandBlock.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.nfc.cblock.bpGeneric.dlength.get(data_length = 1, commandBlock = repcap.CommandBlock.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NFC:CBLock{commandBlock_cmd_val}:BPGeneric:DLENgth? {param}')
