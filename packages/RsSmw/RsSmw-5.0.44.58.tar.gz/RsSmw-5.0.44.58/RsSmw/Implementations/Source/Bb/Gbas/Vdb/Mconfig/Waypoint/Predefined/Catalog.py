...
class Catalog:
	...
	def get(self, vdbTransmitter=repcap.VdbTransmitter.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.source.bb.gbas.vdb.mconfig.waypoint.predefined.catalog.get(vdbTransmitter = repcap.VdbTransmitter.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GBAS:VDB{vdbTransmitter_cmd_val}:MCONfig:WAYPoint:PREDefined:CATalog?')
