...
class Avcces:
	...
	def get(self, subframeNull=repcap.SubframeNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:[SUBF<ST0>]:ENCC:PDCCh:AVCCes \n
		Snippet: value: int = driver.source.bb.oneweb.downlink.subf.encc.pdcch.avcces.get(subframeNull = repcap.SubframeNull.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ONEWeb:DL:SUBF{subframeNull_cmd_val}:ENCC:PDCCh:AVCCes?')
