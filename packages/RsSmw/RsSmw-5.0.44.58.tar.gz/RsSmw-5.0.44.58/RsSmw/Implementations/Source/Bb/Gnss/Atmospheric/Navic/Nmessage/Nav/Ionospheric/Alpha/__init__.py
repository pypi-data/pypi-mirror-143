...
class Alpha:
	...
	def set(self, alpha: int, alphaNull=repcap.AlphaNull.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.atmospheric.navic.nmessage.nav.ionospheric.alpha.set(alpha = 1, alphaNull = repcap.AlphaNull.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ATMospheric:NAVic:NMESsage:NAV:IONospheric:ALPHa{alphaNull_cmd_val} {param}')

	def get(self, alphaNull=repcap.AlphaNull.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ATMospheric:NAVic:NMESsage:NAV:IONospheric:ALPHa<CH0> \n
		Snippet: value: int = driver.source.bb.gnss.atmospheric.navic.nmessage.nav.ionospheric.alpha.get(alphaNull = repcap.AlphaNull.Default) \n
		Sets the parameters alpha_0 to alpha_3 of the satellite's navigation message. \n
			:param alphaNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Alpha')
			:return: alpha: integer Range: -128 to 127"""
		alphaNull_cmd_val = self._cmd_group.get_repcap_cmd_value(alphaNull, repcap.AlphaNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:ATMospheric:NAVic:NMESsage:NAV:IONospheric:ALPHa{alphaNull_cmd_val}?')
