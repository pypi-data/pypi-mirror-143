...
class Drop:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:RTCI:[SEQuencer<ST>]:STReam:DROP \n
		Snippet: value: float = driver.source.bb.esequencer.rtci.sequencer.stream.drop.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:RTCI:SEQuencer{sequencer_cmd_val}:STReam:DROP?')
