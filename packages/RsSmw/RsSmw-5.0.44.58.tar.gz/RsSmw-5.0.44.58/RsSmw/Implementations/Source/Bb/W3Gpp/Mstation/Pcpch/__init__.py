...
class Pcpch:
	"""Pcpch commands group definition. 30 total commands, 18 Subgroups, 0 group commands"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("pcpch", core, parent)
	...
	def cpsFormat(self):
		...
		if not hasattr(self, '_cpsFormat'):
			from .CpsFormat import CpsFormat
			self._cpsFormat = CpsFormat(self._core, self._cmd_group)
	...
	def clone(self) -> 'Pcpch':
		...
		new_group = Pcpch(self._core, self._cmd_group.parent)
