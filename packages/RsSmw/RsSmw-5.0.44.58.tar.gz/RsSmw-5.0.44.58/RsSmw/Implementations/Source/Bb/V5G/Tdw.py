...
class Tdw:
	...
	def get_state(self) -> bool:
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:V5G:TDW:STATe?')
	...
	def set_state(self, state: bool) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:V5G:TDW:STATe {param}')
	...
	def get_tr_time(self) -> float:
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:V5G:TDW:TRTime?')
	...
	def set_tr_time(self, transition_time: float) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:V5G:TDW:TRTime {param}')
