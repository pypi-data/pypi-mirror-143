...
class Roffset:
	...
	def set(self, mark_rise_offs: int, output=repcap.Output.Default) -> None:
		...
		Snippet: driver.source.bb.huwb.trigger.output.roffset.set(mark_rise_offs = 1, output = repcap.Output.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:HUWB:TRIGger:OUTPut{output_cmd_val}:ROFFset {param}')

	def get(self, output=repcap.Output.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:HUWB:TRIGger:OUTPut<CH>:ROFFset \n
		Snippet: value: int = driver.source.bb.huwb.trigger.output.roffset.get(output = repcap.Output.Default) \n
		Sets the rise/fall offset. \n
			:param output: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Output')
			:return: mark_rise_offs: No help available"""
		output_cmd_val = self._cmd_group.get_repcap_cmd_value(output, repcap.Output)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:HUWB:TRIGger:OUTPut{output_cmd_val}:ROFFset?')
