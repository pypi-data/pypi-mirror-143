...
class Evdo:
	"""Evdo commands group definition. 153 total commands, 13 Subgroups, 8 group commands"""
	...
	def anetwork(self):
		...
		if not hasattr(self, '_anetwork'):
			...
			self._anetwork = Anetwork(self._core, self._cmd_group)
	...
	def terminal(self):
		...
		if not hasattr(self, '_terminal'):
			...
			self._terminal = Terminal(self._core, self._cmd_group)
	...
	def clone(self) -> 'Evdo':
		...
		new_group = Evdo(self._core, self._cmd_group.parent)
