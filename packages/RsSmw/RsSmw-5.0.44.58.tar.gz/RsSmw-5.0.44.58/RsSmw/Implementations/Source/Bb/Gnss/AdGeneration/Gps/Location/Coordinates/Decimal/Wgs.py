...
class Wgs:
	...
	def set(self, longitude: float, latitude: float, altitude: float) -> None:
		...
		Snippet: driver.source.bb.gnss.adGeneration.gps.location.coordinates.decimal.wgs.set(longitude = 1.0, latitude = 1.0, altitude = 1.0) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS:LOCation:COORdinates:DECimal:WGS {param}'.rstrip())
	...
	def get(self) -> WgsStruct:
		"""SCPI: [SOURce<HW>]:BB:GNSS:ADGeneration:GPS:LOCation:COORdinates:DECimal:[WGS] \n
		Snippet: value: WgsStruct = driver.source.bb.gnss.adGeneration.gps.location.coordinates.decimal.wgs.get() \n
		Sets the geographic reference location in decimal format. \n
			:return: structure: for return value, see the help for WgsStruct structure arguments."""
		return self._core.io.query_struct(f'SOURce<HwInstance>:BB:GNSS:ADGeneration:GPS:LOCation:COORdinates:DECimal:WGS?', self.__class__.WgsStruct())
