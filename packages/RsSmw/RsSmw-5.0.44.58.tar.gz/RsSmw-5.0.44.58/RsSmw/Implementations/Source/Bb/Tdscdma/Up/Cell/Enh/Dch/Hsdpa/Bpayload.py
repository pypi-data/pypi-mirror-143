...
class Bpayload:
	...
	def get(self, cell=repcap.Cell.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:ENH:DCH:HSDPA:BPAYload \n
		Snippet: value: int = driver.source.bb.tdscdma.up.cell.enh.dch.hsdpa.bpayload.get(cell = repcap.Cell.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:ENH:DCH:HSDPA:BPAYload?')
