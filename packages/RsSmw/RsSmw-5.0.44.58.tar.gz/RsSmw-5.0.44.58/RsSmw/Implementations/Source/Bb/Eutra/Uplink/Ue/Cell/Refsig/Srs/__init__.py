...
class Srs:
	...
	def nktc(self):
		...
		if not hasattr(self, '_nktc'):
			from .Nktc import Nktc
			self._nktc = Nktc(self._core, self._cmd_group)
		return self._nktc
	...
	def ntrans(self):
		...
		if not hasattr(self, '_ntrans'):
			...
			self._ntrans = Ntrans(self._core, self._cmd_group)
		return self._ntrans
	...
	def upptsadd(self):
		...
		if not hasattr(self, '_upptsadd'):
			from .Upptsadd import Upptsadd
			self._upptsadd = Upptsadd(self._core, self._cmd_group)
