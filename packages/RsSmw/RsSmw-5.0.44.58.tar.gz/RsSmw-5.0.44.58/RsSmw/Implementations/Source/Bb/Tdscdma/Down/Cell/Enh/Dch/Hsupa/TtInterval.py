...
class TtInterval:
	...
	def get(self, cell=repcap.Cell.Default) -> enums.TdscdmaEnhTchTti:
		...
		Snippet: value: enums.TdscdmaEnhTchTti = driver.source.bb.tdscdma.down.cell.enh.dch.hsupa.ttInterval.get(cell = repcap.Cell.Default) \n
		Queries the transmission time interval (TTI) . \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: tt_interval: 5MS"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:DCH:HSUPA:TTINterval?')
