...
class S60K:
	"""S60K commands group definition. 4 total commands, 4 Subgroups, 0 group commands"""
	...
	def komu(self):
		...
		if not hasattr(self, '_komu'):
			from .Komu import Komu
			self._komu = Komu(self._core, self._cmd_group)
		return self._komu
	...
	def nrb(self):
		...
		if not hasattr(self, '_nrb'):
			from .Nrb import Nrb
			self._nrb = Nrb(self._core, self._cmd_group)
		return self._nrb
	...
	def clone(self) -> 'S60K':
		...
		new_group = S60K(self._core, self._cmd_group.parent)
