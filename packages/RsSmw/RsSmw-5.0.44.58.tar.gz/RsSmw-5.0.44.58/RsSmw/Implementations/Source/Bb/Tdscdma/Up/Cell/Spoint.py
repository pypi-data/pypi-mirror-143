...
class Spoint:
	...
	def set(self, spoint: int, cell=repcap.Cell.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:SPOint \n
		Snippet: driver.source.bb.tdscdma.up.cell.spoint.set(spoint = 1, cell = repcap.Cell.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:SPOint {param}')

	def get(self, cell=repcap.Cell.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:UP:CELL<ST>:SPOint \n
		Snippet: value: int = driver.source.bb.tdscdma.up.cell.spoint.get(cell = repcap.Cell.Default) \n
		Sets the switching point between the uplink slots and the downlink slots in the frame. \n
			:param cell: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cell')
			:return: spoint: integer Range: 1 to 6"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:SPOint?')
