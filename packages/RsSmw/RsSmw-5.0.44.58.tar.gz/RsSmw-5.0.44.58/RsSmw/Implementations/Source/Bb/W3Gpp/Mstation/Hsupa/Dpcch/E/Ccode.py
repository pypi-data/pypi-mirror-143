...
class Ccode:
	...
	def get(self, mobileStation=repcap.MobileStation.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.hsupa.dpcch.e.ccode.get(mobileStation = repcap.MobileStation.Default) \n
		Querries the channelization code. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: ccode: integer Range: 1 to max"""
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:HSUPa:DPCCh:E:CCODe?')
