...
class Signals:
	...
	def get(self, stream=repcap.Stream.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:GNSS:RT:[STReam<CH>]:SBAS:ACTive:SIGNals \n
		Snippet: value: str = driver.source.bb.gnss.rt.stream.sbas.active.signals.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:RT:STReam{stream_cmd_val}:SBAS:ACTive:SIGNals?')
