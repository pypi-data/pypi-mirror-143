...
class Clipping:
	...
	def get_level(self) -> int:
		...
		if [:SOURce<hw>]:BB:WLNN:CLIPping:STATe is set to ON. \n
	...
	def get_spps_state(self) -> bool:
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:WLNN:CLIPping:SPPState?')
		return Conversions.str_to_bool(response)

	def set_spps_state(self, state: bool) -> None:
		"""SCPI: [SOURce<HW>]:BB:WLNN:CLIPping:SPPState \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:WLNN:CLIPping:SPPState {param}')

	def get_state(self) -> bool:
		"""SCPI: [SOURce<HW>]:BB:WLNN:CLIPping:STATe \n
		Snippet: value: bool = driver.source.bb.wlnn.clipping.get_state() \n
		Activates level clipping (Clipping) . The value is defined with [:SOURce<hw>]:BB:WLNN:CLIPping:LEVel, the mode of
		calculation with [:SOURce<hw>]:BB:WLNN:CLIPping:MODE. \n
			:return: state: 1| ON| 0| OFF
		"""
