...
class Mcode:
	...
	def get(self, cell=repcap.Cell.Default) -> int:
		...
		Snippet: value: int = driver.source.bb.tdscdma.up.cell.mcode.get(cell = repcap.Cell.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:UP:CELL{cell_cmd_val}:MCODe?')
