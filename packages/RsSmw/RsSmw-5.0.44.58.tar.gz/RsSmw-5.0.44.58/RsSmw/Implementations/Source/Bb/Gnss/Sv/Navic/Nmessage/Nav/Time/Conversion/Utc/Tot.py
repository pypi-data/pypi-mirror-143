...
class Tot:
	...
	def get_unscaled(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT:UNSCaled \n
		...
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT:UNSCaled?')
	...
	def set_unscaled(self, tot: int) -> None:
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT:UNSCaled {param}')

	def get_value(self) -> int:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT \n
		Snippet: value: int = driver.source.bb.gnss.sv.navic.nmessage.nav.time.conversion.utc.tot.get_value() \n
		Sets the parameter Tot. \n
			:return: tot: integer Range: 0 to 65535
		"""
		response = self._core.io.query_str('SOURce<HwInstance>:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT?')
		return Conversions.str_to_int(response)
	...
	def set_value(self, tot: int) -> None:
		...
		"""
		param = Conversions.decimal_value_to_str(tot)
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SV:NAVic:NMESsage:NAV:TIME:CONVersion:UTC:TOT {param}')
