...
class L1Cdma:
	"""L1Cdma commands group definition. 1 total commands, 0 Subgroups, 1 group commands
	Repeated Capability: IndexNull, default value after init: IndexNull.Nr0"""

	def __init__(self, core: Core, parent):
		self._core = core
		self._cmd_group = CommandsGroup("l1Cdma", core, parent)
	...
	def set(self, signal_state: bool, monitorPane=repcap.MonitorPane.Default, indexNull=repcap.IndexNull.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:MONitor<CH>:DISPlay:POWer:SYSTem:GLONass:SIGNal:L1Band:L1CDma<US0> \n
		Snippet: driver.source.bb.gnss.monitor.display.power.system.glonass.signal.l1Band.l1Cdma.set(signal_state = False, monitorPane = repcap.MonitorPane.Default, indexNull = repcap.IndexNull.Default) \n
		No command help available \n
			:param signal_state: No help available
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:MONitor{monitorPane_cmd_val}:DISPlay:POWer:SYSTem:GLONass:SIGNal:L1Band:L1CDma{indexNull_cmd_val} {param}')

	def get(self, monitorPane=repcap.MonitorPane.Default, indexNull=repcap.IndexNull.Default) -> bool:
		"""SCPI: [SOURce<HW>]:BB:GNSS:MONitor<CH>:DISPlay:POWer:SYSTem:GLONass:SIGNal:L1Band:L1CDma<US0> \n
		Snippet: value: bool = driver.source.bb.gnss.monitor.display.power.system.glonass.signal.l1Band.l1Cdma.get(monitorPane = repcap.MonitorPane.Default, indexNull = repcap.IndexNull.Default) \n
		No command help available \n
			:param monitorPane: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Monitor')
			:param indexNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'L1Cdma')
			:return: signal_state: No help available"""
		monitorPane_cmd_val = self._cmd_group.get_repcap_cmd_value(monitorPane, repcap.MonitorPane)
		indexNull_cmd_val = self._cmd_group.get_repcap_cmd_value(indexNull, repcap.IndexNull)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:MONitor{monitorPane_cmd_val}:DISPlay:POWer:SYSTem:GLONass:SIGNal:L1Band:L1CDma{indexNull_cmd_val}?')
		return Conversions.str_to_bool(response)

	def clone(self) -> 'L1Cdma':
		"""Clones the group by creating new object from it and its whole existing subgroups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = L1Cdma(self._core, self._cmd_group.parent)
