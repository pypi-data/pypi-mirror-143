...
class Drop:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:ASEQuencing:[SEQuencer<ST>]:STReam:DROP \n
		Snippet: value: float = driver.source.bb.esequencer.asequencing.sequencer.stream.drop.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:ASEQuencing:SEQuencer{sequencer_cmd_val}:STReam:DROP?')
