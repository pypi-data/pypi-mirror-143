...
class Release:
	...
	def set(self, release: enums.ReleaseNbiotDl, userIx=repcap.UserIx.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:RELease \n
		Snippet: driver.source.bb.oneweb.downlink.user.release.set(release = enums.ReleaseNbiotDl.EMTC, userIx = repcap.UserIx.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:RELease {param}')

	# noinspection PyTypeChecker
	def get(self, userIx=repcap.UserIx.Default) -> enums.ReleaseNbiotDl:
		"""SCPI: [SOURce<HW>]:BB:ONEWeb:DL:USER<CH>:RELease \n
		Snippet: value: enums.ReleaseNbiotDl = driver.source.bb.oneweb.downlink.user.release.get(userIx = repcap.UserIx.Default) \n
		No command help available \n
			:param userIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'User')
			:return: release: No help available"""
		userIx_cmd_val = self._cmd_group.get_repcap_cmd_value(userIx, repcap.UserIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ONEWeb:DL:USER{userIx_cmd_val}:RELease?')
