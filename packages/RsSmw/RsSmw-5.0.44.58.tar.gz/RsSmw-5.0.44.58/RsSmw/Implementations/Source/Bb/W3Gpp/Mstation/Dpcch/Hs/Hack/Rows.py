...
class Rows:
	...
	def set(self, row_count: int, mobileStation=repcap.MobileStation.Default) -> None:
		...
		Snippet: driver.source.bb.w3Gpp.mstation.dpcch.hs.hack.rows.set(row_count = 1, mobileStation = repcap.MobileStation.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:DPCCh:HS:HACK:ROWS {param}')

	def get(self, mobileStation=repcap.MobileStation.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:W3GPp:MSTation<ST>:DPCCh:HS:HACK:ROWS \n
		Snippet: value: int = driver.source.bb.w3Gpp.mstation.dpcch.hs.hack.rows.get(mobileStation = repcap.MobileStation.Default) \n
		Determines the number of the rows in the HARQ-ACK respectively in the PCI/CQI scheduling table. \n
			:param mobileStation: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Mstation')
			:return: row_count: integer Range: 1 to 32"""
		mobileStation_cmd_val = self._cmd_group.get_repcap_cmd_value(mobileStation, repcap.MobileStation)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:W3GPp:MSTation{mobileStation_cmd_val}:DPCCh:HS:HACK:ROWS?')
