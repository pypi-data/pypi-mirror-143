...
class CaPeriod:
	...
	def set(self, const_acc_period: float, satelliteSvid=repcap.SatelliteSvid.Default) -> None:
		...
		Snippet: driver.source.bb.gnss.svid.glonass.sdynamics.caPeriod.set(const_acc_period = 1.0, satelliteSvid = repcap.SatelliteSvid.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:GLONass:SDYNamics:CAPeriod {param}')

	def get(self, satelliteSvid=repcap.SatelliteSvid.Default) -> float:
		"""SCPI: [SOURce<HW>]:BB:GNSS:SVID<CH>:GLONass:SDYNamics:CAPeriod \n
		Snippet: value: float = driver.source.bb.gnss.svid.glonass.sdynamics.caPeriod.get(satelliteSvid = repcap.SatelliteSvid.Default) \n
		Sets the time duration during that acceleration is applied and thus the velocity varies. \n
			:param satelliteSvid: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Svid')
			:return: const_acc_period: float Range: 0.1 to 10800"""
		satelliteSvid_cmd_val = self._cmd_group.get_repcap_cmd_value(satelliteSvid, repcap.SatelliteSvid)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:SVID{satelliteSvid_cmd_val}:GLONass:SDYNamics:CAPeriod?')
