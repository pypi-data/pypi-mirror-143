...
class StsFrame:
	...
	def set(self, start_subframe: int, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> None:
		...
		Snippet: driver.source.bb.eutra.uplink.ue.emtc.trans.stsFrame.set(start_subframe = 1, userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
	...
	def get(self, userEquipment=repcap.UserEquipment.Default, transmission=repcap.Transmission.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:EUTRa:UL:UE<ST>:EMTC:TRANs<CH>:STSFrame \n
		Snippet: value: int = driver.source.bb.eutra.uplink.ue.emtc.trans.stsFrame.get(userEquipment = repcap.UserEquipment.Default, transmission = repcap.Transmission.Default) \n
		Sets the subframe number in that the PUSCH/PUCCH allocation is scheduled for the first time. \n
			:param userEquipment: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Ue')
			:param transmission: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Trans')
