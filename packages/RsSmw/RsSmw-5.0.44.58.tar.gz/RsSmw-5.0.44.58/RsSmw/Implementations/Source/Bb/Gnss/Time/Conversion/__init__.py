...
class Conversion:
	"""Conversion commands group definition. 93 total commands, 9 Subgroups, 0 group commands"""
	...
	@property
	def leap(self):
		"""leap commands group. 2 Sub-classes, 1 commands."""
		if not hasattr(self, '_leap'):
			from .Leap import Leap
			self._leap = Leap(self._core, self._cmd_group)
		return self._leap
	...
	def utcsu(self):
		...
		if not hasattr(self, '_utcsu'):
			from .Utcsu import Utcsu
			self._utcsu = Utcsu(self._core, self._cmd_group)
		return self._utcsu
