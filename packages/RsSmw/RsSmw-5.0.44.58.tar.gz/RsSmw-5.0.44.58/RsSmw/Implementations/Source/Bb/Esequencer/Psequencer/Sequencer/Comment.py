...
class Comment:
	...
	def get(self, sequencer=repcap.Sequencer.Default) -> str:
		"""SCPI: [SOURce<HW>]:BB:ESEQuencer:PSEQuencer:[SEQuencer<ST>]:COMMent \n
		Snippet: value: str = driver.source.bb.esequencer.psequencer.sequencer.comment.get(sequencer = repcap.Sequencer.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:ESEQuencer:PSEQuencer:SEQuencer{sequencer_cmd_val}:COMMent?')
