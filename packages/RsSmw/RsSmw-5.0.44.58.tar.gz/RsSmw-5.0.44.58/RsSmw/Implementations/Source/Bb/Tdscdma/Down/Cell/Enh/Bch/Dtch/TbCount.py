...
class TbCount:
	...
	def get(self, cell=repcap.Cell.Default, channel=repcap.Channel.Default) -> int:
		"""SCPI: [SOURce<HW>]:BB:TDSCdma:DOWN:CELL<ST>:ENH:BCH:DTCH<CH>:TBCount \n
		Snippet: value: int = driver.source.bb.tdscdma.down.cell.enh.bch.dtch.tbCount.get(cell = repcap.Cell.Default, channel = repcap.Channel.Default) \n
		Queries the number of transport blocks for the TCH. \n
			...
			:return: tb_count: integer"""
		cell_cmd_val = self._cmd_group.get_repcap_cmd_value(cell, repcap.Cell)
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:TDSCdma:DOWN:CELL{cell_cmd_val}:ENH:BCH:DTCH{channel_cmd_val}:TBCount?')
