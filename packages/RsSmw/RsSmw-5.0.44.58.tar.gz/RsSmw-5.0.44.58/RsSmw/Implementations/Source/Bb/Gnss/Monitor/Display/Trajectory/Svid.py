...
class Svid:
	...
	def set(self, svid: enums.Svid, monitorPane=repcap.MonitorPane.Default) -> None:
		"""SCPI: [SOURce<HW>]:BB:GNSS:MONitor<CH>:DISPlay:TRAJectory:SVID \n
		Snippet: driver.source.bb.gnss.monitor.display.trajectory.svid.set(svid = enums.Svid._1, monitorPane = repcap.MonitorPane.Default) \n
		Selects the SV ID which elevation/azimuth variation over 24 hours is displayed. \n
		...
		self._core.io.write(f'SOURce<HwInstance>:BB:GNSS:MONitor{monitorPane_cmd_val}:DISPlay:TRAJectory:SVID {param}')

	# noinspection PyTypeChecker
	def get(self, monitorPane=repcap.MonitorPane.Default) -> enums.Svid:
		"""SCPI: [SOURce<HW>]:BB:GNSS:MONitor<CH>:DISPlay:TRAJectory:SVID \n
		Snippet: value: enums.Svid = driver.source.bb.gnss.monitor.display.trajectory.svid.get(monitorPane = repcap.MonitorPane.Default) \n
		Selects the SV ID which elevation/azimuth variation over 24 hours is displayed. \n
			:param monitorPane: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Monitor')
		...
		monitorPane_cmd_val = self._cmd_group.get_repcap_cmd_value(monitorPane, repcap.MonitorPane)
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:GNSS:MONitor{monitorPane_cmd_val}:DISPlay:TRAJectory:SVID?')
