...
class State:
	...
	def set(self, state: bool, generatorIx=repcap.GeneratorIx.Default) -> None:
		...
		Snippet: driver.source.fm.state.set(state = False, generatorIx = repcap.GeneratorIx.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:FM{generatorIx_cmd_val}:STATe {param}')

	def get(self, generatorIx=repcap.GeneratorIx.Default) -> bool:
		"""SCPI: [SOURce<HW>]:FM<CH>:STATe \n
		Snippet: value: bool = driver.source.fm.state.get(generatorIx = repcap.GeneratorIx.Default) \n
		Activates frequency modulation. \n
			:param generatorIx: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fm')
			:return: state: 1| ON| 0| OFF"""
		generatorIx_cmd_val = self._cmd_group.get_repcap_cmd_value(generatorIx, repcap.GeneratorIx)
		response = self._core.io.query_str(f'SOURce<HwInstance>:FM{generatorIx_cmd_val}:STATe?')
