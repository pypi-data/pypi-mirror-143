...
class Correction:
	"""Correction commands group definition. 76 total commands, 6 Subgroups, 2 group commands"""
	...
	def cset(self):
		...
		if not hasattr(self, '_cset'):
			...
			self._cset = Cset(self._core, self._cmd_group)
		return self._cset
	...
	def fresponse(self):
		...
		if not hasattr(self, '_fresponse'):
			from .Fresponse import Fresponse
			self._fresponse = Fresponse(self._core, self._cmd_group)
	...
	def zeroing(self):
		...
		if not hasattr(self, '_zeroing'):
			from .Zeroing import Zeroing
			self._zeroing = Zeroing(self._core, self._cmd_group)
