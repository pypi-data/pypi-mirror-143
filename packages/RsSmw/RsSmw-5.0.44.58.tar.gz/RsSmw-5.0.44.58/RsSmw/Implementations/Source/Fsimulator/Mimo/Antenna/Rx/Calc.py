...
class Calc:
	...
	def get_mode(self) -> enums.AntModCalcMode:
		"""SCPI: [SOURce]:FSIMulator:MIMO:ANTenna:RX:CALC:MODE \n
		Snippet: value: enums.AntModCalcMode = driver.source.fsimulator.mimo.antenna.rx.calc.get_mode() \n
		...
		"""
		response = self._core.io.query_str('SOURce:FSIMulator:MIMO:ANTenna:RX:CALC:MODE?')
	...
	def set_mode(self, ant_mod_calc_rx_mod: enums.AntModCalcMode) -> None:
		"""SCPI: [SOURce]:FSIMulator:MIMO:ANTenna:RX:CALC:MODE \n
		Snippet: driver.source.fsimulator.mimo.antenna.rx.calc.set_mode(ant_mod_calc_rx_mod = enums.AntModCalcMode.RELativphase) \n
		Set how the distance between the antenna elements is defined: based on the physical distance or on the relative phase. \n
		...
		self._core.io.write(f'SOURce:FSIMulator:MIMO:ANTenna:RX:CALC:MODE {param}')
