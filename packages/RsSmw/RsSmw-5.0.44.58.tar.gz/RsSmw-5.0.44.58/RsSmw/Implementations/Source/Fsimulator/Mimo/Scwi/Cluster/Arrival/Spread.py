...
class Spread:
	...
	def set(self, spread: float, cluster=repcap.Cluster.Default) -> None:
		...
		Snippet: driver.source.fsimulator.mimo.scwi.cluster.arrival.spread.set(spread = 1.0, cluster = repcap.Cluster.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:FSIMulator:MIMO:SCWI:CLUSter{cluster_cmd_val}:ARRival:SPRead {param}')

	def get(self, cluster=repcap.Cluster.Default) -> float:
		"""SCPI: [SOURce<HW>]:FSIMulator:MIMO:SCWI:CLUSter<CH>:ARRival:SPRead \n
		Snippet: value: float = driver.source.fsimulator.mimo.scwi.cluster.arrival.spread.get(cluster = repcap.Cluster.Default) \n
		Sets the AoA (angle of arrival) / AoD (angle of departure) spread (AS) of the selected cluster. \n
			:param cluster: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cluster')
			:return: spread: float Range: 1 to 75"""
		cluster_cmd_val = self._cmd_group.get_repcap_cmd_value(cluster, repcap.Cluster)
		response = self._core.io.query_str(f'SOURce<HwInstance>:FSIMulator:MIMO:SCWI:CLUSter{cluster_cmd_val}:ARRival:SPRead?')
