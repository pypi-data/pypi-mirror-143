...
class Pfile:
	...
	def set(self, filename: str, index=repcap.Index.Default) -> None:
		...
		Snippet: driver.source.fsimulator.scm.antenna.tx.antenna.pfile.set(filename = '1', index = repcap.Index.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:FSIMulator:SCM:ANTenna:TX:ANTenna{index_cmd_val}:PFILe {param}')

	def get(self, index=repcap.Index.Default) -> str:
		"""SCPI: [SOURce<HW>]:FSIMulator:SCM:ANTenna:TX:ANTenna<DI>:PFILe \n
		Snippet: value: str = driver.source.fsimulator.scm.antenna.tx.antenna.pfile.get(index = repcap.Index.Default) \n
		Selects the antenna pattern file (*.ant_pat) per antenna. Querry the existing files with the command
		[:SOURce]:FSIMulator:MIMO:ANTenna:PATTern:CATalog:USER?. \n
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Antenna')
			:return: filename: string Complete file path, incl. the filename; the file extension can be omitted"""
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:FSIMulator:SCM:ANTenna:TX:ANTenna{index_cmd_val}:PFILe?')
