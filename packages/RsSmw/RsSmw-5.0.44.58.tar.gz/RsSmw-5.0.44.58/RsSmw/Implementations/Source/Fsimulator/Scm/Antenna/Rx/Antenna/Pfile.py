...
class Pfile:
	...
	def set(self, filename: str, index=repcap.Index.Default) -> None:
		"""SCPI: [SOURce]:FSIMulator:SCM:ANTenna:RX:ANTenna<DI>:PFILe \n
		Snippet: driver.source.fsimulator.scm.antenna.rx.antenna.pfile.set(filename = '1', index = repcap.Index.Default) \n
		...
		self._core.io.write(f'SOURce:FSIMulator:SCM:ANTenna:RX:ANTenna{index_cmd_val}:PFILe {param}')
	...
	def get(self, index=repcap.Index.Default) -> str:
		...
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce:FSIMulator:SCM:ANTenna:RX:ANTenna{index_cmd_val}:PFILe?')
