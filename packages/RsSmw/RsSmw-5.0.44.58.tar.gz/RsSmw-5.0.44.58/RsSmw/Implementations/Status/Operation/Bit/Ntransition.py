...
class Ntransition:
	...
	def set(self, ntransition: str, bitNumberNull=repcap.BitNumberNull.Default) -> None:
		...
		Snippet: driver.status.operation.bit.ntransition.set(ntransition = '1', bitNumberNull = repcap.BitNumberNull.Default) \n
		...
		self._core.io.write(f'STATus:OPERation:BIT{bitNumberNull_cmd_val}:NTRansition {param}')

	def get(self, bitNumberNull=repcap.BitNumberNull.Default) -> str:
		"""SCPI: STATus:OPERation:BIT<BITNR>:NTRansition \n
		Snippet: value: str = driver.status.operation.bit.ntransition.get(bitNumberNull = repcap.BitNumberNull.Default) \n
		No command help available \n
			:param bitNumberNull: optional repeated capability selector. Default value: Nr0 (settable in the interface 'Bit')
			:return: ntransition: No help available"""
		bitNumberNull_cmd_val = self._cmd_group.get_repcap_cmd_value(bitNumberNull, repcap.BitNumberNull)
		response = self._core.io.query_str(f'STATus:OPERation:BIT{bitNumberNull_cmd_val}:NTRansition?')
