...
from hestia_earth.validation.validators.shared import (
    ...
    validate_boundary_size, validate_region_size
)
...
def test_validate_linked_source_privacy_indexed(mock_download):
    node = {
        'source': {
            '@type': 'Source',
            '@id': '1'
    ...
    mock_download.return_value = {'dataPrivate': True}
    assert validate_linked_source_privacy(node, 'source') is True

    mock_download.return_value = {'dataPrivate': False}
