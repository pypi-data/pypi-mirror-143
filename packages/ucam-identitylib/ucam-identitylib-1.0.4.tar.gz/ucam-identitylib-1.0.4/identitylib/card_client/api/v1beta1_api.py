    ...
    def v1beta1_card_requests_list(
        ...
        **kwargs
        ...
        Keyword Args:
            ...
            cardholder_status (str): [optional]
    ...
    def v1beta1_card_requests_update(
        ...
        **kwargs
        ...
        """Updates the card request  # noqa: E501
          ...
          ## Update the card request  This method allows a client to submit an action in the request body and optional identifier for a given card request. The available actions are `add`, `start`, `abandon`, `make`, `refute` and `complete`.   For the `make` action, the client can also append identifiers which associates the physically created cards to the card record - for example the card UID which is  pre-encoded into the card by the manufacturer.  The `complete` action returns the UUID of the created `card` entity.  ### Permissions  Principals with the `CARD_CREATOR` permission will be able to affect this endpoint.    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
