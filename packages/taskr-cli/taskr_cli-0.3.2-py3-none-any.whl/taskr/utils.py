...
def bumpVersion(version: Optional[str] = None) -> bool:
    """
...
def addVarsToEnv(envs: Dict[str, str]) -> bool:
    """
    Exposes internal inject path to allow users to set any var
    """
    return _injectPath(envs)
...
def _injectPath(envs: Dict[str, str]) -> bool:
    ...
    temp = envs.copy()
    with open(activationScript, 'r') as f:
